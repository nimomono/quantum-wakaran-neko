...
従来の強化課題であるQ1/Q2の完全周期収支、M64 process--時計--終位置記録--resetの単一反復周期統合、連続空間一様極限、多粒子拡張は引き続き保持する。さらに、直接定めた開放ミクロ模型を明示的なHamiltonian無限浴へ持ち上げること、複数reservoir portを共通単一bathへ統合すること、有限浴化、有限閉鎖Hamiltonian化を、M0より強い横断的実装強化として区別して保持する。
