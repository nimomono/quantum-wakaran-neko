...
従来の強化課題であるQ1/Q2の完全周期収支、M64 process--時計--終位置記録--resetの単一反復周期統合、連続空間一様極限、多粒子拡張は引き続き保持する。さらに、採用open模型を明示的なHamiltonian無限浴へ持ち上げること、複数reservoir portを共通単一bathへ統合すること、有限浴化、有限閉鎖Hamiltonian化を、M0より強い横断的実装強化として区別して保持する。

M0は、複数の固定目標にまたがる主要自由度と物理接続端を1つのjoint microscopic device/processと共通反復周期へ統合する、A1より強い統合目標として区別する。M0はA1と同様に規約を満たす採用開放古典ミクロ方程式を許し、Hamiltonian無限浴への持上げを必須にしない。各固定目標についてA1を達成することだけではM0達成としない。
