...
- 一般Bloch方向のfinite-lock余弦則、finite-thickness全変動誤差、exact非信号周辺、measurement-dependence監査、一試行二端interfaceを閉じる。
- Q2-2の達成ラベルは維持し、直接依存をR180C--M65/R181DからM66/R205--R207へ切り替える。R180A/R180Cはactive alternate witnessとして本draftでは退役させない。
- Q2-2-Sのfinite-speed spatial reservoirとsetting確定後timing closure、A1/A2/B判定、M0判定は変更しない。旧4-setting R207 candidateはnotes/Git履歴へ保存する。
...
| Q2-2 | 達成 | M66/R205 thermal phase-volume preparation | A/B二つのhidden-direction rotor、局所projection rail、二つの物理読出し端 | joint thermal preparation、passive separation、local sign latch、local record | R207A--R207C | 一般Bloch方向のsinglet共同分布を任意精度で構成し、finite-lock/finite-thickness誤差、exact非信号周辺、measurement-dependenceとlocal response factorizationを監査。finite-speed因果隔離はQ2-2-S、全周期統合はM0課題 |
