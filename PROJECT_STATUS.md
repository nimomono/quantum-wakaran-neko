...
| ID | 固定目標 | 達成判定の中心 |
|---|---|---|
| Q4-1 | 単一量子ビット型可逆力学のアナログ回路設計 | 古典アナログ回路上に2モード有効状態を構成し、Bloch球に相当する状態空間、共通位相不変性、任意の $SU(2)$ 操作、Rabi振動の振幅・振動数・離調依存性を、有限精度と有限運転範囲を持つ具体的な回路設計として再現する |
| Q4-2 | 射影測定統計とZeno効果のアナログ回路設計 | 単一量子ビット型回路担体に準備、排他的結果選択、記録、測定後状態更新を接続し、任意のBloch軸に対する2値Born分布、同軸反復分布、異軸逐次分布、Rabi型遷移に対する有限回反復測定のZeno型抑制を、有限精度と無反応を含む具体的な回路設計として再現する |
| Q4-3 | 2量子ビット型結合ゲートのアナログ回路設計 | 2つの単一量子ビット型回路担体と共同担体を接続し、積入力を非分離な共同内部状態へ移し得る2量子ビット型結合ゲートを具体的な回路として設計する。共同入力--出力関係、相対位相の保持、局所操作との合成、逆ゲートによる回帰を有限精度で示す |
| Q4-4 | Bell型測定統計のアナログ回路設計 | 二体系の共同内部状態または同等の共同信号源を2つの測定端へ接続し、各端の局所操作、排他的結果選択、記録から、余弦共同確率、CHSH不等式の破れ、Tsirelson限界、非信号性を有限精度で再現する具体的な回路を設計する。測定設定独立性が破れる場合は、その準備過程と因果経路も回路上で明示する |

Q4-1、Q4-2、Q4-3、Q4-4は、それぞれQ1-1、Q1-2、Q2-1、Q2-2に対応する。

#### Q4の達成判定の補足

- Q4の達成には、回路図、部品定数、電源、駆動、雑音源、制御、較正、読出しの仕様、回路方程式と対応する理論模型の関係、有限帯域、部品ばらつき、寄生要素、熱雑音、ドリフト、測定負荷を含む回路シミュレーション、および実機で検証可能な測定計画を要求する。実機の製作と実測は強い拡張とし、固定目標の達成条件には含めない。
- 外部デジタル機器は、設定、タイミング制御、データ記録、統計解析に用いてよい。対象となる状態発展、結合ゲート、枝選択、結果形成をデジタル計算で代行すること、最終確率を外部で計算して抽選すること、期待する測定後状態を数値的に書き込むことは認めない。
- 入力、測定軸、対照条件、試行数、許容誤差を含む有限の試験計画を示す。無反応、閾値近傍、準備失敗を完全結果集合に残し、除外後の結果だけを再規格化しない。
- Q4-1からQ4-4は個別に判定する。先行するQ4目標の達成ラベルだけを後続目標の根拠にせず、各目標について使用する回路、接続、回路方程式、シミュレーション結果、誤差範囲を示す。部品またはinterfaceの共有は禁止しない。
- Q4-2のZeno型抑制では同じ総時間のRabi対照と比較し、駆動停止、摩擦、傾斜だけによる抑制および事後選別と区別する。
- Q4-4で設定が中央準備過程へ入る場合は、その因果経路を回路図とタイミング図で明示する。この構成を、測定設定独立性、自由設定、空間的分離を満たす標準Bell実験の古典的再現とは呼ばない。

### 現在地

Q1からQ3の「達成」は、固定範囲で基準を厳密に満たすか、任意の $\epsilon>0$ に対して誤差を $\epsilon$ 未満にする有限構成を選べる状態を指す。形式極限、構成のない収束仮定、無反応試行の事後除外は含めない。Q4の「達成」は、固定した有限精度と有限運転範囲について、具体的な回路設計、非理想性を含む回路シミュレーション、実機で検証可能な測定計画がそろった状態を指し、実機製作は要求しない。各固定目標は下表の根拠モデルと根拠結果から独立に判定し、共通ハードウェアへの統合状態を判定へ混ぜない。Q1とQ2はM54の同じ完全状態型と外部interfaceから派生する。R181Aは物理template準備、R181Bは固定入力tensor-lift、R181Cは永続register gate、R181DはR170駆動projector-treeを与える。Q2-4の旧aperture経路は現行依存から外し、R181Dへ一本化した。これは達成ラベルを変更しない。
...
| Q4-1 | 未着手 | 完結回路設計なし | — | 2モード担体、任意の $SU(2)$ 操作、Rabi振動を具体的なアナログ回路と非理想シミュレーションへ落とす |
| Q4-2 | 未着手 | 完結回路設計なし | — | 準備、排他的結果選択、記録、状態更新、Zeno対照を同じ回路設計へ接続する |
| Q4-3 | 未着手 | 完結回路設計なし | — | 2つの単一量子ビット型担体と共同担体による結合ゲート回路を設計する |
| Q4-4 | 未着手 | 完結回路設計なし | — | 2端測定、Bell型統計、設定先行準備の因果経路を一つの回路設計へ統合する |
