...
| Q3-2 | Nelson流の作用変分または時間対称Newton則の導出 | 明示的な古典ミクロモデルの縮約から、Nelson流の作用の停留原理、または前進・後退平均加速度を対称に組み合わせたNewton則を導く。確率過程、平均微分、適用範囲、近似誤差を明示する。作用変分経路では作用汎関数、許容変分、端点条件も示す |
...
| Q3-3C | W型の束縛状態 | 1次元のW型（有限障壁を持つ二重井戸型）ポテンシャルについて有限個の非縮退低位固有状態の固有値、密度、節構造を再現し、有限環境との弱結合を縮約したエネルギー固有基底で、非対角相関の有限時間減衰と対角占有率の安定性を導く |
...
旧Q3-2「位相量子化」は意味と判定基準を保ってQ3-6へ移動した。新Q3-2は別の導出課題である。旧Q3-3はQ3-3A（井戸型）とQ3-3B（調和型）へ分割し、Q3-3C（W型）を追加した。Q3-3は系列の総称とし、独立した達成行は置かない。

#### Q3-2からQ3-6の達成判定の補足

- 新Q3-2は二つの経路の少なくとも一方を満たせばよく、両方の導出を必須にしない。対象の停留原理または時間対称Newton則をそのまま仮定することや、比較対象のSchrödinger方程式を外部から前提にして書き換えるだけでは達成としない。Schrödinger型発展をミクロから導出済みの場合も、対応する粒子確率過程と平均微分・縮約の接続を示す。位相量子化の大域条件はQ3-6で別に判定する。
...
| Q3-3C | 部分達成 | M37、M47のW型2モード解析 | R86、R140（低2モード部分） | 有限個の低位固有値・密度・節の収束と、同じW型エネルギー固有基底での有限環境純位相緩和をそろえる。R140の射影内操作だけで全達成としない |
| Q3-4 | 条件付き達成 | M54、M37、M42、M50 | R86、R124、R181A、R172--R174 | M54準備、M37担体、初期作用殻、M42輸送、終位置記録までの単一装置統合 |
...
今回の固定目標再編は方針文書を先行する。現行論文draft-69内の旧Q3-2は新Q3-6、旧Q3-3の達成は新Q3-3A・Q3-3Bに対応する。本文・研究メモ・生成原稿・PDFの参照同期は後続の論文改訂で行い、旧稿の達成表現を新Q3-2・Q3-3Cへ読み替えない。
...
6. 新Q3-2の作用変分または時間対称Newton則、Q3-3CのW型束縛状態と純位相緩和、Q3-6の位相量子化を扱う。連続空間、多粒子は別途検討する。
