...
M67/R208--R209は、この三実体Q3経路の上位Hamiltonian parent候補である。M37型coherent sectorとthermal-reservoir sectorを一つのstructured reservoirへまとめ、別実体をclassical markerだけにした二実体構成からM64 open lawを有限時間で回収する。R209A--R209Cはlocal-flow、finite-bath Markov/FDT、small-mass/process-law compatibilityを明示誤差付きで定量化する。M67はなおcandidateであり、現行Q3 fixed-goal依存やM64/M66の運用状態は変更しない。
