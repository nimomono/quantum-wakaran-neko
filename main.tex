...
\hypersetup{
  ...
  citecolor=PaperBlue,
  urlcolor=PaperBlue,
  pdftitle={構造化2成分誘導場を持つ弱開放古典 Hamiltonian 系：位相接続による Schrödinger 型力学、作用殻標本化、Bell 型統計},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{PaperBlue}2成分誘導場の位相接続と作用殻}
...
\begin{titlepage}
  ...
  {\sffamily\bfseries\color{PaperNavy}\fontsize{20}{29}\selectfont
    ...
    Schrödinger 型力学、作用殻標本化、Bell 型統計\par}
  \vspace{9mm}
  {\Large\color{PaperBlue}Nelson--Madelung 縮約、Born 型入口流束、配置拡散候補、Bell 型残余ファイバー体積\par}
  \vspace{14mm}
  {\color{PaperRule}\rule{0.68\textwidth}{0.9pt}\par}
  \vfill
  {\large draft-15 改訂稿\par}
  \vspace{22mm}
\end{titlepage}
...
\tightlist
\item
  位相接続経路は、粒子と2成分場を接続で結合し、coherent縮約多様体上で Nelson--Madelung 作用と局所 Schrödinger 型方程式を与える。
\item
  運動量結合経路は、有限誘導場の速度揺らぎから配置拡散と実在的な前後 Markov 過程へ進む候補を与える。
\item
  境界作用殻経路は、位置入口の Born 型重みと Bell 型共同統計を、Liouville 流束と作用殻体積から与える。
\end{enumerate}
...
\tightlist
\item
  有限2成分場との位相接続から、Nelson--Madelung 作用と Schrödinger 型力学を得られるか。
\item
  場強度と粒子密度の一致を定義として置かず、入口の Liouville 流束から位置の Born 型重みを得られるか。
\item
  運動量結合した有限誘導場から、実在的な前後 Markov 拡散と同じ有効作用定数を得られるか。
\item
  境界作用殻の幾何から Bell 型共同統計を得て、Bell の前提違反と非信号条件を特定できるか。
\end{enumerate}
...
\hypertarget{ux3064ux306eux7e2eux7d04ux7d4cux8def}{%
\section{3つの縮約経路}\label{ux3064ux306eux7e2eux7d04ux7d4cux8def}}
...
\hypertarget{ux56faux5b9aux90e8ux5206ux7a7aux9593ux306bux3088ux308bux5171ux5b58}{%
\section{固定部分空間による共存}\label{ux56faux5b9aux90e8ux5206ux7a7aux9593ux306bux3088ux308bux5171ux5b58}}
...
\hypertarget{ux8a66ux884cux5468ux671fux3068ux6392ux4ed6ux7684ux30c1ux30e3ux30f3ux30cdux30eb}{%
\section{試行周期と排他的チャンネル}\label{ux8a66ux884cux5468ux671fux3068ux6392ux4ed6ux7684ux30c1ux30e3ux30f3ux30cdux30eb}}
...
\tightlist
\item
  coherent活性場と2モード作用殻の準備。
\item
  全入口履歴を捨てない Liouville 流束標本化。
\item
  結果情報の暗モードまたは記録器への可逆転送。
\item
  活性場の coherent 部分空間への再埋め込み。
\item
  明反応座標の基準状態への復元。
\item
  設定、記録、garbage自由度の消去または外部への移送。
\item
  次試行の再準備。
\end{enumerate}
...
\hypertarget{ux7e2eux7d04ux591aux69d8ux4f53ux3068-nelsonmadelung-ux4f5cux7528}{%
\chapter{縮約多様体と Nelson--Madelung 作用}\label{ux7e2eux7d04ux591aux69d8ux4f53ux3068-nelsonmadelung-ux4f5cux7528}}
...
\hypertarget{ux30e2ux30fcux30c9ux6bbbux306eux7b49ux65b9ux6df7ux5408}{%
\section{2モード殻の等方混合}\label{ux30e2ux30fcux30c9ux6bbbux306eux7b49ux65b9ux6df7ux5408}}
...
\hypertarget{ux904bux52d5ux91cfux7d50ux5408ux306bux3088ux308bux914dux7f6eux62e1ux6563ux7d4cux8def}{%
\chapter{運動量結合による配置拡散経路}\label{ux904bux52d5ux91cfux7d50ux5408ux306bux3088ux308bux914dux7f6eux62e1ux6563ux7d4cux8def}}
...
\hypertarget{ux4f4dux76f8ux63a5ux7d9aux7d4cux8defux3068ux306eux4fc2ux6570ux6574ux5408}{%
\section{位相接続経路との係数整合}\label{ux4f4dux76f8ux63a5ux7d9aux7d4cux8defux3068ux306eux4fc2ux6570ux6574ux5408}}
...
\hypertarget{ux7d4cux8defux306eux5c0eux51faux72b6ux614b}{%
\section{3経路の導出状態}\label{ux7d4cux8defux306eux5c0eux51faux72b6ux614b}}
...
\hypertarget{bell-ux5074ux306eux8aa4ux5dee}{%
\section{Bell 側の誤差}\label{bell-ux5074ux306eux8aa4ux5dee}}
...
\item
  直接作用分配方向が複数なら、入口重みは一般に \(A_i\) の高いべきになる。
\item
  暗モードへ作用を直接移す結合は、2モード保存則を壊す。
\item
  運動量結合が速度揺らぎを作っても、その積分が Brown 運動へ収束するとは限らない。
\item
  \((X,P)\) が Markov でも、\(X\) だけの射影は一般に Markov ではない。
\item
  二側配置拡散から Fisher 項が得られても、時間対称 Newton 則は自動的に従わない。
\item
  残余2モードの \(U(2)\) 等方性だけでは、Bell 結果ファイバー間の質量を決めない。
\item
  固定正準接合部は Liouville 測度を保存するが、一様殻測度を単一状態から生成しない。
\item
  純粋な一方向漏れは非零の定常作用殻を準備しない。
\item
  Bell 型余弦則は、Born 則、位相量子化、一般 Tsirelson 原理を単独では導かない。
\end{enumerate}
...
\tightlist
\item
  2モード入口作用殻を、場強度に対応する局所作用 \(A_i\) と共通流束因子で偏りなく準備する。
\item
  標本化後の結果情報を保存しつつ、活性場を coherent部分空間へ再埋め込みする。
\item
  共有明反応座標、記録、garbage自由度を事後選別なしで復元し、次試行へ戻す。
\item
  coherent多様体上の縮約作用が、有限 Hamiltonian 粗視化運動を実際に支配することを示す。
\item
  節を含む位相接続と循環量子化を制御する。
\item
  運動量結合浴から Brown 極限と配置 Markov 閉鎖を導き、\(2m\nu_{\rm bath}=|\mathcal J_\phi|\) を得る。
\item
  同じ装置内で位相活性場、配置拡散浴、局所測定器、Bell 境界殻を同時に実現し、交差誤差を小さくする。
\item
  任意基底と一般測定器へ Born 型流束を拡張する。
\item
  偏った準備装置まで含めた非信号条件を示す。
\end{enumerate}
...
\hypertarget{bornux5074ux3068bellux5074ux306eux5bfeux5fdc}{%
\section{Born側とBell側の対応}\label{bornux5074ux3068bellux5074ux306eux5bfeux5fdc}}
