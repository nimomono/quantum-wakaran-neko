...
\begin{statusbox}
位置づけ：M38の2モード完全装置、M37の空間振動子網、M35の一般有限基底装置、M36のBell 二側履歴を分離する。Q1は厳密、Q2--Q4は制御された任意精度、Q9は弱結合・有限時間で達成とする。Q5とQ7は部分達成であり、未構成のM0と統一母測度を現行定理の仮定に使わない。
\end{statusbox}
...
M38とM35の任意複素基底変換は、局所位相回転と \(QQ+PP\) 交換の有限列で実装する。\(QQ+PP\) 交換は運動量間結合を含むため、M37の位置ばね網と同じハードウェアではない。\(L=2\) のQ1--Q4はM38内部で閉じ、M37との統合を達成条件へ入れない。
...
が理想Poincaré 不変測度になる。\(1,\alpha_1,\ldots,\alpha_{N_{\rm Z}}\) の有理数体上の1次独立性により、固定有限次元の角平行移動は一意エルゴード的である。Q2の2段測定は \(N_{\rm Z}=2\) の場合である。
...
有限 \(K\) 実験周期は外部セルを含む有限閉鎖Hamiltonian 系へ埋め込める。無期限運転では外部セル流が必要であり、情報と旧偏差を消去していない。従ってQ3は制御された任意精度で達成である。
...
測定窓中もRabi 項を非零に保つ。幅 \(h_j\) の有限パルスは理想測定写像から \(O(h_j)\) だけずれ、無反応込みの全履歴分布誤差は各段の条件付き全変動誤差の和以下になる。固定有限 \(N_{\rm Z}\) ごとにこの誤差と周期帰還誤差を任意に小さくできるため、Q4は有限Zeno抑制の範囲で達成である。完全凍結または \(N_{\rm Z}\to\infty\) に一様な固定性能は含まない。
...
Q2--Q4の「達成」は、任意の \(\epsilon>0\) に対して固定有限プログラムの無反応率、分布誤差、周期帰還誤差を \(\epsilon\) 未満にできることを意味する。Q4では \(N_{\rm Z}\) と測定間隔列も先に有限値へ固定する。有限資源で誤差が厳密に零であること、または \(N_{\rm Z}\to\infty\) に一様であることを意味しない。
...
第2章と第3章はM37によるQ9のミクロ導出を扱う。第4章はM38によるQ1--Q4、第5章はM35によるQ5の一般有限基底部分、第6章はM36によるQ7の条件付きBell 二側履歴、第7章は各模型の誤差と資源、第8章は達成範囲、未完成目標、反証条件を扱う。
...
\begin{statusbox}
位置づけ：M35と同じ2モード正準信号担体を使うM38を構成し、Bloch 球、共通位相不変性、任意の $SU(2)$ 操作、自律Rabi 振動、任意軸測定、永久外部記録、内部逆計算、外部セル交換 reset、固定有限回のZeno周期を接続する。Q1は厳密、Q2--Q4は無反応と装置誤差を含む制御された任意精度で達成する。M37との同一ハードウェア化は主張しない。
\end{statusbox}
...
\hypertarget{ux5b8cux5168ux5468ux671fux5b9aux7406ux3068q4ux9054ux6210ux5224ux5b9a}{%
\section{完全周期定理とQ4達成判定}\label{ux5b8cux5168ux5468ux671fux5b9aux7406ux3068q4ux9054ux6210ux5224ux5b9a}}
...
\begin{statusbox}
位置づけ：M35について、固定純粋準備・固定有限基底・一般有限Lの局所基底回路、滑らかな比較、無反応結果、測定後状態、内部逆計算、無理数回転Poincaré 写像を構成する。M38で閉じたL=2のQ1--Q4と分離し、一般有限Lの外部記録、弱開放 reset、逐次基底一般化はQ5へ残す。
\end{statusbox}
...
\begin{statusbox}
位置づけ：M38のQ1--Q4誤差、M37からM35への分布誤差、一般有限基底装置、Bell 二側履歴の性能指標を分離して集計する。Q2--Q4の達成は固定有限プログラムごとの任意精度を意味し、有限資源での零誤差、無限回測定、総エネルギー最適性を意味しない。量子ゲート型計算機の実装コストを扱うQ8は未着手である。
\end{statusbox}
...
M38はQ1--Q4を閉じるが、M37との同一ハードウェア化を含まない。M37からM35への接続は、状態方向誤差から有限基底分布誤差への数学的上界であり、同一装置の構成ではない。M0への統合、一般有限 \(L\) の完全周期、量子計算機としての規模依存性は別の課題である。
...
Q4の1回の有限観測では、信号2対、測定コア補助6対、共通制御時計1対、Rabi 駆動作用・角1対を再利用する。各測定段は独立選択器1対、補助状態交換セル6対、永久記録セル1対を使う。従って単純上界は
...
M38は1--3を構成する。Q4では1個の測定コアを使い、補助状態だけを段セルへ退避する。4はHamiltonian 流の1対1性と両立せず、要求しない。固定有限 \(N_{\rm Z}\) と \(K\) なら履歴を有限個の外部セルへ保存し、全系を有限閉鎖Hamiltonian として扱える。無期限運転では空セル流入と使用済みセル流出を持つ弱開放系として扱う。
...
\item
  M38の2段能動正準対数17と、Q4の \(10+8N_{\rm Z}\) 資源上界。
\item
...
残る課題は、強度制約下の最小測定間隔、全時間域の抑制・増速境界、総エネルギー収支、雑音耐性、M38の長期安定性、資源下界、一般有限 \(L\) の完全周期、多体系での規模依存性である。これらは各目標の検証項目であり、有限Zeno抑制を超える一般化または \(n\) 量子ビット回路の実装コストを扱うQ8の達成とは数えない。Q8は未着手である。
...
\begin{statusbox}
位置づけ：Q1--Q4はM38の範囲で達成、Q9はM37の限定範囲で達成である。Q5とQ7は部分達成、Q6とQ8は未着手、Q10--Q13は未達であり、達成済み主張の反証条件と未完成目標の合格条件を分けて示す。
\end{statusbox}

\hypertarget{ux6709ux9650zenoux52b9ux679cux306eux9054ux6210ux7bc4ux56f2q4}{%
\section{有限Zeno効果の達成範囲（Q4）}\label{ux6709ux9650zenoux52b9ux679cux306eux9054ux6210ux7bc4ux56f2q4}}

Q4は、任意に指定した有限測定回数 \(N_{\rm Z}\) と有限間隔列について達成とする。理想履歴分布、一度も反転しない確率、最終正占有率は
...
\hypertarget{ux4e00ux822cux6709ux9650lux3078ux306eux62e1ux5f35q5}{%
\section{一般有限Lへの拡張（Q5）}\label{ux4e00ux822cux6709ux9650lux3078ux306eux62e1ux5f35q5}}
...
\hypertarget{ux91cfux5b50ux30b2ux30fcux30c8ux578bux8a08ux7b97ux6a5fux306eux5b9fux88c5ux30b3ux30b9ux30c8q8}{%
\section{量子ゲート型計算機の実装コスト（Q8）}\label{ux91cfux5b50ux30b2ux30fcux30c8ux578bux8a08ux7b97ux6a5fux306eux5b9fux88c5ux30b3ux30b9ux30c8q8}}
...
\hypertarget{ux91cdux30b9ux30eaux30c3ux30c8ux5e72ux6e09q13}{%
\section{2重スリット干渉（Q13）}\label{ux91cdux30b9ux30eaux30c3ux30c8ux5e72ux6e09q13}}
...
\item
  Q4の全履歴分布が \(E_{N_{\rm Z}}^{\rm fwd}\) の全変動距離上界を超える。
\item
  Q4の前向き測定窓でRabi 項の係数が零になる。
\item
  観測中の摩擦または観測後reset だけでQ4の抑制が生じる。
\item
  無反応または反転履歴を除外してQ4の分布を再規格化する。
\item
...
Q1--Q4をM38で閉じた後の中心課題は、次の3本に分かれる。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Q5側では、M38の状態空間、操作、複数段測定、外部セル周期を一般有限 \(L\) へ拡張する。
\item
  Q7側では、集団交差相関を単一試行で読まず、余弦区間多重度と二側支持をミクロHamiltonian の解空間から作る。
\item
  Q10--Q13側では、M37の空間包絡を位相量子化、束縛状態、トンネル、局所検出へ拡張する。
\end{enumerate}
