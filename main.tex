...
Q2のM41では測定開始面の完全状態がA設定に依存するため、Bellの測定設定独立性は成立しない。Q3-2の位相量子化は未達のまま凍結する。第8章と付録Hでは、周期反応座標、対数型捕捉エントロピー座標、熱雑音、散逸、能動自由エネルギー供給を持つM45開放捕捉器を採用する。準臨界殻、位相すべり、帰還、指数位相体積を直接検査し、量子基底密度と定常Nelson則は3つの橋を仮定した条件付き結果として分離する。旧M46のcurrent transducerは現行因果模型から外し、R130--R132の再利用可能な有限次元計算だけを補助結果として保持する。第8.15節と付録Iでは、対称W型ポテンシャルの最低二modeに限定したM47を追加する。M47の複素振幅は独立場でなく実現配置--浴共同統計核のrank-one因子であり、外部 \(\lambda_{\rm prep}(t)\) による開放Hopf準備と閉鎖作用角伝播を分ける。R135とR136は閉鎖古典流と左右占有振動を厳密に与え、R137はmatching保存を仮定した条件付き閉包、R138は逆設計存在構成を与える。自然なHopf準備からmatching多様体への到達とその保存、連続空間の一様極限、多粒子、総エネルギー収支、統一M0、多量子ビットの指数資源回避は未完成である。
...
\item
  全結果について理想式、有限Hamiltonian近似、開放古典モデル、無反応、記録後の帰還誤差、反証条件を区別する。
\end{enumerate}

共通有効モデルをM42、Q1装置をM38、一般有限基底補助をM35、Q2結合ゲートをM39、Q2 Bell周期をM41、Q3ミクロ振動子網をM37、Q3有限環境純位相緩和をM43と呼ぶ。M45は、周期反応座標と対数型捕捉エントロピー座標を開放Langevin方程式で動かし、定常基底状態の初期共通原因測度を準備できるかを調べる現行補助モデルである。旧M46の三角形capacity-current模型は、複素振幅を実現配置--浴統計として読む新方針と因果律が両立しないため不採用とする。R130--R132の再利用可能な計算だけを補助結果として保持する。M47は対称W型ポテンシャルの最低二modeに限定し、共同統計から複素振幅を定義して、外部制御された開放Hopf準備と閉鎖作用角伝播を分ける現行限定モデルである。これらを1つのハードウェア、時計周期、反復母測度へまとめたM0は未完成である。
...
固定目標は再現すべき物理現象、観測結果、数学的性質を定め、達成判定は完全結果集合、対照条件、誤差、適用範囲を定める。これらは特定の現行モデルに依存させない。モデルID、内部変数、装置構成、近似、証明予定は現在の実現方針として別に管理する。現在の実現方針が成立しない場合も固定目標そのものが否定されたことにはならず、別の明示的な古典構成が同じ判定を満たしてよい。開放古典モデルは、状態変数、確率規約、駆動・散逸・雑音、エネルギー・エントロピー流、定常条件、近似、検証を明示した場合に候補とする。有限閉鎖Hamiltonian系への持ち上げはミクロな根拠を強める上位段階だが、モデル採用の必須条件とはしない。開放方程式を仮定したことと上位モデルから導出したことは区別する。

Q3-2は科学的には未達のまま凍結する。Q3-3はR123で低位束縛状態とエネルギー保存型純位相緩和、Q3-4はR124で3頂点有限障壁の障壁値未満確率移動、Q3-5はR125で最小2経路干渉を導出した。後2者はM42の固定時刻位置読出しへ接続する。従ってQ3-3からQ3-5を第1.5節の主結果表へ追加し、改訂後の固定範囲で達成と判定する。モデル非依存な達成判定は \texttt{PROJECT\_STATUS.md}、定理と個別の非主張は第7章、完全証明は付録Gに置く。
...
旧開放層M46 & 複素振幅場、三角形capacity、局所位相比較 & 場からrateを作るcurrent transducer & 不採用。R130--R132の補助計算だけを保持し、R133、R134は不採用模型内の結果として管理 \\
W型共同統計層M47 & 実現配置 \(X\)、bath作用角、W型最低二mode、外部 \(\lambda_{\rm prep}(t)\) & 開放Hopf回転軌道、閉鎖共分散回転、左右占有振動 & R135、R136は厳密。R137はmatching保存に条件付き。R138は逆設計存在構成。自然なmatching準備は未導出 \\
\end{longtable}
...
M47 & W型最低二modeの共同統計核、閉鎖作用角回転、左右占有振動 & 限定模型。閉鎖回転と振動は厳密、統計閉包はmatching保存に条件付き、自然なHopf準備は未導出 & R135--R138、第8.15節、付録I \\
\end{longtable}
...
共通モデルは、量子力学の全構造、同一ハードウェアのM0、独立同分布型有限標本統計、総エネルギー収支、一般連続空間、多粒子、多量子ビットの多項式資源構成を与えない。M41は標準的な空間分離Bell実験ではなく、R123--R125は有限モード・有限時間・固定時刻読出しの範囲を越えない。M45は定常基底状態の準備候補であり、一般の時間依存Nelson流、励起状態、複素位相を単独では導かない。M47は対称W型の最低二modeだけを扱い、M42の場またはM46のcurrent transducerをW型伝播の原因には使わない。閉鎖bath共分散の回転と、matching条件下の左右占有振動は導出するが、自然な局所Hopf装置が実現配置周辺、bath共分散、条件付きbath分布を同時にmatching多様体へ準備し、切断後も保存する定理はない。結果別の非主張、資源上の未評価量、反証条件は第7章、第8章、付録Iへ集約する。
...
付録Aは共通作用とM35、付録BはQ1完全周期と有限Zeno、付録CはQ2結合ゲート、付録DはM41、付録EはM37包絡誤差、付録Fは有限配置グラフ上の実現配置過程、付録GはQ3-3からQ3-5の証明、付録HはM45の開放自己組織化準臨界準備、付録IはM47のW型二mode共同統計と閉鎖伝播を扱う。
...
\textbf{共通構造（R112：有限配置グラフ模型のQ1--Q2--Q3共通構造）}
...
R113はM42の旧来の操作模型に属する。M47のW型共同統計模型は、この最小率または旧M46のcurrent transducerを使わず、実現配置--浴共同測度から統計核を定義する。従ってM47がR113を導出、物理化または検証したとは扱わない。今回の改訂では、R113--R121に依存する既存Q1--Q3の達成判定を維持するためM42を既存結果に限定して残す。M42を全面退役させるには、測定、共同配置、固定時刻位置読出しを共同統計模型から再導出する別の基礎改訂が必要である。
...
\begin{corollary}[R118：鋭い基準配置からの共通準備]
...
\hypertarget{m38ux306e2ux9802ux70b9ux64cdux4f5cux6e2cux5b9aux6709ux9650zenoux5468ux671f}{%
\chapter{M38の2頂点操作・測定・有限Zeno周期}\label{m38ux306e2ux9802ux70b9ux64cdux4f5cux6e2cux5b9aux6709ux9650zenoux5468ux671f}}
...
\begin{proposition}[R92：固定作用面の共通位相縮約]
...
\begin{corollary}[R94：自律2モードRabi 公式]
...
\begin{theorem}[R97：連結入力に対する厳密2値像の障害]
...
\begin{lemma}[R119：実現配置による2頂点操作・逐次測定周期]
...
\begin{theorem}[R100：2頂点複素振幅場・実現配置の弱開放完全周期]
...
実装では測定、記録、段セル交換に有限時間を要する。\(\tau_j\) は理想測定時刻間隔とし、有限幅窓をその近傍へ置く。観測開始から最後の前向き記録まで、Rabi 項の係数を零にしない。有限幅による時刻ずれと非可換な重なりは第3.16節の誤差へ入れる。
...
と進める。\(1,\alpha_1,\ldots,\alpha_{N_{\rm Z}}\) が有理数体上で1次独立なら、固定有限 \(N_{\rm Z}\) の更新角ベクトルはトーラス上の積Haar測度を持つ。履歴頻度は第3.14節の逐次重みの積になるが、独立同分布性は主張しない。
...
\begin{theorem}[R103：実現配置履歴による有限Zeno周期]
...
M47ではM37を、対称W型生成子 \(h_W\)、最低二つの古典正常mode \(\phi_0,\phi_1\)、mode分裂 \(E_1-E_0\) を供給する層としてだけ使う。M37の物理包絡 \(b\) をM47の独立実在場とはせず、\(b\) からrateを作って実現配置を動かさない。M47の統計振幅は、実現配置--浴共同測度のrank-one核として別に定義する。M37のHamiltonianと反回転項の評価は変更しない。外部 \(\lambda_{\rm prep}(t)\) による開放準備と閉鎖作用角伝播、matching受渡しの条件は第8.15節と付録Iに示す。
...
第8.15節と付録IのM47は、対称W型ポテンシャルの最低二modeについて、固有状態単独でなく重ね合わせの左右占有分布が時間振動することを解析的に扱う追加模型である。R123--R125の数値または有限グラフ証明をM47の証拠へ読み替えず、M47をQ3-4、Q3-5の既存位置読出し根拠にも使わない。

今回の改訂では新しいW型数値解析を追加しない。M47のR135、R136は古典作用角と偶奇二modeから得る解析結果であり、R137のmatching保存は条件付きのまま残す。従ってQ3-3からQ3-5の現行達成判定と根拠結果は変更しない。
...
\item
  M47について、具体的な開放Hopf--bath結合がW型最低二modeの相対作用・相対位相、実現配置周辺、bath共分散、条件付きbath分布を同じmatching多様体へ吸引し、外部 \(\lambda_{\rm prep}(t)\) の切断後に閉鎖古典流がその多様体を有限時間ほぼ保存するかを判定する。局所実現配置軌道、切断時の仕事と記憶、二mode外漏れ、一般ポテンシャルへの拡張は別の解析課題とする。
\end{enumerate}
...
\hypertarget{m47-wux578bux4e8cmode-hopfux5171ux540cux7d71ux8a08ux6a21ux578b}{%
\section{M47 W型二mode Hopf共同統計模型}\label{m47-wux578bux4e8cmode-hopfux5171ux540cux7d71ux8a08ux6a21ux578b}}
...
\hypertarget{m38ux306e2ux30e2ux30fcux30c9ux5b8cux5168ux5468ux671fux3068ux6709ux9650zenoux5468ux671f}{%
\chapter{M38の2モード完全周期と有限Zeno周期}\label{m38ux306e2ux30e2ux30fcux30c9ux5b8cux5168ux5468ux671fux3068ux6709ux9650zenoux5468ux671f}}
...
M47は対称W型ポテンシャルの最低二modeへ対象を限定し、実現配置 \(X\) とbath作用角 \(\xi\) の共同測度から統計核を定義する。外部 \(\lambda_{\rm prep}(t)\) が正の間は開放Hopf準備、零にした後は閉鎖作用角伝播とする。内部反応座標 \(s\) を準備--伝播切替のlatchには使わない。
...
\hypertarget{wux578bux4e8cmode-hopfux5171ux540cux7d71ux8a08ux6a21ux578b}{%
\chapter{W型二mode Hopf共同統計模型}\label{wux578bux4e8cmode-hopfux5171ux540cux7d71ux8a08ux6a21ux578b}}

\begin{statusbox}
位置づけ：第8.15節のM47について、実現配置--浴共同統計、外部制御された開放Hopf準備、閉鎖作用角伝播、左右占有振動、条件付き閉包、逆設計存在構成を詳述する。R130--R132から再利用する補助結果と、未導出のmatching多様体準備を区別する。
\end{statusbox}
...
\hypertarget{ux53e4ux5178wux578bux4f5cux7528ux7d20ux3068ux6700ux4f4eux4e8cmode}{%
\section{古典W型作用素と最低二mode}\label{ux53e4ux5178wux578bux4f5cux7528ux7d20ux3068ux6700ux4f4eux4e8cmode}}
...
\hypertarget{r132ux7ddaux5f62ux5171ux5206ux6563ux306eux4e3bmodeux9078ux629e}{%
\subsection{R132：線形共分散の主mode選択}\label{r132ux7ddaux5f62ux5171ux5206ux6563ux306eux4e3bmodeux9078ux629e}}
...
\begin{theorem}[R136：W型二mode共同統計の占有振動]
...
\tightlist
\item
  M45または具体的負性抵抗回路からI.5の二mode開放方程式を導くこと。
\item
  \(X\) 周辺、bath共分散、条件付きbath分布を同時に \(\mathcal M_W\) へ吸引すること。
\item
...
\item
  W型最低二modeを越える一般ポテンシャル、多mode、node、連続空間一様極限へ拡張すること。
\end{enumerate}
