...
\begin{titlepage}
  ...
  \vspace{9mm}
  {\Large\color{PaperBlue}有限 Gaussian 縮約、2モード台帳、相補的内部時計\par}
  \vspace{14mm}
...
4つの結果領域の基準質量は、台帳幾何だけでは決まらない。準備 Hamiltonian、準備巨視領域、基準 Liouville 測度が左右の符号反転に不変であるという \texttt{{[}S{]}} の下でのみ、
...
を定める追加原理を \texttt{{[}R{]}} と呼ぶ。これは「2境界統計原理」であり、時間反転共変な完成模型が得られたことを意味しない。
...
\hypertarget{ux554fux984cux8a2dux5b9a2ux90e8ux69cbux6210ux4eeeux5b9aux306eux53f0ux5e33}{%
\chapter{問題設定、2部構成、仮定の台帳}\label{ux554fux984cux8a2dux5b9a2ux90e8ux69cbux6210ux4eeeux5b9aux306eux53f0ux5e33}}
...
第1の問題には、第4章のパラメータ \(C^1\) 収束定理として答える。第2の問題には、第7章の2モード台帳定理として条件付きで答える。ただし第2の定理は、Hamilton 方程式だけから全履歴測度を導く定理ではない。物理的試行測度を定める \texttt{{[}R{]}} を明示的に入力する。
...
\hypertarget{m-2ux30e2ux30fcux30c9ux53f0ux5e33ux306eux5165ux53e3ux6e2cux5ea6}{%
\subsection{{[}M{]} 2モード台帳の入口測度}\label{m-2ux30e2ux30fcux30c9ux53f0ux5e33ux306eux5165ux53e3ux6e2cux5ea6}}
...
\tightlist
\item
  各結果領域の中で軟モードのエネルギーが一様であることは、固定総作用を持つ2モード台帳の Liouville 幾何から導く。
\item
...
\item
  比較器が差動作用 \(I_-\) を計算する。
\item
  初期相対時計運動量を \(\Pi_R=E_*\) とし、台帳変数 \(h\) と差動作用 \(\kappa I_-\) を比較する。
\item
...
第2章から第4章は、可逆な調和 Hamiltonian 中核、Gaussian 条件づけ、作用形式の定量的 Nelson 極限を扱う。第5章から第7章は、有限 Hamiltonian 装置部品、2モード台帳、2境界統計原理の下での Bell 型共同確率を扱う。第8章は適用範囲、否定結果、反証条件をまとめる。長い収束評価、正準計算、2境界照合、測定設定依存度、数値検証手順は付録と \texttt{VALIDATION.md} に置く。
...
\hypertarget{ux5171ux901aux672aux6765ux306eux6bd4ux8f03ux5668ux30682ux30e2ux30fcux30c9ux53f0ux5e33}{%
\chapter{共通未来の比較器と2モード台帳}\label{ux5171ux901aux672aux6765ux306eux6bd4ux8f03ux5668ux30682ux30e2ux30fcux30c9ux53f0ux5e33}}
...
\hypertarget{ux30e2ux30fcux30c9ux53f0ux5e33}{%
\section{2モード台帳}\label{ux30e2ux30fcux30c9ux53f0ux5e33}}
...
\hypertarget{ux4e00ux69d8ux306aux8edfux30a8ux30cdux30ebux30aeux30fcux5468ux8fbaux5b9aux7406}{%
\section{一様な軟エネルギー周辺定理}\label{ux4e00ux69d8ux306aux8edfux30a8ux30cdux30ebux30aeux30fcux5468ux8fbaux5b9aux7406}}
...
この定理は、2つの1自由度調和モードの状態密度がともに定数であることの帰結である。結果領域、測定設定、生成源位相は台帳作用殻の定義に現れないため、\texttt{{[}M{]}} の入口測度が各結果領域で共通なら
...
\(\tau_{\rm mix}\) は粗視化頻度分布の緩和、\(\tau_{\rm cmp}\) は比較器が台帳状態を読み出す前の混合窓、\(T_{\rm rec}\) は有限混合器の再帰尺度である。本論文は一般の \(K_M\) に対してこの不等式を証明しない。これは数値検証すべき \texttt{{[}M{]}} の動力学部分である。
...
\hypertarget{ux30e2ux30fcux30c9ux53f0ux5e33-bell-ux578bux6574ux5408ux6027ux5b9aux7406}{%
\section{2モード台帳 Bell 型整合性定理}\label{ux30e2ux30fcux30c9ux53f0ux5e33-bell-ux578bux6574ux5408ux6027ux5b9aux7406}}
...
\hypertarget{bell-ux524dux63d0ux306eux53f0ux5e33}{%
\section{Bell 前提の台帳}\label{bell-ux524dux63d0ux306eux53f0ux5e33}}
...
2モード台帳の一様密度、対称準備、\texttt{{[}R{]}} を用いると Bell 型余弦共同確率を得る。確率重みの原理は \texttt{{[}R{]}} であり、時計相補性や順時間的待ち時間ではない。Bell の前提違反は測定設定独立性に位置し、巨視的な測定設定頻度と対称準備での非信号性は保たれる。
...
\hypertarget{ux30e2ux30fcux30c9ux53f0ux5e33ux306eux9811ux5065ux6027}{%
\section{2モード台帳の頑健性}\label{ux30e2ux30fcux30c9ux53f0ux5e33ux306eux9811ux5065ux6027}}
...
\item
  軟エネルギーの累積分布と Bell 角依存性の高次調波を測り、2モード台帳からのずれを検査する。
\item
...
\item
  追加台帳モードを加え、予測される \(F_N(x)\) と高次調波を比較する。
\end{enumerate}
...
\hypertarget{ux53f0ux5e33ux3060ux3051ux3092ux4f7fux3046ux8868ux73fe}{%
\section{台帳だけを使う表現}\label{ux53f0ux5e33ux3060ux3051ux3092ux4f7fux3046ux8868ux73fe}}
...
とする。理想 \texttt{{[}S{]}} 集団では \(\epsilon_{\rm NS}=0\) である。偏った準備に対して \(\epsilon_{\rm NS}=O(1)\) が現れれば、対称準備だけで成り立つ模型の操作上の限界が直接検出される。
