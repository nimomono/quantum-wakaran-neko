...
\begin{titlepage}
  ...
  {\sffamily\bfseries\color{PaperNavy}\fontsize{20}{29}\selectfont
    ...
    Bell 型二側履歴\par}
  \vspace{9mm}
  {\Large\color{PaperBlue}Bloch 球、Rabi 振動、外部記録、\\[1mm]
    弱開放 reset\par}
  \vspace{14mm}
  {\color{PaperRule}\rule{0.68\textwidth}{0.9pt}\par}
  \vfill
  {\large draft-25 改訂稿\par}
  \vspace{22mm}
\end{titlepage}
...
\tightlist
\item
  M38の2モード操作、2段測定、外部記録、弱開放 reset 。
\item
  M37の局所振動子網による空間伝播。
\item
  M35の一般有限基底内部測定周期。
...
\tightlist
\item
  安全な測定後信号を新しい第2測定段へ渡す同軸または異軸逐次測定。
\item
  外部記録後に内部逆計算し、同じ能動装置で次周期を始めること。
\item
...
\tightlist
\item
  Q4とQ5側では、M38のRabi 遷移、逐次測定、外部セル周期をZeno 効果と一般有限 \(L\) へ拡張する。
\item
