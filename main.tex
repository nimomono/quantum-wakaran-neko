...
\tightlist
\item
  固定状態 \(\chi\) を準備し、\(W\chi\) を作る。
\item
  各出力作用と選択閾値を正準レジスターへ読む。
\item
  累積作用区間から、境界を除いて唯一の結果 \(k\) を選ぶ。
\item
  空テンプレートを \(e_k\) へ条件付き回転する。
\item
  信号とテンプレートを正準 SWAP する。
\item
  測定直後の信号を \(|u_k\rangle\)、内部記録を \(k\) にする。
\item
  保持窓の後に全操作を逆順に実行する。
\item
  周期末に選択器角だけを無理数回転させる。
...
\item
  有限観測窓の時間積分流束。
\item
  click、no-click、複数clickを含む完全結果集合。
\item
  理想的には各試行で唯一の局所記録。
\item
  検出網内の信号伝播速度と比較時間。
...
\tightlist
\item
  単粒子側では、Q1の局所包絡誤差と作用変動を保ったまま、M35の理想正準制御を物理的な局所装置へ置き換える。
\item
