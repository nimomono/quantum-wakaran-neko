...
Q2のM41では測定開始面の完全状態がA設定に依存するため、Bellの測定設定独立性は成立しない。Q3-2の位相量子化は未達のまま凍結する。連続空間、多粒子、総エネルギー収支、統一M0、多量子ビットの指数資源回避は未完成である。
...
共通モデルは、量子力学の全構造、同一ハードウェアのM0、独立同分布型有限標本統計、総エネルギー収支、連続空間、多粒子、多量子ビットの多項式資源構成を与えない。M41は標準的な空間分離Bell実験ではなく、R123--R125は有限モード・有限時間・固定時刻読出しの範囲を越えない。結果別の非主張、資源上の未評価量、反証条件は第7章と第8章へ集約する。
...
同一ハードウェアと反復母測度を持つM0、独立同分布型有限標本統計、総エネルギー収支、一般有限 \(L\) の完全外部周期、多量子ビットの多項式資源構成は未完成である。M41は空間分離Bell実験ではなく、Q3-2は未達のまま凍結する。R123--R125は不可逆な無限環境、散乱極限、幾何学的開口、初回到達、吸収、連続空間、多粒子を導かない。完全な境界一覧は第7章と第8章に置いた。

\hypertarget{ux6b21ux306eux6c7aux5b9aux7684ux691cux67fb}{%
\section{次の決定的検査}\label{ux6b21ux306eux6c7aux5b9aux7684ux691cux67fb}}

次は、Q2-3の指数資源を回避できる構成、R123の連続環境極限と摂動耐性、R124・R125を散乱・幾何学的開口・吸収へ拡張する際の追加仮定を独立に検査する。M41の状態的接続、準備後設定変更、空間分離も別課題として維持する。成立しない場合は対応する達成範囲を縮小し、成立する場合だけ統一M0への橋として採用する。
