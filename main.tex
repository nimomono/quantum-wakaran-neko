\documentclass[11pt,a4paper,openany]{report}

\usepackage{fontspec}
\usepackage{mathtools}
\usepackage{unicode-math}
\usepackage[inner=27mm,outer=24mm,top=26mm,bottom=25mm,headheight=23pt]{geometry}
\usepackage{xcolor}
\usepackage{hyperref}
\usepackage{fancyhdr}
\usepackage{titlesec}
\usepackage{enumitem}
\usepackage{cite}
\usepackage{amsthm}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{graphicx}

\setmainfont[
  Path=fonts/,
  BoldFont=NotoSerifJP-Bold.ttf,
  ItalicFont=NotoSerifJP-Regular.ttf,
  ItalicFeatures={FakeSlant=0.15},
  BoldItalicFont=NotoSerifJP-Bold.ttf,
  BoldItalicFeatures={FakeSlant=0.15}
]{NotoSerifJP-Regular.ttf}
\setsansfont[
  Path=fonts/,
  BoldFont=NotoSansJP-Bold.ttf,
  ItalicFont=NotoSansJP-Regular.ttf,
  ItalicFeatures={FakeSlant=0.15},
  BoldItalicFont=NotoSansJP-Bold.ttf,
  BoldItalicFeatures={FakeSlant=0.15}
]{NotoSansJP-Regular.ttf}
\setmonofont{Latin Modern Mono}
\setmathfont{Latin Modern Math}

\XeTeXlinebreaklocale "ja"
\XeTeXlinebreakskip=0pt plus 1pt
\emergencystretch=2em
\allowdisplaybreaks[2]
\numberwithin{equation}{chapter}
\setcounter{tocdepth}{2}

\definecolor{PaperNavy}{HTML}{173B57}
\definecolor{PaperBlue}{HTML}{275D78}
\definecolor{PaperRule}{HTML}{9AB4C3}
\definecolor{PaperTint}{HTML}{F2F7FA}

\hypersetup{
  unicode=true,
  colorlinks=true,
  linkcolor=PaperBlue,
  citecolor=PaperBlue,
  urlcolor=PaperBlue,
  pdftitle={構造化有限浴を持つ弱開放古典 Hamiltonian 系における Nelson 作用極限と境界作用殻測度の下での Bell 型統計},
  pdfauthor={Open research manuscript}
}

\titleformat{\chapter}[display]
  {\normalfont\sffamily\bfseries\color{PaperNavy}\centering}
  {\Large 第\thechapter 章}
  {0.8ex}
  {\Huge}
  [\vspace{1.1ex}\color{PaperRule}\titlerule]
\titleformat{name=\chapter,numberless}[display]
  {\normalfont\sffamily\bfseries\color{PaperNavy}\centering}
  {}{0pt}{\Huge}
  [\vspace{1.1ex}\color{PaperRule}\titlerule]
\titlespacing*{\chapter}{0pt}{-8pt}{28pt}

\titleformat{\section}
  {\normalfont\Large\sffamily\bfseries\color{PaperNavy}}
  {\thesection}{0.7em}{}
\titleformat{\subsection}
  {\normalfont\large\sffamily\bfseries\color{PaperBlue}}
  {\thesubsection}{0.7em}{}
\titlespacing*{\section}{0pt}{2.8ex plus 0.6ex minus 0.2ex}{1.2ex}
\titlespacing*{\subsection}{0pt}{2.3ex plus 0.5ex minus 0.2ex}{0.9ex}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{PaperBlue}構造化有限浴と境界作用殻測度}
\fancyhead[R]{\small\color{PaperBlue}\nouppercase{\leftmark}}
\fancyfoot[C]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\headrule}{\hbox to\headwidth{\color{PaperRule}\leaders\hrule height \headrulewidth\hfill}}
\renewcommand{\chaptermark}[1]{\markboth{\thechapter\quad #1}{}}

\setlength{\parindent}{1em}
\setlength{\parskip}{0.35em}
\linespread{1.08}
\clubpenalty=10000
\widowpenalty=10000
\displaywidowpenalty=10000
\setlist[itemize]{leftmargin=2.2em,itemsep=0.25em,topsep=0.45em}
\setlist[enumerate]{leftmargin=2.4em,itemsep=0.25em,topsep=0.45em}

\newenvironment{statusbox}{%
  \par\medskip\noindent\begin{center}\begin{minipage}{0.92\linewidth}%
  \color{PaperBlue}\small\sffamily\hrule height 0.6pt\vspace{0.7em}%
}{%
  \vspace{0.7em}\hrule height 0.6pt\end{minipage}\end{center}\medskip%
}

\newtheorem{theorem}{定理}[chapter]
\newtheorem{proposition}[theorem]{命題}
\newtheorem{lemma}[theorem]{補題}
\newtheorem{corollary}[theorem]{系}
\theoremstyle{definition}
\newtheorem{assumption}[theorem]{仮定}
\newtheorem{definition}[theorem]{定義}
\theoremstyle{remark}
\newtheorem{remark}[theorem]{注意}
\renewcommand{\proofname}{証明}

\newcommand{\tightlist}{\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}
\renewcommand{\contentsname}{目次}
\renewcommand{\bibname}{参考文献}
\renewcommand{\figurename}{図}
\renewcommand{\tablename}{表}

\makeatletter
\renewcommand*\@pnumwidth{2.2em}
\renewcommand*\@tocrmarg{3.2em}
\renewcommand*\l@section{\@dottedtocline{1}{1.5em}{3.2em}}
\renewcommand*\l@subsection{\@dottedtocline{2}{4.7em}{3.8em}}
\makeatother

\begin{document}

\begin{titlepage}
  \centering
  \vspace*{29mm}
  {\sffamily\bfseries\color{PaperNavy}\fontsize{21}{30}\selectfont
    構造化有限浴を持つ弱開放古典 Hamiltonian 系における\\[2mm]
    Nelson 作用極限と境界作用殻測度の下での\\[2mm]
    Bell 型統計\par}
  \vspace{9mm}
  {\Large\color{PaperBlue}線形 Gauss 型作用、U(3) 等方拡散、Bell 型残余ファイバー体積\par}
  \vspace{14mm}
  {\color{PaperRule}\rule{0.68\textwidth}{0.9pt}\par}
  \vfill
  {\large draft-12 改訂稿\par}
  \vspace{22mm}
\end{titlepage}

\pagenumbering{roman}
\tableofcontents
\clearpage
\pagenumbering{arabic}

\chapter*{概要}

\addcontentsline{toc}{chapter}{概要}

本論文は、有限自由度の古典 Hamiltonian 系を基礎としながら、実験装置を外部との微小なエネルギー交換を伴う弱開放系として扱う。対象部分と外部自由度を合わせた拡大系を

\[
H_{\rm all}
=
H_{\rm fin}(z)
+
H_{\rm ext}(y)
+
\varepsilon_{\rm ext}H_{\rm link}(z,y)
+
H_{\rm work}
\]

と書く。\(H_{\rm fin}\) は生成源、測定器、1つの構造化有限浴、記録器、境界モードを含む有限 Hamiltonian 部分、\(H_{\rm ext}\) は外部環境、\(H_{\rm work}\) は設定変更、記録消去、再準備へ仕事を供給する自由度である。有限部分の収支は

\[
\dot E_{\rm fin}
=
J_{\rm in}
-
J_{\rm out}
+
P_{\rm ctrl}
\]

であり、外部からの流入、外部への流出、制御仕事を区別して追跡する。

1試行の局所測定窓では

\[
\varepsilon_{\rm open}
=
\frac{
\sup_{0\leq t\leq T_{\rm meas}}
\left|
E_{\rm fin}(t)-E_{\rm fin}(0)
\right|
}{
E_{\rm ref}
}
\ll1
\]

を要求する。これにより、有限 Hamiltonian 部分を測定窓内では近似的に閉じた補助モデルとして扱う。一方、外部へのごく弱い漏れと流入は常時存在し、準備、記録安定化、殻幅の維持、消去、再初期化では捨てない。

論文は2つの論理的に独立な問題を扱う。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  補助的な有限 Fourier--Gauss 型確率表示から、Nelson 型作用の形式をどこまで定量的に得られるか。
\item
  1つの構造化有限浴が局所測定セクターと共通境界セクターを持つとき、境界作用殻の残余体積から Bell 型共同確率をどこまで導けるか。
\end{enumerate}

第I部では、有限分解能の前後記録で条件づけた線形 Gauss 型経路法則を考える。時間刻み \(h\) の繰り込み済み粗視化作用を \(\mathcal A_{N,h}^{R,U}\) とすると、滑らかな有限次元パラメータ集合 \(K\) 上で

\[
\left\|
\mathcal A_{N,h}^{R,U}
-
\mathcal A_{\rm GM}^{R,U}
\right\|_{C^1(K)}
\leq
C_K
\left(
\frac hT+
\frac{T^2}{Nh^2}
\right)
\]

を得る。\(h_N=TN^{-1/3}\) なら誤差は \(O(N^{-1/3})\) である。極限作用は正の Gauss 型密度領域で Guerra--Morato 表示と Nelson 表示を持つ。

これは、定義した補助表示の作用値と有限次元パラメータ微分についての厳密結果である。弱開放ミクロモデルから一般の線形表示を導くこと、微視的運動が作用の停留点を選ぶこと、任意の Nelson 停留点を有限モデルで実現すること、Nelson 表示から量子力学全体を導くことは含まれない。

第II部は第I部の Nelson 極限から Bell 重みを導かない。構造化有限浴は、時間に依存しない正準基底で

\[
\mathcal B
\simeq
\mathcal B_A^{\rm loc}
\oplus
\mathcal B_B^{\rm loc}
\oplus
\mathcal B_\partial^{\rm glob}
\oplus
\mathcal B^{\rm dark}
\]

と近似分解する。\(\mathcal B_A^{\rm loc}\) と \(\mathcal B_B^{\rm loc}\) は左右の反応座標を含む局所セクター、\(\mathcal B_\partial^{\rm glob}\) は共通未来の境界作用を担うセクター、\(\mathcal B^{\rm dark}\) は直接読み出さない暗モードである。これは複数の独立浴を置くことではなく、同じ浴の静的な明・暗モード分解である。

局所測定窓では、左右の交差応答を表す無次元量 \(\varepsilon_{\rm loc}\) が十分小さいとする。従って局所記録は \(O(\varepsilon_{\rm loc})\) の範囲で分離して扱える。左右が同じ有限浴に属することだけでは Bell 相関は生じず、共通未来の境界適合条件は残る。

局所記録後に共通未来へ運ばれる2つの実正準ベクトルを \(u_A,u_B\) とする。静的な和・差基底の作用を

\[
I_+^{AB}
=
\frac14
\left\|
u_A+u_B
\right\|^2,
\qquad
I_-^{AB}
=
\frac14
\left\|
u_A-u_B
\right\|^2
\]

と定める。固定総入力作用 \(I_+^{AB}+I_-^{AB}=2I_0\) と位相同期した生成源の下で、

\[
\overline I_-^{AB}
=
I_0
\left[
1-ABV\cos\Delta_{ab}
\right],
\]

\[
\overline I_+^{AB}
=
I_0
\left[
1+ABV\cos\Delta_{ab}
\right]
\]

となる。余弦は量子振幅ではなく、2つの実2次元ベクトルの内積から生じる。

境界セクターには3つの複素正準モード

\[
a_+,
\qquad
a_s,
\qquad
a_r
\]

を置き、それぞれの作用を \(J_\nu=|a_\nu|^2\) とする。共通総作用殻は

\[
J_+ + J_s + J_r = C_0
\]

である。結果セクターの境界適合条件は、和モードの作用を

\[
J_+
=
I_+^{AB}(a,b)
\]

へ合わせる。同じ作用殻の中で残る2モードは

\[
J_s+J_r
=
C_0-I_+^{AB}.
\]

固定 \(J_+\) の下で残余2モードだけを \(U(2)\) 等方にしても、異なる \(J_+\) のファイバー間の相対質量は決まらない。必要なのは、3モード共通殻全体の測度である。

縮約された準備力学が概念的に

\[
\partial_t f
=
D_\partial
\Delta_{S^5}f
\]

という非退化な \(U(3)\) 等方拡散を持つなら、共通作用殻上の正規化 Liouville 測度が一意な定常分布になる。この縮約方程式の下での一意性は厳密結果である。有限 Hamiltonian 浴と常時の弱い外部交換から、この生成子を一様誤差付きで導くことは予想・未解決である。

共通作用殻上では、\(J_+\) の周辺密度は

\[
p(J_+)
=
\frac{
2(C_0-J_+)
}{
C_0^2
},
\qquad
0\leq J_+\leq C_0
\]

となる。従って、同じ分解能と同じ coarea Jacobian で結果セクターを切れば、その未規格化質量は

\[
W_{AB}
\propto
C_0-I_+^{AB}.
\]

\(C_0=J_*+2I_0\) と置くと、

\[
C_0-I_+^{AB}
=
J_*+I_-^{AB}.
\]

4つの結果セクターの基準質量が左右の符号反転対称性により等しい場合、規格化された共同確率は

\[
P(A,B\mid a,b)
=
\frac14
\left[
1
-
V_{\rm eff}
AB\cos\Delta_{ab}
\right],
\]

\[
V_{\rm eff}
=
\frac{I_0}{J_*+I_0}V
\]

となる。\(J_*=0\) かつ \(V=1\) なら完全な余弦共同確率、CHSH 超過には \(V_{\rm eff}>1/\sqrt2\) が必要である。

この導出は、結果ごとに終端関数を置かず、共通作用殻上の残余ファイバー体積を比較する。旧来の Bell 固有な終端統計原理 \texttt{{[}R{]}} と中央比較器は現行モデルから除く。ただし、共通未来を含む境界条件付けそのものを除いたわけではない。全境界正準位相空間の Liouville 測度を作用保存と境界適合条件で制限し、許容履歴空間へ押し出すという2境界構造は残る。

完全な微視履歴分布は、境界適合ファイバーが \(a,b\) によって変わるため、一般に測定設定へ依存する。Bell の前提違反は測定設定独立性にあり、Bell の定理を否定しない。対称セクターでは一側周辺が \(1/2\) となり非信号性が成り立つ。

主な未解決問題は、同じ構造化有限浴から3モード全殻の等方拡散を導くこと、一般測定器を構成すること、境界適合条件を含む1試行周期を外部交換まで含めて閉じること、偏った準備での非信号性、Born 則、Tsirelson 限界の選択、Wallstrom 問題である。

\hypertarget{ux554fux984cux8a2dux5b9aux30e2ux30c7ux30ebux968eux5c64ux8a66ux884cux5468ux671f}{%
\chapter{問題設定、モデル階層、試行周期}\label{ux554fux984cux8a2dux5b9aux30e2ux30c7ux30ebux968eux5c64ux8a66ux884cux5468ux671f}}

\begin{statusbox}
位置づけ：弱開放な現行モデルと補助モデルを分け、第II部では境界作用殻の厳密な体積計算と、全殻等方拡散の未完成なミクロ導出を区別する。
\end{statusbox}

\hypertarget{ux554fux984cux8a2dux5b9a}{%
\section{問題設定}\label{ux554fux984cux8a2dux5b9a}}

Bell の定理は、局所応答、結果の確定性、測定設定独立性などを同時に満たす理論が、特定の Bell--CHSH 相関を再現できないことを示す \cite{bell1964,chsh1969}。古典 Hamiltonian モデルが Bell--CHSH 不等式を超える相関を与えるなら、Bell の前提の少なくとも1つが成立していない。本論文はこの論理を回避せず、軌道法則、準備測度、測定過程、境界条件、試行の数え方を個別に監査する。

扱う問題は次の2つである。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  補助的な有限 Fourier--Gauss 型経路法則から、Nelson 型作用の形式をどこまで定量的に得られるか。
\item
  構造化有限浴の共通作用殻と境界適合条件から、Bell 型共同確率をどこまで導けるか。
\end{enumerate}

第1の問題には、第4章のパラメータ \(C^1\) 収束定理として答える。第2の問題では、共通作用殻上の Liouville 測度を入力すれば、残余ファイバー体積と Bell 型共同確率を厳密に計算できる。さらに、縮約された \(U(3)\) 等方拡散の一意な定常分布として、その作用殻測度を力学的に準備できる。

ただし、縮約拡散を同じ有限 Hamiltonian 浴と常時の弱い外部交換から一様誤差付きで導く定理はない。従って、作用殻上の体積計算と、作用殻測度のミクロな生成を分ける。

\hypertarget{ux90e8ux306eux8ad6ux7406ux7684ux72ecux7acbux6027}{%
\section{2部の論理的独立性}\label{ux90e8ux306eux8ad6ux7406ux7684ux72ecux7acbux6027}}

第I部は、有限 Fourier--Gauss 型駆動、有限分解能の前後記録、繰り込み済み作用を扱う。中心結果は、作用値と指定した有限次元パラメータ方向の第1変分が Guerra--Morato 型 Nelson 作用へ収束するという厳密な定量定理である。

第II部は、1つの構造化有限浴、局所反応座標、静的な明・暗モード分解、共通境界3モード、作用殻拡散、境界適合ファイバーを扱う。第I部の Nelson 極限は Bell 重みを生成しない。両者に共通するのは、局所の時間発展と、完結履歴へ置く条件付き測度を同一視しないという方法である。

従って、「線形 Gauss 型確率表示から Nelson 作用形式へ進む論理鎖」と、「構造化浴の境界作用殻から Bell 型統計へ進む論理鎖」は別である。第I部の厳密結果は第II部の共通作用殻を必要とせず、第II部の Bell 計算は Nelson 作用を必要としない。

\hypertarget{ux73feux884cux30e2ux30c7ux30ebux3068ux88dcux52a9ux30e2ux30c7ux30eb}{%
\section{現行モデルと補助モデル}\label{ux73feux884cux30e2ux30c7ux30ebux3068ux88dcux52a9ux30e2ux30c7ux30eb}}

現行モデルは、有限 Hamiltonian 部分と外部自由度を弱く結合した系である。

\[
H_{\rm all}
=
H_{\rm fin}
+
H_{\rm ext}
+
\varepsilon_{\rm ext}H_{\rm link}
+
H_{\rm work}.
\]

\(H_{\rm fin}\) には生成源、設定制御器、局所装置、1つの構造化有限浴、固定指針、共通境界3モードを含める。\(H_{\rm ext}\) は記録安定化、熱交換、作用殻幅の維持、装置再準備に関わる外部環境、\(H_{\rm work}\) は設定変更、記録消去、再初期化へ仕事を供給する自由度である。

この式は、現行モデルが含むべき部分と交換経路を示す構成目標である。全ての自由項、外部結合、自律制御を1本に統合し、1周期を有限時間で実行する完成 Hamiltonian をすでに与えたという意味ではない。

現行モデルに対して、次の補助モデルを用いる。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
補助モデル
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
役割
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
現行モデルとの関係
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
1試行内の有限閉鎖系 & 正準写像、保存量、位相体積を厳密に計算する & 弱開放結合を短い測定窓で無視した近似 \\
線形 Gauss 型確率表示 & 第I部の作用収束を証明する & 弱結合、線形化、時間尺度分離後の候補表示 \\
最小結果符号化器 & 既存の2値符号を固定指針へ写す & 一般測定器の一部だけを抽出 \\
3モード固定作用殻 & 境界適合ファイバーの体積を厳密に計算する & 弱開放系の狭い準定常殻を固定殻で近似 \\
等方殻拡散 & 共通作用殻上の一意な定常測度を与える & 有限浴と外部交換を縮約した候補方程式 \\
残余2モードの \(U(2)\) 混合 & ファイバー内部の一様作用分配を示す & 全殻測度には不足することを示す比較モデル \\
\end{longtable}

閉鎖補助モデルの厳密結果を、そのまま弱開放な現行モデルの厳密結果と呼ばない。両者の接続には、交換エネルギー、観測時間、浴相関時間、殻幅、有限分解能、局所セクター間の交差応答を用いた近似評価が必要である。

\hypertarget{ux8a66ux884cux5185ux306eux8fd1ux4f3cux9589ux9396ux7a93}{%
\section{1試行内の近似閉鎖窓}\label{ux8a66ux884cux5185ux306eux8fd1ux4f3cux9589ux9396ux7a93}}

1試行の局所測定窓では、有限 Hamiltonian 部分の位相点を \(z\)、標準シンプレクティック行列を \(J\) と書き、

\[
\dot z
=
J\nabla H_{\rm fin}(z)
\]

を0次近似として用いる。各部品は滑らかな Hamiltonian 生成子を持ち、閉鎖補助モデルでは位相体積を保存する。

外部交換の小さい量を

\[
\varepsilon_{\rm open}
=
\frac{
\sup_{0\leq t\leq T_{\rm meas}}
\left|
E_{\rm fin}(t)-E_{\rm fin}(0)
\right|
}{
E_{\rm ref}
}
\]

とする。適用条件は

\[
\varepsilon_{\rm open}\ll1,
\qquad
T_{\rm meas}\ll\tau_{\rm exch}
\]

である。\(\tau_{\rm exch}\) は外部との交換により中心的な作用または記録領域が変化する時間尺度である。

記録については

\[
\tau_{\rm meas}
\ll
\tau_{\rm record}
\ll
T_{\rm rec}
\]

も要求する。\(\tau_{\rm meas}\) は記録形成時間、\(\tau_{\rm record}\) は必要な保持時間、\(T_{\rm rec}\) は閉鎖有限浴の再帰尺度である。現行モデルでは外部への漏れを常時完全には切らず、記録形成を乱さない強さに保ちながら、長時間の再位相化を抑える。

同じ浴の左右局所セクターを分離して扱うため、

\[
\varepsilon_{\rm loc}
=
\frac{
\displaystyle
\sup_{0\leq t\leq T_{\rm meas}}
\max\left(
\|\chi_{AB}(t)\|,
\|\chi_{BA}(t)\|
\right)
}{
\displaystyle
\sup_{0\leq t\leq T_{\rm meas}}
\min\left(
\|\chi_{AA}(t)\|,
\|\chi_{BB}(t)\|
\right)
}
\ll1
\]

を要求する。\(\chi_{XY}\) は反応座標 \(Y\) への摂動が反応座標 \(X\) に現れる線形応答核である。基底ベクトルが直交するだけでは十分でなく、有限時間応答核そのものが小さい必要がある。

\hypertarget{ux30a8ux30cdux30ebux30aeux30fcux4ea4ux63dbux3068ux8a66ux884cux5468ux671f}{%
\section{エネルギー交換と試行周期}\label{ux30a8ux30cdux30ebux30aeux30fcux4ea4ux63dbux3068ux8a66ux884cux5468ux671f}}

有限 Hamiltonian 部分のエネルギー収支を

\[
\Delta E_{\rm fin}
=
W_{\rm src}
+
W_{\rm ctrl}
+
W_{\rm reset}
+
Q_{\rm in}
-
Q_{\rm out}
\]

と分ける。各項の意味は次である。

\begin{itemize}
\tightlist
\item
  \(W_{\rm src}\)：生成源と位相基準を準備する仕事。
\item
  \(W_{\rm ctrl}\)：設定制御器と局所記録器へ供給する仕事。
\item
  \(W_{\rm reset}\)：指針、反応座標、境界モードを再準備する仕事。
\item
  \(Q_{\rm in}\)：外部揺らぎと作用殻の半径方向安定化による流入。
\item
  \(Q_{\rm out}\)：局所浴、暗モード、外部環境への流出。
\end{itemize}

閉鎖補助モデルでは、これらは拡大した時計または仕事貯蔵自由度との内部交換として表せる。現行モデルでは、実験室の外部供給と排熱まで含めて追跡する。

繰り返し試行は次の5段階からなる。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{殻準備}：境界3モードを狭い総作用分布へ置き、殻接方向の浴拡散で偏りを緩和する。
\item
  \textbf{生成源と設定準備}：位相同期した伝達ベクトル対と左右の設定制御器を準備する。
\item
  \textbf{局所測定と記録}：左右の局所反応座標を介して結果を形成し、固定指針へ記録する。
\item
  \textbf{境界適合}：記録と伝達ベクトルを共通未来へ運び、静的な和・差基底と共通作用殻の条件に適合する履歴を定める。
\item
  \textbf{消去と再初期化}：記録を消去し、局所セクター、境界セクター、仕事貯蔵系を次の入口巨視状態へ戻す。
\end{enumerate}

記録消去と再設定は一般に外部へのエントロピーとエネルギーの移送を伴う \cite{landauer1961,bennett1982}。確率的リセットと有限環境への散逸に関する既知の結果は、試行周期を構成するときの比較対象になる \cite{fuchs_goldt_seifert2016,evans_majumdar_schehr2020,marchiori_deaguiar2011}。

純粋な一方向漏れは殻半径を縮めるだけで、殻上の等方分布を作らない。必要なのは、殻接方向の非退化な混合と、半径方向の漏れ・流入の釣り合いである。これらを同じ有限浴と外部結合から導く部分は未完成である。

\hypertarget{ux7b2ciiux90e8ux306eux6761ux4ef6}{%
\section{第II部の条件}\label{ux7b2ciiux90e8ux306eux6761ux4ef6}}

第II部の条件を次の5つに分ける。

\hypertarget{h-ux69cbux9020ux5316ux6709ux9650ux6d74}{%
\subsection{{[}H{]} 構造化有限浴}\label{h-ux69cbux9020ux5316ux6709ux9650ux6d74}}

生成源、設定制御器、局所記録器、境界3モード、有限浴を有限個の正準対と滑らかな Hamiltonian 生成子で記述する。浴の明・暗モード分解、左右の反応座標、共通境界セクターは時間に依存しない同じ正準基底で定める。局所測定窓では \(\varepsilon_{\rm loc}\ll1\) とする。

現在の明示装置は、あらかじめ存在する2値の結果種を指針へ写す最小結果符号化器である。一般の

\[
A=\mathscr A(a,\lambda_A),
\qquad
B=\mathscr B(b,\lambda_B)
\]

を到来信号との相互作用から形成する装置は未構成である。

\hypertarget{p-ux4f4dux76f8ux540cux671fux3057ux305fux751fux6210ux6e90}{%
\subsection{{[}P{]} 位相同期した生成源}\label{p-ux4f4dux76f8ux540cux671fux3057ux305fux751fux6210ux6e90}}

左右へ送る2つの実2次元伝達ベクトルは、共通の位相基準と固定した総入力作用を持つ。和・差作用は

\[
I_+^{AB}+I_-^{AB}
=
2I_0
\]

を満たす。有限の位相雑音は可視度 \(V\) に含める。総入力作用の揺らぎは作用殻幅の誤差へ含め、中心定理では固定する。

\hypertarget{s-ux5bfeux79f0ux306aux7d50ux679cux30bbux30afux30bfux30fc}{%
\subsection{{[}S{]} 対称な結果セクター}\label{s-ux5bfeux79f0ux306aux7d50ux679cux30bbux30afux30bfux30fc}}

境界条件を課す前の準備 Hamiltonian、準備巨視領域、基準 Liouville 測度は、左右の結果種を独立に反転する2つの変換に不変である。4つの結果セクターが1つの軌道に沿って相互遷移する必要はない。必要なのは、共通殻を切る前の基準質量、境界分解能、coarea Jacobian が4セクターで等しいことである。

\hypertarget{c-ux5171ux901aux4f5cux7528ux6bbbux3068ux5883ux754cux9069ux5408}{%
\subsection{{[}C{]} 共通作用殻と境界適合}\label{c-ux5171ux901aux4f5cux7528ux6bbbux3068ux5883ux754cux9069ux5408}}

境界3モードは

\[
J_++J_s+J_r=C_0
\]

という1つの共通作用殻を持つ。局所記録後の伝達ベクトルを静的な和・差基底へ写し、各結果セクターで

\[
J_+=I_+^{AB}(a,b)
\]

を同じ物理的分解能で要求する。設定名や目標確率を直接引数に持つ結果別終端関数は置かない。

この条件は、共通未来を含む境界値問題を残す。共有浴だけで境界条件が不要になるわけではない。

\hypertarget{d-ux5168ux6bbbux7b49ux65b9ux62e1ux6563}{%
\subsection{{[}D{]} 全殻等方拡散}\label{d-ux5168ux6bbbux7b49ux65b9ux62e1ux6563}}

準備窓の縮約生成子は、3モード共通殻の接方向で非退化かつ \(U(3)\) 不変である。理想化した生成子を

\[
\mathcal L_{\rm iso}
=
D_\partial\Delta_{S^5}
\]

とする。準備時間 \(\tau_{\rm prep}\) は混合時間より十分長く、半径方向の殻幅は

\[
\varepsilon_C
=
\frac{\sigma_C}{C_0}
\ll1
\]

を満たす。

この条件から共通殻上の正規化 Liouville 測度が一意な定常分布として従う。従って旧来の入口測度条件 \texttt{{[}M{]}} は、縮約拡散の定常結果 \texttt{{[}M\_\{\textbackslash{}rm\ shell\}{]}} へ置き換える。ただし、\texttt{{[}D{]}} 自体を同じ有限浴から導く部分は予想・未解決である。

旧来の Bell 固有な終端統計原理 \texttt{{[}R{]}} は用いない。Bell 重みは \texttt{{[}C{]}} と \texttt{{[}M\_\{\textbackslash{}rm\ shell\}{]}} の下で残余ファイバー体積から得る。

\hypertarget{ux8a66ux884cux306eux7269ux7406ux7684ux6642ux7cfbux5217}{%
\section{1試行の物理的時系列}\label{ux8a66ux884cux306eux7269ux7406ux7684ux6642ux7cfbux5217}}

1試行を次の9段階に分ける。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  境界3モードの総作用を \(C_0\) 付近の狭い分布へ準備する。
\item
  浴由来の殻接方向拡散を混合時間より長く作用させる。
\item
  位相同期した伝達ベクトル対と局所装置を準備する。
\item
  左右の制御器巨視状態により設定 \(a,b\) を生成する。
\item
  到来変数、設定、局所装置微視状態の相互作用で結果を形成する。現在の明示計算では、この段階を結果種の入力で置き換える。
\item
  局所反応座標と固定指針が結果を記録する。左右の交差応答は \(O(\varepsilon_{\rm loc})\) である。
\item
  記録と伝達ベクトルを通常の時間順序で共通未来へ運び、静的な和・差基底へ写す。
\item
  共通総作用殻と \(J_+=I_+^{AB}\) の境界適合条件により、完結履歴のファイバーを定める。
\item
  完了後に記録、浴、境界モードを次試行の入口巨視状態へ戻す。
\end{enumerate}

境界条件に適合しない軌道は、記録後に捨てた試行とは定義しない。最初から同じ2境界値問題の解ではない。ただし、実験装置がこの集団を実現する際に開始数、記録数、完了数が結果または設定に依存して減るなら、操作上は事後選別になる。この監査は省略しない。

\hypertarget{ux4e3bux8981ux7d50ux679cux306eux5c0eux51faux72b6ux614b}{%
\section{主要結果の導出状態}\label{ux4e3bux8981ux7d50ux679cux306eux5c0eux51faux72b6ux614b}}

第I部の結果を分ける。

\begin{longtable}[]{@{}ll@{}}
\toprule\noalign{}
主張 & 導出状態 \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
線形 Gauss 型条件付き作用の \(C^1\) 収束 & 厳密結果 \\
Guerra--Morato 表示と Nelson 表示の一致 & 厳密結果 \\
弱開放ミクロモデルから線形表示への縮約 & 予想・未解決 \\
微視的運動による停留点選択 & 予想・未解決 \\
\end{longtable}

第II部の結果を分ける。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
主張
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
導出状態
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
実2次元和・差作用の余弦恒等式 & 厳密結果 \\
\(U(2)\) 等方性だけではファイバー間質量が決まらないこと & 厳密結果。否定的結果 \\
\(U(3)\) 等方拡散の定常分布が共通殻 Haar 測度になること & 縮約方程式の下で厳密結果。仮説依存 \\
3モード共通殻の \(J_+\) 周辺と残余ファイバー体積 & 厳密結果 \\
共通殻、対称セクター、共通分解能の下での Bell 型共同確率 & 厳密結果。仮説依存 \\
弱開放な有限浴から全殻等方拡散を導くこと & 予想・未解決 \\
異方性、混合不足、殻幅、交差応答による補正 & 近似結果 \\
一般測定器と境界適合を含む全試行周期 & 予想・未解決 \\
\end{longtable}

固定された損失のない接合部は Liouville 測度を保存するが生成せず、残余2モードの \(U(2)\) 混合は固定 \(J_+\) の内部しか一様化しない。純粋な漏れも殻上等方化を与えない。これらは現行モデルの成立条件を限定する否定的結果として本文に残す。

\hypertarget{bell-ux306eux524dux63d0ux76e3ux67fb}{%
\section{Bell の前提監査}\label{bell-ux306eux524dux63d0ux76e3ux67fb}}

第II部の監査結果を先に要約する。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
監査項目
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
現時点の結論
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
局所性 & 局所記録窓の左右交差応答は \(O(\varepsilon_{\rm loc})\)。共通未来の境界セクターは残る \\
測定設定独立性 & 境界適合ファイバーが \(a,b\) に依存するため成立しない \\
結果の一意性 & 最小結果符号化器では一意。一般測定器は未構成 \\
事後選別 & 理論上は2境界解集合を定義するが、装置実現では開始数・記録数・完了数の監査が必要 \\
非信号性 & 対称セクターと共通分解能の下で厳密。偏った準備では未証明 \\
試行測度 & 共通殻の Liouville 測度を履歴空間へ押し出す。浴からのミクロ導出は未完成 \\
\end{longtable}

従って Bell の定理の前提違反は測定設定独立性にある。局所 Hamilton 方程式へ遠隔力を加えたことや、Bell の定理を否定したことによるものではない。

\hypertarget{ux5148ux884cux7814ux7a76ux3068ux306eux4f4dux7f6eux95a2ux4fc2}{%
\section{先行研究との位置関係}\label{ux5148ux884cux7814ux7a76ux3068ux306eux4f4dux7f6eux95a2ux4fc2}}

Nelson の確率力学、確率変分法、Guerra--Morato 作用には先行研究がある \cite{nelson1966,guerra_morato1983,yasue1981,zambrini1986,knorst_lopes2024}。Gauss 型条件づけは固定区間平滑化、相反過程、Schrödinger 橋、経路単位の Gauss 型条件づけと関係する \cite{jamison1974,doob1957,leonard2014,chen_georgiou_pavon2016,rauch_tung_striebel1965,wilson_et_al2021,leonard_roelly_zambrini2014}。第I部の新規性は、指定した線形 Gauss 型クラスに対し、有限 Fourier 表示から繰り込み作用のパラメータ \(C^1\) 極限を明示率付きで与える点に限定される。

2境界条件、局所逆因果モデル、測定設定独立性を緩めた Bell モデルにも先行研究がある \cite{wharton2010,wharton_argaman2020,hall2010,leifer_pusey2017,wood_spekkens2015,price_wharton2023,price_wharton2024,argaman2010,hossenfelder_palmer2020,thooft2016}。共通未来と境界制約による設定依存履歴測度という発想自体は新規ではない。第II部が追加するのは、1つの構造化有限浴の局所反応座標と共通境界セクターを分け、3モード共通作用殻の残余ファイバー体積を Bell 重みへ接続し、2モード等方性の不足と全殻等方拡散の未導出性を明示する点である。

\hypertarget{ux672cux8ad6ux6587ux304cux4e3bux5f35ux3057ux306aux3044ux3053ux3068}{%
\section{本論文が主張しないこと}\label{ux672cux8ad6ux6587ux304cux4e3bux5f35ux3057ux306aux3044ux3053ux3068}}

本論文は次を主張しない。

\begin{itemize}
\tightlist
\item
  Bell の全仮定を保った古典モデルによる Bell 不等式の破れ。
\item
  共有浴にしただけで共通未来の境界条件が不要になること。
\item
  全殻 \(U(3)\) 等方拡散が有限 Hamiltonian 浴だけから厳密に導かれたこと。
\item
  純粋なエネルギー漏れや固定された損失のない接合部が一様殻測度を生成すること。
\item
  残余2モードの \(U(2)\) 等方性だけで Bell 重みが決まること。
\item
  局所装置が一般の設定依存結果を到来信号から生成する完成測定器であること。
\item
  全装置、外部自由度、仕事貯蔵系を1本に統合した有限幅 Hamiltonian が完成していること。
\item
  弱開放ミクロモデルから第I部の一般線形確率表示が導出済みであること。
\item
  任意の偏った準備に対する非信号性。
\item
  余弦則または Tsirelson 限界が追加原理から一意に選ばれること。
\item
  Bell 型共同確率が Born 則または Wallstrom 問題を解くこと。
\item
  第I部の作用収束が微視的な停留点選択または量子力学全体を導くこと。
\end{itemize}

\hypertarget{ux8ad6ux6587ux306eux69cbux6210}{%
\section{論文の構成}\label{ux8ad6ux6587ux306eux69cbux6210}}

第2章から第4章は、弱開放な現行モデルと閉鎖補助モデルの関係、Gauss 型条件づけ、作用形式の定量的 Nelson 極限を扱う。第5章は1つの構造化有限浴、静的な明・暗モード分解、局所反応座標、共通未来の境界セクターを扱う。第6章は3モード共通作用殻、\(U(2)\) の限界、\(U(3)\) 等方拡散、残余ファイバー体積を導く。第7章は境界 Liouville 測度の履歴空間への押し出し、Bell 型共同確率、非信号性、Bell の前提監査を扱う。第8章は近似誤差、適用限界、反証条件、未解決問題をまとめる。主定理の収束評価と、本文の構造化浴・作用殻計算を支える正準計算だけを付録に置き、数値検算は \texttt{VALIDATION.md} に集約する。

\part{補助的な線形 Gauss 型確率表示の Nelson 作用極限}

\hypertarget{ux6709ux9650-hamiltonian-ux90e8ux5206ux3068ux7ddaux5f62-gauss-ux578bux78baux7387ux8868ux793a}{%
\chapter{有限 Hamiltonian 部分と線形 Gauss 型確率表示}\label{ux6709ux9650-hamiltonian-ux90e8ux5206ux3068ux7ddaux5f62-gauss-ux578bux78baux7387ux8868ux793a}}

\begin{statusbox}
位置づけ：弱開放な現行モデル、閉鎖調和補助モデル、証明用の線形 Gauss 型確率表示を分離する。
\end{statusbox}

\hypertarget{ux73feux884cux30e2ux30c7ux30ebux3068ux6709ux96502ux6b21ux88dcux52a9ux30e2ux30c7ux30eb}{%
\section{現行モデルと有限2次補助モデル}\label{ux73feux884cux30e2ux30c7ux30ebux3068ux6709ux96502ux6b21ux88dcux52a9ux30e2ux30c7ux30eb}}

現行モデルでは、観測対象と装置を含む有限 Hamiltonian 部分 \(z\) を、外部自由度 \(y\) と弱く結合する。

\[
H_{\rm all}(z,y)
=
H_{\rm fin}(z)
+
H_{\rm ext}(y)
+
\varepsilon_{\rm ext}H_{\rm link}(z,y)
+
H_{\rm work}.
\]

\(H_{\rm fin}\) だけのエネルギーを \(E_{\rm fin}=H_{\rm fin}\) とすると、

\[
\dot E_{\rm fin}
=
\varepsilon_{\rm ext}
\{H_{\rm fin},H_{\rm link}\}
+
P_{\rm ctrl}
\]

である。第1項は外部との流入または流出、第2項は仕事貯蔵系と自律時計からの制御仕事である。従って、有限 Hamiltonian 部分は現行モデルの中で厳密な保存系ではない。

第I部の厳密計算では、観測窓 \(0\leq t\leq T\) における

\[
\varepsilon_{\rm open}
=
\frac{
\sup_{0\leq t\leq T}
\left|
E_{\rm fin}(t)-E_{\rm fin}(0)
\right|
}{
E_{\rm ref}
}
\ll1
\]

を仮定し、\(\varepsilon_{\rm ext}=0\) とした閉鎖有限系を補助モデルとして用いる。この補助モデルへの置換は、中心的な作用または記録領域が変わる交換時間 \(\tau_{\rm exch}\) に対して \(T/\tau_{\rm exch}\ll1\) のときに限る。

さらに、作用収束定理は一般の弱開放ミクロモデルを直接扱わず、線形化後の Gauss 型経路法則を対象とする。従って本章では、次の3層を混同しない。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  有限 Hamiltonian 部分を持つ弱開放な現行モデル。
\item
  1試行内で用いる閉鎖有限系の補助モデル。
\item
  第4章の証明に用いる線形 Gauss 型確率表示。
\end{enumerate}

閉鎖補助モデルの位相空間を \(\mathbb{R}^{2M}\)、正準座標を \(Z=(Q,P)\) とし、

\[
H_N(Z)=\frac12 Z^{\mathsf T}G_N Z,
\qquad
G_N=G_N^{\mathsf T}>0
\]

を考える。運動方程式は

\[
\dot Z=JG_NZ,
\qquad
Z(t)=e^{tJG_N}Z(0)
\]

である。\(G_N\) が運動量に関して偶であれば、標準時間反転 \(\Theta(Q,P)=(Q,-P)\) に対して

\[
\Theta e^{tJG_N}\Theta=e^{-tJG_N}
\]

が成立する。従って全微視軌道は時間反転対称であり、Liouville 体積を保存する。

正準変換で正規モードへ移れば、安定な部分は

\[
H_N
=
\sum_{n=1}^{N}
\frac12
\left(
P_n^2+\omega_n^2Q_n^2
\right)
\]

の形にできる。初期正準変数が中心 Gauss 分布を持つなら、任意の線形観測量

\[
X_N(t)=L e^{tJG_N}Z(0)+\mu_N(t)
\]

は有限次元 Gauss 型過程である。従って、閉鎖調和補助モデルの線形観測座標は、平均と2時刻共分散だけで完全に記述できる。

\hypertarget{ux53cdux4f5cux7528ux3092ux542bux3080ux6a19ux6e96ux7684ux306aux8abfux548cux6d74}{%
\section{反作用を含む標準的な調和浴}\label{ux53cdux4f5cux7528ux3092ux542bux3080ux6a19ux6e96ux7684ux306aux8abfux548cux6d74}}

粒子座標 \(q\) と調和浴を明示する代表例は

\[
H_{\rm core}
=
\frac{p^2}{2m}+V(q)
+
\sum_{n=1}^{N}
\left[
\frac{P_n^2}{2m_n}
+
\frac{m_n\omega_n^2}{2}
\left(
Q_n-\frac{c_nq}{m_n\omega_n^2}
\right)^2
\right]
\]

である \cite{ford1965,mori1965,zwanzig1973}。平方完成された結合は反作用と周波数補正を同時に含む。浴変数を厳密に消去すると、粒子は有限記憶核を持つ一般化 Langevin 方程式に従う。

\[
m\ddot q(t)
+V'(q(t))
+\int_0^t\Gamma_N(t-s)\dot q(s)\,\mathrm{d} s
=
\xi_N(t)+F_{\rm slip}(t).
\]

ここで

\[
\Gamma_N(t)
=
\sum_{n=1}^{N}
\frac{c_n^2}{m_n\omega_n^2}
\cos\omega_nt
\]

であり、\(\xi_N\) は浴初期座標の線形結合である。浴初期分布が Gauss 分布なら \(\xi_N\) も有限 Gauss 型過程になる。有限 \(N\) では記憶核も雑音も再帰的であり、白色雑音や散逸は閉鎖補助モデルの基本法則ではない。

本論文の \(C^1\) 定理は、この一般化 Langevin 方程式を任意の非線形 \(V\) について直接扱うものではない。調和領域または線形化領域で観測される Gauss 型経路法則を、次節の有限 Fourier 表示で計算する。従って、閉鎖調和系は微視的な可逆性を検査する補助モデル、線形 Gauss 型表示は観測法則を計算する補助モデルである。

弱開放な現行モデルからこの表示へ進むには、少なくとも弱結合、線形化、外部相関時間と観測時間の分離を要する。この縮約の一様誤差評価は本論文では完了していない。

\hypertarget{ux5b8cux5168ux306aux6709ux9650-fourier-ux6d74}{%
\section{完全な有限 Fourier 浴}\label{ux5b8cux5168ux306aux6709ux9650-fourier-ux6d74}}

時間区間を \([0,T]\)、\(\omega_n=2\pi n/T\) とする。独立な標準 Gauss 型ベクトル \(Z_0,A_n,B_n\in\mathbb{R}^d\) を用いて

\[
\widetilde\eta_N(t)
=
\sqrt{\frac{2\nu}{T}}Z_0
+
\sqrt{\frac{4\nu}{T}}
\sum_{n=1}^{N}
\left[
A_n\cos\omega_nt
+B_n\sin\omega_nt
\right]
\]

と定義する。この過程は調和正規モードの初期振幅を読み出すことで実現できる。零周波数 \(Z_0\) は保存された正準運動量または自由モードに対応する。

共分散は

\[
\mathbb{E}\left[
\widetilde\eta_N^i(t)
\widetilde\eta_N^j(s)
\right]
=
2\nu\,\delta^{ij}\delta_{T,N}(t-s),
\]

\[
\delta_{T,N}(\tau)
=
\frac1T
+
\frac2T
\sum_{n=1}^{N}\cos\omega_n\tau
\]

である。\(\delta_{T,N}\) は周期 Dirichlet 核であり、滑らかな試験関数に対して周期デルタ分布へ収束する。

零周波数を最初から除いた

\[
2\nu
\left[
\delta_{T,N}(t-s)-\frac1T
\right]
\]

を普遍的な浴共分散とみなしてはならない。これは全ての線形系に共通な浴ではなく、自由増分の全期間積分を零にする条件を課したときに現れる特殊な条件付き共分散である。一般の線形な流れでは、終端条件による共分散修正は流れと観測行列に依存する Schur 補完になる。

\hypertarget{ux8a3cux660eux7528ux306eux7ddaux5f62-gauss-ux578bux78baux7387ux8868ux793a}{%
\section{証明用の線形 Gauss 型確率表示}\label{ux8a3cux660eux7528ux306eux7ddaux5f62-gauss-ux578bux78baux7387ux8868ux793a}}

実際の証明では、観測座標の確率法則を

\[
\dot X_N(t)
=
F_\theta(t)X_N(t)
+f_\theta(t)
+\widetilde\eta_N(t),
\qquad
X_N(0)\sim N(m_{0,\theta},P_{0,\theta})
\]

で表す。\(\theta\) は質量、周波数、外力、終端記録などをまとめた有限次元パラメータである。\(F_\theta\) と \(f_\theta\) は時間について十分滑らかとする。

基本行列 \(\Phi_\theta(t,s)\) を

\[
\partial_t\Phi_\theta(t,s)
=
F_\theta(t)\Phi_\theta(t,s),
\qquad
\Phi_\theta(s,s)=I
\]

で定めると、

\[
X_N(t)
=
\Phi_\theta(t,0)X_N(0)
+\int_0^t
\Phi_\theta(t,s)
\left[
f_\theta(s)+\widetilde\eta_N(s)
\right]
\,\mathrm{d} s
\]

である。従って \(X_N\) は有限個の Gauss 変数の線形像であり、平均 \(\mu_N\) と共分散 \(C_N\) を有限和として厳密に計算できる。

この方程式は、有限 \(N\) の平均と共分散を計算するための補助的な確率表示である。\(\widetilde\eta_N\) 自体は有限 Hamiltonian 正規モードの初期振幅から作れるが、任意の時間依存係数 \(F_\theta(t)\)、\(f_\theta(t)\) を含む上式全体が、1つの有限自律 Hamiltonian の観測座標として実現されることまでは示さない。

従って、第4章の定理が直接扱うのはこの線形 Gauss 型確率表示のクラスである。特定の有限 Hamiltonian モデルと平均・共分散が一致する場合には同じ作用計算を移せるが、一般の時間依存線形系に対する Hamiltonian 埋め込みを定理の結論へ含めない。この区別により、「有限 Hamiltonian 部分を持つ現行モデル」と「証明用確率表示の Hamiltonian 実現」を混同しない。

\hypertarget{ux6975ux9650ux62e1ux6563}{%
\section{極限拡散}\label{ux6975ux9650ux62e1ux6563}}

\(N\to\infty\) で積分雑音

\[
W_N(t)=\int_0^t\widetilde\eta_N(s)\,\mathrm{d} s
\]

は、有限次元分布で共分散 \(2\nu\min(s,t)\) を持つ Wiener 増分へ近づく。本論文の作用とパラメータ第1微分は2時刻の平均・共分散だけで評価するため、一般の経路空間位相における弱収束は主定理の仮定にも結論にも用いない。対応する線形拡散表示は

\[
\,\mathrm{d} X(t)
=
\left[
F_\theta(t)X(t)+f_\theta(t)
\right]\,\mathrm{d} t
+\sqrt{2\nu}\,\,\mathrm{d} W_t
\]

である。有限 \(N\) の各経路は微分可能であるが、極限経路は微分不可能である。粗視化作用に現れる発散は、この正則性の変化に由来する。

\hypertarget{ux672cux7ae0ux306eux7d50ux8ad6}{%
\section{本章の結論}\label{ux672cux7ae0ux306eux7d50ux8ad6}}

現行モデルは、有限 Hamiltonian 部分を持つ弱開放系である。閉鎖調和系は1試行内の可逆性、正準構造、再帰を検査する補助モデルであり、第4章の証明にはさらに線形 Gauss 型確率表示を用いる。

厳密に確立したのは、補助表示内部の平均、共分散、有限 Fourier 近似である。弱開放ミクロモデルから一般の線形表示を導くこと、白色雑音近似と Markov 近似の誤差を現行モデルの尺度で閉じることは未解決である。終端条件は浴そのものではなく Gauss 型条件づけとして導入する。次章では、その数学的条件づけと、物理的試行測度の選択を分けて記述する。第II部の構造化浴と境界作用殻は別の補助構成であり、本章の線形表示から導かない。

\hypertarget{ux524dux5f8cux4e21ux5074ux304bux3089ux6761ux4ef6ux3065ux3051ux305fux7ddaux5f62-gauss-ux578bux7d4cux8defux6cd5ux5247}{%
\chapter{前後両側から条件づけた線形 Gauss 型経路法則}\label{ux524dux5f8cux4e21ux5074ux304bux3089ux6761ux4ef6ux3065ux3051ux305fux7ddaux5f62-gauss-ux578bux7d4cux8defux6cd5ux5247}}

\begin{statusbox}
位置づけ：有限分解能の終端記録を Gauss 型 Schur 補完として厳密に定義し、物理的試行測度とは分離する。
\end{statusbox}

\hypertarget{ux306aux305cux6709ux9650ux5206ux89e3ux80fdux3092ux7528ux3044ux308bux304b}{%
\section{なぜ有限分解能を用いるか}\label{ux306aux305cux6709ux9650ux5206ux89e3ux80fdux3092ux7528ux3044ux308bux304b}}

前章の \(X_N\) に対し、初期側では Gauss 準備分布を与え、終端側では測定装置が残す有限分解能の記録を条件として用いる。終端位置をデルタ関数で厳密固定すると、極限拡散の終端近傍で流れが特異になり、\(C^1\) 評価に不要な境界層が生じる。実在する測定記録は有限分解能を持つため、本論文では正定値の読み出し雑音を含む条件づけを主定理に採用する。

時刻 \(T\) の記録を

\[
Y=HX_N(T)+\varepsilon,
\qquad
\varepsilon\sim N(0,R),
\qquad
R\geq r_*I>0
\]

とする。実際に得られた記録値を \(y\) とする。この条件は、尤度

\[
L_R(x)
=
\exp\left[
-\frac12(Hx-y)^{\mathsf T}R^{-1}(Hx-y)
\right]
\]

で経路を重みづけすることと同値である。

\(R\) は単に特異性を避けるための正則化定数ではない。装置の有限分解能、外部雑音、記録読出しのばらつきを合わせた共分散である。主定理は \(R\geq r_*I>0\) を固定した範囲で成立し、点終端 \(R\downarrow0\) を同時に取る極限は含めない。物理的に用いるには、未記述の装置補正による共分散変化が \(r_*\) より十分小さいことを別に確かめる必要がある。

\hypertarget{ux7121ux6761ux4ef6-gauss-ux578bux6cd5ux5247}{%
\section{無条件 Gauss 型法則}\label{ux7121ux6761ux4ef6-gauss-ux578bux6cd5ux5247}}

有限 \(N\) の平均と2時刻共分散を

\[
\mu_N(t)=\mathbb{E}[X_N(t)],
\]

\[
C_N(s,t)
=
\mathbb{E}\left[
(X_N(s)-\mu_N(s))
(X_N(t)-\mu_N(t))^{\mathsf T}
\right]
\]

とする。基本行列を使えば、平均は

\[
\mu_N(t)
=
\Phi(t,0)m_0
+\int_0^t\Phi(t,r)f(r)\,\mathrm{d} r
\]

であり、共分散は初期共分散と有限 Fourier モードの寄与の和として書ける。

雑音を基底関数 \(e_\alpha(t)\) と独立 Gauss 係数 \(\zeta_\alpha\) で

\[
\widetilde\eta_N(t)
=
\sum_{\alpha=0}^{2N}e_\alpha(t)\zeta_\alpha
\]

と書けば、

\[
K_{N,\alpha}(t)
=
\int_0^t\Phi(t,r)e_\alpha(r)\,\mathrm{d} r
\]

により

\[
C_N(s,t)
=
\Phi(s,0)P_0\Phi(t,0)^{\mathsf T}
+\sum_{\alpha=0}^{2N}
K_{N,\alpha}(s)K_{N,\alpha}(t)^{\mathsf T}
\]

となる。この表示は、条件づけとパラメータ微分を有限行列計算へ帰着させる。

\hypertarget{schur-ux88dcux5b8cux306bux3088ux308bux6761ux4ef6ux4ed8ux304dux5e73ux5747ux3068ux5171ux5206ux6563}{%
\section{Schur 補完による条件付き平均と共分散}\label{schur-ux88dcux5b8cux306bux3088ux308bux6761ux4ef6ux4ed8ux304dux5e73ux5747ux3068ux5171ux5206ux6563}}

記録共分散を

\[
S_N
=
HC_N(T,T)H^{\mathsf T}+R
\]

とする。\(R\geq r_*I\) なので \(S_N\) は一様に可逆である。

\begin{proposition}[有限 Gauss 型条件づけ]
条件 $Y=y$ の下で $X_N$ は Gauss 型過程のままであり、その平均と共分散は

$$
\mu_N^R(t)
=
\mu_N(t)
+C_N(t,T)H^{\mathsf T}S_N^{-1}
\left[y-H\mu_N(T)\right],
$$

$$
C_N^R(s,t)
=
C_N(s,t)
-C_N(s,T)H^{\mathsf T}S_N^{-1}HC_N(T,t)
$$

である。
\end{proposition}

\begin{proof}
有限個の時刻 $t_1,\ldots,t_k$ を固定すると、$(X_N(t_1),\ldots,X_N(t_k),Y)$ は結合 Gauss ベクトルである。結合共分散行列の $Y$ 成分に関する Schur 補完を取れば上式を得る。任意の有限時刻集合で整合するため、条件付き過程全体が定まる。
\end{proof}

条件付き共分散の第2項は、終端記録により減少した不確かさを表す。これは力ではない。ある経路が終端記録とどれだけ整合するかという統計的更新である。

この計算は、新しい種類の Gauss 型条件づけではない。有限次元の状態を拡大して Fourier 係数まで含めれば、固定区間の線形 Gauss 型平滑化と同じ Schur 補完になる \cite{rauch_tung_striebel1965}。経路測度の立場では相反過程および Schrödinger 橋の線形 Gauss 型部分に属し \cite{jamison1974,leonard2014,chen_georgiou_pavon2016,leonard_roelly_zambrini2014}、経路単位の Gauss 型条件づけとしても標準的に表せる \cite{wilson_et_al2021}。本論文で必要なのは、この既知の条件づけを有限 Fourier 切断数 \(N\) とパラメータ \(\theta\) について一様に微分し、第4章の定量的 \(C^1\) 評価へ接続することである。

\hypertarget{ux30d1ux30e9ux30e1ux30fcux30bfux5faeux5206}{%
\section{パラメータ微分}\label{ux30d1ux30e9ux30e1ux30fcux30bfux5faeux5206}}

\(F_\theta\) の変分 \(\delta F\) に対して基本行列の第1変分は

\[
D\Phi_\theta[\delta F](t,s)
=
\int_s^t
\Phi_\theta(t,r)
\delta F(r)
\Phi_\theta(r,s)
\,\mathrm{d} r
\]

である。逆行列の微分

\[
D(S^{-1})[\delta S]
=
-S^{-1}(\delta S)S^{-1}
\]

と合わせると、\(\mu_N^R\)、\(C_N^R\) のパラメータ第1微分を明示できる。\(S_N\geq r_*I\) により、条件づけの微分は \(N\) に依存しない定数で制御される。

有限分解能 \(R>0\) は、物理的に自然であるだけでなく、数学的にも重要である。\(R=0\) で \(H\) が全座標を固定すると、終端に近づくにつれて条件付き流れが \((T-t)^{-1}\) 型に発散し得る。点終端は \(R\downarrow0\) の別極限として扱うべきであり、主定理には含めない。

\hypertarget{ux6975ux9650ux62e1ux6563ux306eux6761ux4ef6ux4ed8ux304dux6d41ux308c}{%
\section{極限拡散の条件付き流れ}\label{ux6975ux9650ux62e1ux6563ux306eux6761ux4ef6ux4ed8ux304dux6d41ux308c}}

\(N\to\infty\) の無条件拡散を

\[
\,\mathrm{d} X_t=b(X_t,t)\,\mathrm{d} t+\sqrt{2\nu}\,\,\mathrm{d} W_t,
\qquad
b(x,t)=F(t)x+f(t)
\]

とする。終端尤度の後方伝播を

\[
h_R(x,t)
=
\mathbb{E}\left[L_R(X_T)\mid X_t=x\right]
\]

と置く。線形 Gauss 型系では \(h_R\) は指数2次関数で正である。条件付き前進流れは Doob 変換により

\[
b_+^R(x,t)
=
b(x,t)+2\nu\nabla\log h_R(x,t)
\]

となる \cite{jamison1974,doob1957}。\(\nabla\log h_R\) は \(x\) の1次式であるため、条件付き過程も線形 Gauss 型である。

条件付き時刻密度を \(\rho^R(x,t)\) とすると、後退流れは

\[
b_-^R(x,t)
=
b_+^R(x,t)-2\nu\nabla\log\rho^R(x,t)
\]

である。そこで

\[
v^R
=
\frac{b_+^R+b_-^R}{2},
\qquad
u^R
=
\frac{b_+^R-b_-^R}{2}
=
\nu\nabla\log\rho^R
\]

と定義する。\(v^R\) は確率流の速度、\(u^R\) は密度勾配に伴う浸透速度である。

\hypertarget{ux81eaux7531ux7cfbux3067ux73feux308cux308b--1t}{%
\section{\texorpdfstring{自由系で現れる \(-1/T\)}{自由系で現れる -1/T}}\label{ux81eaux7531ux7cfbux3067ux73feux308cux308b--1t}}

\(F=0\)、\(f=0\)、\(X_N(0)=x_0\) とし、終端を厳密に \(X_N(T)=x_0\) へ固定する特殊な場合を考える。非零 Fourier モードは1周期積分すると零になるため、全期間変位を担うのは零周波数 \(Z_0\) だけである。終端条件は \(Z_0=0\) を意味する。従って条件付き雑音共分散は

\[
\mathbb{E}\left[
\widetilde\eta_N(t)
\widetilde\eta_N(s)^{\mathsf T}
\mid X_N(T)=x_0
\right]
=
2\nu
\left[
\delta_{T,N}(t-s)-\frac1T
\right]I.
\]

ここで初めて \(-1/T\) が現れる。一般の \(F\neq0\) では終端値は全 Fourier モードの線形結合に依存するため、条件付き修正は

\[
-\operatorname{Cov}(\widetilde\eta_N,Y)
\operatorname{Cov}(Y,Y)^{-1}
\operatorname{Cov}(Y,\widetilde\eta_N)
\]

という流れ依存の Schur 補完であり、単純な \(-1/T\) ではない。

\hypertarget{ux524dux5f8cux4e21ux5074ux6761ux4ef6ux3065ux3051ux306eux7269ux7406ux7684ux610fux5473}{%
\section{前後両側条件づけの物理的意味}\label{ux524dux5f8cux4e21ux5074ux6761ux4ef6ux3065ux3051ux306eux7269ux7406ux7684ux610fux5473}}

初期準備と終端記録の双方を知った後に、途中経路の統計を求めることは、通常の条件付き確率である。終端記録が途中経路の条件付き平均を変えることは、終端装置が過去へ力を送ることを意味しない。

ただし、条件付き経路分布を物理的試行頻度として採用するには、どの完結履歴へ確率を置くかという追加の物理原理が必要である。Gauss 型 Schur 補完は、指定した記録を得た後の条件付き法則を計算する厳密な数学であるが、その法則が実験の無条件頻度として選ばれることまでは証明しない。

弱開放な試行周期では、開始、記録、完了、再初期化の各段階で異なる条件付き分布が現れ得る。結果依存の完了率で記録済み試行を捨てれば事後選別になる。これと、初期・終端の両境界で定義された試行集団とを区別する必要がある。第II部では、共通作用殻の Liouville 測度を境界適合条件で制限し、履歴空間へ押し出す。これは Bell 固有の終端関数を置く構成ではないが、2境界条件付けであることは変わらない。また、第4章の Nelson 作用極限からも、本章の Schur 補完からも導かない。

\hypertarget{ux672cux7ae0ux306eux7d50ux8ad6-1}{%
\section{本章の結論}\label{ux672cux7ae0ux306eux7d50ux8ad6-1}}

有限分解能の終端記録を用いれば、前後両側から条件づけた経路法則は通常の Gauss 型 Schur 補完として完全に定義できる。条件付き平均、共分散、そのパラメータ微分は一様に制御される。これは補助確率表示内部の厳密結果である。

物理的には、記録共分散 \(R\) の装置起源、弱開放ミクロモデルから線形表示への縮約、条件付き法則を無条件の試行頻度へ変える原理が別に必要である。次章では、これらを仮定せず、定義した補助表示の作用収束だけを示す。

\hypertarget{ux7e70ux308aux8fbcux307fux6e08ux307fux7c97ux8996ux5316ux4f5cux7528ux5f62ux5f0fux306e-nelson-ux6975ux9650}{%
\chapter{繰り込み済み粗視化作用形式の Nelson 極限}\label{ux7e70ux308aux8fbcux307fux6e08ux307fux7c97ux8996ux5316ux4f5cux7528ux5f62ux5f0fux306e-nelson-ux6975ux9650}}

\begin{statusbox}
位置づけ：線形 Gauss 型・有限分解能・2次ポテンシャルの補助表示で、定義した作用形式の定量的 $C^1$ 収束を厳密に示す。
\end{statusbox}

\hypertarget{ux7c97ux8996ux5316ux4f5cux7528}{%
\section{粗視化作用}\label{ux7c97ux8996ux5316ux4f5cux7528}}

有限 \(N\) の経路は微分可能であるが、\(N\to\infty\) の拡散経路は微分不可能である。そのため、単純な運動エネルギー

\[
\frac m2\int_0^T|\dot X_N(t)|^2\,\mathrm{d} t
\]

は極限で発散する。時間分解能 \(h>0\) を固定し、有限差分

\[
D_hX_N(t)=\frac{X_N(t+h)-X_N(t)}{h}
\]

を用いる。拡散係数が \(\nu\)、空間次元が \(d\) なら、雑音の普遍的発散は

\[
\frac m2\mathbb{E}|D_hX|^2
\sim
\frac{md\nu}{h}
\]

である。

差分商の運動項から軌道に依存しない発散定数を除き、有限な Guerra--Morato 項を残す原理自体は既知である \cite{nelson1966,guerra_morato1983}。本章の新規な主張は、有限 Fourier 切断、有限分解能の終端記録、滑らかな有限次元パラメータ族を同時に扱い、作用値とその第1偏微分へ共通の明示誤差評価を与える点にある。

ここで扱う粗視化作用は、補助的な線形 Gauss 型経路法則から定義する評価汎関数である。特定の有限 Hamiltonian 軌道集団と平均・共分散が一致するときには同じ計算を移せるが、弱開放な現行モデルからこの経路法則が得られることを本章では仮定しない。Hamilton 方程式そのものから、この汎関数を最小化または停留化する選択則が生じるとも仮定しない。本章の結論は作用形式の収束であり、ミクロな縮約と有効運動方程式の動力学的選択は含まない。

外部ポテンシャルを \(U_\theta(x,t)\) と書き、条件付き経路法則に対する繰り込み済み作用を

\[
\mathcal A_{N,h}^{R,U}(\theta)
=
\mathbb{E}_{N,\theta}^{R}
\int_0^{T-h}
\left[
\frac m{2h^2}
|X_N(t+h)-X_N(t)|^2
-\frac{md\nu}{h}
-U_\theta(X_N(t),t)
\right]\,\mathrm{d} t
\]

と定義する。差し引く項は結果や設定に依存せず、有限差分の Gauss 型自己揺らぎだけを除く。

\hypertarget{ux8a31ux5bb9ux3059ux308bux30d1ux30e9ux30e1ux30fcux30bfux65cf}{%
\section{許容するパラメータ族}\label{ux8a31ux5bb9ux3059ux308bux30d1ux30e9ux30e1ux30fcux30bfux65cf}}

パラメータ集合 \(K\subset\mathbb{R}^p\) をコンパクトとする。次を仮定する。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \(F_\theta(t)\)、\(f_\theta(t)\) は \((\theta,t)\) について \(C^2\) であり、\(K\times[0,T]\) 上で2階まで一様有界である。
\item
  初期平均 \(m_{0,\theta}\) と初期共分散 \(P_{0,\theta}\) は \(C^2\) で、\(P_{0,\theta}\geq p_*I>0\) である。
\item
  終端観測 \(H_\theta\)、\(y_\theta\)、\(R_\theta\) は \(C^2\) で、\(R_\theta\geq r_*I>0\) である。
\item
  外部ポテンシャルは
\end{enumerate}

\[
U_\theta(x,t)
=
\frac12x^{\mathsf T}K_\theta(t)x
+\ell_\theta(t)^{\mathsf T}x
+c_\theta(t)
\]

の形で、係数は \(C^2\) かつ一様有界である。 5. Fourier 切断数 \(N\) と粗視化幅 \(h=h_N\) は

\[
h_N\longrightarrow0,
\qquad
N\left(\frac{h_N}{T}\right)^2\longrightarrow\infty
\]

を満たす。

\(C^1(K)\) は作用値と \(\theta\) に関する全ての第1偏微分の一様ノルムを表す。この定理は、任意の非線形な経路変分についての無限次元 \(C^1\) 定理ではなく、指定した線形 Gauss 型パラメータ族上の定理である。

\hypertarget{ux6975ux9650ux4f5cux7528}{%
\section{極限作用}\label{ux6975ux9650ux4f5cux7528}}

極限の条件付き拡散の前進流れを \(b_{+,\theta}^R\)、時刻密度を \(\rho_\theta^R\) とする。Guerra--Morato 型作用を

\[
\mathcal A_{\mathrm{GM}}^{R,U}(\theta)
=
\int_0^T\int_{\mathbb{R}^d}
\rho_\theta^R(x,t)
\left[
\frac m2|b_{+,\theta}^R(x,t)|^2
+m\nu\nabla\cdot b_{+,\theta}^R(x,t)
-U_\theta(x,t)
\right]
\,\mathrm{d} x\,\mathrm{d} t
\]

と定義する \cite{guerra_morato1983}。線形 Gauss 型系では \(b_+^R\) は \(x\) の1次式、\(\rho^R\) は正の Gauss 型密度なので、全ての積分は有限である。

\hypertarget{ux7ddaux5f62-gauss-ux578b-c1-ux53ceux675fux5b9aux7406}{%
\section{\texorpdfstring{線形 Gauss 型 \(C^1\) 収束定理}{線形 Gauss 型 C\^{}1 収束定理}}\label{ux7ddaux5f62-gauss-ux578b-c1-ux53ceux675fux5b9aux7406}}

\begin{theorem}[線形 Gauss 型 $C^1$ 極限]
第4.2節の仮定を満たすとする。ある定数 $C_K<\infty$ が存在し、十分大きい $N$ と $0<h<T/4$ に対して

$$
\left\|
\mathcal A_{N,h}^{R,U}
-
\mathcal A_{\mathrm{GM}}^{R,U}
\right\|_{C^1(K)}
\leq
C_K
\left(
\frac hT
+
\frac{T^2}{Nh^2}
\right)
$$

が成立する。従って $h_N\to0$ かつ $N(h_N/T)^2\to\infty$ なら、次の収束が $C^1(K)$ で成り立つ。

$$
\mathcal A_{N,h_N}^{R,U}
\longrightarrow
\mathcal A_{\mathrm{GM}}^{R,U}.
$$

特に $h_N=TN^{-1/3}$ なら誤差は $O(N^{-1/3})$ である。
\end{theorem}

この \(N^{-1/3}\) は、共分散尾部を \(O(N^{-1})\) と評価し、増分商の \(h^{-2}\) と釣り合わせた現在の証明から得られる率である。下界または最適性は示していない。より滑らかな核、端点適合基底、相殺を用いれば改善される可能性があり、本質的な普遍指数とは主張しない。

証明の中心は、Fourier 切断による共分散誤差 \(O(T^2/N)\) と、時間対角展開による粗視化誤差 \(O(h/T)\) を分離することである。増分商には \(h^{-2}\) が掛かるため、前者は作用上で \(O(T^2/(Nh^2))\) になる。Fourier 尾部と Schur 補完の安定性は付録A、粗視化作用とパラメータ第1微分の評価は付録Bに示す。

\hypertarget{guerramorato-ux8868ux793aux3068-nelson-ux8868ux793a}{%
\section{Guerra--Morato 表示と Nelson 表示}\label{guerramorato-ux8868ux793aux3068-nelson-ux8868ux793a}}

前進・後退流れから

\[
v^R=\frac{b_+^R+b_-^R}{2},
\qquad
u^R=\frac{b_+^R-b_-^R}{2}
=\nu\nabla\log\rho^R
\]

を定義する。境界項が消える条件、例えば全空間での Gauss 型減衰、周期境界、または無流束境界を仮定する。

\begin{theorem}[Guerra--Morato 作用と Nelson 作用の一致]

$$
\mathcal A_{\mathrm{GM}}^{R,U}
=
\mathcal A_{\mathrm{N}}^{R,U},
$$

$$
\mathcal A_{\mathrm{N}}^{R,U}
=
\int_0^T\int_{\mathbb{R}^d}
\rho^R
\left[
\frac m2|v^R|^2
-\frac m2|u^R|^2
-U
\right]
\,\mathrm{d} x\,\mathrm{d} t.
$$
\end{theorem}

\begin{proof}
$b_+^R=v^R+u^R$ と $\nu\nabla\rho^R=\rho^Ru^R$ を用いる。空間部分積分により

$$
\int\rho^R m\nu\nabla\cdot b_+^R\,\mathrm{d} x
=
-m\int\rho^R b_+^R\cdot u^R\,\mathrm{d} x.
$$

従って

$$
\frac m2|b_+^R|^2
-m b_+^R\cdot u^R
=
\frac m2|v^R|^2
-\frac m2|u^R|^2.
$$

ポテンシャル項は共通なので結論を得る。
\end{proof}

この一致は近似ではない。\(C^1\) 極限で得られた Guerra--Morato 作用は、正の Gauss 型密度領域では Nelson 作用そのものである \cite{nelson1966,guerra_morato1983,yasue1981,zambrini1986}。Guerra--Morato 作用の臨界点と第2変分を扱う近年の研究もあるが \cite{knorst_lopes2024}、本定理が扱う有限 Fourier 条件付き族の2尺度 \(C^1\) 収束とは問題設定が異なる。

さらに、\(\rho>0\) で \(mv^R=\nabla S\) と局所的に書ける領域では、

\[
\hbar_{\rm eff}=2m\nu,
\qquad
\psi=\sqrt{\rho^R}
\exp\left(\frac{iS}{\hbar_{\rm eff}}\right)
\]

と置くことで、連続の式と Nelson 作用の停留条件を

\[
i\hbar_{\rm eff}\partial_t\psi
=
\left[
-\frac{\hbar_{\rm eff}^2}{2m}\Delta+U
\right]\psi
\]

へ書き換えられる \cite{nelson1966,guerra_morato1983,yasue1981,zambrini1986}。これは正の密度を持つ局所領域での厳密な表示変換であり、多重連結領域の循環量子化、節を横切る位相接続、微視的な停留点選択を与えない。

\hypertarget{ux505cux7559ux70b9ux306bux3064ux3044ux3066ux8a00ux3048ux308bux3053ux3068}{%
\section{停留点について言えること}\label{ux505cux7559ux70b9ux306bux3064ux3044ux3066ux8a00ux3048ux308bux3053ux3068}}

\begin{corollary}[収束する停留点]
$\theta_N\in\operatorname{int}K$ が

$$
D_\theta\mathcal A_{N,h_N}^{R,U}(\theta_N)=0,
\qquad
\theta_N\longrightarrow\theta_*
$$

を満たすなら、

$$
D_\theta\mathcal A_{\mathrm{N}}^{R,U}(\theta_*)=0
$$

である。
\end{corollary}

\begin{proof}
$C^1(K)$ 収束と $D\mathcal A_{N,h_N}(\theta_N)=0$ から

$$
\|D\mathcal A_{\mathrm{N}}(\theta_*)\|
\leq
\|D\mathcal A_{\mathrm{N}}(\theta_*)-D\mathcal A_{\mathrm{N}}(\theta_N)\|
+
\|D\mathcal A_{\mathrm{N}}(\theta_N)-D\mathcal A_{N,h_N}(\theta_N)\|
\longrightarrow0.
$$
\end{proof}

これは一方向の条件付き主張である。まず有限モデルのパラメータ停留点列が存在し、その列が収束する場合に限って、極限点が Nelson 作用の停留点になる。任意の Nelson 停留点を有限浴の停留点列で近似するには、Hessian の非退化性と少なくとも局所 \(C^2\) 収束が必要である。さらに、微視的 Hamiltonian 方程式が粗視化作用の停留点を力学的に選ぶことは、この系から従わない。

\hypertarget{ux8abfux548c-gauss-ux578bux5206ux5e03ux306eux7269ux7406ux50cf}{%
\section{調和 Gauss 型分布の物理像}\label{ux8abfux548c-gauss-ux578bux5206ux5e03ux306eux7269ux7406ux50cf}}

1次元で

\[
\rho(x,t)
=
\frac1{\sqrt{2\pi}\sigma(t)}
\exp\left[
-\frac{(x-q(t))^2}{2\sigma(t)^2}
\right]
\]

とし、連続の式を満たす速度を

\[
v(x,t)
=
\dot q(t)
+\frac{\dot\sigma(t)}{\sigma(t)}[x-q(t)]
\]

とする。浸透速度は

\[
u(x,t)
=
-\nu\frac{x-q(t)}{\sigma(t)^2}
\]

である。調和ポテンシャル \(U=m\Omega^2x^2/2\) に対する Nelson 作用は

\[
\mathcal A_G[q,\sigma]
=
\frac m2
\int_0^T
\left[
\dot q^2+\dot\sigma^2
-\frac{\nu^2}{\sigma^2}
-\Omega^2(q^2+\sigma^2)
\right]\,\mathrm{d} t.
\]

変分すると

\[
\ddot q+\Omega^2q=0,
\]

\[
\ddot\sigma+\Omega^2\sigma
-\frac{\nu^2}{\sigma^3}=0
\]

を得る。中心は古典的な調和運動を行い、幅は通常の拡散で単調に広がるのではなく、調和閉じ込めと密度勾配の項の釣り合いで振動する。定常幅は

\[
\sigma_*^2=\frac\nu\Omega
\]

である。これは Nelson 作用が、単なる熱拡散ではなく、確率流と密度勾配の前後対称な変分力学を表すことを示す。

\hypertarget{ux7d50ux679cux306eux5c0eux51faux72b6ux614bux3068ux7269ux7406ux7684ux63a5ux7d9a}{%
\section{結果の導出状態と物理的接続}\label{ux7d50ux679cux306eux5c0eux51faux72b6ux614bux3068ux7269ux7406ux7684ux63a5ux7d9a}}

本章の主張を導出状態で分ける。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
主張
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
導出状態
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
根拠
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
定義した線形 Gauss 型作用の \(C^1\) 収束 & 厳密結果 & 第4.4節と付録B \\
Guerra--Morato 表示と Nelson 表示の一致 & 厳密結果 & 第4.5節の部分積分 \\
収束する有限次元停留点列の極限 & 厳密結果 & 第4.6節。停留点列の存在と収束に仮説依存 \\
弱開放ミクロモデルから線形 Gauss 型表示を得ること & 予想・未解決 & 一様な縮約誤差が未評価 \\
微視的時間発展が Nelson 停留点を選ぶこと & 予想・未解決 & 選択機構が未構成 \\
\end{longtable}

現行モデルとの接続に必要な近似を次に整理する。最初の3項は本定理の外側にあり、物理的導出を完成するには別途検証が必要である。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
近似
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
小さい量
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
適用範囲
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
主要誤差と中心結論への影響
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
検証
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
弱開放窓 & \(\varepsilon_{\rm open}\) & \(T\ll\tau_{\rm exch}\) & 平均・共分散と作用に \(O(\varepsilon_{\rm open})\) の補正を生じ得る & 拡大 Hamiltonian と閉鎖補助モデルを同じ入口集団で比較 \\
線形化 & \(\varepsilon_{\rm lin}\) & 観測経路が指定した調和領域に留まる時間 & 捨てた非線形流れが平均・共分散へ \(O(\varepsilon_{\rm lin})\) の補正を与える & 領域幅を変え、非線形有限モデルと比較 \\
Markov・白色雑音近似 & \(\varepsilon_{\rm M}=\tau_{\rm corr}/\tau_{\rm slow}\) & 外部相関時間が遅い運動より十分短い範囲 & 記憶核と有色雑音の残差が \(O(\varepsilon_{\rm M})\) で残る & 相関時間と観測時間を変え、2時刻共分散を比較 \\
時間粗視化 & \(\varepsilon_h=h/T\) & \(0<h<T/4\) & 作用と第1微分に \(O(h/T)\) & \(h\) を変え収束率を測る \\
Fourier 切断 & \(\varepsilon_N=T^2/(Nh^2)\) & \(N(h/T)^2\gg1\) & 作用と第1微分に \(O(T^2/(Nh^2))\) & \(N\) を変え収束率を測る \\
有限分解能記録 & \(R\geq r_*I>0\) & 固定した非退化分解能 & 点終端極限の境界層を除外する。\(R\downarrow0\) は未評価 & 装置分解能を変え条件付き共分散を比較 \\
\end{longtable}

\(\varepsilon_{\rm lin}\) は、指定した観測領域で捨てた非線形流れの大きさを、保持した線形流れの代表値で割った無次元量である。完全な物理的接続には

\[
\varepsilon_{\rm open}
+
\varepsilon_{\rm lin}
+
\varepsilon_{\rm M}
+
\frac hT
+
\frac{T^2}{Nh^2}
\ll1
\]

が必要になる。ただし、本論文が厳密に上界を与えるのは最後の2項だけである。最初の3項を小さいと仮定しただけで、弱開放ミクロモデルから Nelson 有効力学を導出したとは呼ばない。

\hypertarget{ux5b9aux7406ux306eux7bc4ux56f2}{%
\section{定理の範囲}\label{ux5b9aux7406ux306eux7bc4ux56f2}}

本章で証明したのは、線形 Gauss 型範囲における作用形式の \(C^1\) 極限である。次は主定理に含まれない。

\begin{itemize}
\tightlist
\item
  状態依存の非線形な流れ。
\item
  退化した点終端 \(R=0\)。
\item
  硬いしきい値条件による非滑らかな経路選択。
\item
  2次を超える一般ポテンシャルに対する一様第1変分評価。
\item
  密度の節を横切る大域位相。
\item
  全ての Nelson 変分を尽くす無限次元 \(C^1\) 収束。
\item
  微視的 Hamiltonian 運動による粗視化作用の停留点選択。
\item
  一般の時間依存線形 Gauss 型表示を有限自律 Hamiltonian へ埋め込む定理。
\item
  弱開放ミクロモデルから線形表示への縮約誤差。
\end{itemize}

従って本章の結果を「弱開放な古典 Hamiltonian 系からの Nelson 有効力学の動力学的出現」とは呼ばない。確立した内容は、明示した補助確率表示と粗視化規則に対する作用形式の収束である。

\part{構造化有限浴と境界作用殻測度による Bell 型統計}

\hypertarget{ux69cbux9020ux5316ux6709ux9650ux6d74ux5c40ux6240ux53cdux5fdcux5ea7ux6a19ux5171ux901aux5883ux754cux30bbux30afux30bfux30fc}{%
\chapter{構造化有限浴、局所反応座標、共通境界セクター}\label{ux69cbux9020ux5316ux6709ux9650ux6d74ux5c40ux6240ux53cdux5fdcux5ea7ux6a19ux5171ux901aux5883ux754cux30bbux30afux30bfux30fc}}

\begin{statusbox}
位置づけ：1つの有限浴の静的な明・暗モード分解と局所交差応答の評価は Hamiltonian 補助モデルで明示する。局所記録の安定化と全殻拡散への縮約は近似または予想・未解決である。
\end{statusbox}

\hypertarget{ux7b2ciiux90e8ux306eux76eeux7684}{%
\section{第II部の目的}\label{ux7b2ciiux90e8ux306eux76eeux7684}}

第2章から第4章は、観測座標の線形 Gauss 型経路法則と Nelson 作用形式を扱った。そこから Bell 型結果重みは出ない。第II部では、測定器と浴を次の順序で組み立てる。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  左右の局所記録と共通境界モードを、1つの構造化有限浴の中に置く。
\item
  浴の結合ベクトルから、時間に依存しない明・暗モード基底を定める。
\item
  局所測定窓では、左右反応座標間の交差応答が小さいことを条件にする。
\item
  局所記録後の伝達ベクトルを、共通未来で静的な和・差基底へ写す。
\item
  共通境界3モードの作用殻と境界適合条件から、結果ごとの残余ファイバーを定める。
\end{enumerate}

旧構成の中央比較器、相補時計、結果重みを直接与える終端関数は用いない。Bell 重みは第6章と第7章で、浴が準備する共通作用殻の残余体積として求める。

本章で完全に構成するのは、一般測定器の全過程ではない。既存の結果符号を固定指針へ写す最小結果符号化器、静的な浴基底、局所交差応答の評価量、共通未来へ運ぶ伝達ベクトルを明示する。設定、到来変数、装置微視状態から結果を形成する一般測定相互作用は未構成である。

\hypertarget{ux3064ux306eux69cbux9020ux5316ux6709ux9650ux6d74}{%
\section{1つの構造化有限浴}\label{ux3064ux306eux69cbux9020ux5316ux6709ux9650ux6d74}}

質量で規格化した浴の正準座標を

\[
Q
=
\begin{pmatrix}
Q_1&\cdots&Q_N
\end{pmatrix}^{\mathsf T},
\qquad
P
=
\begin{pmatrix}
P_1&\cdots&P_N
\end{pmatrix}^{\mathsf T}
\]

とする。有限浴の Hamiltonian を

\[
H_{\mathcal B}
=
\frac12P^{\mathsf T}P
+
\frac12Q^{\mathsf T}KQ
+
\varepsilon_{\rm nl}V_{\rm nl}(Q)
\]

と書く。\(K\) は正定値実対称行列、\(V_{\rm nl}\) は有限時間混合を補助する滑らかな非線形項である。

左右の局所応答座標を \(x_A,x_B\) とし、浴への結合を

\[
H_{\rm loc-link}
=
\epsilon_A x_A c_A^{\mathsf T}Q
+
\epsilon_B x_B c_B^{\mathsf T}Q
\]

とする。\(c_A,c_B\) は同じ \(N\) 次元浴内の結合ベクトルである。共通境界3モードへの結合ベクトルも同じ浴内に置く。従って左右局所浴と境界浴を別々の独立系として追加するのではない。

有限装置部分は概念的に

\[
H_{\rm fin}
=
H_{\rm src}
+
H_{\rm set}
+
H_{\rm meas,A}
+
H_{\rm meas,B}
+
H_{\rm ptr}
+
H_{\mathcal B}
+
H_{\partial}
+
H_{\rm fin-link}
\]

と書ける。\(H_\partial\) は第6章の境界3モード、\(H_{\rm fin-link}\) は局所反応座標、境界モード、浴暗モードの結合を含む。この式は部品表であり、全操作を1本の自律有限幅 Hamiltonian で実行する完成モデルではない。

現行の拡大全系は

\[
H_{\rm all}
=
H_{\rm fin}
+
H_{\rm ext}(z_{\rm ext})
+
H_{\rm work}(z_{\rm work})
+
\varepsilon_{\rm ext}H_{\rm link}
\]

である。\(\varepsilon_{\rm ext}H_{\rm link}\) は常時存在し、局所浴と境界浴から外部へのごく弱い漏れと、逆向きの微小な流入の双方を許す。

\hypertarget{ux9759ux7684ux306aux660eux6697ux30e2ux30fcux30c9ux5206ux89e3}{%
\section{静的な明・暗モード分解}\label{ux9759ux7684ux306aux660eux6697ux30e2ux30fcux30c9ux5206ux89e3}}

結合ベクトル

\[
\mathcal C
=
\operatorname{span}
\left\{
c_A,c_B,c_{\partial,1},\ldots,c_{\partial,m}
\right\}
\]

を浴の明部分空間とする。\(c_{\partial,\alpha}\) は境界3モードを混合する浴方向である。\(\mathcal C\) の正規直交基底を先頭に並べる直交行列 \(O\) を1つ固定し、

\[
\widetilde Q=OQ,
\qquad
\widetilde P=OP
\]

とする。同じ \(O\) を座標と運動量へ作用させるため、

\[
dP^{\mathsf T}\wedge dQ
=
d\widetilde P^{\mathsf T}\wedge d\widetilde Q
\]

が成り立つ。従ってこれは正準変換である。

変換後の先頭成分には、左右の局所反応座標と共通境界反応座標を取る。残りは外部装置と直接結合しない暗モードである。記号上、

\[
\mathcal B
\simeq
\mathcal B_A^{\rm loc}
\oplus
\mathcal B_B^{\rm loc}
\oplus
\mathcal B_\partial^{\rm glob}
\oplus
\mathcal B^{\rm dark}
\]

と書く。

この直和は、一般には Hamiltonian の厳密な直和ではない。\(O K O^{\mathsf T}\) と非線形項は各部分空間を弱く結び得る。従って変換後の浴 Hamiltonian を

\[
H_{\mathcal B}
=
H_A^{\rm loc}
+
H_B^{\rm loc}
+
H_\partial^{\rm glob}
+
H^{\rm dark}
+
\varepsilon_{\rm cross}H_{\rm cross}
\]

と整理する。\(\varepsilon_{\rm cross}\) は単一の結合定数ではなく、対象時間窓で測る交差応答の代表尺度である。

明・暗モード分解は測定設定や結果に応じて切り替えない。\(O\) は装置を組み立てた時点で固定され、全試行に同じ基底を用いる。局所、共通、和、差という名称は、同じ正準位相空間内の異なる固定部分空間を表す。

\hypertarget{ux5c40ux6240ux4ea4ux5deeux5fdcux7b54}{%
\section{局所交差応答}\label{ux5c40ux6240ux4ea4ux5deeux5fdcux7b54}}

まず \(\varepsilon_{\rm nl}=0\) の線形浴を考える。浴を消去すると、局所反応座標には自己応答と交差応答を含む記憶項が現れる \cite{ford1965,mori1965,zwanzig1973}。単位質量表示では、代表的な応答核を

\[
\chi_{XY}(t)
=
c_X^{\mathsf T}
K^{-1/2}
\sin
\left(
K^{1/2}t
\right)
c_Y,
\qquad
X,Y\in\{A,B\}
\]

と書ける。規格化や時間微分の位置は、どの局所変数へ結合するかにより変わる。本論文で必要なのは、対角核と非対角核の比である。

局所性の誤差を

\[
\varepsilon_{\rm loc}
=
\frac{
\displaystyle
\sup_{0\leq t\leq T_{\rm meas}}
\max\left(
|\chi_{AB}(t)|,
|\chi_{BA}(t)|
\right)
}{
\displaystyle
\sup_{0\leq t\leq T_{\rm meas}}
\min\left(
|\chi_{AA}(t)|,
|\chi_{BB}(t)|
\right)
}
\]

と定める。適用条件は

\[
\varepsilon_{\rm loc}\ll1.
\]

\(c_A^{\mathsf T}c_B=0\) だけでは、この条件は保証されない。\(K\) が2方向を動力学的に混ぜれば、有限時間後に交差応答が現れる。従って局所性は、結合ベクトルの直交性ではなく、実際の応答核または有限モデルの時間発展で検査する。

非線形浴と弱開放補正を含む場合は、同じ初期小摂動に対する線形化応答、または条件付き相関を用いて \(\varepsilon_{\rm loc}\) を定義する。第7章の厳密な Bell 代数は \(\varepsilon_{\rm loc}=0\) の補助モデルで行い、現行モデルへの接続では \(O(\varepsilon_{\rm loc})\) の局所誤差として扱う。

\hypertarget{ux5c40ux6240ux5206ux6790ux5668ux3068ux6700ux5c0fux7d50ux679cux7b26ux53f7ux5316ux5668}{%
\section{局所分析器と最小結果符号化器}\label{ux5c40ux6240ux5206ux6790ux5668ux3068ux6700ux5c0fux7d50ux679cux7b26ux53f7ux5316ux5668}}

結果種を表す円周座標を \(s_A,s_B\) とし、平坦領域上で

\[
A=\sigma(s_A)\in\{-1,+1\},
\qquad
B=\sigma(s_B)\in\{-1,+1\}
\]

とする。生成源が左右へ送る実2次元伝達ベクトルを

\[
u_X
=
\begin{pmatrix}
Q_X\\
P_X
\end{pmatrix},
\qquad
X=A,B
\]

とする。

局所分析器の理想正準写像は

\[
u_A^{\rm out}
=
A R[\phi(a)]u_A^{\rm in},
\qquad
u_B^{\rm out}
=
B R[\phi(b)]u_B^{\rm in}
\]

である。\(R[\phi]\) は実2次元回転行列、\(\phi(a),\phi(b)\) は装置の設定較正である。この写像は、結果符号と設定位相を伝達ベクトルへ記録する。

応答モードの運動量を \(p_X\)、固定指針の運動量を \(\Pi_X\) とする。短時間の局所 Hamiltonian パルスにより

\[
p_A^{\rm out}=A,
\qquad
p_B^{\rm out}=B,
\]

\[
\Pi_A^{\rm out}=A,
\qquad
\Pi_B^{\rm out}=B
\]

を得る補助構成を用いる。有限幅パルスと浴交差応答を含めると、

\[
\Pi_X^{\rm out}
=
X
+
O(\varepsilon_{\rm pulse})
+
O(\varepsilon_{\rm loc})
\]

となる。右辺の \(X\) は \(A\) または \(B\) を表す。補正が指針の2つの巨視領域の間隔より小さければ、記録符号は変わらない。

この構成は、既存の2値符号を記録する最小結果符号化器である。設定、到来変数、装置微視状態から結果そのものを形成する一般の局所 Hamiltonian は与えていない。従って、結果の形成は現行モデルの未解決部分であり、後段の厳密な体積計算によって解決済みにはならない。

\hypertarget{ux5e38ux6642ux306eux5f31ux3044ux6f0fux308cux3068ux8a18ux9332ux5b89ux5b9aux6027}{%
\section{常時の弱い漏れと記録安定性}\label{ux5e38ux6642ux306eux5f31ux3044ux6f0fux308cux3068ux8a18ux9332ux5b89ux5b9aux6027}}

局所反応座標と暗モードを外部環境へ弱く結合する。結合は測定窓だけ切るのではなく常時存在する。局所有限部分の収支を

\[
\dot E_X^{\rm loc}
=
P_X^{\rm ctrl}
+
J_X^{\rm in}
-
J_X^{\rm out}
+
J_X^{\rm cross}
\]

と分ける。\(J_X^{\rm cross}\) は同じ浴内の他セクターとの交換である。

局所記録に必要な時間尺度条件は

\[
\tau_{\rm form}
\ll
\tau_{\rm leak},
\qquad
\tau_{\rm record}
\ll
T_{\rm rec},
\]

\[
\tau_{\rm mix}^{\rm dark}
\lesssim
\tau_{\rm record}
\]

である。\(\tau_{\rm form}\) は記録形成、\(\tau_{\rm leak}\) は記録を担う明モードの作用が外部交換で変わる尺度、\(\tau_{\rm mix}^{\rm dark}\) は一時情報が暗モードへ分散する尺度である。

漏れが強すぎれば記録形成を壊し、弱すぎれば有限浴の再帰を抑えられない。外部揺らぎの流入も指針領域をまたがない強さでなければならない。本論文は、具体的な外部スペクトルについてこの安定域を一様に証明しない。弱開放記録の安定性は予想・未解決である。

\hypertarget{ux5171ux901aux672aux6765ux306eux9759ux7684ux306aux548cux5deeux57faux5e95}{%
\section{共通未来の静的な和・差基底}\label{ux5171ux901aux672aux6765ux306eux9759ux7684ux306aux548cux5deeux57faux5e95}}

局所記録時刻を \(t_A,t_B\)、両伝達ベクトルが同じ境界領域へ到達できる時刻を \(t_C\) とし、

\[
t_A,t_B<t_C
\]

とする。\(t_C\) より前には、A側の局所 Hamiltonian はB側の設定と結果を含まず、B側も同様である。左右交差応答は \(O(\varepsilon_{\rm loc})\) に抑える。

\(t_C\) 以後、伝達ベクトルを静的な和・差基底へ写す。

\[
u_+
=
\frac{
u_A+u_B
}{
\sqrt2
},
\qquad
u_-
=
\frac{
u_A-u_B
}{
\sqrt2
}.
\]

この変換は直交変換であり、対応する座標と運動量へ同じ行列を作用させれば正準である。和・差作用は

\[
I_+
=
\frac12\|u_+\|^2
=
\frac14\|u_A+u_B\|^2,
\]

\[
I_-
=
\frac12\|u_-\|^2
=
\frac14\|u_A-u_B\|^2.
\]

和・差基底は、測定後に結果に応じて選ぶ比較規則ではない。装置の固定された線形正常モードである。固定された損失のない接合部は位相体積を保存するが、共通作用殻上の一様測度を生成しない。この区別は第6章で用いる。

\hypertarget{ux5171ux901aux5883ux754c3ux30e2ux30fcux30c9}{%
\section{共通境界3モード}\label{ux5171ux901aux5883ux754c3ux30e2ux30fcux30c9}}

共通境界セクターに、和モード \(a_+\)、ソフトモード \(a_s\)、残余モード \(a_r\) を置く。

\[
a_\nu
=
\frac{
q_\nu+ip_\nu
}{
\sqrt2
},
\qquad
J_\nu
=
|a_\nu|^2,
\qquad
\nu\in\{+,s,r\}.
\]

\(a_+\) は静的和モード \(u_+\) の複素正準表示であり、

\[
J_+=I_+.
\]

差モードの作用 \(I_-\) と準備時の基準作用 \(J_*\) は、境界接合部の損失のない正準写像により \(a_s,a_r\) の総作用へ入る。

\[
J_s+J_r
=
J_*+I_-.
\]

従って、

\[
C
=
J_++J_s+J_r
=
J_*+I_++I_-.
\]

生成源が固定総入力作用 \(I_++I_-=2I_0\) を持つなら、

\[
C=C_0,
\qquad
C_0=J_*+2I_0.
\]

この関係は作用保存を表す。どの結果が何倍起こるかはまだ決めない。結果重みを得るには、3モード共通殻上の準備測度と、各結果セクターを切る共通分解能が必要である。

\hypertarget{ux5171ux901aux672aux6765ux3092ux6b8bux3059ux7406ux7531}{%
\section{共通未来を残す理由}\label{ux5171ux901aux672aux6765ux3092ux6b8bux3059ux7406ux7531}}

左右局所セクターを同じ有限浴の中に置いても、共通未来の境界構造は不要にならない。同じ浴を共有するだけで Bell 重みが出るなら、その重みは共通過去の浴変数、局所窓の交差応答、または非局所相互作用に書き込まれていなければならない。本モデルはその経路を採らない。

局所測定窓では左右交差応答を小さく保ち、共同重みは共通未来の作用殻を切る残余体積から得る。従って、局所 Hamilton 方程式の因果伝播と、完結履歴へ置く境界条件付けを分ける。

旧構成の Bell 固有な終端関数を除くことは、2境界値問題そのものを初期値問題へ置き換えることではない。本モデルに残るのは、

\begin{itemize}
\tightlist
\item
  全結果に共通な作用保存則。
\item
  設定名を直接参照しない静的和・差基底。
\item
  3モード共通殻。
\item
  同じ分解能で課す境界適合条件。
\end{itemize}

である。

\hypertarget{ux672cux7ae0ux306eux7d50ux8ad6-2}{%
\section{本章の結論}\label{ux672cux7ae0ux306eux7d50ux8ad6-2}}

左右局所装置と共通境界モードを、複数の独立浴ではなく1つの構造化有限浴の固定正準基底として整理した。静的な明・暗モード分解は厳密な正準変換であるが、その後の Hamiltonian が完全な直和になるとは限らない。局所性は、測定窓内の交差応答 \(\varepsilon_{\rm loc}\) を実際に小さくする条件として置く。

局所装置について明示したのは最小結果符号化器であり、一般測定器ではない。常時の弱い漏れは有限浴の再帰を抑える候補だが、漏れだけでは境界作用殻を一様化しない。

共通未来では、固定された和・差正準基底から \(I_+\) と \(I_-\) を得る。境界3モードの総作用は \(C_0=J_*+I_++I_-\) である。次章では、3モード全体の等方拡散が共通殻測度をどう準備し、その測度が残余ファイバー体積をどう決めるかを示す。

\hypertarget{ux30e2ux30fcux30c9ux5883ux754cux4f5cux7528ux6bbbux3068ux6d74ux306bux3088ux308bux7b49ux65b9ux6e96ux5099}{%
\chapter{3モード境界作用殻と浴による等方準備}\label{ux30e2ux30fcux30c9ux5883ux754cux4f5cux7528ux6bbbux3068ux6d74ux306bux3088ux308bux7b49ux65b9ux6e96ux5099}}

\begin{statusbox}
位置づけ：和・差作用、U(2) の不足、U(3) 不変測度、共通殻の周辺密度、残余ファイバー体積は厳密結果である。全殻等方拡散のミクロ導出と弱開放補正は未解決または近似結果である。
\end{statusbox}

\hypertarget{ux548cux5deeux4f5cux7528ux306eux4f59ux5f26ux5e7eux4f55}{%
\section{和・差作用の余弦幾何}\label{ux548cux5deeux4f5cux7528ux306eux4f59ux5f26ux5e7eux4f55}}

生成源が準備する2つの伝達ベクトルを

\[
u_A^{(0)}
=
r_A n(\Theta_A),
\qquad
u_B^{(0)}
=
r_B n(\Theta_B),
\]

\[
n(\Theta)
=
\begin{pmatrix}
\cos\Theta\\
\sin\Theta
\end{pmatrix}
\]

とする。第5章の局所記録後には

\[
u_A
=
A r_A R[\phi(a)]n(\Theta_A),
\]

\[
u_B
=
B r_B R[\phi(b)]n(\Theta_B)
\]

となる。\(A,B\in\{-1,+1\}\) は局所固定指針に記録済みである。

相対角を

\[
\Delta_{ab}
=
\phi(a)-\phi(b)+\Theta_A-\Theta_B
\]

とする。和・差作用は

\[
I_+^{AB}
=
\frac14
\left\|
u_A+u_B
\right\|^2,
\]

\[
I_-^{AB}
=
\frac14
\left\|
u_A-u_B
\right\|^2
\]

である。直接展開すると、

\[
I_\pm^{AB}
=
\frac14
\left[
r_A^2+r_B^2
\pm
2ABr_Ar_B\cos\Delta_{ab}
\right].
\]

\begin{proposition}[和・差作用の余弦恒等式]
等振幅 $r_A=r_B=r$、固定相対生成源位相の下で、

$$
I_\pm^{AB}
=
I_0
\left[
1
\pm
AB\cos\Delta_{ab}
\right],
\qquad
I_0=\frac{r^2}{2}.
$$

さらに、

$$
I_+^{AB}+I_-^{AB}=2I_0
$$

である。
\end{proposition}

\begin{proof}
2つのベクトルの内積は

$$
u_A\cdot u_B
=
ABr^2\cos\Delta_{ab}.
$$

これを和ベクトルと差ベクトルの2乗へ代入すればよい。和と差を加えると内積項が消える。
\end{proof}

位相雑音 \(\delta\) を許し、その分布が設定と結果符号に依存しないとする。重みが \(I_\pm\) に線形であるため、位相を先に平均できる。可視度 \(0\leq V\leq1\) と位相ずれを \(\Delta_{ab}\) へ吸収すれば、

\[
\overline I_\pm^{AB}
=
I_0
\left[
1
\pm
ABV\cos\Delta_{ab}
\right].
\]

総入力作用は平均後も \(2I_0\) である。振幅揺らぎを許す場合は、各試行で \(I_++I_-\) が固定される範囲に中心定理を限定する。総作用自体の揺らぎは第8章の殻幅誤差へ含める。

\hypertarget{ux30e2ux30fcux30c9ux5171ux901aux4f5cux7528ux6bbb}{%
\section{3モード共通作用殻}\label{ux30e2ux30fcux30c9ux5171ux901aux4f5cux7528ux6bbb}}

境界3モードを

\[
a_\nu
=
\frac{
q_\nu+ip_\nu
}{
\sqrt2
},
\qquad
J_\nu=|a_\nu|^2,
\qquad
\nu\in\{+,s,r\}
\]

とする。作用角変数では

\[
q_\nu
=
\sqrt{2J_\nu}\cos\theta_\nu,
\qquad
p_\nu
=
\sqrt{2J_\nu}\sin\theta_\nu,
\]

\[
dq_\nu\,dp_\nu
=
dJ_\nu\,d\theta_\nu.
\]

総作用を

\[
C
=
J_++J_s+J_r
\]

とする。固定値 \(C_0>0\) の共通作用殻上の Liouville 測度は

\[
d\mu_{C_0}
=
\frac{
\delta
\left(
C_0-J_+-J_s-J_r
\right)
\prod_{\nu}
dJ_\nu\,d\theta_\nu
}{
\Omega_3(C_0)
},
\]

\[
\Omega_3(C_0)
=
\frac{
(2\pi)^3C_0^2
}{
2
}.
\]

複素3成分ベクトル \(a=(a_+,a_s,a_r)^{\mathsf T}\) で見れば、この殻は

\[
a^\dagger a=C_0
\]

という5次元球面である。

局所記録と静的和・差変換が与える境界適合条件は

\[
J_+=I_+^{AB}(a,b).
\]

従って結果セクター \((A,B)\) に残る作用は

\[
C_0-I_+^{AB}
=
J_s+J_r.
\]

\(C_0=J_*+2I_0\) とすれば、

\[
C_0-I_+^{AB}
=
J_*+I_-^{AB}.
\]

この恒等式は作用保存だけから従う。相対確率を得るには、異なる \(J_+\) のファイバーにどの測度を置くかを決めなければならない。

\hypertarget{ux6b8bux4f592ux30e2ux30fcux30c9ux306e-u2-ux7b49ux65b9ux6027ux3067ux306fux4e0dux8db3ux3059ux308b}{%
\section{残余2モードの U(2) 等方性では不足する}\label{ux6b8bux4f592ux30e2ux30fcux30c9ux306e-u2-ux7b49ux65b9ux6027ux3067ux306fux4e0dux8db3ux3059ux308b}}

固定 \(J_+=x\) の下では、残余2モードは

\[
J_s+J_r=C_0-x
\]

という3次元球面を作る。\((a_s,a_r)\) に \(U(2)\) が作用すると、この球面上で推移的である。従って、固定 \(x\) の内部では正規化された \(U(2)\) 不変測度が一意になる。

この事実から、

\[
p
\left(
J_s\mid J_+=x
\right)
=
\frac{
1
}{
C_0-x
},
\qquad
0\leq J_s\leq C_0-x
\]

という一様作用分配が従う。しかし、これは各ファイバー内部の条件付き分布であり、異なる \(x\) のファイバー間の相対質量を決めない。

\begin{proposition}[U(2) 等方性の不足]
任意の非負可積分関数 $f$ に対し、

$$
d\mu_f
\propto
f(J_+)
\delta
\left(
C_0-J_+-J_s-J_r
\right)
\prod_\nu dJ_\nu\,d\theta_\nu
$$

は、残余2モード $(a_s,a_r)$ に関して $U(2)$ 不変である。しかし、$J_+=x$ のファイバー質量は

$$
W_f(x)
\propto
f(x)(C_0-x)
$$

となる。
\end{proposition}

\begin{proof}
$f(J_+)$ は残余2モードの回転で変化しない。固定 $J_+=x$ で角変数を積分し、$J_r$ のデルタ関数積分を行うと、

$$
\int_0^{C_0-x}dJ_s
=
C_0-x
$$

が残る。従って任意の $f(x)$ がファイバー間の相対質量へ残る。
\end{proof}

従って、残余2モードの等方性だけから Bell 重みを導くことはできない。必要なのは、\(J_+\) を含む3モード共通殻全体の測度である。

\hypertarget{u3-ux4e0dux5909ux6e2cux5ea6}{%
\section{U(3) 不変測度}\label{u3-ux4e0dux5909ux6e2cux5ea6}}

\(U(3)\) は \(a^\dagger a=C_0\) の球面上へ推移的に作用する。従って、正規化された \(U(3)\) 不変確率測度は一意であり、固定作用殻の Liouville 測度 \(d\mu_{C_0}\) と一致する。

\begin{proposition}[共通殻不変測度の一意性]
$S_{C_0}^5=\{a\in\mathbb C^3:a^\dagger a=C_0\}$ とする。$S_{C_0}^5$ 上の正規化 Borel 測度が全ての $U(3)$ 変換に不変なら、その測度は $d\mu_{C_0}$ に等しい。
\end{proposition}

\begin{proof}
$U(3)$ は球面上で推移的であり、安定部分群は $U(2)$ である。従って球面は同次空間 $U(3)/U(2)$ と同一視できる。コンパクト群の正規化 Haar 測度を商空間へ押し出した測度は不変である。不変確率測度の平均作用を用いれば一意性が従う。作用角変数で書けば、その押し出しは $d\mu_{C_0}$ である。
\end{proof}

固定された損失のない3入力3出力接合部は \(U(3)\) 変換を1つ実行するだけであり、単一の入力位相点を殻全体へ広げない。上の命題は不変測度の一意性を述べるのであって、Hamilton 方程式が確率測度を無から生成することを述べない。

\hypertarget{ux6d74ux306bux3088ux308bux5168ux6bbbux7b49ux65b9ux62e1ux6563}{%
\section{浴による全殻等方拡散}\label{ux6d74ux306bux3088ux308bux5168ux6bbbux7b49ux65b9ux62e1ux6563}}

境界3モードの準備窓で、構造化浴の多数の未読自由度を消去した縮約方程式を考える。理想的な殻接方向生成子を

\[
\mathcal L_{\rm iso}
=
D_\partial\Delta_{S^5},
\qquad
D_\partial>0
\]

とする。\(\Delta_{S^5}\) は固定作用殻の Laplace--Beltrami 作用素である。

\begin{theorem}[等方殻拡散の定常測度]
連結な固定作用殻 $S_{C_0}^5$ 上で

$$
\partial_t f
=
D_\partial\Delta_{S^5}f
$$

を考える。規格化された非負密度の定常解は定数だけであり、$d\mu_{C_0}$ が一意な定常確率測度である。初期密度が $L^2$ なら、

$$
\left\|
f_t-1
\right\|_{L^2(d\mu_{C_0})}
\leq
e^{-D_\partial\lambda_1t}
\left\|
f_0-1
\right\|_{L^2(d\mu_{C_0})},
$$

ここで $\lambda_1>0$ は殻上 Laplacian の第1非零固有値である。
\end{theorem}

\begin{proof}
定常密度 $f$ に対し、部分積分から

$$
\int
f\Delta_{S^5}f\,d\mu_{C_0}
=
-
\int
\left|
\nabla_{S^5}f
\right|^2
d\mu_{C_0}.
$$

左辺は0なので、連結性から $f$ は定数である。規格化により $f=1$ となる。時間発展については定数成分を除いた固有関数展開を用い、第1非零固有値で評価する。
\end{proof}

混合不足の尺度を

\[
\varepsilon_{\rm mix}
=
\exp
\left(
-D_\partial\lambda_1\tau_{\rm prep}
\right)
\]

とする。\(\varepsilon_{\rm mix}\ll1\) なら、滑らかな観測量について初期の方向偏りは小さくなる。

有限 Hamiltonian 浴からこの拡散を得る候補は、\(U(3)\) の Hamiltonian 生成子を、等方な相関行列を持つ浴変数へ弱く結合することである。弱結合・短相関時間極限では、2次の縮約生成子が \(U(3)\) の Casimir 作用素へ近づく。付録Cに具体形を示す。

ただし、有限閉鎖 Hamiltonian 流れは微細 Liouville 密度を保存し、一般には \(L^1\) で一様密度へ収束しない。有限浴だけで主張できるのは、有限時間・有限分解能の混合または弱い観測量収束である。不可逆な一意定常分布を用いる場合は、常時の弱い外部交換まで含む縮約が必要である。

\hypertarget{ux7570ux65b9ux6027ux3068ux534aux5f84ux65b9ux5411ux306eux5f31ux958bux653eux88dcux6b63}{%
\section{異方性と半径方向の弱開放補正}\label{ux7570ux65b9ux6027ux3068ux534aux5f84ux65b9ux5411ux306eux5f31ux958bux653eux88dcux6b63}}

現行の縮約生成子を

\[
\mathcal L
=
D_\partial\Delta_{S^5}
+
\varepsilon_{\rm aniso}\mathcal L_{\rm aniso}
+
\mathcal L_C
\]

と分ける。\(\mathcal L_{\rm aniso}\) は殻接方向の異方成分、\(\mathcal L_C\) は総作用 \(C\) の半径方向変化を表す。

半径方向の有効式の候補を

\[
dC_t
=
-\gamma_C
\left(
C_t-C_0
\right)dt
+
\sqrt{2D_C}\,dW_t
\]

とする。これは、外部への漏れと仕事貯蔵系または微小揺らぎからの流入が \(C_0\) 付近で釣り合う線形化である。定常幅は

\[
\sigma_C^2
=
\frac{D_C}{\gamma_C},
\qquad
\varepsilon_C
=
\frac{\sigma_C}{C_0}.
\]

\(\varepsilon_C\ll1\) なら、固定殻計算を狭い準定常殻へ適用できる。純粋な漏れ

\[
\dot C=-\gamma_C C
\]

だけでは、\(C=0\) へ落ちるだけで \(C_0>0\) の定常殻を作らない。

異方性と半径幅が小さく、境界ファイバーが殻端から離れていれば、滑らかな結果重みの補正を

\[
O
\left(
\varepsilon_{\rm aniso}
+
\varepsilon_{\rm mix}
+
\varepsilon_C
\right)
\]

と整理できる。全係数を同じ有限浴の尺度から与える一様上界は未完成であるため、現行モデルへの接続は近似結果である。

\hypertarget{ux5171ux901aux6bbbux306eux5468ux8fbaux5bc6ux5ea6}{%
\section{共通殻の周辺密度}\label{ux5171ux901aux6bbbux306eux5468ux8fbaux5bc6ux5ea6}}

\begin{theorem}[和モード作用の周辺密度]
3モード固定作用殻の正規化 Liouville 測度について、$J_+=x$ の周辺密度は

$$
p_+(x)
=
\frac{
2(C_0-x)
}{
C_0^2
}
\mathbf1_{\{0\leq x\leq C_0\}}.
$$
\end{theorem}

\begin{proof}
角変数を積分すると $(2\pi)^3$ を得る。$J_+=x$ を固定した未規格化密度は

$$
(2\pi)^3
\int_0^\infty dJ_s
\int_0^\infty dJ_r\,
\delta
\left(
C_0-x-J_s-J_r
\right)
=
(2\pi)^3(C_0-x).
$$

これを

$$
\Omega_3(C_0)
=
\frac{
(2\pi)^3C_0^2
}{
2
}
$$

で規格化すればよい。
\end{proof}

この線形密度が Bell 重みの起源になる。重要なのは、各 \(x\) の残余2モード分布を別々に質量1へ規格化しないことである。4つの結果セクターを同じ3モード作用殻の切断として比較する。

\hypertarget{ux6b8bux4f59ux30d5ux30a1ux30a4ux30d0ux30fcux4f53ux7a4d}{%
\section{残余ファイバー体積}\label{ux6b8bux4f59ux30d5ux30a1ux30a4ux30d0ux30fcux4f53ux7a4d}}

結果セクター \((A,B)\) の理想境界条件を

\[
g_{AB}
=
J_+-I_+^{AB}(a,b)
=
0
\]

とする。作用座標でこの制約を用いる場合、\(|\partial g_{AB}/\partial J_+|=1\) であり、coarea Jacobian は全結果と全設定に共通である。

\begin{proposition}[残余ファイバーの線形体積]
$0<I_+^{AB}<C_0$ とする。固定作用殻と $g_{AB}=0$ の交わりに誘導される未規格化 Liouville 体積は

$$
\Omega_{AB}
=
(2\pi)^3
\left(
C_0-I_+^{AB}
\right)
$$

に比例する。共通定数を除けば、

$$
W_{AB}
\propto
C_0-I_+^{AB}
=
J_*+I_-^{AB}.
$$
\end{proposition}

\begin{proof}
$J_+=I_+^{AB}$ を固定し、$J_r$ のデルタ関数積分を行う。許される $J_s$ は

$$
0\leq J_s\leq C_0-I_+^{AB}
$$

なので、その長さが残る。3つの角変数は共通因子 $(2\pi)^3$ を与える。
\end{proof}

一般の境界写像では、解集合自体を無条件にシンプレクティック多様体と呼ばない。全境界正準位相空間の Liouville 測度を、作用殻と境界適合写像で制限して得る誘導測度として定義する。複数解、分岐、caustic がある場合は、Jacobianと多重度を含める必要がある。

\hypertarget{ux6709ux9650ux5206ux89e3ux80fd}{%
\section{有限分解能}\label{ux6709ux9650ux5206ux89e3ux80fd}}

現実の境界適合条件には幅 \(\delta_J>0\) がある。共通窓関数 \(K_{\delta_J}\) を用い、

\[
K_{\delta_J}
\left(
J_+-I_+^{AB}
\right)
\]

で制限する。\(I_+^{AB}\) が殻端から \(\delta_J\) より十分離れ、\(K_{\delta_J}\) が全結果に共通なら、

\[
W_{AB}^{(\delta_J)}
=
c_{\delta_J}
\left(
C_0-I_+^{AB}
\right)
+
O
\left(
\frac{\delta_J^2}{C_0}
\right).
\]

\(c_{\delta_J}\) は全セクターに共通で規格化時に消える。窓幅または Jacobian が結果や設定に依存する場合は、余分な重みを直接導入するため認めない。

\hypertarget{ux672cux7ae0ux306eux7d50ux8ad6-3}{%
\section{本章の結論}\label{ux672cux7ae0ux306eux7d50ux8ad6-3}}

残余2モードの \(U(2)\) 等方性は、固定 \(J_+\) のファイバー内部を一様にするだけで、ファイバー間の質量を決めない。この不足は任意関数 \(f(J_+)\) を用いた反例で厳密に示せる。

3モード全体の \(U(3)\) 等方性は共通作用殻上の測度を一意に決める。縮約された等方拡散では、その測度が一意な定常分布になる。固定殻の \(J_+\) 周辺は \(C_0-J_+\) に比例し、境界条件 \(J_+=I_+^{AB}\) を課した残余ファイバー体積は

\[
W_{AB}
\propto
J_*+I_-^{AB}
\]

となる。

固定殻の幾何と縮約拡散の定常測度は、指定した補助モデル内で厳密である。一方、同じ構造化有限浴と常時の弱い外部交換から、非退化な \(U(3)\) 等方生成子、混合時間、異方誤差、殻幅を導くことは未完成である。次章では、共通殻の誘導測度を履歴空間へ押し出し、Bell 型共同確率と前提違反を計算する。

\hypertarget{ux5883ux754cux4f5cux7528ux6bbbux6e2cux5ea6ux304bux3089ux306e-bell-ux578bux5171ux540cux6cd5ux5247}{%
\chapter{境界作用殻測度からの Bell 型共同法則}\label{ux5883ux754cux4f5cux7528ux6bbbux6e2cux5ea6ux304bux3089ux306e-bell-ux578bux5171ux540cux6cd5ux5247}}

\begin{statusbox}
位置づけ：共通殻測度、対称な結果セクター、共通の境界分解能を条件とした共同確率は厳密結果である。全殻測度のミクロな準備と一般測定器への適用は予想・未解決である。
\end{statusbox}

\hypertarget{ux5c65ux6b74ux6e2cux5ea6ux306eux5b9aux7fa9}{%
\section{履歴測度の定義}\label{ux5c65ux6b74ux6e2cux5ea6ux306eux5b9aux7fa9}}

第6章で得た Liouville 測度は、境界3モードの正準位相空間上にある。Bell 実験の結果確率として用いるには、その測度を完結履歴の空間へ移す必要がある。

初期側の生成源、設定制御器、局所装置、指針、浴暗モードをまとめて \(\lambda\) とし、境界3モードを \(\eta=(a_+,a_s,a_r)\) とする。設定制御器の巨視領域が \(a,b\) を定め、局所 Hamiltonian 流れが結果 \(A,B\) と伝達ベクトル \(u_A,u_B\) を定める。

全境界正準位相空間の基準測度を

\[
d\mu_0
=
\rho_{\rm prep}(\lambda)
d\lambda\,
d\mu_{C_0}(\eta)
\]

とする。ここで \(d\mu_{C_0}\) は3モード共通作用殻の正規化 Liouville 測度である。結果セクターを

\[
\Sigma_{AB}
=
\left\{
\lambda:
A(\lambda,a)=A,\,
B(\lambda,b)=B
\right\}
\]

とする。

境界適合写像を

\[
g_{AB}^{a,b}(\lambda,\eta)
=
J_+(\eta)
-
I_+^{AB}(a,b;\lambda)
\]

とする。理想境界条件は \(g_{AB}^{a,b}=0\) である。有限分解能では、全セクターに共通な窓関数 \(K_{\delta_J}\) を用いる。

設定 \((a,b)\) の履歴測度は、\(d\mu_0\) を \(\Sigma_{AB}\) と境界適合窓で制限し、Hamiltonian の解写像

\[
\mathcal F_{a,b}:
(\lambda,\eta)
\longmapsto
\gamma_{a,b}
\]

により履歴空間へ押し出して定める。記号的には、

\[
d\mu_{\rm hist}^{a,b}
\propto
\left(
\mathcal F_{a,b}
\right)_\#
\left[
\rho_{\rm prep}(\lambda)
K_{\delta_J}
\left(
g_{AB}^{a,b}
\right)
d\lambda\,
d\mu_{C_0}(\eta)
\right].
\]

この定義で数えるのは、解集合そのものに無条件で置いた Liouville 体積ではない。全境界正準位相空間の Liouville 測度を、作用保存と境界適合条件で制限し、解写像で履歴空間へ押し出した測度である。

\hypertarget{ux7d50ux679cux30bbux30afux30bfux30fcux306eux57faux6e96ux5bfeux79f0ux6027}{%
\section{結果セクターの基準対称性}\label{ux7d50ux679cux30bbux30afux30bfux30fcux306eux57faux6e96ux5bfeux79f0ux6027}}

共通作用殻が決めるのは、各境界適合ファイバーの作用体積である。局所結果セクター自体の基準質量は、準備対称性から別に決める。

境界条件を課す前の質量を

\[
w_{AB}
=
\int_{\Sigma_{AB}}
\rho_{\rm prep}(\lambda)\,d\lambda
\]

とする。左右の結果符号を独立に反転する測度保存対合を

\[
\mathcal S_A:
\Sigma_{AB}
\longrightarrow
\Sigma_{-A,B},
\]

\[
\mathcal S_B:
\Sigma_{AB}
\longrightarrow
\Sigma_{A,-B}
\]

とする。\(\mathcal S_A,\mathcal S_B\) が準備 Hamiltonian、準備巨視領域、\(\rho_{\rm prep}d\lambda\) を保つなら、

\[
w_{++}
=
w_{+-}
=
w_{-+}
=
w_{--}
=
\frac14.
\]

Hamiltonian の形式が符号反転対称なだけでは不十分である。同じ Hamiltonian に非対称な準備密度を置けるため、準備巨視領域と基準測度まで対称でなければならない。

\hypertarget{ux7d50ux679cux306eux672aux898fux683cux5316ux8ceaux91cf}{%
\section{結果の未規格化質量}\label{ux7d50ux679cux306eux672aux898fux683cux5316ux8ceaux91cf}}

理想境界条件と共通 Jacobian の下で、結果 \((A,B)\) の未規格化質量は

\[
\widetilde W_{AB}
=
w_{AB}
\int
\delta
\left(
C_0-J_+-J_s-J_r
\right)
\delta
\left(
J_+-I_+^{AB}
\right)
\prod_\nu dJ_\nu\,d\theta_\nu.
\]

第6.8節の計算から、

\[
\widetilde W_{AB}
\propto
w_{AB}
\left(
C_0-I_+^{AB}
\right).
\]

\(C_0=J_*+2I_0\) と \(I_++I_-=2I_0\) を用いれば、

\[
\widetilde W_{AB}
\propto
w_{AB}
\left(
J_*+I_-^{AB}
\right).
\]

ここで \(J_*\) は結果と設定に依存しない基準作用である。\(J_*\) は、境界ファイバーが全結果で正の幅を持つための余裕でもある。\(J_*\) が大きいほど角度依存の可視度は低下する。

有限分解能では、

\[
\widetilde W_{AB}^{(\delta_J)}
=
c_{\delta_J}
w_{AB}
\left(
C_0-I_+^{AB}
\right)
+
O
\left(
w_{AB}
\frac{\delta_J^2}{C_0}
\right).
\]

\(c_{\delta_J}\) が全結果と全設定に共通なら規格化で消える。結果ごとに異なる窓幅を用いると、欲しい重みを装置分解能へ直接書き込むことになる。

\hypertarget{bell-ux578bux5171ux540cux78baux7387}{%
\section{Bell 型共同確率}\label{bell-ux578bux5171ux540cux78baux7387}}

位相雑音を平均した差作用を

\[
\overline I_-^{AB}
=
I_0
\left[
1
-
ABV\cos\Delta_{ab}
\right]
\]

とする。\([S]\) の対称準備により \(w_{AB}=1/4\) とする。

\begin{theorem}[境界作用殻からの Bell 型共同法則]
次を仮定する。

1. 境界3モードは1つの共通作用殻 $C_0=J_*+2I_0$ 上の Liouville 測度を持つ。
2. 4つの結果セクターの基準質量は等しい。
3. 境界条件は $J_+=I_+^{AB}$ であり、分解能と coarea Jacobian は全結果・全設定に共通である。
4. 生成源は固定総入力作用と可視度 $V$ を持つ。

このとき、

$$
P(A,B\mid a,b)
=
\frac14
\left[
1
-
V_{\rm eff}
AB\cos\Delta_{ab}
\right],
$$

$$
V_{\rm eff}
=
\frac{
I_0
}{
J_*+I_0
}V.
$$
\end{theorem}

\begin{proof}
未規格化質量は

$$
\widetilde W_{AB}
\propto
\frac14
\left[
J_*+I_0
-
AB I_0V\cos\Delta_{ab}
\right].
$$

4セクターについて和を取ると、$AB$ に奇な項が消え、

$$
\sum_{A,B}
\widetilde W_{AB}
\propto
J_*+I_0.
$$

従って各質量を総和で割れば主張を得る。
\end{proof}

\(J_*=0\) かつ \(V=1\) なら、

\[
P(A,B\mid a,b)
=
\frac14
\left[
1
-
AB\cos\Delta_{ab}
\right].
\]

\(J_*>0\) または \(V<1\) なら、同じ余弦形で可視度が低下する。

\hypertarget{chsh-ux5024}{%
\section{CHSH 値}\label{chsh-ux5024}}

相関関数は

\[
E(a,b)
=
\sum_{A,B}
AB
P(A,B\mid a,b)
=
-V_{\rm eff}\cos\Delta_{ab}.
\]

標準的な4設定を選ぶと、

\[
|S_{\rm CHSH}|
=
2\sqrt2\,V_{\rm eff}.
\]

従って CHSH 超過の条件は

\[
V_{\rm eff}
>
\frac1{\sqrt2}.
\]

小さい補正 \(\varepsilon_{\rm Bell}\) が相関へ一様に加わる場合、十分な超過余裕を持つには

\[
2\sqrt2\,V_{\rm eff}
-
2
\gg
O(\varepsilon_{\rm Bell})
\]

が必要である。誤差の内訳は第8章で整理する。

\hypertarget{ux975eux4fe1ux53f7ux6027}{%
\section{非信号性}\label{ux975eux4fe1ux53f7ux6027}}

対称な共同確率をBについて和を取ると、

\[
\sum_B
P(A,B\mid a,b)
=
\frac12.
\]

同様に、

\[
\sum_A
P(A,B\mid a,b)
=
\frac12.
\]

従って一側周辺は反対側の設定に依存しない。この非信号性は、次の条件に依存する。

\begin{itemize}
\tightlist
\item
  \(w_{AB}=1/4\)。
\item
  共通の境界分解能。
\item
  共通の coarea Jacobian。
\item
  \(J_*\) と総入力作用が結果・設定に依存しない。
\item
  位相雑音と殻幅の分布が左右符号反転に不変。
\end{itemize}

これらが崩れる場合、一側周辺に残差が現れ得る。任意の偏った準備に対して非信号性が動的に回復することは示していない。

\hypertarget{ux6e2cux5b9aux8a2dux5b9aux72ecux7acbux6027}{%
\section{測定設定独立性}\label{ux6e2cux5b9aux8a2dux5b9aux72ecux7acbux6027}}

Bell の測定設定独立性は、完全な隠れた変数または履歴変数 \(\Lambda\) の分布が

\[
\rho
\left(
\Lambda\mid a,b
\right)
=
\rho(\Lambda)
\]

を満たすことを要求する。本モデルでは境界適合条件が

\[
J_+
=
I_+^{AB}(a,b)
\]

であり、許容される残余ファイバーが設定によって変わる。従って、

\[
\rho_{\rm hist}
\left(
\Lambda\mid a,b
\right)
\neq
\rho_{\rm hist}(\Lambda)
\]

が一般に成り立つ。

測定設定依存性は、初期密度へ結果式を直接書き込むことからではなく、同じ共通作用殻を設定依存の境界適合面で切ることから生じる。局所結果形成時に反対側の設定が Hamilton 方程式へ入ることを意味しない。

この構成は Bell の定理を否定しない。Bell 不等式を超えるために必要な前提違反を、完結履歴の測定設定独立性の破れとして明示する。

\hypertarget{ux4e8bux5f8cux9078ux5225ux3068ux306eux533aux5225}{%
\section{事後選別との区別}\label{ux4e8bux5f8cux9078ux5225ux3068ux306eux533aux5225}}

境界条件に適合しない軌道は、本理論上は実験後に捨てた記録済み試行ではなく、最初から指定した2境界値問題の解ではない。しかし、数学上の定義だけでは、実験装置が事後選別なしにその履歴集団を実現することを保証しない。

操作上は、少なくとも次を結果別・設定別に監査する。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  外部から与えた開始信号数。
\item
  左右の局所指針が形成した記録数。
\item
  境界段階まで完了した試行数。
\item
  次試行へ再初期化できた回数。
\item
  記録形成、殻準備、消去に要した時間と交換エネルギー。
\item
  境界分解能と作用殻幅。
\end{enumerate}

観測済み試行の一部を結果または設定に依存して除外しなければ Bell 値が出ない場合、本構成は検出事後選別へ退化する。開始数、記録数、完了数が共通係数だけで異なるなら、共同確率の規格化には影響しない。

\hypertarget{bell-ux524dux63d0ux306eux4e00ux89a7}{%
\section{Bell 前提の一覧}\label{bell-ux524dux63d0ux306eux4e00ux89a7}}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
前提または条件
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
本モデルでの地位
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
局所 Hamiltonian 応答 & 局所測定窓で \(O(\varepsilon_{\rm loc})\) まで成立 \\
結果の一意性 & 最小結果符号化器では成立。一般測定器は未構成 \\
測定設定独立性 & 境界適合ファイバーの設定依存性により成立しない \\
非信号性 & 対称準備と共通分解能の下で厳密 \\
事後選別の不在 & 理論上は2境界解集合。装置実現では開始・記録・完了数の監査が必要 \\
共通試行測度 & 全殻等方拡散の定常測度として縮約レベルで導出 \\
全殻拡散のミクロ生成 & 予想・未解決 \\
\end{longtable}

局所性と測定設定独立性を同じ条件として扱わない。局所 Hamilton 方程式の交差応答が小さくても、完結履歴の条件付き測度は設定に依存し得る。

\hypertarget{ux65e7ux7d42ux7aefux95a2ux6570ux3068ux306eux7f6eux63dbux95a2ux4fc2}{%
\section{旧終端関数との置換関係}\label{ux65e7ux7d42ux7aefux95a2ux6570ux3068ux306eux7f6eux63dbux95a2ux4fc2}}

旧構成では、基準初期密度へ終端関数を引き戻して

\[
\rho_{\rm old}
\propto
\rho_S
G_R\circ\Phi_{a,b}^{T}
\]

とし、終端半空間の体積を Bell 重みへ変換していた。この構成では、Bell 固有の終端統計原理を独立入力として必要とした。

現構成では、

\[
d\mu_{C_0}
\longrightarrow
J_+=I_+^{AB}
\longrightarrow
W_{AB}
\propto
C_0-I_+^{AB}
\]

という1つの共通作用殻の切断を用いる。設定名、結果名、目標相関を引数に持つ終端関数はない。中央比較器と相補時計も用いない。

ただし、これは境界条件付けを消したのではない。共通未来で \(J_+=I_+^{AB}\) を要求し、許容履歴の測度を数える点で2境界構造は残る。未解決問題は、Bell 固有の終端関数の生成から、同じ構造化浴による全殻等方準備と共通境界適合の物理的実現へ移った。

\hypertarget{ux672cux7ae0ux306eux7d50ux8ad6-4}{%
\section{本章の結論}\label{ux672cux7ae0ux306eux7d50ux8ad6-4}}

3モード共通作用殻の Liouville 測度を、結果セクターと境界適合条件で制限し、履歴空間へ押し出す測度を定義した。対称な結果セクター、共通分解能、共通 Jacobian の下で、残余ファイバー体積は

\[
W_{AB}
\propto
J_*+I_-^{AB}
\]

となり、Bell 型余弦共同確率が厳密に従う。

完全な余弦則には \(J_*=0\) と \(V=1\)、CHSH 超過には \(V_{\rm eff}>1/\sqrt2\) が必要である。対称準備では非信号性が成り立つ一方、完全履歴分布は設定に依存する。Bell の前提違反は測定設定独立性にある。

旧来の Bell 固有な終端関数と中央比較器は除いたが、共通未来を含む境界値問題は残る。全殻測度を同じ有限浴から準備すること、一般測定器を構成すること、事後選別なしの試行周期として実装することは未解決である。

\hypertarget{ux8fd1ux4f3cux8aa4ux5deeux9069ux7528ux9650ux754cux53cdux8a3cux6761ux4ef6}{%
\chapter{近似誤差、適用限界、反証条件}\label{ux8fd1ux4f3cux8aa4ux5deeux9069ux7528ux9650ux754cux53cdux8a3cux6761ux4ef6}}

\begin{statusbox}
位置づけ：共通作用殻の厳密結果と現行の弱開放モデルへの近似接続を分け、全殻拡散、局所交差応答、殻幅、異方性、境界 Jacobian の誤差と未解決問題をまとめる。
\end{statusbox}

\hypertarget{ux8aa4ux5deeux306eux5206ux89e3}{%
\section{誤差の分解}\label{ux8aa4ux5deeux306eux5206ux89e3}}

第7章の Bell 型共同確率は、理想化した共通作用殻、対称セクター、共通境界分解能の下で厳密である。弱開放な現行モデルへ接続するときの主要誤差を

\[
\varepsilon_{\rm Bell}
=
\varepsilon_{\rm loc}
+
\varepsilon_{\rm pulse}
+
\varepsilon_{\rm aniso}
+
\varepsilon_{\rm mix}
+
\varepsilon_C
+
\varepsilon_{\rm res}
+
\varepsilon_J
+
\varepsilon_S
\]

と整理する。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
記号
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
物理的意味
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
主な検証
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\(\varepsilon_{\rm loc}\) & 局所測定窓の左右交差応答 & 有限浴の応答核、局所摂動実験 \\
\(\varepsilon_{\rm pulse}\) & 局所正準写像の有限幅補正 & パルス幅を変えた収束 \\
\(\varepsilon_{\rm aniso}\) & 殻接方向拡散の \(U(3)\) 異方性 & 生成子の固有値、方向別相関 \\
\(\varepsilon_{\rm mix}\) & 準備時間が有限であることによる初期偏り & 準備時間を変えた収束 \\
\(\varepsilon_C\) & 総作用の半径方向幅 & \(C\) の定常分布 \\
\(\varepsilon_{\rm res}\) & 境界適合の有限分解能 & 窓幅を変えた収束 \\
\(\varepsilon_J\) & coarea Jacobian と解多重度の変動 & 境界写像の微分、分岐監査 \\
\(\varepsilon_S\) & 結果セクター基準質量の非対称 & 設定前の結果符号反転試験 \\
\end{longtable}

各誤差が結果と設定に一様であり、境界ファイバーが殻端から離れていれば、

\[
P_{\rm phys}(A,B\mid a,b)
=
P_{\rm shell}(A,B\mid a,b)
+
O(\varepsilon_{\rm Bell})
\]

と整理できる。全係数を同じ有限 Hamiltonian 浴から与える一様上界は未完成であるため、この接続は近似結果である。

\hypertarget{ux5c40ux6240ux4ea4ux5deeux5fdcux7b54-1}{%
\section{局所交差応答}\label{ux5c40ux6240ux4ea4ux5deeux5fdcux7b54-1}}

同じ有限浴の中で左右の局所セクターを定める以上、厳密な独立性を自動的には持たない。中心条件は

\[
\varepsilon_{\rm loc}\ll1
\]

である。

交差応答が結果形成前に大きければ、B側の設定がA側の局所結果へ順時間的に影響する経路が生じる。この場合、測定設定独立性だけでなく局所性の監査も失敗する。交差応答が小さいが零でない場合は、一側周辺、設定頻度、相関関数へ現れる残差を同時に測る。

静的な明・暗モード基底の直交性は必要な構成要素だが、十分条件ではない。有限時間発展を支配する \(K\) と非線形項を含めて、\(\chi_{AB}(t)\) と \(\chi_{BA}(t)\) を評価する。

\hypertarget{ux5168ux6bbbux6df7ux5408ux3068-u2-ux306eux9650ux754c}{%
\section{全殻混合と U(2) の限界}\label{ux5168ux6bbbux6df7ux5408ux3068-u2-ux306eux9650ux754c}}

残余2モードだけの \(U(2)\) 等方性では、

\[
d\mu_f
\propto
f(J_+)
\delta
\left(
C_0-J_+-J_s-J_r
\right)
d\Gamma
\]

という任意の \(f(J_+)\) が残る。従って Bell 重みを決めるには、\(J_+\) を含む全3モード殻の測度が必要である。

縮約生成子が

\[
\mathcal L
=
D_\partial\Delta_{S^5}
+
\varepsilon_{\rm aniso}\mathcal L_{\rm aniso}
\]

なら、異方成分は定常密度を変形する。第1近似では、

\[
f_{\rm stat}
=
1
+
O(\varepsilon_{\rm aniso})
\]

となるが、異方生成子の零モード近傍に小さい固有値がある場合は補正係数が大きくなり得る。単に結合定数が小さいことではなく、等方生成子のスペクトルギャップに対する相対量を評価する必要がある。

準備時間が有限なら、

\[
\varepsilon_{\rm mix}
\lesssim
\exp
\left(
-D_\partial\lambda_1\tau_{\rm prep}
\right)
\]

で初期偏りを評価する。有限 Hamiltonian 浴だけを用いる場合は、この指数収束を微細密度へ主張せず、有限分解能観測量の混合誤差として測る。

\hypertarget{ux5e38ux6642ux306eux6f0fux308cux3068ux6bbbux5e45}{%
\section{常時の漏れと殻幅}\label{ux5e38ux6642ux306eux6f0fux308cux3068ux6bbbux5e45}}

外部へのごく弱い漏れは、有限浴の再帰抑制と記録安定化に役立ち得る。しかし、純粋な減衰は作用殻を一様化しない。半径方向に

\[
\dot C=-\gamma_C C
\]

だけが働けば、定常状態は \(C=0\) である。

\(C_0>0\) の狭い準定常殻には、外部からの流入または仕事貯蔵系による復元が必要である。線形化した定常幅を

\[
\varepsilon_C
=
\frac1{C_0}
\sqrt{
\frac{D_C}{\gamma_C}
}
\]

とする。結果ファイバーが殻端から

\[
\min_{A,B,a,b}
\left(
C_0-I_+^{AB}
\right)
\gg
\sigma_C+\delta_J
\]

だけ離れている範囲で、固定殻の線形体積を安定に用いる。

総作用の平均または幅が結果や設定に依存する場合、規格化で消えない余分な重みが生じる。これは単なる可視度低下ではなく、非信号性残差や高次調波を生み得る。

\hypertarget{ux5883ux754cux5206ux89e3ux80fdjacobianux8907ux6570ux89e3}{%
\section{境界分解能、Jacobian、複数解}\label{ux5883ux754cux5206ux89e3ux80fdjacobianux8907ux6570ux89e3}}

理想線形制約

\[
g_{AB}=J_+-I_+^{AB}=0
\]

では、作用座標に関する Jacobian は1である。現実の境界写像を \(g_{AB}^{\rm phys}\) とすると、coarea 公式は

\[
W_{AB}
\propto
\int_{
\left(
g_{AB}^{\rm phys}
\right)^{-1}(0)
}
\frac{
\rho_\partial
}{
\left|
\nabla g_{AB}^{\rm phys}
\right|
}
d\Sigma
\]

を与える。

次の場合は単純な線形体積則を再検討する。

\begin{itemize}
\tightlist
\item
  \(|\nabla g_{AB}^{\rm phys}|\) が結果または設定に依存する。
\item
  1つの境界データに複数の解が対応する。
\item
  caustic で解の多重度が変わる。
\item
  境界面が殻端へ接する。
\item
  分解能窓が非対称または結果依存である。
\end{itemize}

これらの効果をまとめて \(\varepsilon_J+\varepsilon_{\rm res}\) とする。共通分解能を較正するだけでなく、境界写像の微分と解多重度を検査しなければならない。

\hypertarget{chsh-ux8d85ux904eux306eux5b89ux5b9aux6761ux4ef6}{%
\section{CHSH 超過の安定条件}\label{chsh-ux8d85ux904eux306eux5b89ux5b9aux6761ux4ef6}}

理想相関は

\[
E(a,b)
=
-V_{\rm eff}\cos\Delta_{ab},
\qquad
V_{\rm eff}
=
\frac{I_0}{J_*+I_0}V.
\]

誤差が4つの相関値それぞれに最大 \(\delta_E\) 入るなら、

\[
\left|
\delta S_{\rm CHSH}
\right|
\leq
4\delta_E.
\]

従って、CHSH 超過を誤差込みで主張する安全条件は

\[
2\sqrt2\,V_{\rm eff}
-
4\delta_E
>
2.
\]

\(\delta_E\) は相関関数の誤差であり、確率表の最大絶対誤差と同じとは限らない。数値検算では共同確率、相関、周辺確率を別々に報告する。

\hypertarget{ux5426ux5b9aux7684ux7d50ux679cux3068ux9069ux7528ux9650ux754c}{%
\section{否定的結果と適用限界}\label{ux5426ux5b9aux7684ux7d50ux679cux3068ux9069ux7528ux9650ux754c}}

現時点で確立している否定的結果をまとめる。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  残余2モードの \(U(2)\) 等方性だけでは、異なる \(J_+\) のファイバー間質量を決められない。
\item
  固定された損失のない接合部は Liouville 測度を保存するが、単一状態から一様測度を生成しない。
\item
  純粋な一方向漏れは \(C_0>0\) の一様作用殻を準備しない。
\item
  有限閉鎖 Hamiltonian 流れは微細 Liouville 密度を一般に強収束させない。
\item
  左右を同じ浴へ入れただけでは、共通未来の境界条件は不要にならない。
\item
  Bell 型余弦則は配置空間位相の循環量子化を含意せず、Wallstrom 問題を解かない \cite{wallstrom1994}。
\item
  余弦相関が得られても、Tsirelson 限界を一般原理から選んだことにはならない。
\end{enumerate}

旧終端関数を削除したことを、確率測度のミクロな生成問題が解決したとは表現しない。問題は、結果依存終端重みの生成から、全3モード殻の等方準備と共通境界適合の実現へ移った。

\hypertarget{ux53cdux8a3cux306bux4f7fux3048ux308bux89b3ux6e2cux91cf}{%
\section{反証に使える観測量}\label{ux53cdux8a3cux306bux4f7fux3048ux308bux89b3ux6e2cux91cf}}

現行モデルは、少なくとも次の量で検査できる。

\begin{itemize}
\tightlist
\item
  局所測定窓の交差応答核 \(\chi_{AB}(t),\chi_{BA}(t)\)。
\item
  境界3モードの \(J_+\) 周辺が \(2(C_0-J_+)/C_0^2\) に従うか。
\item
  準備時間に対する初期方向偏りの減衰。
\item
  作用殻幅 \(\sigma_C/C_0\) と結果別・設定別の平均作用。
\item
  境界分解能を変えたときの共同確率の収束。
\item
  高次調波 \(\cos(2\Delta)\)、\(\cos(3\Delta)\) の残差。
\item
  一側周辺の反対側設定依存性。
\item
  開始数、記録数、完了数、再初期化数の結果別・設定別差。
\item
  \(J_*\) を変えたときの可視度 \(I_0V/(J_*+I_0)\)。
\end{itemize}

\(J_+\) 周辺が線形でない、Jacobianが結果依存、開始後の棄却率が設定依存、局所交差応答が無視できない、といういずれかが確認されれば、中心導出の適用条件は破れる。

\hypertarget{ux6b8bux308bux8ab2ux984c}{%
\section{残る課題}\label{ux6b8bux308bux8ab2ux984c}}

中心的な未解決問題は次である。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  1つの明示的な構造化有限浴から、局所反応座標と境界3モードの双方を導く。
\item
  同じ浴の相関関数から \(U(3)\) 等方な殻接方向生成子を導き、\(\varepsilon_{\rm aniso}\) と混合時間を評価する。
\item
  常時の弱い漏れと流入から、狭い総作用殻の定常幅とエネルギー収支を導く。
\item
  設定、到来変数、装置微視状態から結果を形成する一般測定 Hamiltonian を構成する。
\item
  境界適合、記録、消去、再初期化を1周期の明示モデルへ統合し、事後選別がないことを示す。
\item
  偏った準備でも非信号性が回復する条件を判定する。
\item
  M0から第I部の線形 Gauss 型表示への縮約を導く。
\item
  微視的時間発展による Nelson 停留点選択を示す。
\item
  Born 則、Tsirelson 限界、Wallstrom 量子化へ進む追加構造を特定する。
\end{enumerate}

\hypertarget{ux6700ux7d42ux7d50ux8ad6}{%
\section{最終結論}\label{ux6700ux7d42ux7d50ux8ad6}}

第I部では、指定した線形 Gauss 型補助表示について、繰り込み済み粗視化作用のパラメータ \(C^1\) 収束を厳密に得た。これは弱開放ミクロモデルからの一般導出や停留点選択を含まない。

第II部では、1つの構造化有限浴を局所反応座標セクターと共通境界セクターへ静的に分けた。共通境界3モードの \(U(3)\) 等方拡散は、縮約方程式の下で共通作用殻の一意な定常測度を与える。その作用殻を \(J_+=I_+^{AB}\) で切ると、残余ファイバー体積は

\[
W_{AB}
\propto
J_*+I_-^{AB}
\]

となり、対称セクターでは Bell 型余弦共同確率が厳密に従う。

Bell 固有の終端関数、中央比較器、相補時計は現行モデルから除いた。しかし、共通未来の境界適合と全殻測度は残る。現在の前進は、求める Bell 重みを直接入力する必要をなくし、その起源を共通作用殻の幾何へ移したことである。残る中心課題は、その共通殻測度と境界適合を同じ弱開放な有限 Hamiltonian モデルから導くことである。

\appendix

\hypertarget{fouriergauss-ux578bux8fd1ux4f3cux3068-schur-ux88dcux5b8cux306eux8a55ux4fa1}{%
\chapter{Fourier--Gauss 型近似と Schur 補完の評価}\label{fouriergauss-ux578bux8fd1ux4f3cux3068-schur-ux88dcux5b8cux306eux8a55ux4fa1}}

\begin{statusbox}
位置づけ：補助的な線形 Gauss 型表示について、有限モード収束と条件づけの安定性を厳密に補足する。
\end{statusbox}

\hypertarget{ux57faux672cux6838ux306e-fourier-ux4fc2ux6570}{%
\section{基本核の Fourier 係数}\label{ux57faux672cux6838ux306e-fourier-ux4fc2ux6570}}

線形方程式の雑音応答核を

\[
G_\theta(t,s)
=
\mathbf 1_{0\leq s\leq t}
\Phi_\theta(t,s)
\]

とする。\(s=t\) に跳びがあるため、\(s\) に関する Fourier 係数は一般に \(O(n^{-1})\) である。

\begin{lemma}[一様 Fourier 尾部]
$F_\theta$ が第4.2節の仮定を満たすなら、ある $C_K$ が存在して

$$
\sup_{\theta\in K,\,t\in[0,T]}
\|\widehat G_{\theta,n}(t)\|
\leq
\frac{C_KT}{1+|n|},
$$

$$
\sup_{\theta\in K,\,t\in[0,T]}
\|D_\theta\widehat G_{\theta,n}(t)\|
\leq
\frac{C_KT}{1+|n|}
$$

が成立する。従って共分散尾部は

$$
\sup_{\theta,s,t}
\left(
\|C_N(s,t)-C(s,t)\|
+\|D_\theta C_N(s,t)-D_\theta C(s,t)\|
\right)
\leq
\frac{C_KT^2}{N}
$$

である。
\end{lemma}

\begin{proof}
$n\neq0$ に対して $e^{-i\omega_ns}$ を部分積分する。区間端と $s=t$ の跳びから $1/\omega_n$ の境界項が生じ、区間内部では $\partial_s\Phi(t,s)=-\Phi(t,s)F(s)$ が一様有界である。従って $|\widehat G_n|\leq C/|\omega_n|$ を得る。

$\theta$ 微分については

$$
D\Phi_\theta[\delta F](t,s)
=
\int_s^t
\Phi_\theta(t,r)
\delta F(r)
\Phi_\theta(r,s)
\,\mathrm{d} r
$$

を使う。$D\Phi$ とその $s$ 微分も一様有界なので同じ部分積分評価が成立する。共分散は Fourier 係数の積の和であり、

$$
\sum_{|n|>N}\frac1{n^2}\leq\frac{C}{N}
$$

から結論を得る。
\end{proof}

\hypertarget{ux5e73ux5747ux306eux53ceux675f}{%
\section{平均の収束}\label{ux5e73ux5747ux306eux53ceux675f}}

本論文では \(F_\theta\)、\(f_\theta\)、初期平均は \(N\) に依存しないため、無条件平均は \(\mu_N=\mu\) である。浴切断に依存する補正平均を許す場合でも、Fourier 尾部が中心化されていれば平均差は零であり、非零の決定論的尾部を加えた場合はその \(L^1\) ノルムで直接評価できる。

条件付き平均は \(C_N(t,T)\) と \(C_N(T,T)\) に依存するため、共分散尾部から \(O(1/N)\) の差を持つ。

\hypertarget{schur-ux88dcux5b8cux306eux5b89ux5b9aux6027}{%
\section{Schur 補完の安定性}\label{schur-ux88dcux5b8cux306eux5b89ux5b9aux6027}}

\(S_N=HC_N(T,T)H^{\mathsf T}+R\)、\(S=HC(T,T)H^{\mathsf T}+R\) とする。\(R\geq r_*I\) なので

\[
\|S_N^{-1}\|\leq r_*^{-1},
\qquad
\|S^{-1}\|\leq r_*^{-1}.
\]

逆行列恒等式

\[
S_N^{-1}-S^{-1}
=
S_N^{-1}(S-S_N)S^{-1}
\]

から

\[
\|S_N^{-1}-S^{-1}\|
\leq
r_*^{-2}\|S_N-S\|
\]

を得る。従って条件付き共分散について

\[
\sup_{s,t,\theta}
\|C_N^R(s,t)-C^R(s,t)\|
\leq
\frac{C_KT^2}{N}
\]

である。

第1微分では

\[
D(S^{-1})=-S^{-1}(DS)S^{-1}
\]

を用いる。\(S_N^{-1}\)、\(DS_N\) が一様有界なので、積の各因子を1つずつ差し替えることで

\[
\sup_{s,t,\theta}
\|D C_N^R(s,t)-D C^R(s,t)\|
\leq
\frac{C_KT^2}{N}
\]

を得る。条件付き平均も同様である。

\hypertarget{ux6709ux9650ux5206ux89e3ux80fdux306eux5f79ux5272}{%
\section{有限分解能の役割}\label{ux6709ux9650ux5206ux89e3ux80fdux306eux5f79ux5272}}

\(R>0\) は、観測値 \(y\) の周囲に有限幅の終端領域を持たせる。これにより、条件付き共分散は \(T\) で完全には消えず、条件付き流れの係数は閉区間 \([0,T]\) 上で有界に保たれる。

\(R\downarrow0\) とすると、完全観測された方向の終端共分散は零へ近づく。自由拡散では前進条件付き流れに

\[
\frac{y-x}{T-t}
\]

型の項が現れる。点終端での定理を得るには、\(t=T\) の境界層を除いた区間で先に \(N\to\infty\)、\(h\to0\) を取り、その後に境界層と \(R\downarrow0\) を別に評価する必要がある。

\hypertarget{ux81eaux7531ux7d42ux7aefux56faux5b9aux3068ux96f6ux5468ux6ce2ux6570}{%
\section{自由終端固定と零周波数}\label{ux81eaux7531ux7d42ux7aefux56faux5b9aux3068ux96f6ux5468ux6ce2ux6570}}

自由系では

\[
X_N(T)-X_N(0)
=
\int_0^T\widetilde\eta_N(t)\,\mathrm{d} t
=
\sqrt{2\nu T}\,Z_0.
\]

従って \(X_N(T)=X_N(0)\) は \(Z_0=0\) と同値である。非零モードは終端条件と独立なので、条件付き共分散から零モードの寄与 \(2\nu/T\) だけが除かれる。この計算は、旧来の \(-1/T\) が浴の基本性質ではなく、自由終端固定の結果であることを最も直接に示す。

\hypertarget{ux4e00ux822cux7ddaux5f62ux7cfbux3067ux306fux5168ux30e2ux30fcux30c9ux304cux6761ux4ef6ux3065ux3051ux3089ux308cux308b}{%
\section{一般線形系では全モードが条件づけられる}\label{ux4e00ux822cux7ddaux5f62ux7cfbux3067ux306fux5168ux30e2ux30fcux30c9ux304cux6761ux4ef6ux3065ux3051ux3089ux308cux308b}}

\(F\neq0\) では

\[
X_N(T)
=
\Phi(T,0)X_N(0)
+\sum_\alpha K_{N,\alpha}(T)\zeta_\alpha
+d_N(T)
\]

である。ここで \(d_N(T)\) は決定論項であり、一般に \(K_{N,\alpha}(T)\neq0\) である。終端記録は零周波数だけでなく全ての Fourier 係数の線形結合を拘束する。そのため条件付き雑音共分散の修正は階数有限の Schur 項となり、流れ \(F\)、観測 \(H\)、分解能 \(R\) に依存する。

\hypertarget{ux7c97ux8996ux5316ux4f5cux7528ux306e-c1-ux8a55ux4fa1}{%
\chapter{\texorpdfstring{粗視化作用の \(C^1\) 評価}{粗視化作用の C\^{}1 評価}}\label{ux7c97ux8996ux5316ux4f5cux7528ux306e-c1-ux8a55ux4fa1}}

\begin{statusbox}
位置づけ：補助的な線形 Gauss 型表示について、時間粗視化誤差と Fourier 切断誤差を分離し、厳密な主定理を補足する。
\end{statusbox}

\hypertarget{gauss-ux578bux5897ux5206ux306eux6b63ux78baux306aux8868ux793a}{%
\section{Gauss 型増分の正確な表示}\label{gauss-ux578bux5897ux5206ux306eux6b63ux78baux306aux8868ux793a}}

条件付き Gauss 型過程の増分 \(\Delta_hX(t)=X(t+h)-X(t)\) に対して

\[
\mathbb{E}^R|\Delta_hX(t)|^2
=
|\mu^R(t+h)-\mu^R(t)|^2
+\operatorname{tr}\left[
C^R(t+h,t+h)+C^R(t,t)-2C^R(t+h,t)
\right]
\]

が厳密に成立する。従って粗視化作用の運動項は、条件付き平均と共分散だけで計算できる。

有限 \(N\) でも同じ式が成立する。\(C_N^R-C^R=O(1/N)\) なので、増分2乗の有限モード誤差は粗い評価で \(O(1/N)\)、\(h^{-2}\) を掛けた作用誤差は \(O(1/(Nh^2))\) となる。

\hypertarget{ux6642ux9593ux5bfeux89d2ux306eux5c55ux958b}{%
\section{時間対角の展開}\label{ux6642ux9593ux5bfeux89d2ux306eux5c55ux958b}}

極限拡散について、\(X_t=x\) を固定した短時間増分は

\[
\Delta_hX
=
b_+^R(x,t)h
+\sqrt{2\nu}\Delta_hW
+O_{L^2}(h^{3/2})
\]

である。流れの空間依存と雑音の相関による交差項まで含めて平均すると

\[
\mathbb{E}^R
\left[
|\Delta_hX|^2\mid X_t=x
\right]
=
2d\nu h
+h^2
\left[
|b_+^R(x,t)|^2
+2\nu\nabla\cdot b_+^R(x,t)
\right]
+O(h^3).
\]

線形流れでは3階剰余を平均・微分した量も一様に有界である。\(m/(2h^2)\) を掛けると

\[
\frac m{2h^2}\mathbb{E}^R|\Delta_hX|^2
-\frac{md\nu}{h}
=
\mathbb{E}^R
\left[
\frac m2|b_+^R|^2
+m\nu\nabla\cdot b_+^R
\right]
+O(h).
\]

積分上端を \(T-h\) で止めたことによる欠落も \(O(h)\) である。

\hypertarget{ux306aux305cux767aux6563ux9805ux304cux6b8bux308bux304b}{%
\section{なぜ発散項が残るか}\label{ux306aux305cux767aux6563ux9805ux304cux6b8bux308bux304b}}

雑音の主要項 \(2d\nu h\) だけを差し引いても、流れと短時間雑音の交差効果は \(h^2\) の有限項として残る。それが

\[
m\nu\nabla\cdot b_+^R
\]

である。この項を落とすと、極限は正しい Guerra--Morato 作用にならず、Nelson 表示の負の Fisher 項も得られない。

\hypertarget{fourier-ux5207ux65adux8aa4ux5dee}{%
\section{Fourier 切断誤差}\label{fourier-ux5207ux65adux8aa4ux5dee}}

付録Aの評価から

\[
\|C_N^R-C^R\|_{C^1(K;C([0,T]^2))}
\leq
\frac{C_KT^2}{N}
\]

である。増分共分散は4つの共分散値の線形結合なので、

\[
\left|
\mathbb{E}_N^R|\Delta_hX_N|^2
-
\mathbb{E}^R|\Delta_hX|^2
\right|_{C^1(K)}
\leq
\frac{C_KT^2}{N}.
\]

従って運動項の差は

\[
\frac{C_KT^2}{Nh^2}
\]

で抑えられる。この評価は最適とは限らないが、\(N(h/T)^2\to\infty\) という単純な対角極限を与える。

\hypertarget{ux6b21ux30ddux30c6ux30f3ux30b7ux30e3ux30eb}{%
\section{2次ポテンシャル}\label{ux6b21ux30ddux30c6ux30f3ux30b7ux30e3ux30eb}}

\(U(x,t)=x^{\mathsf T}K(t)x/2+\ell(t)^{\mathsf T}x+c(t)\) なら

\[
\mathbb{E}^R[U(X_t,t)]
=
\frac12\mu^R(t)^{\mathsf T}K(t)\mu^R(t)
+\frac12\operatorname{tr}[K(t)C^R(t,t)]
+\ell(t)^{\mathsf T}\mu^R(t)
+c(t).
\]

従ってポテンシャル期待値とそのパラメータ第1微分は、\(\mu_N^R\) と \(C_N^R\) の \(O(1/N)\) 収束から直接従う。これは運動項の \(O(1/(Nh^2))\) より小さく、主定理の右辺へ吸収できる。

一般の滑らかな非2次ポテンシャルでは、Gauss 型モーメント展開または一様可積分性を用いて同様の結果を拡張できる可能性がある。しかし第1微分には解写像の応答と \(\nabla U\) の積が現れるため、本論文では証明が閉じる2次範囲に限定する。

\hypertarget{ux30d1ux30e9ux30e1ux30fcux30bfux7b2c1ux5faeux5206}{%
\section{パラメータ第1微分}\label{ux30d1ux30e9ux30e1ux30fcux30bfux7b2c1ux5faeux5206}}

作用を \(\theta_j\) で微分すると、平均、共分散、条件付き Schur 項、ポテンシャル係数の微分が現れる。基本行列の微分公式と \(R\geq r_*I\) により、全ての係数は \(K\) 上で一様有界である。

時間対角展開を微分した剰余も \(O(h)\)、Fourier 尾部を微分した誤差も \(O(T^2/(Nh^2))\) である。有限個の \(\theta_j\) について最大を取れば

\[
\|\mathcal A_{N,h}^{R,U}-\mathcal A_{\mathrm{GM}}^{R,U}\|_{C^1(K)}
\leq
C_K
\left(
\frac hT+\frac{T^2}{Nh^2}
\right)
\]

を得る。

\hypertarget{ux5bfeux89d2ux5c3aux5ea6ux306eux9078ux629e}{%
\section{対角尺度の選択}\label{ux5bfeux89d2ux5c3aux5ea6ux306eux9078ux629e}}

\(h/T=N^{-\alpha}\) と置くと、2つの誤差は

\[
N^{-\alpha},
\qquad
N^{2\alpha-1}
\]

である。両者を同じ次数にするには \(\alpha=1/3\) とすればよい。従って

\[
h_N=TN^{-1/3},
\qquad
\varepsilon_N=O(N^{-1/3})
\]

となる。ここで \(\varepsilon_N\) は全評価誤差を表す。

この選択は、粗視化窓を短くしすぎると未解像の Fourier 尾部が増幅され、長くしすぎると局所 Nelson 作用から外れる、という物理的な釣り合いを表す。

\hypertarget{ux69cbux9020ux5316ux6709ux9650ux6d74u3-ux6bbbux62e1ux6563ux5883ux754cux30d5ux30a1ux30a4ux30d0ux30fcux4f53ux7a4d}{%
\chapter{構造化有限浴、U(3) 殻拡散、境界ファイバー体積}\label{ux69cbux9020ux5316ux6709ux9650ux6d74u3-ux6bbbux62e1ux6563ux5883ux754cux30d5ux30a1ux30a4ux30d0ux30fcux4f53ux7a4d}}

\begin{statusbox}
位置づけ：静的浴基底、和・差変換、作用保存、固定殻体積、coarea 計算を厳密に補足する。有限浴から等方拡散への縮約は候補構成であり、未完成である。
\end{statusbox}

\hypertarget{ux62e1ux5927ux5168ux7cfbux3068ux30a8ux30cdux30ebux30aeux30fcux53ceux652f}{%
\section{拡大全系とエネルギー収支}\label{ux62e1ux5927ux5168ux7cfbux3068ux30a8ux30cdux30ebux30aeux30fcux53ceux652f}}

第II部の拡大全系を

\[
H_{\rm all}
=
H_{\rm src}
+
H_{\rm set}
+
H_{\rm loc}
+
H_{\rm ptr}
+
H_{\mathcal B}
+
H_\partial
+
H_{\rm fin-link}
+
H_{\rm ext}
+
\varepsilon_{\rm ext}H_{\rm link}
+
H_{\rm work}
\]

と書く。各項の役割は次である。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
項
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
役割
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\(H_{\rm src}\) & 固定総入力作用と位相基準を持つ伝達ベクトル対の準備 \\
\(H_{\rm set}\) & 左右の設定制御器 \\
\(H_{\rm loc}\) & 局所結果形成または最小結果符号化 \\
\(H_{\rm ptr}\) & 固定指針への記録 \\
\(H_{\mathcal B}\) & 左右局所反応座標、共通境界反応座標、暗モードを含む1つの有限浴 \\
\(H_\partial\) & 境界3モードの自由運動と弱い混合 \\
\(H_{\rm fin-link}\) & 有限装置部分内の結合 \\
\(H_{\rm ext}\) & 外部環境 \\
\(\varepsilon_{\rm ext}H_{\rm link}\) & 常時のごく弱い漏れと流入 \\
\(H_{\rm work}\) & 設定変更、記録消去、再初期化の仕事源 \\
\end{longtable}

全 Hamiltonian に明示的な時間依存性がなければ、拡大全エネルギーは保存される。一方、有限装置部分

\[
H_{\rm fin}
=
H_{\rm all}
-
H_{\rm ext}
-
H_{\rm work}
\]

の収支は

\[
\frac{dH_{\rm fin}}{dt}
=
\left\{
H_{\rm fin},
\varepsilon_{\rm ext}H_{\rm link}
+
H_{\rm work}
\right\}.
\]

実験室の記号では、

\[
\dot E_{\rm fin}
=
J_{\rm in}
-
J_{\rm out}
+
P_{\rm ctrl}.
\]

閉鎖補助モデルは \(\varepsilon_{\rm ext}=0\) とし、仕事源を有限装置へ含めた短時間窓で用いる。現行モデルでは \(\varepsilon_{\rm ext}\) を常時零にせず、測定窓内の相対エネルギー変化を小さい量として評価する。

\hypertarget{ux9759ux7684ux6d74ux57faux5e95ux306eux6b63ux6e96ux6027}{%
\section{静的浴基底の正準性}\label{ux9759ux7684ux6d74ux57faux5e95ux306eux6b63ux6e96ux6027}}

浴座標を \(Q,P\in\mathbb R^N\) とし、

\[
H_{\mathcal B}
=
\frac12P^{\mathsf T}P
+
\frac12Q^{\mathsf T}KQ
+
\varepsilon_{\rm nl}V_{\rm nl}(Q)
\]

とする。\(K\) は正定値実対称行列である。

局所装置と境界装置が浴へ結合する方向を

\[
c_A,
\quad
c_B,
\quad
c_{\partial,1},
\ldots,
c_{\partial,m}
\]

とする。その張る部分空間の正規直交基底を先頭に並べる直交行列 \(O\) を固定し、

\[
\widetilde Q=OQ,
\qquad
\widetilde P=OP
\]

と変換する。

\begin{proposition}[直交浴基底変換の正準性]
$O^{\mathsf T}O=I$ なら、$(Q,P)\mapsto(\widetilde Q,\widetilde P)$ は正準変換である。
\end{proposition}

\begin{proof}
正準1形式は

$$
P^{\mathsf T}dQ
=
\widetilde P^{\mathsf T}
O
O^{\mathsf T}
d\widetilde Q
=
\widetilde P^{\mathsf T}d\widetilde Q
$$

と保存される。従ってシンプレクティック2形式も保存される。
\end{proof}

変換後の2次形式は

\[
H_{\mathcal B}^{(2)}
=
\frac12\widetilde P^{\mathsf T}\widetilde P
+
\frac12
\widetilde Q^{\mathsf T}
\widetilde K
\widetilde Q,
\qquad
\widetilde K
=
OKO^{\mathsf T}.
\]

結合ベクトルに適合した基底を取っても、\(\widetilde K\) は一般にブロック対角ではない。局所、境界、暗モード間の動的結合は \(\widetilde K\) の非対角ブロックと \(V_{\rm nl}\) に残る。

従って、

\[
H_{\mathcal B}
=
H_A^{\rm loc}
+
H_B^{\rm loc}
+
H_\partial^{\rm glob}
+
H^{\rm dark}
+
H_{\rm cross}
\]

という分解は、正準座標の厳密な分類と、動力学的な近似直和を分けて読む必要がある。

\hypertarget{ux7ddaux5f62ux5fdcux7b54ux6838}{%
\section{線形応答核}\label{ux7ddaux5f62ux5fdcux7b54ux6838}}

線形浴の運動方程式は

\[
\ddot Q+KQ
=
-\epsilon_Ac_Ax_A
-\epsilon_Bc_Bx_B
-\sum_\alpha
c_{\partial,\alpha}F_\alpha.
\]

ここで \(F_\alpha\) は境界モードから浴へ加わる一般化力である。初期値解は

\[
Q(t)
=
\cos
\left(
K^{1/2}t
\right)Q(0)
+
K^{-1/2}
\sin
\left(
K^{1/2}t
\right)P(0)
\]

\[
\quad
-
\int_0^t
K^{-1/2}
\sin
\left[
K^{1/2}(t-s)
\right]
F_{\rm dev}(s)\,ds,
\]

\[
F_{\rm dev}
=
\epsilon_Ac_Ax_A
+
\epsilon_Bc_Bx_B
+
\sum_\alpha
c_{\partial,\alpha}F_\alpha.
\]

A側の一般化力 \(c_A^{\mathsf T}Q(t)\) にB側の \(x_B\) が与える寄与は

\[
-\epsilon_B
\int_0^t
\chi_{AB}(t-s)x_B(s)\,ds,
\]

\[
\chi_{AB}(t)
=
c_A^{\mathsf T}
K^{-1/2}
\sin
\left(
K^{1/2}t
\right)
c_B.
\]

同様に \(\chi_{BA}\) を得る。従って、\(c_A^{\mathsf T}c_B=0\) でも \(\chi_{AB}(t)\) は一般に零ではない。\(K\) が \(c_A,c_B\) の張る部分空間を別々に不変にするときだけ、線形交差応答は厳密に消える。

局所測定窓 \(0\leq t\leq T_{\rm meas}\) で

\[
\varepsilon_{\rm loc}
=
\frac{
\sup_t
\max
\left(
|\chi_{AB}(t)|,
|\chi_{BA}(t)|
\right)
}{
\sup_t
\min
\left(
|\chi_{AA}(t)|,
|\chi_{BB}(t)|
\right)
}
\]

を用いる理由はここにある。非線形補正については、指定した準備領域のまわりで変分方程式を解き、同じ比を定義する。

\hypertarget{ux5c40ux6240ux7d50ux679cux7b26ux53f7ux5316ux306eux751fux6210ux5b50}{%
\section{局所結果符号化の生成子}\label{ux5c40ux6240ux7d50ux679cux7b26ux53f7ux5316ux306eux751fux6210ux5b50}}

結果種座標を \(s_X\)、伝達ベクトルを \((Q_X,P_X)\)、応答モードを \((x_X,p_X)\) とする。平坦結果領域上で \(\sigma(s_X)=X\in\{-1,+1\}\) とする。

局所分析の生成子を

\[
K_X^{\rm an}
=
-
\left[
\phi(a_X)
+
\pi\chi_-(s_X)
\right]
I_X
-
x_X\sigma(s_X),
\]

\[
I_X
=
\frac12
\left(
Q_X^2+P_X^2
\right),
\qquad
\chi_-(s)
=
\frac{
1-\sigma(s)
}{
2
}
\]

とする。生成子の単位流れで、

\[
u_X^{\rm out}
=
X R[\phi(a_X)]u_X^{\rm in},
\qquad
p_X^{\rm out}
=
p_X^{\rm in}+X.
\]

\(p_X^{\rm in}=0\) なら \(p_X^{\rm out}=X\) である。

固定指針対を \((Y_X,\Pi_X)\) とし、平坦関数 \(\zeta(p_X)\) が \(p_X=\pm1\) の近傍で \(\pm1\) を取るとする。転写生成子

\[
K_X^{\rm lock}
=
-Y_X\zeta(p_X)
\]

の単位流れは

\[
\Pi_X^{\rm out}
=
\Pi_X^{\rm in}
+
\zeta(p_X).
\]

\(\Pi_X^{\rm in}=0\) なら \(\Pi_X^{\rm out}=X\) となる。

自由 Hamiltonian と幅 \(\tau_{\rm pulse}\) のパルスが同時に働く場合、理想単位流れとの差は、有界な適用領域で

\[
O(\varepsilon_{\rm pulse}),
\qquad
\varepsilon_{\rm pulse}
=
\tau_{\rm pulse}
\sup_{\mathcal K}
\left\|
X_{H_0}
\right\|
\]

と評価する。局所浴の交差応答も加えると、指針の誤差は

\[
O
\left(
\varepsilon_{\rm pulse}
+
\varepsilon_{\rm loc}
+
\varepsilon_{\rm open}
\right).
\]

この補助装置は既存の結果種を記録するだけであり、一般測定器ではない。

\hypertarget{ux548cux5deeux57faux5e95ux306eux6b63ux6e96ux6027}{%
\section{和・差基底の正準性}\label{ux548cux5deeux57faux5e95ux306eux6b63ux6e96ux6027}}

左右伝達ベクトルを正準対 \((Q_A,P_A)\)、\((Q_B,P_B)\) とする。和・差座標を

\[
Q_\pm
=
\frac{
Q_A\pm Q_B
}{
\sqrt2
},
\qquad
P_\pm
=
\frac{
P_A\pm P_B
}{
\sqrt2
}
\]

と定める。

\begin{proposition}[和・差変換の正準性]
上の変換は正準であり、

$$
P_A\,dQ_A
+
P_B\,dQ_B
=
P_+\,dQ_+
+
P_-\,dQ_-.
$$
\end{proposition}

\begin{proof}
変換行列

$$
U
=
\frac1{\sqrt2}
\begin{pmatrix}
1&1\\
1&-1
\end{pmatrix}
$$

は直交行列である。座標と運動量へ同じ $U$ を作用させるため、C.1節と同じ計算で正準1形式が保存される。
\end{proof}

和・差作用は

\[
I_\pm
=
\frac12
\left(
Q_\pm^2+P_\pm^2
\right)
=
\frac14
\left\|
u_A\pm u_B
\right\|^2.
\]

直交性から、

\[
I_++I_-
=
I_A+I_B.
\]

設定と結果は \(I_+-I_-\) に入るが、総入力作用には入らない。

固定された \(U\) は1つの正準写像であり、測度を生成しない。入力集団が Liouville 測度を持てば保存するが、単一入力を一様集団へ変えない。

\hypertarget{ux30e2ux30fcux30c9ux4f5cux7528ux3068-u3-ux751fux6210ux5b50}{%
\section{3モード作用と U(3) 生成子}\label{ux30e2ux30fcux30c9ux4f5cux7528ux3068-u3-ux751fux6210ux5b50}}

境界3モードを複素正準変数

\[
a
=
\begin{pmatrix}
a_+\\
a_s\\
a_r
\end{pmatrix},
\qquad
a_\nu
=
\frac{
q_\nu+ip_\nu
}{
\sqrt2
}
\]

で表す。Poisson 括弧を

\[
\left\{
a_j,a_k^*
\right\}
=
-i\delta_{jk}
\]

とする。総作用は

\[
C
=
a^\dagger a.
\]

\(T_\alpha\) を \(u(3)\) の Hermitian 基底とし、

\[
L_\alpha
=
a^\dagger T_\alpha a
\]

を Hamiltonian 生成子とする。\(L_\alpha\) の流れは

\[
\dot a
=
-iT_\alpha a
\]

であり、

\[
\left\{
C,L_\alpha
\right\}
=
0.
\]

従って全ての \(U(3)\) 生成子は総作用殻に接する。

構造化浴の未読変数 \(\xi_\alpha(z_{\mathcal B})\) と弱く結合する候補 Hamiltonian を

\[
H_{\rm iso-link}
=
\epsilon_{\rm iso}
\sum_{\alpha=1}^{9}
\xi_\alpha(z_{\mathcal B})
L_\alpha(a)
\]

とする。各瞬間の全 Hamiltonian 流れは \(C\) を保存する。

浴相関が準備窓で

\[
\left\langle
\xi_\alpha(t)
\xi_\beta(0)
\right\rangle
\simeq
\delta_{\alpha\beta}
\kappa(t)
\]

となり、相関時間が境界モードの緩和時間より短いとする。弱結合・長時間尺度での2次縮約生成子は概念的に

\[
\mathcal L_{\rm eff}
=
D_\partial
\sum_{\alpha=1}^{9}
X_{L_\alpha}^2
\]

となる。同次空間 \(U(3)/U(2)\) 上では、この Casimir 作用素は規格化を除いて \(\Delta_{S^5}\) に一致する。

この縮約には、少なくとも次の近似が必要である。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \(\epsilon_{\rm iso}\ll1\) の弱結合。
\item
  浴相関時間と準備時間の分離。
\item
  9方向の相関行列の等方性。
\item
  有限浴の再帰時間より短い準備窓。
\item
  外部交換による長時間再位相化の抑制。
\end{enumerate}

本論文は、特定の有限 \(V_{\rm nl}\) と外部結合について、これらの条件から \(\mathcal L_{\rm eff}\) への一様誤差上界を証明しない。従ってこれはミクロ実現候補であり、導出済みの定理ではない。

\hypertarget{ux7b49ux65b9ux62e1ux6563ux306eux5b9aux5e38ux6e2cux5ea6}{%
\section{等方拡散の定常測度}\label{ux7b49ux65b9ux62e1ux6563ux306eux5b9aux5e38ux6e2cux5ea6}}

固定殻 \(S_{C_0}^5\) 上の密度 \(f\) が

\[
\partial_t f
=
D_\partial\Delta_{S^5}f
\]

に従うとする。定常解は

\[
\Delta_{S^5}f=0
\]

を満たす。部分積分により、

\[
0
=
\int
f\Delta_{S^5}f\,d\mu_{C_0}
=
-
\int
\left|
\nabla_{S^5}f
\right|^2
d\mu_{C_0}.
\]

従って \(f\) は定数であり、規格化すれば \(f=1\) である。

異方摂動を

\[
\mathcal L_\varepsilon
=
D_\partial\Delta_{S^5}
+
\varepsilon_{\rm aniso}\mathcal L_1
\]

とする。\(\mathcal L_1\) が質量を保存し、等方生成子のスペクトルギャップに対して相対有界なら、定常密度の形式展開は

\[
f_\varepsilon
=
1
-
\frac{
\varepsilon_{\rm aniso}
}{
D_\partial
}
\left(
\Delta_{S^5}
\right)^{-1}_{0}
\mathcal L_1^*1
+
O
\left(
\varepsilon_{\rm aniso}^2
\right)
\]

となる。逆作用素の添字0は平均零部分空間への制限である。この式は、異方性の影響が結合定数だけでなくスペクトルギャップに依存することを示す。

\hypertarget{ux56faux5b9aux6bbbux4f53ux7a4dux3068ux5468ux8fbaux5bc6ux5ea6}{%
\section{固定殻体積と周辺密度}\label{ux56faux5b9aux6bbbux4f53ux7a4dux3068ux5468ux8fbaux5bc6ux5ea6}}

作用角変数で、3モード固定殻の未規格化体積は

\[
\Omega_3(C_0)
=
(2\pi)^3
\int_0^\infty dJ_+
\int_0^\infty dJ_s
\int_0^\infty dJ_r\,
\delta
\left(
C_0-J_+-J_s-J_r
\right).
\]

\(J_r\) を積分すると、\(J_+,J_s\geq0\) かつ \(J_++J_s\leq C_0\) の三角形が残る。従って、

\[
\Omega_3(C_0)
=
(2\pi)^3
\int_0^{C_0}dJ_+
\int_0^{C_0-J_+}dJ_s
=
\frac{
(2\pi)^3C_0^2
}{
2
}.
\]

\(J_+=x\) の未規格化周辺は

\[
\omega_+(x)
=
(2\pi)^3
\int_0^\infty dJ_s
\int_0^\infty dJ_r\,
\delta
\left(
C_0-x-J_s-J_r
\right)
\]

\[
=
(2\pi)^3
\left(
C_0-x
\right)
\mathbf1_{\{0\leq x\leq C_0\}}.
\]

従って、

\[
p_+(x)
=
\frac{
\omega_+(x)
}{
\Omega_3(C_0)
}
=
\frac{
2(C_0-x)
}{
C_0^2
}.
\]

固定 \(x\) の条件付き分布では、

\[
p
\left(
J_s\mid J_+=x
\right)
=
\frac1{C_0-x}
\]

である。条件付き密度の \(1/(C_0-x)\) と、ファイバー総質量の \(C_0-x\) を混同してはならない。

\hypertarget{coarea-ux3068ux5883ux754cux30d5ux30a1ux30a4ux30d0ux30fc}{%
\section{coarea と境界ファイバー}\label{coarea-ux3068ux5883ux754cux30d5ux30a1ux30a4ux30d0ux30fc}}

一般の境界正準位相空間を \(\Gamma_\partial\)、基準体積を \(d\Gamma_\partial\) とする。2つの制約を

\[
F_1
=
C_0-J_+-J_s-J_r,
\]

\[
F_2
=
J_+-I_+^{AB}
\]

とする。理想線形モデルでは、

\[
W_{AB}
\propto
\int_{\Gamma_\partial}
\delta(F_1)
\delta(F_2)
d\Gamma_\partial.
\]

作用角座標に変換すると、変換 Jacobian は1である。\(J_+\) と \(J_r\) のデルタ関数積分を行えば、

\[
W_{AB}
\propto
(2\pi)^3
\int_0^{C_0-I_+^{AB}}dJ_s
\]

\[
=
(2\pi)^3
\left(
C_0-I_+^{AB}
\right).
\]

一般の滑らかな境界写像 \(F=(F_1,F_2)\) では coarea 公式により、

\[
\int_{\Gamma_\partial}
\rho(z)
\delta
\left(
F(z)
\right)
d\Gamma_\partial
=
\int_{F^{-1}(0)}
\frac{
\rho(z)
}{
J_F(z)
}
d\Sigma(z),
\]

\[
J_F
=
\sqrt{
\det
\left[
DF
\left(
DF
\right)^{\mathsf T}
\right]
}.
\]

理想作用座標では \(J_F\) が結果と設定に共通な定数へなる。非線形境界写像、分岐、caustic、解多重度がある場合は、この単純化を使えない。同じ巨視的結果へ対応する複数の解は、その局所 Jacobian と多重度を含めて数える。

\hypertarget{ux6709ux9650ux5206ux89e3ux80fdux306eux5c55ux958b}{%
\section{有限分解能の展開}\label{ux6709ux9650ux5206ux89e3ux80fdux306eux5c55ux958b}}

偶関数 \(K\) を

\[
\int_{\mathbb R}K(y)\,dy=1,
\qquad
\int_{\mathbb R}yK(y)\,dy=0
\]

と規格化し、

\[
K_{\delta_J}(y)
=
\frac1{\delta_J}
K
\left(
\frac y{\delta_J}
\right)
\]

とする。\(J_+\) 周辺密度 \(p_+(x)\) は殻内部で線形なので、窓が殻端に触れない限り、

\[
\int
p_+(x)
K_{\delta_J}(x-I_+)\,dx
=
p_+(I_+)
\]

が偶対称窓では厳密に成り立つ。一般の滑らかな Jacobian または殻幅分布を含めると、

\[
W_{AB}^{(\delta_J)}
=
c_{\delta_J}
\left(
C_0-I_+^{AB}
\right)
+
O
\left(
\delta_J^2
\sup
\left|
\partial_{J_+}^2
\frac{\rho}{J_F}
\right|
\right).
\]

従って、理想線形作用殻では有限分解能自体が1次の角度誤差を生まない。主要な分解能誤差は、殻端の切断、非線形 Jacobian、結果依存窓、総作用幅との重なりから生じる。

\hypertarget{ux534aux5f84ux65b9ux5411ux306eux5f31ux958bux653eux529bux5b66}{%
\section{半径方向の弱開放力学}\label{ux534aux5f84ux65b9ux5411ux306eux5f31ux958bux653eux529bux5b66}}

総作用 \(C\) の縮約式を

\[
dC_t
=
-\gamma_C
\left(
C_t-C_0
\right)dt
+
\sqrt{2D_C}\,dW_t
\]

とする。定常密度は

\[
\rho_C(C)
\propto
\exp
\left[
-
\frac{
\gamma_C
}{
2D_C
}
\left(
C-C_0
\right)^2
\right],
\]

その分散は

\[
\sigma_C^2
=
\frac{D_C}{\gamma_C}
\]

である。

この式は、外部への漏れと流入を線形化した縮約候補である。基礎 Hamiltonian の総エネルギーが確率的に失われると仮定したものではない。外部環境と仕事源を消去した有限部分の有効式として読む。

角方向と半径方向が近似的に分離する条件を

\[
\tau_{\rm corr}
\ll
\tau_{\rm mix}
\ll
\tau_C,
\qquad
\tau_C=\gamma_C^{-1}
\]

とする。\(\tau_{\rm corr}\) は浴相関時間、\(\tau_{\rm mix}\) は殻接方向混合、\(\tau_C\) は半径変化である。\(\tau_{\rm mix}\ll\tau_C\) なら、各半径で方向分布が先に等方化する。

純粋漏れでは

\[
dC_t=-\gamma_CC_t\,dt
\]

となり、非零の定常殻は存在しない。流入または仕事源を含む復元項は、作用殻準備に不可欠である。

\hypertarget{ux30dfux30afux30edux69cbux6210ux304bux3089ux672aux5c0eux51faux306eux4e8bux9805}{%
\section{ミクロ構成から未導出の事項}\label{ux30dfux30afux30edux69cbux6210ux304bux3089ux672aux5c0eux51faux306eux4e8bux9805}}

本付録で厳密に示したのは次である。

\begin{itemize}
\tightlist
\item
  1つの有限浴に対する静的な直交正準基底。
\item
  線形浴の交差応答核。
\item
  最小結果符号化器の理想正準写像。
\item
  和・差変換の正準性と作用保存。
\item
  \(U(3)\) Hamiltonian 生成子が総作用を保存すること。
\item
  3モード固定作用殻の体積、周辺密度、残余ファイバー体積。
\item
  理想線形境界写像の共通 coarea Jacobian。
\end{itemize}

次は未導出である。

\begin{itemize}
\tightlist
\item
  特定の有限非線形浴が必要な時間窓で等方な相関行列を持つこと。
\item
  有限浴と常時の外部交換から \(D_\partial\Delta_{S^5}\) を一様誤差付きで得ること。
\item
  異方誤差、混合時間、再帰時間を同じパラメータから同時に閉じること。
\item
  半径方向の復元式とエネルギー収支を明示的な外部 Hamiltonian から導くこと。
\item
  一般測定器、境界適合、記録、消去、再初期化を1本の有限幅 Hamiltonian へ統合すること。
\end{itemize}

従って、作用殻の幾何計算は厳密結果、全殻拡散と半径安定化は縮約候補、現行の弱開放モデルへの接続は近似または予想・未解決として扱う。

\begin{thebibliography}{99}

\addcontentsline{toc}{chapter}{参考文献}

\bibitem{bell1964} J. S. Bell, ``On the Einstein Podolsky Rosen Paradox,'' Physics Physique Fizika 1, 195--200 (1964). \url{https://doi.org/10.1103/PhysicsPhysiqueFizika.1.195}

\bibitem{chsh1969} J. F. Clauser, M. A. Horne, A. Shimony, and R. A. Holt, ``Proposed Experiment to Test Local Hidden-Variable Theories,'' Physical Review Letters 23, 880--884 (1969). \url{https://doi.org/10.1103/PhysRevLett.23.880}

\bibitem{nelson1966} E. Nelson, ``Derivation of the Schrödinger Equation from Newtonian Mechanics,'' Physical Review 150, 1079--1085 (1966). \url{https://doi.org/10.1103/PhysRev.150.1079}

\bibitem{guerra_morato1983} F. Guerra and L. M. Morato, ``Quantization of Dynamical Systems and Stochastic Control Theory,'' Physical Review D 27, 1774--1786 (1983). \url{https://doi.org/10.1103/PhysRevD.27.1774}

\bibitem{yasue1981} K. Yasue, ``Stochastic Calculus of Variations,'' Journal of Functional Analysis 41, 327--340 (1981). \url{https://doi.org/10.1016/0022-1236(81)90079-3}

\bibitem{zambrini1986} J.-C. Zambrini, ``Stochastic Mechanics According to E. Schrödinger,'' Physical Review A 33, 1532--1548 (1986). \url{https://doi.org/10.1103/PhysRevA.33.1532}

\bibitem{wharton2010} K. B. Wharton, ``Time-Symmetric Boundary Conditions and Quantum Foundations,'' Symmetry 2, 272--283 (2010). \url{https://doi.org/10.3390/sym2010272}

\bibitem{wharton_argaman2020} K. B. Wharton and N. Argaman, ``Colloquium: Bell's Theorem and Locally Mediated Reformulations of Quantum Mechanics,'' Reviews of Modern Physics 92, 021002 (2020). \url{https://doi.org/10.1103/RevModPhys.92.021002}

\bibitem{hall2010} M. J. W. Hall, ``Local Deterministic Model of Singlet State Correlations Based on Relaxing Measurement Independence,'' Physical Review Letters 105, 250404 (2010). \url{https://doi.org/10.1103/PhysRevLett.105.250404}

\bibitem{leifer_pusey2017} M. S. Leifer and M. F. Pusey, ``Is a Time Symmetric Interpretation of Quantum Theory Possible without Retrocausality?,'' Proceedings of the Royal Society A 473, 20160607 (2017). \url{https://doi.org/10.1098/rspa.2016.0607}

\bibitem{wood_spekkens2015} C. J. Wood and R. W. Spekkens, ``The Lesson of Causal Discovery Algorithms for Quantum Correlations,'' New Journal of Physics 17, 033002 (2015). \url{https://doi.org/10.1088/1367-2630/17/3/033002}

\bibitem{ford1965} G. W. Ford, M. Kac, and P. Mazur, ``Statistical Mechanics of Assemblies of Coupled Oscillators,'' Journal of Mathematical Physics 6, 504--515 (1965). \url{https://doi.org/10.1063/1.1704304}

\bibitem{mori1965} H. Mori, ``Transport, Collective Motion, and Brownian Motion,'' Progress of Theoretical Physics 33, 423--455 (1965). \url{https://doi.org/10.1143/PTP.33.423}

\bibitem{zwanzig1973} R. Zwanzig, ``Nonlinear Generalized Langevin Equations,'' Journal of Statistical Physics 9, 215--220 (1973). \url{https://doi.org/10.1007/BF01008729}

\bibitem{jamison1974} B. Jamison, ``Reciprocal Processes,'' Zeitschrift für Wahrscheinlichkeitstheorie und Verwandte Gebiete 30, 65--86 (1974). \url{https://doi.org/10.1007/BF00532864}

\bibitem{doob1957} J. L. Doob, ``Conditional Brownian Motion and the Boundary Limits of Harmonic Functions,'' Bulletin de la Société Mathématique de France 85, 431--458 (1957). \url{https://doi.org/10.24033/bsmf.1495}

\bibitem{landauer1961} R. Landauer, ``Irreversibility and Heat Generation in the Computing Process,'' IBM Journal of Research and Development 5, 183--191 (1961). \url{https://doi.org/10.1147/rd.53.0183}

\bibitem{bennett1982} C. H. Bennett, ``The Thermodynamics of Computation: A Review,'' International Journal of Theoretical Physics 21, 905--940 (1982). \url{https://doi.org/10.1007/BF02084158}

\bibitem{wallstrom1994} T. C. Wallstrom, ``Inequivalence between the Schrödinger Equation and the Madelung Hydrodynamic Equations,'' Physical Review A 49, 1613--1617 (1994). \url{https://doi.org/10.1103/PhysRevA.49.1613}

\bibitem{price_wharton2023} H. Price and K. Wharton, ``Bell Correlations as Selection Artefacts,'' arXiv:2309.10969v3 (2024). \url{https://arxiv.org/abs/2309.10969}

\bibitem{price_wharton2024} H. Price and K. Wharton, ``A Mechanism for Entanglement?,'' arXiv:2406.04571v1 (2024). \url{https://arxiv.org/abs/2406.04571}

\bibitem{argaman2010} N. Argaman, ``Bell's Theorem and the Causal Arrow of Time,'' American Journal of Physics 78, 1007--1013 (2010). \url{https://doi.org/10.1119/1.3456564}

\bibitem{hossenfelder_palmer2020} S. Hossenfelder and T. Palmer, ``Rethinking Superdeterminism,'' Frontiers in Physics 8, 139 (2020). \url{https://doi.org/10.3389/fphy.2020.00139}

\bibitem{thooft2016} G. 't Hooft, The Cellular Automaton Interpretation of Quantum Mechanics, Springer (2016). \url{https://doi.org/10.1007/978-3-319-41285-6}

\bibitem{leonard2014} C. Léonard, ``A Survey of the Schrödinger Problem and Some of Its Connections with Optimal Transport,'' Discrete and Continuous Dynamical Systems A 34, 1533--1574 (2014). \url{https://doi.org/10.3934/dcds.2014.34.1533}

\bibitem{chen_georgiou_pavon2016} Y. Chen, T. T. Georgiou, and M. Pavon, ``On the Relation between Optimal Transport and Schrödinger Bridges: A Stochastic Control Viewpoint,'' Journal of Optimization Theory and Applications 169, 671--691 (2016). \url{https://doi.org/10.1007/s10957-015-0803-z}

\bibitem{rauch_tung_striebel1965} H. E. Rauch, F. Tung, and C. T. Striebel, ``Maximum Likelihood Estimates of Linear Dynamic Systems,'' AIAA Journal 3, 1445--1450 (1965). \url{https://doi.org/10.2514/3.3166}

\bibitem{fuchs_goldt_seifert2016} J. Fuchs, S. Goldt, and U. Seifert, ``Stochastic Thermodynamics of Resetting,'' Europhysics Letters 113, 60009 (2016). \url{https://doi.org/10.1209/0295-5075/113/60009}

\bibitem{evans_majumdar_schehr2020} M. R. Evans, S. N. Majumdar, and G. Schehr, ``Stochastic Resetting and Applications,'' Journal of Physics A: Mathematical and Theoretical 53, 193001 (2020). \url{https://doi.org/10.1088/1751-8121/ab7cfe}

\bibitem{knorst_lopes2024} J. Knorst and A. O. Lopes, ``On the Quantum Guerra--Morato Action Functional,'' Journal of Mathematical Physics 65, 082102 (2024). \url{https://doi.org/10.1063/5.0207422}

\bibitem{wilson_et_al2021} J. T. Wilson, V. Borovitskiy, A. Terenin, P. Mostowsky, and M. P. Deisenroth, ``Pathwise Conditioning of Gaussian Processes,'' Journal of Machine Learning Research 22, 1--47 (2021). \url{https://jmlr.org/papers/v22/20-1260.html}

\bibitem{leonard_roelly_zambrini2014} C. Léonard, S. Rœlly, and J.-C. Zambrini, ``Reciprocal Processes. A Measure-Theoretical Point of View,'' Probability Surveys 11, 237--269 (2014). \url{https://doi.org/10.1214/13-PS220}

\bibitem{marchiori_deaguiar2011} M. A. Marchiori and M. A. M. de Aguiar, ``Energy Dissipation Via Coupling With a Finite Chaotic Environment,'' Physical Review E 83, 061112 (2011). \url{https://doi.org/10.1103/PhysRevE.83.061112}

\end{thebibliography}


\end{document}
