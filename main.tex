...
\hypersetup{
  ...
  citecolor=PaperBlue,
  urlcolor=PaperBlue,
  pdftitle={有限 Hamiltonian 中核の定量的 Nelson 極限と時間対称境界測度による Bell 型統計},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{PaperBlue}定量的 Nelson 極限と境界測度による Bell 型統計}
...
\begin{titlepage}
  ...
  {\sffamily\bfseries\color{PaperNavy}\fontsize{21}{30}\selectfont
    有限 Hamiltonian 中核の定量的 Nelson 極限と\\[2mm]
    時間対称境界測度による Bell 型統計\par}
  \vspace{9mm}
  {\Large\color{PaperBlue}有限 Gaussian 縮約、二モード台帳、相補的内部時計\par}
  \vspace{14mm}
  {\color{PaperRule}\rule{0.68\textwidth}{0.9pt}\par}
  \vfill
  {\large 相補時計による終端半空間の正準実現 改訂稿\par}
  \vspace{22mm}
\end{titlepage}
...
が従う。本論文ではこの対称準備条件を \texttt{{[}S{]}} と呼ぶ。従って旧 equilibrium 仮定は、sector 内の一様密度を与える二モード定理と、sector 質量をそろえる準備対称性へ分解される。

時間対称境界統計原理 \texttt{{[}R{]}} は、基準初期密度 \(\rho_S\) と固定 terminal function \(G_R\) から物理的履歴測度を
...
と定める。\texttt{{[}R{]}} は有限性、再帰性、時間反転対称性だけから導かれるとは仮定しない。これは Hamilton 方程式とは独立に、どの完結履歴を物理的確率集団とみなすかを定める境界原理である。
...
本論文の Bell 部分は、順時間的な共有浴ノイズ、ゲート通過速度、長い滞在時間によって過去の結果頻度を作り直す模型ではない。結果確定後の有限浴と reaction coordinate は記録保持、比較、完了事象を物理化するが、試行測度を定めるのは \texttt{{[}R{]}} である。また、soft mode が複数の通常浴 mode と自由にエネルギー交換すると、
...
相補的時計は terminal half-space を正準実現するが、\(\Pi_R(T)<0\) の軌道を Hamilton 方程式から消去しない。この軌道では時計向きが交換されるだけである。各時計枝をそれぞれの clock-past 境界から準備し、同一 Hamiltonian 履歴として matching する二境界積測度を追加すれば \texttt{{[}R{]}} の積形式を得るが、その matching rule 自体が追加の統計原理である。また二つの向きに対応する相補的半空間を同じ読出しに対して等重みで平均すると、\(x=E_*+\kappa I_-\) に対し
...
本論文は \texttt{{[}R{]}} の物理的必然性、任意初期密度からの厳密な一様化、任意の非平衡準備に対する無信号性、cos 則または Tsirelson 限界の一意な選択、一般の非 Gaussian 状態や節を持つ状態、および Wallstrom の位相量子化問題を解かない。確立するのは、有限 Hamiltonian 測定器、対称準備、二モード台帳、固定 terminal condition、時間対称境界測度を組み合わせたときの明示的な Bell compatibility 定理、および terminal half-space の相補時計実現とその限界である。

\hypertarget{ux8ad6ux7406ux69cbux9020}{%
\section{論理構造}\label{ux8ad6ux7406ux69cbux9020}}
...
\bottomrule\noalign{}
\endlastfoot
可逆中核 \texttt{{[}H{]}} & 有限2次 Hamiltonian & 系と調和浴の微視運動 & 明示構成 \\
Gaussian 縮約 & 有限 Fourier 経路法則 & 観測座標の粗視化 & 厳密計算 \\
Nelson 極限 & 繰り込み作用のパラメータ \(C^1\) 収束 & 有効確率力学の変分構造 & 主定理 \\
局所測定器 \texttt{{[}H{]}} & bright response、anchor record、有限浴 & definite local record & 明示構成 \\
phase-locked source \texttt{{[}P{]}} & 二つの実正準ベクトル & 差動作用の cos 幾何 & 準備条件 \\
対称準備 \texttt{{[}S{]}} & 符号反転不変な基準測度 & \(w_{AB}=1/4\) & 対称性からの系 \\
二モード台帳 & 固定 \(J_s+J_0\) 殻 & \(p(h)=1/E_\ell\) & 不変測度について厳密 \\
粗視化混合 \texttt{{[}M{]}} & 有限非線形 mixer & 入口測度の典型化 & 時間尺度仮定 \\
相補的内部時計 & \((\bar\tau,P_c;Y_R,\Pi_R)\) & terminal half-space の正準実現 & 半空間は厳密、測度選択は未導出 \\
境界統計 \texttt{{[}R{]}} & \(G_R\circ\Phi_{a,b}^{T}\) & 物理的履歴測度 & 独立原理 \\
Bell compatibility & terminal-compatible phase volume & cos 共同確率 & \texttt{{[}H,P,S,M,R{]}} の下で厳密 \\
\end{longtable}

\hypertarget{ux672cux6587ux69cbux6210}{%
\section{本文構成}\label{ux672cux6587ux69cbux6210}}

第1章は二部の独立性、有限測定窓、五つの仮定 \texttt{{[}H{]}}、\texttt{{[}P{]}}、\texttt{{[}S{]}}、\texttt{{[}M{]}}、\texttt{{[}R{]}} を定める。第I部の第2章から第4章は、可逆な調和 Hamiltonian 中核、Gaussian 条件づけ、定量的 \(C^1\) Nelson 極限を扱う。第II部の第5章は局所測定器、境界集団、相補的内部時計による terminal half-space の正準実現、第6章は共通未来の比較器と二モード台帳、第7章は終端整合測度と Bell 共同確率、第8章は頑健性、反証条件、証明台帳を扱う。付録は Fourier--Schur 評価、\(C^1\) 評価、明示 Hamiltonian、相補時計の二境界 matching と向き平均 no-go、台帳位相体積、Gaussian Nelson 方程式を収録する。

\hypertarget{ux554fux984cux8a2dux5b9aux4e8cux90e8ux69cbux6210ux4eeeux5b9aux306eux53f0ux5e33}{%
\chapter{問題設定、二部構成、仮定の台帳}\label{ux554fux984cux8a2dux5b9aux4e8cux90e8ux69cbux6210ux4eeeux5b9aux306eux53f0ux5e33}}
...
\tightlist
\item
  可逆な有限 Hamiltonian 系の Gaussian 縮約から、Nelson 型作用をどこまで定量的に得られるか。
\item
  有限 Hamiltonian 測定器に時間対称な境界統計原理を加えたとき、setting-blind な終端条件の整合体積から Bell 型共同確率を構成できるか。
\end{enumerate}

第1の問題には、第4章のパラメータ \(C^1\) 収束定理として答える。第2の問題には、第7章の二モード台帳 Bell compatibility 定理として答える。ただし第2の定理は、Hamilton 方程式だけから全履歴測度を導く定理ではない。物理的試行測度を定める境界原理 \texttt{{[}R{]}} を明示的に入力する。

\hypertarget{ux4e8cux90e8ux306eux8ad6ux7406ux7684ux72ecux7acbux6027}{%
\section{二部の論理的独立性}\label{ux4e8cux90e8ux306eux8ad6ux7406ux7684ux72ecux7acbux6027}}
...
は別の論理鎖である。第I部だけを用いる読者は第4章で完結でき、第II部だけを検討する読者は \texttt{{[}R{]}} と準備条件を独立に監査できる。

\hypertarget{ux6709ux9650ux9589ux9396ux6e2cux5b9aux7a93}{%
\section{有限閉鎖測定窓}\label{ux6709ux9650ux9589ux9396ux6e2cux5b9aux7a93}}
...
\hypertarget{ux4eeeux5b9aux306eux53f0ux5e33}{%
\section{仮定の台帳}\label{ux4eeeux5b9aux306eux53f0ux5e33}}

第II部の仮定を次の五つに分ける。

\hypertarget{h-ux6709ux9650-hamiltonian-ux69cbux6210}{%
\subsection{{[}H{]} 有限 Hamiltonian 構成}\label{h-ux6709ux9650-hamiltonian-ux69cbux6210}}
...
\hypertarget{p-phase-locked-source}{%
\subsection{{[}P{]} phase-locked source}\label{p-phase-locked-source}}
...
\texttt{{[}R{]}} を適用する前の preparation Hamiltonian、準備 macroregion、基準 Liouville 測度は、左右の outcome seed を独立に反転する二つの変換に不変である。四つの seed sector が一つの軌道に沿って相互遷移する必要はない。必要なのは、基準測度に関して四 sector が同じ体積を持つことである。

\hypertarget{m-ux4e8cux30e2ux30fcux30c9ux53f0ux5e33ux306eux5165ux53e3ux6e2cux5ea6}{%
\subsection{{[}M{]} 二モード台帳の入口測度}\label{m-ux4e8cux30e2ux30fcux30c9ux53f0ux5e33ux306eux5165ux53e3ux6e2cux5ea6}}
...
\hypertarget{r-ux6642ux9593ux5bfeux79f0ux5883ux754cux7d71ux8a08ux539fux7406}{%
\subsection{{[}R{]} 時間対称境界統計原理}\label{r-ux6642ux9593ux5bfeux79f0ux5883ux754cux7d71ux8a08ux539fux7406}}
...
とする。\(G_R\) は setting label、outcome label、目標相関を引数に持たない。\texttt{{[}R{]}} は有限性、再帰性、時間反転対称性から一意に導かれるとは主張しない。これはどの terminal-compatible histories を物理的 ensemble とみなすかを定める追加の境界統計原理である。相補的内部時計は後に \(G_R=\mathbf1_{\{\Pi_R\geq0\}}\) の半空間を時計向き保存条件として実現するが、その半空間だけから物理的履歴測度を選ぶ原理は出ない。

\hypertarget{s-ux3068ux65e7-equilibrium-ux4eeeux5b9a}{%
\section{\texorpdfstring{\texttt{{[}S{]}} と旧 equilibrium 仮定}{{[}S{]} と旧 equilibrium 仮定}}\label{s-ux3068ux65e7-equilibrium-ux4eeeux5b9a}}
...
\tightlist
\item
  sector 内で soft energy が一様であることは、固定総作用を持つ二モード台帳の Liouville 幾何から導く。
\item
  sector の基準質量が等しいことは、準備測度の独立符号反転対称性から導く。
\end{enumerate}
...
のうち、\(1/E_\ell\) は二モード定理、\(w_{AB}=1/4\) は \texttt{{[}S{]}} の対称性系論である。Hamiltonian の形だけでは初期密度を決められないため、準備測度という統計条件まで消えるわけではない。

\hypertarget{ux4e00ux8a66ux884cux306eux7269ux7406ux7684ux6642ux7cfbux5217}{%
\section{一試行の物理的時系列}\label{ux4e00ux8a66ux884cux306eux7269ux7406ux7684ux6642ux7cfbux5217}}
...
\tightlist
\item
  phase-locked messenger pair と outcome seed を準備する。
\item
  左右の controller macrostate により設定 \(a,b\) を選ぶ。
\item
  左右の局所 analyzer が messenger を回転し、局所 outcome \(A,B\) を生成する。
\item
  bright response mode が局所信号を受ける。
\item
  anchor pointer が符号を記録し、局所有限浴が bright transient を分散する。
\item
  記録の写しと messenger を通常の時間順序で共通未来へ搬送する。
\item
  setting-blind comparator が差動作用 \(I_-\) を計算する。
\item
  相補時計の初期相対運動量を \(\Pi_R(0)=E_*\) とし、二モード台帳の soft energy \(h\) と差動作用 \(\kappa I_-\) による kick を加える。
\item
  最終相対時計運動量 \(\Pi_R(T)=E_*+\kappa I_- -h\) に固定 terminal macroregion \(G_R\) を課す。
\item
  \texttt{{[}R{]}} により terminal-compatible histories の規格化測度を物理的試行集団とする。
\end{enumerate}

左と右は局所記録が形成される前には共通浴を持たない。共通未来の相互作用は、すでに確定した記録の写しを処理する。このため Bell 相関を「共有浴ノイズが左右へ順時間的に漏れた結果」とは解釈しない。

\hypertarget{ux4e3bux8981ux7d50ux679c}{%
\section{主要結果}\label{ux4e3bux8981ux7d50ux679c}}
...
\item
  branch-wise boundary preparation と matching を追加したときの \texttt{{[}R{]}} 積形式、および相補時計だけでは測度を選べないという限界。
\item
  \texttt{{[}H,P,S,M,R{]}} の下での Bell 型共同確率。
\item
...
\item
  \texttt{{[}R{]}} が有限 Hamiltonian 力学または相補的時計運動量だけから必然的に導かれること。
\item
...
\hypertarget{ux53efux9006ux306aux8abfux548c-hamiltonian-ux4e2dux6838ux3068ux6709ux9650-gaussian-ux8868ux793a}{%
\chapter{可逆な調和 Hamiltonian 中核と有限 Gaussian 表示}\label{ux53efux9006ux306aux8abfux548c-hamiltonian-ux4e2dux6838ux3068ux6709ux9650-gaussian-ux8868ux793a}}
...
\hypertarget{ux7ddaux5f62-gaussian-ux8a08ux7b97ux6a21ux578b}{%
\section{線形 Gaussian 計算模型}\label{ux7ddaux5f62-gaussian-ux8a08ux7b97ux6a21ux578b}}
...
\hypertarget{ou-ux6a21ux578bux3068-stratonovich-ux8868ux73fe}{%
\section{OU 模型と Stratonovich 表現}\label{ou-ux6a21ux578bux3068-stratonovich-ux8868ux73fe}}
...
\hypertarget{ux524dux5f8cux4e21ux5074ux304bux3089ux6761ux4ef6ux3065ux3051ux305fux7ddaux5f62-gaussian-ux7d4cux8defux6cd5ux5247}{%
\chapter{前後両側から条件づけた線形 Gaussian 経路法則}\label{ux524dux5f8cux4e21ux5074ux304bux3089ux6761ux4ef6ux3065ux3051ux305fux7ddaux5f62-gaussian-ux7d4cux8defux6cd5ux5247}}
...
\hypertarget{ux7121ux6761ux4ef6-gaussian-ux6cd5ux5247}{%
\section{無条件 Gaussian 法則}\label{ux7121ux6761ux4ef6-gaussian-ux6cd5ux5247}}
...
ただし、条件付き経路分布を物理的試行頻度として採用するには、どの完結履歴へ確率を置くかという追加の物理原理が必要である。Gaussian Schur 補完は、記録を与えた後の条件付き法則を計算するが、その法則が実験の無条件頻度として選ばれることまでは証明しない。第II部ではこの役割を時間対称境界統計原理 \texttt{{[}R{]}} として明示し、第4章の Nelson 極限からは導かない。
...
\hypertarget{ux7e70ux308aux8fbcux307fux6e08ux307fux7c97ux8996ux5316ux4f5cux7528ux306e-nelson-ux6975ux9650}{%
\chapter{繰り込み済み粗視化作用の Nelson 極限}\label{ux7e70ux308aux8fbcux307fux6e08ux307fux7c97ux8996ux5316ux4f5cux7528ux306e-nelson-ux6975ux9650}}
...
\hypertarget{ux7ddaux5f62-gaussian-c1-ux53ceux675fux5b9aux7406}{%
\section{\texorpdfstring{線形 Gaussian \(C^1\) 収束定理}{線形 Gaussian C\^{}1 収束定理}}\label{ux7ddaux5f62-gaussian-c1-ux53ceux675fux5b9aux7406}}
...
\hypertarget{ux8abfux548c-gaussian-ux306eux7269ux7406ux50cf}{%
\section{調和 Gaussian の物理像}\label{ux8abfux548c-gaussian-ux306eux7269ux7406ux50cf}}
...
\hypertarget{ux6709ux9650-hamiltonian-ux6e2cux5b9aux5668ux3068ux6642ux9593ux5bfeux79f0ux5883ux754cux96c6ux56e3}{%
\chapter{有限 Hamiltonian 測定器と時間対称境界集団}\label{ux6709ux9650-hamiltonian-ux6e2cux5b9aux5668ux3068ux6642ux9593ux5bfeux79f0ux5883ux754cux96c6ux56e3}}
...
\tightlist
\item
  左右の setting controller。
\item
  局所的に確定する2値 record。
\item
  設定と結果符号を共通未来へ運ぶ messenger。
\item
  二つの messenger を比較する setting-blind な二次形式。
\item
  比較結果と未読台帳変数を照合する terminal coordinate。
\item
  terminal-compatible histories を物理的集団とする境界統計原理。
\end{enumerate}
...
\hypertarget{outcome-seed-ux3068ux8a2dux5b9a-controller}{%
\section{outcome seed と設定 controller}\label{outcome-seed-ux3068ux8a2dux5b9a-controller}}
...
\hypertarget{autonomous-pulse-hamiltonian}{%
\section{autonomous pulse Hamiltonian}\label{autonomous-pulse-hamiltonian}}
...
\hypertarget{ux5c40ux6240-analyzer-ux3068-bright-response}{%
\section{局所 analyzer と bright response}\label{ux5c40ux6240-analyzer-ux3068-bright-response}}
...
\hypertarget{anchor-pointer-ux3078ux306eux8a18ux9332ux56faux5b9a}{%
\section{anchor pointer への記録固定}\label{anchor-pointer-ux3078ux306eux8a18ux9332ux56faux5b9a}}
...
\hypertarget{common-future-propagation}{%
\section{common-future propagation}\label{common-future-propagation}}
...
\(t_C\) 以後に二つの messenger を同じ comparator へ入れる。これは局所記録後の timelike common future における通常の相互作用であり、spacelike separated な記録形成へ遠隔 force を導入しない。第7章で現れる setting dependence は、common-future interaction 自体が過去を変更するためではなく、その interaction を含む全軌道に terminal boundary measure \texttt{{[}R{]}} を適用するためである。

\hypertarget{terminal-function-ux3068ux5c65ux6b74ux6e2cux5ea6}{%
\section{terminal function と履歴測度}\label{terminal-function-ux3068ux5c65ux6b74ux6e2cux5ea6}}
...
\hypertarget{ux76f8ux88dcux7684ux5185ux90e8ux6642ux8a08ux306bux3088ux308b-terminal-half-space-ux306eux6b63ux6e96ux5b9fux73fe}{%
\section{相補的内部時計による terminal half-space の正準実現}\label{ux76f8ux88dcux7684ux5185ux90e8ux6642ux8a08ux306bux3088ux308b-terminal-half-space-ux306eux6b63ux6e96ux5b9fux73fe}}
...
となり、時計向きが交換されるだけである。従って相補的時計は terminal half-space の形を導くが、その半空間に入る履歴だけを物理的 ensemble とする \texttt{{[}R{]}} までは導かない。

\hypertarget{r-ux3068-postselection}{%
\section{\texorpdfstring{\texttt{{[}R{]}} と postselection}{{[}R{]} と postselection}}\label{r-ux3068-postselection}}
...
は、実験後の rejection sampling と同じ条件付き確率に見える。本論文が \texttt{{[}R{]}} を境界 ontology として用いるためには、少なくとも次を要求する。

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \(G_R\) は Bell data を見る前に apparatus の terminal macroregion として固定する。
\item
  全 setting と全 outcome に同じ terminal device と分解能を用いる。
\item
  実現した pointer record を後から除外しない。
\item
  terminal width、ledger energy、comparator scale を独立 calibration で決める。
\item
  external launch count、pointer record count、terminal completion count の関係を報告する。
\end{enumerate}

これらを満たせず、観測済み trial の一部を捨てて初めて Bell 値が出るなら、本構成は detector postselection に退化する。\texttt{{[}R{]}} を公理として書くだけではこの操作的区別は自動的に保証されない。
...
一方、結果頻度を定める原理は局所浴の散逸、pointer の保持時間、comparison の速度、または時計相補性だけではない。物理的 ensemble を定める \texttt{{[}R{]}} と、次章で導く terminal compatibility の位相体積である。

\hypertarget{ux5171ux901aux672aux6765ux306eux6bd4ux8f03ux5668ux3068ux4e8cux30e2ux30fcux30c9ux53f0ux5e33}{%
\chapter{共通未来の比較器と二モード台帳}\label{ux5171ux901aux672aux6765ux306eux6bd4ux8f03ux5668ux3068ux4e8cux30e2ux30fcux30c9ux53f0ux5e33}}

\begin{statusbox}
位置づけ：cos 差動作用と sector 内の一様 soft-energy 密度を別々の位相空間幾何から導く。
\end{statusbox}

\hypertarget{phase-locked-messenger}{%
\section{phase-locked messenger}\label{phase-locked-messenger}}
...
\hypertarget{ux5deeux52d5ux4f5cux7528ux306e-cos-ux5e7eux4f55}{%
\section{差動作用の cos 幾何}\label{ux5deeux52d5ux4f5cux7528ux306e-cos-ux5e7eux4f55}}
...
\hypertarget{amplitude-mismatch-ux3068-phase-noise}{%
\section{amplitude mismatch と phase noise}\label{amplitude-mismatch-ux3068-phase-noise}}
...
\hypertarget{ux4e8cux30e2ux30fcux30c9ux53f0ux5e33}{%
\section{二モード台帳}\label{ux4e8cux30e2ux30fcux30c9ux53f0ux5e33}}
...
\hypertarget{ux4e00ux69d8-soft-energy-ux5468ux8fbaux5b9aux7406}{%
\section{一様 soft-energy 周辺定理}\label{ux4e00ux69d8-soft-energy-ux5468ux8fbaux5b9aux7406}}
...
この定理は、二つの一自由度 harmonic mode の density of states がともに定数であることの帰結である。結果 sector、setting、source phase は ledger shell の定義に現れないため、\texttt{{[}M{]}} の入口測度が各 sector で共通なら
...
\hypertarget{ux6709ux9650ux975eux7ddaux5f62-mixer}{%
\section{有限非線形 mixer}\label{ux6709ux9650ux975eux7ddaux5f62-mixer}}
...
\tightlist
\item
  \textbf{ensemble reading}：比較器入口を固定作用殻の正規化 Liouville 測度で準備する。この場合 \(p_\ell(h)=1/E_\ell\) は厳密である。
\item
  \textbf{typical-time reading}：一つの初期 microstate を finite mixer で長時間発展させ、有限分解能の time histogram を入口測度として用いる。この場合は mixing と時間尺度分離が必要である。
\end{enumerate}
...
\(\tau_{\rm mix}\) は coarse histogram の緩和、\(\tau_{\rm cmp}\) は comparator が ledger state を読み出す前の混合窓、\(T_{\rm rec}\) は有限 mixer の再帰尺度である。本論文は一般の \(K_M\) に対してこの不等式を証明しない。これは数値検証すべき \texttt{{[}M{]}} の動力学部分である。
...
\hypertarget{sector-ux8ceaux91cfux306eux5bfeux79f0ux6027}{%
\section{sector 質量の対称性}\label{sector-ux8ceaux91cfux306eux5bfeux79f0ux6027}}
...
Hamiltonian の符号対称性だけでは不十分である。同じ Hamiltonian に非対称な初期密度を置くことも可能だからである。\texttt{{[}S{]}} は「対称な preparation macrostate 上の不変基準測度を採用する」という統計条件を含む。
...
finite nonlinear bath は後者の不変測度を作る論理原理ではなく、その orientation を有限時間で典型化する候補機構である。sector mass はさらに preparation symmetry \texttt{{[}S{]}} を必要とする。次章では、setting-blind terminal coordinate へこの二つの結果を代入し、共同確率を導く。

\hypertarget{ux7d42ux7aefux6574ux5408ux6e2cux5ea6ux3068-bell-ux5171ux540cux78baux7387}{%
\chapter{終端整合測度と Bell 共同確率}\label{ux7d42ux7aefux6574ux5408ux6e2cux5ea6ux3068-bell-ux5171ux540cux78baux7387}}

\begin{statusbox}
位置づけ：setting-blind terminal condition の Hamiltonian pullback と二モード位相体積から Bell compatibility weight を導く。
\end{statusbox}

\hypertarget{return-comparator}{%
\section{return comparator}\label{return-comparator}}
...
\hypertarget{ux56faux5b9a-terminal-condition}{%
\section{固定 terminal condition}\label{ux56faux5b9a-terminal-condition}}
...
が終端まで保たれた条件である。これは terminal half-space の正準力学的起源を与えるが、その半空間だけを物理的履歴集団として数える \texttt{{[}R{]}} の統計原理を置き換えない。
...
\hypertarget{terminal-compatibility-ux306eux7a4dux5206}{%
\section{terminal compatibility の積分}\label{terminal-compatibility-ux306eux7a4dux5206}}
...
\hypertarget{ux4e8cux30e2ux30fcux30c9ux53f0ux5e33-bell-compatibility-ux5b9aux7406}{%
\section{二モード台帳 Bell compatibility 定理}\label{ux4e8cux30e2ux30fcux30c9ux53f0ux5e33-bell-compatibility-ux5b9aux7406}}
...
Proof. \texttt{{[}S{]}} から \(w_{AB}=1/4\) である。第7.3節より
...
と因子化する。しかし \texttt{{[}R{]}} 後の \(\lambda\) 分布は第7.6節の通り setting-dependent である。

\hypertarget{ux521dux671fux6642ux8a08ux5411ux304d-margin-e_-ux306eux5f79ux5272}{%
\section{\texorpdfstring{初期時計向き margin \(E_*\) の役割}{初期時計向き margin E\_* の役割}}\label{ux521dux671fux6642ux8a08ux5411ux304d-margin-e_-ux306eux5f79ux5272}}
...
\hypertarget{microscopic-posterior-ux3068-measurement-independence}{%
\section{microscopic posterior と measurement independence}\label{microscopic-posterior-ux3068-measurement-independence}}

理想化して、Bell-relevant hidden data を \((A,B,h)\) とする。他の source variables が setting-independently factorize する場合、\texttt{{[}R{]}} 後の密度は
...
\hypertarget{setting-frequency-ux306eux4fddux8b77}{%
\section{setting frequency の保護}\label{setting-frequency-ux306eux4fddux8b77}}

controller の基準分布を \(P_S(a,b)\) とする。\texttt{{[}R{]}} を controller を含む全 ensemble へ適用すると、
...
\hypertarget{no-signalling-ux3068ux76f8ux95a2}{%
\section{no-signalling と相関}\label{no-signalling-ux3068ux76f8ux95a2}}
...
従って \texttt{{[}S{]}} の対称 ensemble では operational no-signalling が成立する。これは microscopic measurement independence が回復したことを意味しない。遠隔 setting dependence は hidden posterior に残るが、outcome-sign symmetry により一側周辺で相殺される。
...
\hypertarget{bell-ux524dux63d0ux53f0ux5e33}{%
\section{Bell 前提台帳}\label{bell-ux524dux63d0ux53f0ux5e33}}
...
\item
  equilibrium no-signalling：\texttt{{[}S{]}} の下で満たす。
\item
...
\hypertarget{ux6d41ux675fux6edeux5728ux6642ux9593ux7d50ux679cux983bux5ea6}{%
\section{流束、滞在時間、結果頻度}\label{ux6d41ux675fux6edeux5728ux6642ux9593ux7d50ux679cux983bux5ea6}}
...
は、それだけでは \texttt{{[}R{]}} の代わりにならない。本論文の \(W_{AB}\) は gate capacity または completion flux ではなく、\texttt{{[}R{]}} が物理的集団とする terminal-compatible phase volume である。
...
確率の起源は \texttt{{[}R{]}} であり、時計相補性だけではない。cos dependence、linear threshold volume、terminal half-space の正準構造は Hamiltonian apparatus が計算する。Bell の前提違反は measurement independence に位置し、macroscopic setting frequency と equilibrium no-signalling は四 sector の対称性により保たれる。

\hypertarget{ux9811ux5065ux6027ux53cdux8a3cux6761ux4ef6ux8a3cux660eux53f0ux5e33}{%
\chapter{頑健性、反証条件、証明台帳}\label{ux9811ux5065ux6027ux53cdux8a3cux6761ux4ef6ux8a3cux660eux53f0ux5e33}}

\begin{statusbox}
位置づけ：`[R]`、mixing、preparation symmetry の残る地位と、模型を区別する観測量を明示する。
\end{statusbox}

\hypertarget{ux5230ux9054ux3057ux305fux7d50ux679c}{%
\section{到達した結果}\label{ux5230ux9054ux3057ux305fux7d50ux679c}}
...
\tightlist
\item
  finite Hamiltonian local analyzer が definite anchor record を作る。
\item
  二つの real messenger が common future で差動作用を形成する。
\item
  差動作用は \(1-ABV\cos\Delta_{ab}\) に比例する。
\item
  一つの soft pair と一つの ledger pair の固定総作用殻が一様 threshold density を与える。
\item
  symmetric preparation が四 outcome sector の基準質量を \(1/4\) にする。
\item
  return pair と center pair が、向きの相補的な二つの内部時計へ正準分解される。
\item
  setting-blind comparator が初期相対時計運動量 \(E_*\) に \(\kappa I_- -h\) を加え、\(\Pi_R(T)=E_*+\kappa I_- -h\) を作る。
\item
  terminal half-space は順序付き時計向き保存条件として実現される。
\item
  \texttt{{[}R{]}} が terminal-compatible histories を物理的 ensemble とする。
\item
  規格化後に Bell 型 joint law、no-signalling marginal、CHSH violation が得られる。
\item
  Bell の前提違反は measurement independence に現れる。
\end{enumerate}
...
を物理的 probability law として一意に選ぶことはできない。\texttt{{[}R{]}} は本理論を標準的な初期値統計力学から区別する中心原理である。

従って \texttt{{[}R{]}} を削除するなら、少なくとも次のいずれかで同じ役割を置き換えなければならない。
...
となり \texttt{{[}R{]}} の積形式を再現する。ただし、branch-wise boundary preparation と matching が追加の all-at-once 統計原理である。
...
\hypertarget{ux5171ux6709ux6d74ux30ceux30a4ux30baux306eux9806ux6642ux9593ux7684ux6f0fux308cux306bux95a2ux3059ux308bux5426ux5b9aux7d50ux679c}{%
\section{共有浴ノイズの順時間的漏れに関する否定結果}\label{ux5171ux6709ux6d74ux30ceux30a4ux30baux306eux9806ux6642ux9593ux7684ux6f0fux308cux306bux95a2ux3059ux308bux5426ux5b9aux7d50ux679c}}
...
従って本論文の Bell correlation は、共有浴ノイズの leakage ではない。共通未来の bath と mixer は、\texttt{{[}R{]}} の terminal compatibility を計算する装置部分である。

\hypertarget{sbiased-preparationno-signalling}{%
\section{\texorpdfstring{\texttt{{[}S{]}}、biased preparation、no-signalling}{{[}S{]}、biased preparation、no-signalling}}\label{sbiased-preparationno-signalling}}
...
となり、Alice の marginal は Bob の setting に依存する。従って \texttt{{[}S{]}} の equilibrium no-signalling は arbitrary-preparation no-signalling ではない。
...
\tightlist
\item
  biased preparation apparatus を含む boundary problem が symmetry を回復する。
\item
  biased seed macroregion の terminal-compatible volume が零または操作不能になる。
\item
  biased preparation が可能で、模型は signalling prediction を持つ。
\end{enumerate}

第三の場合、本模型は実験的に排除される。\texttt{{[}S{]}} は反証不能な言葉で隠すのではなく、preparability test の対象にすべきである。

\hypertarget{mixing-ux306eux9811ux5065ux6027}{%
\section{mixing の頑健性}\label{mixing-ux306eux9811ux5065ux6027}}
...
\tightlist
\item
  \textbf{不完全 mixing}：finite observation window で \(p(h)\) に residual structure が残る。
\item
  \textbf{action leakage}：\(J_s+J_0\) が他 mode へ漏れ、fixed shell が崩れる。
\item
  \textbf{追加 ledger mode}：threshold-dependent energy を三つ以上の canonical pair が共有する。
\end{enumerate}
...
\hypertarget{terminal-width-ux3068-cutoff}{%
\section{terminal width と cutoff}\label{terminal-width-ux3068-cutoff}}
...
\hypertarget{reaction-time-ux3068-e_-ux306eux4ea4ux63dbux95a2ux4fc2}{%
\section{\texorpdfstring{reaction time と \(E_*\) の交換関係}{reaction time と E\_* の交換関係}}\label{reaction-time-ux3068-e_-ux306eux4ea4ux63dbux95a2ux4fc2}}
...
従って \(E_*\) を増やすと completion time は一様化するが、CHSH visibility は下がる。この同時変化は装置模型の反証可能な予測である。ただし \(\tau_g\) は \texttt{{[}R{]}} で定まった weight を表示する後段時間であり、weight の起源ではない。

\hypertarget{cos-ux5247ux3068-tsirelson-ux9650ux754c}{%
\section{cos 則と Tsirelson 限界}\label{cos-ux5247ux3068-tsirelson-ux9650ux754c}}
...
\hypertarget{wallstrom-ux554fux984cux3068ux306eux95a2ux4fc2}{%
\section{Wallstrom 問題との関係}\label{wallstrom-ux554fux984cux3068ux306eux95a2ux4fc2}}
...
\hypertarget{ux691cux8a3cux30d7ux30edux30b0ux30e9ux30e0}{%
\section{検証プログラム}\label{ux691cux8a3cux30d7ux30edux30b0ux30e9ux30e0}}

最小の検証を次の五段階に分ける。

\hypertarget{ux6bb5ux968e1local-measurement}{%
\subsection{段階1：local measurement}\label{ux6bb5ux968e1local-measurement}}
...
\hypertarget{ux6bb5ux968e2two-mode-ledger}{%
\subsection{段階2：two-mode ledger}\label{ux6bb5ux968e2two-mode-ledger}}
...
\hypertarget{ux6bb5ux968e3terminal-compatibility}{%
\subsection{段階3：terminal compatibility}\label{ux6bb5ux968e3terminal-compatibility}}

全 setting と outcome に同じ \(G_R\) を用い、\(W_{AB}\) の \(I_-\) に対する線形性、cutoff、terminal-width correction を直接測る。

\hypertarget{ux6bb5ux968e4bell-audit}{%
\subsection{段階4：Bell audit}\label{ux6bb5ux968e4bell-audit}}

joint law、marginal、CHSH、setting normalization、source posterior の全変動距離を同じ sample から計算する。observed trial の除外がないことを確認する。

\hypertarget{ux6bb5ux968e5robustness-and-preparation}{%
\subsection{段階5：robustness and preparation}\label{ux6bb5ux968e5robustness-and-preparation}}
...
\hypertarget{ux8a3cux660eux72b6ux614bux306eux53f0ux5e33}{%
\section{証明状態の台帳}\label{ux8a3cux660eux72b6ux614bux306eux53f0ux5e33}}
...
任意初期密度からの厳密な一様化 & 不成立。粗視化 mixing \texttt{{[}M{]}} のみ \\
\(w_{AB}=1/4\) & symmetric preparation \texttt{{[}S{]}} の下で厳密 \\
...
branch-wise boundary matching から \texttt{{[}R{]}} の積形式 & matching rule を追加した条件付き定理 \\
二つの相補的 half-space の等重み平均 & cos 項が消えるという厳密な no-go \\
Bell joint law & \texttt{{[}H,P,S,M,R{]}} と working range の下で厳密 \\
macroscopic setting normalization & \texttt{{[}S{]}} の下で厳密 \\
equilibrium no-signalling & \texttt{{[}S{]}} の下で厳密 \\
...
相補的内部時計により、return momentum、terminal half-space、\(E_*\) はそれぞれ相対時計運動量、順序付き時計向き保存、向き反転までの初期余裕として解釈できるようになった。これは terminal device の任意性を減らす進展である。一方、\texttt{{[}R{]}} は消去されていない。時計向きが交換される Hamiltonian 軌道を物理的集団から除くには、二境界 matching または同等の all-at-once 統計原理がなお必要である。

従って \texttt{{[}R{]}} は欠陥を隠すための補助仮定ではなく、本理論が通常の初期値統計力学と異なる位置を明示する中心原理である。今後の決定的課題は、branch-wise boundary preparation と matching をより大きな物理的境界値問題から導くこと、順序付き sector の時間反転共変な完成形を構成すること、またはその実験的含意を postselection と区別することである。

\appendix

\hypertarget{fouriergaussian-ux8fd1ux4f3cux3068-schur-ux88dcux5b8cux306eux8a55ux4fa1}{%
\chapter{Fourier--Gaussian 近似と Schur 補完の評価}\label{fouriergaussian-ux8fd1ux4f3cux3068-schur-ux88dcux5b8cux306eux8a55ux4fa1}}
...
\hypertarget{gaussian-ux5897ux5206ux306eux6b63ux78baux306aux8868ux793a}{%
\section{Gaussian 増分の正確な表示}\label{gaussian-ux5897ux5206ux306eux6b63ux78baux306aux8868ux793a}}
...
\hypertarget{ux660eux793a-hamiltonianux4e8cux30e2ux30fcux30c9ux4f4dux76f8ux4f53ux7a4dux88dcux6b63ux9805}{%
\chapter{明示 Hamiltonian、二モード位相体積、補正項}\label{ux660eux793a-hamiltonianux4e8cux30e2ux30fcux30c9ux4f4dux76f8ux4f53ux7a4dux88dcux6b63ux9805}}

\begin{statusbox}
位置づけ：第II部で用いた canonical map、作用殻測度、多モード補正を詳細に計算する。
\end{statusbox}

\hypertarget{poisson-structure}{%
\section{Poisson structure}\label{poisson-structure}}
...
\hypertarget{bright-shift-ux3068-anchor-shift}{%
\section{bright shift と anchor shift}\label{bright-shift-ux3068-anchor-shift}}
...
\hypertarget{autonomous-clock}{%
\section{autonomous clock}\label{autonomous-clock}}
...
\hypertarget{ux76f8ux88dcux7684ux5185ux90e8ux6642ux8a08ux4e8cux5883ux754c-matchingux5411ux304dux5e73ux5747-no-go}{%
\section{相補的内部時計、二境界 matching、向き平均 no-go}\label{ux76f8ux88dcux7684ux5185ux90e8ux6642ux8a08ux4e8cux5883ux754c-matchingux5411ux304dux5e73ux5747-no-go}}
...
この half-space から \texttt{{[}R{]}} の積形式を得るには、さらに二境界の statistical matching を置く必要がある。初期境界の密度を \(\rho_S(z_i)\)、逆向き時計の clock-past に対応する終端関数を \(G_{\rm or}(z_f)\) とし、両枝が同じ Hamiltonian 履歴を表す条件を
...
これは \texttt{{[}R{]}} と同じ積形式である。Hamiltonian flow の Jacobian が1であるため、逆向きに積分しても余分な密度因子は出ない。ただし二つの境界密度を掛け、matching する規則は Hamilton 方程式とは別の all-at-once 統計原理である。
...
\hypertarget{difference-mode-action}{%
\section{difference-mode action}\label{difference-mode-action}}
...
\hypertarget{return-pointer-shift}{%
\section{return-pointer shift}\label{return-pointer-shift}}
...
\hypertarget{ux4e8cux30e2ux30fcux30c9ux4f5cux7528ux6bbbux306eux6b63ux898fux5316}{%
\section{二モード作用殻の正規化}\label{ux4e8cux30e2ux30fcux30c9ux4f5cux7528ux6bbbux306eux6b63ux898fux5316}}
...
\hypertarget{mixer-generators}{%
\section{mixer generators}\label{mixer-generators}}
...
\hypertarget{ux591aux30e2ux30fcux30c9-simplex-marginal}{%
\section{多モード simplex marginal}\label{ux591aux30e2ux30fcux30c9-simplex-marginal}}
...
\hypertarget{finite-terminal-width}{%
\section{finite terminal width}\label{finite-terminal-width}}
...
\hypertarget{forward-bath-no-go}{%
\section{forward-bath no-go}\label{forward-bath-no-go}}
...
\hypertarget{algebraic-consistency-checks}{%
\section{algebraic consistency checks}\label{algebraic-consistency-checks}}
...
\tightlist
\item
  random angle と sign について、直接計算した \(\|u_A-u_B\|^2/4\) と analytic \(I_-\) を比較する。
\item
  \(S^3\) 上の isotropic Gaussian vector を規格化し、\(J_s/J_\ell\) の empirical CDF と uniform CDF を比較する。
\item
  \(\Pi_R(0)=E_*\) と \(\Delta\Pi_R=\kappa I_- -h\) から、相補時計の終端向き条件を検算する。
\item
  \(h\leq E_*+\kappa I_-\) の indicator を Monte Carlo 積分し、analytic \(W_{AB}\) と比較する。
\item
  四 outcome を規格化し、marginal residual と CHSH を計算する。
\item
  \(F_+(x)+F_-(x)=1\) を検算し、等重み orientation average で cos 項が消えることを確認する。
\item
  extra ledger modes を追加し、predicted \(F_N(x)\) と higher harmonics を比較する。
\end{enumerate}

これらは Hamiltonian mixing の証明ではない。幾何、normalization、sampling implementation に循環または符号誤りがないことを確認する代数的検証である。

\hypertarget{gaussian-nelson-ux65b9ux7a0bux5f0fschruxf6dinger-ux8868ux793aou-ux4f8b}{%
\chapter{Gaussian Nelson 方程式、Schrödinger 表示、OU 例}\label{gaussian-nelson-ux65b9ux7a0bux5f0fschruxf6dinger-ux8868ux793aou-ux4f8b}}
...
\hypertarget{ux6b21ux5143-gaussian-ux5909ux5206}{%
\section{1次元 Gaussian 変分}\label{ux6b21ux5143-gaussian-ux5909ux5206}}
...
\hypertarget{ux6b21ux5143-ou-ux4f4dux76f8ux6a21ux578b}{%
\section{2次元 OU 位相模型}\label{ux6b21ux5143-ou-ux4f4dux76f8ux6a21ux578b}}
...
Bell 系論には、phase-locked source \texttt{{[}P{]}}、symmetric preparation \texttt{{[}S{]}}、two-mode entrance measure \texttt{{[}M{]}}、boundary ensemble \texttt{{[}R{]}} が別に必要である。この点を保つことで、Gaussian Nelson 部分と Bell 部分の役割が明確になる。

\hypertarget{ux6e2cux5b9aux4f9dux5b58ux5ea6chsh-ux56dbux8a2dux5b9apostselection-ux76e3ux67fb}{%
\chapter{測定依存度、CHSH 四設定、postselection 監査}\label{ux6e2cux5b9aux4f9dux5b58ux5ea6chsh-ux56dbux8a2dux5b9apostselection-ux76e3ux67fb}}

\begin{statusbox}
位置づけ：第II部の Bell 分類を定量化し、装置 posterior と表現論的最小値を区別する。
\end{statusbox}

\hypertarget{hall-scale}{%
\section{Hall scale}\label{hall-scale}}
...
\hypertarget{ledger-only-representation}{%
\section{ledger-only representation}\label{ledger-only-representation}}
...
\hypertarget{ux6a19ux6e96-chsh-ux56dbux8a2dux5b9aux306eux6700ux5c0fux5024}{%
\section{標準 CHSH 四設定の最小値}\label{ux6a19ux6e96-chsh-ux56dbux8a2dux5b9aux306eux6700ux5c0fux5024}}
...
\hypertarget{setting-frequency-ux3068-source-posterior}{%
\section{setting frequency と source posterior}\label{setting-frequency-ux3068-source-posterior}}
...
\hypertarget{launch-countrecord-countcompletion-count}{%
\section{launch count、record count、completion count}\label{launch-countrecord-countcompletion-count}}
...
\tightlist
\item
  external source が開始した launch count \(N_{\rm launch}\)。
\item
  左右 pointer が definite result を持った record count \(N_{\rm rec}\)。
\item
  terminal apparatus が ready macroregion に入った completion count \(N_R\)。
\end{enumerate}

boundary ontology \texttt{{[}R{]}} では、物理的に実現する trial 自体が terminal-compatible history であると解釈する。しかし laboratory implementation が単に
...
\hypertarget{biased-preparation-test}{%
\section{biased preparation test}\label{biased-preparation-test}}
...
\tightlist
\item
  \(w_{AB}\) の操作が boundary compatibility によって自動的に相殺される。
\item
  biased macrostate が物理的に準備不能になる。
\item
  no-signalling residual が増大する。
\end{enumerate}
...
とする。ideal \texttt{{[}S{]}} ensemble では \(\epsilon_{\rm NS}=0\) である。biased preparation に対し \(\epsilon_{\rm NS}=O(1)\) が現れれば、equilibrium-only model の操作的限界が直接検出される。
