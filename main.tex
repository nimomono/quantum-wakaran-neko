...
\hypersetup{
  ...
  citecolor=PaperBlue,
  urlcolor=PaperBlue,
  pdftitle={有限 Hamiltonian 中核の Nelson 作用極限と2境界統計原理による Bell 型統計},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{PaperBlue}Nelson 作用極限と2境界 Bell 型統計}
...
\begin{titlepage}
  ...
  {\sffamily\bfseries\color{PaperNavy}\fontsize{21}{30}\selectfont
    有限 Hamiltonian 中核の Nelson 作用極限と\\[2mm]
    2境界統計原理による Bell 型統計\par}
  \vspace{9mm}
  {\Large\color{PaperBlue}有限 Gaussian 縮約、2モード作用分配系、相補的内部時計\par}
  \vspace{14mm}
  {\color{PaperRule}\rule{0.68\textwidth}{0.9pt}\par}
  \vfill
  {\large 有限幅比較器と証明状態の改訂稿\par}
  \vspace{22mm}
\end{titlepage}
...
\tightlist
\item
  有限 Fourier--Gaussian 浴の縮約から、Nelson 型作用の形式をどこまで定量的に得られるか。
\item
  有限 Hamiltonian 装置と2境界統計原理を組み合わせたとき、Bell 型共同確率をどこまで明示的に構成できるか。
\end{enumerate}
...
を定める追加原理を \texttt{{[}R{]}} と呼ぶ。これは「2境界統計原理」であり、時間反転共変な完成モデルが得られたことを意味しない。
...
を得る。規格化因子は測定設定に依存せず、\texttt{{[}S{]}} の対称集団では非信号性が成り立つ。ただし、完全な微視状態の事後分布は一般に測定設定へ依存する。Bell の前提違反は測定設定独立性に現れ、Bell の定理を否定しない。
...
Bell 型共同確率 & \texttt{{[}H,P,S,M,R{]}} と適用範囲の下で成立 \\
\texttt{{[}R{]}} & Hamilton 方程式からは未導出。2境界照合を置けば積形式を得る \\
非信号性 & 対称準備 \texttt{{[}S{]}} の下で成立。任意の偏った準備では未証明 \\
Born 則、Tsirelson 限界、Wallstrom 問題 & 力学的な一意導出には未達 \\
\end{longtable}

本文は、物理的な入力、相互作用、保存量、出力、仮定を中心に述べる。長い正準計算、収束評価、2境界照合、測定設定依存度、数値検証手順は付録と \texttt{VALIDATION.md} に分けて置く。

\hypertarget{ux554fux984cux8a2dux5b9a2ux90e8ux69cbux6210ux4eeeux5b9aux4e00ux89a7}{%
\chapter{問題設定、2部構成、仮定一覧}\label{ux554fux984cux8a2dux5b9a2ux90e8ux69cbux6210ux4eeeux5b9aux4e00ux89a7}}
...
Bell の定理は、局所応答、結果の確定性、測定設定独立性などを同時に満たす理論が、特定の Bell--CHSH 相関を再現できないことを示す \cite{bell1964,chsh1969}。古典 Hamiltonian モデルが Bell--CHSH 不等式を超える相関を与えるなら、Bell の前提の少なくとも1つが成立していない。本論文はこの論理を回避せず、軌道法則、準備測度、終端条件、履歴確率を個別に監査する。
...
\tightlist
\item
  可逆な有限 Hamiltonian 系の Gaussian 縮約から、Nelson 型作用の形式をどこまで定量的に得られるか。
\item
  有限 Hamiltonian 装置に2境界統計原理を加えたとき、設定名や結果名を直接参照しない終端条件の整合体積から Bell 型共同確率を構成できるか。
\end{enumerate}

第1の問題には、第4章のパラメータ \(C^1\) 収束定理として答える。第2の問題には、第7章の2モード作用分配系の定理として条件付きで答える。ただし第2の定理は、Hamilton 方程式だけから全履歴測度を導く定理ではない。物理的試行測度を定める \texttt{{[}R{]}} を明示的に入力する。
...
したがって、「有限 Hamiltonian 中核から Nelson 作用形式へ進む論理鎖」と、「有限 Hamiltonian 装置に2境界統計原理を加えて Bell 型統計へ進む論理鎖」は別である。第I部だけを用いる読者は第4章で完結でき、第II部だけを検討する読者は \texttt{{[}R{]}} と準備条件を独立に監査できる。
...
\hypertarget{ux4eeeux5b9aux4e00ux89a7}{%
\section{仮定一覧}\label{ux4eeeux5b9aux4e00ux89a7}}

第II部の仮定を次の5つに分ける。

\hypertarget{h-ux6709ux9650-hamiltonian-ux90e8ux54c1}{%
\subsection{{[}H{]} 有限 Hamiltonian 部品}\label{h-ux6709ux9650-hamiltonian-ux90e8ux54c1}}
...
を到来信号との相互作用から実現する装置は、\texttt{{[}H{]}} に含めて仮定できるが、本論文では具体的に構成しない。
...
とする。\(G_R\) は設定名、結果名、目標相関を引数に持たない。\texttt{{[}R{]}} は有限性、再帰性、時間反転対称性から一意に導かれるとは主張しない。相補的内部時計は
...
\hypertarget{ux6e96ux5099ux5bfeux79f0ux6027ux30682ux30e2ux30fcux30c9ux5e7eux4f55ux306eux5206ux96e2}{%
\section{準備対称性と2モード幾何の分離}\label{ux6e96ux5099ux5bfeux79f0ux6027ux30682ux30e2ux30fcux30c9ux5e7eux4f55ux306eux5206ux96e2}}
...
\tightlist
\item
  各結果領域の中でソフトモードのエネルギーが一様であることは、固定総作用を持つ2モード作用分配系の Liouville 幾何から導く。
\item
  4つの結果領域の基準質量が等しいことは、準備測度の独立符号反転対称性から導く。
\end{enumerate}
...
のうち、\(1/E_\ell\) は2モード定理、\(w_{AB}=1/4\) は \texttt{{[}S{]}} の帰結である。Hamiltonian の形だけでは初期密度を決められないため、準備測度という統計条件は残る。
...
\tightlist
\item
  位相同期した伝達ベクトル対と局所結果種を準備する。
\item
  左右の制御器巨視状態により設定 \(a,b\) を選ぶ。
\item
  局所分析器が伝達ベクトルを回転し、結果符号 \(A,B\) を応答モードへ写す。
\item
  固定指針が結果符号を記録する。
\item
  局所有限浴が応答モードの一時情報を分散する。
\item
  記録の写しと伝達ベクトルを通常の時間順序で共通未来へ運ぶ。
\item
  比較器が差動作用 \(I_-\) を計算する。
\item
  初期相対時計運動量を \(\Pi_R=E_*\) とし、ソフトモードエネルギー \(h\) と差動作用 \(\kappa I_-\) を比較する。
\item
  終端相対時計運動量 \(\Pi_R(T)=E_*+\kappa I_- -h\) に固定終端領域を課す。
\item
  \texttt{{[}R{]}} により終端整合履歴の規格化測度を物理的試行集団とする。
\end{enumerate}
...
2境界条件、局所逆因果モデル、測定設定独立性を緩めた Bell モデルにも先行研究がある \cite{wharton2010,wharton_argaman2020,hall2010,leifer_pusey2017,wood_spekkens2015,price_wharton2023,price_wharton2024,argaman2010,hossenfelder_palmer2020,thooft2016}。共通未来と境界制約による選別という発想自体は新規ではない。第II部が追加するのは、局所指針、実2次元差動比較器、固定総作用をもつ2モード作用分配系、設定名を直接参照しない終端座標を有限 Hamiltonian 部品として接続し、旧共通入口密度を準備対称性と2モード位相体積へ分解する点である。
...
第2章から第4章は、可逆な調和 Hamiltonian 中核、Gaussian 条件づけ、作用形式の定量的 Nelson 極限を扱う。第5章から第7章は、有限 Hamiltonian 装置部品、2モード作用分配系、2境界統計原理の下での Bell 型共同確率を扱う。第8章は適用範囲、否定結果、反証条件をまとめる。長い収束評価、正準計算、2境界照合、測定設定依存度、数値検証手順は付録と \texttt{VALIDATION.md} に置く。

\part{有限調和 Gaussian 中核の Nelson 極限}

\hypertarget{ux53efux9006ux306aux8abfux548c-hamiltonian-ux4e2dux6838ux3068ux6709ux9650-gaussian-ux78baux7387ux8868ux793a}{%
\chapter{可逆な調和 Hamiltonian 中核と有限 Gaussian 確率表示}\label{ux53efux9006ux306aux8abfux548c-hamiltonian-ux4e2dux6838ux3068ux6709ux9650-gaussian-ux78baux7387ux8868ux793a}}
...
\hypertarget{ux8a3cux660eux7528ux306eux7ddaux5f62-gaussian-ux78baux7387ux8868ux793a}{%
\section{証明用の線形 Gaussian 確率表示}\label{ux8a3cux660eux7528ux306eux7ddaux5f62-gaussian-ux78baux7387ux8868ux793a}}
...
\hypertarget{ou-ux30e2ux30c7ux30ebux3068-stratonovich-ux8868ux73fe}{%
\section{OU モデルと Stratonovich 表現}\label{ou-ux30e2ux30c7ux30ebux3068-stratonovich-ux8868ux73fe}}
...
ただし、条件付き経路分布を物理的試行頻度として採用するには、どの完結履歴へ確率を置くかという追加の物理原理が必要である。Gaussian Schur 補完は、記録を与えた後の条件付き法則を計算するが、その法則が実験の無条件頻度として選ばれることまでは証明しない。第II部ではこの役割を2境界統計原理 \texttt{{[}R{]}} として明示し、第4章の Nelson 作用極限からは導かない。
...
\part{2境界統計原理と2モード作用分配系による Bell 型統計}

\hypertarget{ux6709ux9650-hamiltonian-ux88c5ux7f6eux90e8ux54c1ux30682ux5883ux754cux5c65ux6b74ux96c6ux56e3}{%
\chapter{有限 Hamiltonian 装置部品と2境界履歴集団}\label{ux6709ux9650-hamiltonian-ux88c5ux7f6eux90e8ux54c1ux30682ux5883ux754cux5c65ux6b74ux96c6ux56e3}}
...
\item
  2つの伝達ベクトルを比較する、設定名を直接参照しない二次形式。
\item
...
結果頻度を定める原理は、局所浴の散逸、指針の保持時間、比較速度、時計相補性のいずれでもない。物理的履歴集団を定める \texttt{{[}R{]}} と、次章で導く終端整合体積である。
...
有限非線形浴は後者の不変測度を作る論理原理ではなく、その向きを有限時間で典型化する候補機構である。結果領域の質量はさらに準備対称性 \texttt{{[}S{]}} を必要とする。次章では、設定名を直接参照しない終端座標へこの2つの結果を代入し、共同確率を導く。

\hypertarget{ux7d42ux7aefux6574ux5408ux6e2cux5ea6ux3068-bell-ux578bux5171ux540cux78baux7387}{%
\chapter{終端整合測度と Bell 型共同確率}\label{ux7d42ux7aefux6574ux5408ux6e2cux5ea6ux3068-bell-ux578bux5171ux540cux78baux7387}}
...
\hypertarget{ux30e2ux30fcux30c9ux4f5cux7528ux5206ux914dux7cfb-bell-ux578bux6574ux5408ux6027ux5b9aux7406}{%
\section{2モード作用分配系 Bell 型整合性定理}\label{ux30e2ux30fcux30c9ux4f5cux7528ux5206ux914dux7cfb-bell-ux578bux6574ux5408ux6027ux5b9aux7406}}

\begin{theorem}[2モード作用分配系を持つ2境界 Bell 型整合性]
次を仮定する。
...
結果依存の未完了試行や時間切れ試行を除けば、事後選別が生じる。本論文の \(W_{AB}\) は通過流束や完了率ではなく、\texttt{{[}R{]}} が物理的集団とする終端整合位相体積である。共有浴と待ち時間を含む一般的な否定結果は第8.3節と付録C.11にまとめる。
...
2モード作用分配系の一様密度、対称準備、\texttt{{[}R{]}} を用いると Bell 型余弦共同確率を得る。確率重みの原理は \texttt{{[}R{]}} であり、時計相補性や順時間的待ち時間ではない。Bell の前提違反は測定設定独立性に位置し、巨視的な測定設定頻度と対称準備での非信号性は保たれる。

\hypertarget{ux9069ux7528ux7bc4ux56f2ux5426ux5b9aux7d50ux679cux53cdux8a3cux6761ux4ef6}{%
\chapter{適用範囲、否定結果、反証条件}\label{ux9069ux7528ux7bc4ux56f2ux5426ux5b9aux7d50ux679cux53cdux8a3cux6761ux4ef6}}

\begin{statusbox}
位置づけ：導出済み、条件付き、未導出、否定結果を分離し、モデルを区別する観測量を示す。
\end{statusbox}

\hypertarget{ux73feux5728ux306eux5230ux9054ux70b9}{%
\section{現在の到達点}\label{ux73feux5728ux306eux5230ux9054ux70b9}}
...
Bell 型共同確率 & \texttt{{[}H,P,S,M,R{]}} と適用範囲の下で成立 \\
\texttt{{[}R{]}} の Hamiltonian 力学からの導出 & 未達 \\
対称準備での非信号性 & \texttt{{[}S{]}} の下で成立 \\
任意の偏った準備での非信号性 & 未証明 \\
\end{longtable}
...
を物理的確率法則として一意に選ぶことはできない。\texttt{{[}R{]}} は、本構成を通常の初期値統計力学から区別する独立原理である。
...
したがって、順時間的な共有浴雑音の漏れ、結果依存滞在時間、反応座標の通過速度は、それだけでは \texttt{{[}R{]}} の代わりにならない。

\hypertarget{s-ux3068ux504fux3063ux305fux6e96ux5099}{%
\section{\texorpdfstring{\texttt{{[}S{]}} と偏った準備}{{[}S{]} と偏った準備}}\label{s-ux3068ux504fux3063ux305fux6e96ux5099}}
...
となり、A 側周辺は B 側の測定設定に依存する。したがって \texttt{{[}S{]}} の非信号性は平衡的な対称準備についての結果であり、任意準備についての定理ではない。
...
\item
  開始数、指針記録数、終端完了数を分け、結果依存の欠測と事後選別を検査する。
\item
  局所パルス幅を変え、結果符号、伝達作用、Bell 共同確率の補正が \(O(\epsilon)\) で減るかを検査する。
\end{enumerate}
...
\tightlist
\item
  \texttt{{[}R{]}} の2境界照合規則を、より大きな閉じた境界値理論から導けるか。
\item
  設定と到来変数から結果を生成する一般の局所 Hamiltonian 測定相互作用を構成できるか。
\item
  全装置部品と全自由発展を1本の有限 Hamiltonian に統合し、有限幅誤差を一様に評価できるか。
\item
...
を厳密に読出す。対称準備と \texttt{{[}R{]}} を追加すると Bell 型余弦共同確率が得られ、Bell の前提違反は測定設定独立性に現れる。

一方、\texttt{{[}R{]}} は Hamilton 方程式から導かれておらず、局所装置も一般測定相互作用には達していない。したがって現在の到達点は、Bell 型確率構造の完全な力学的導出ではなく、力学的に実現した部分と独立な統計入力の境界を明示した条件付き構成である。
...
\hypertarget{hamiltonian-ux88c5ux7f6eux90e8ux54c12ux30e2ux30fcux30c9ux4f4dux76f8ux4f53ux7a4dux88dcux6b63ux9805}{%
\chapter{Hamiltonian 装置部品、2モード位相体積、補正項}\label{hamiltonian-ux88c5ux7f6eux90e8ux54c12ux30e2ux30fcux30c9ux4f4dux76f8ux4f53ux7a4dux88dcux6b63ux9805}}
...
\hypertarget{ux9806ux6642ux9593ux7684ux5171ux6709ux6d74ux3068ux5f85ux3061ux6642ux9593ux306eux5426ux5b9aux7d50ux679c-1}{%
\section{順時間的共有浴と待ち時間の否定結果}\label{ux9806ux6642ux9593ux7684ux5171ux6709ux6d74ux3068ux5f85ux3061ux6642ux9593ux306eux5426ux5b9aux7d50ux679c-1}}
...
\item
  \(S^3\) 上の等方 Gaussian ベクトルを規格化し、\(J_s/J_\ell\) の経験累積分布と一様累積分布を比較する。
\item
...
\hypertarget{ux6b21ux5143-ou-ux4f4dux76f8ux30e2ux30c7ux30eb}{%
\section{2次元 OU 位相モデル}\label{ux6b21ux5143-ou-ux4f4dux76f8ux30e2ux30c7ux30eb}}
...
Bell 系論には、位相同期した生成源 \texttt{{[}P{]}}、対称準備 \texttt{{[}S{]}}、2モード入口測度 \texttt{{[}M{]}}、2境界履歴集団 \texttt{{[}R{]}} が別に必要である。この点を保つことで、Gaussian Nelson 部分と Bell 部分の役割が明確になる。

\hypertarget{ux6e2cux5b9aux8a2dux5b9aux4f9dux5b58ux5ea6chsh-4ux8a2dux5b9aux4e8bux5f8cux9078ux5225ux76e3ux67fb}{%
\chapter{測定設定依存度、CHSH 4設定、事後選別監査}\label{ux6e2cux5b9aux8a2dux5b9aux4f9dux5b58ux5ea6chsh-4ux8a2dux5b9aux4e8bux5f8cux9078ux5225ux76e3ux67fb}}
...
\hypertarget{ux958bux59cbux6570ux8a18ux9332ux6570ux5b8cux4e86ux6570}{%
\section{開始数、記録数、完了数}\label{ux958bux59cbux6570ux8a18ux9332ux6570ux5b8cux4e86ux6570}}
