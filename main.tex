...
Q1、Q2、Q3を1つの達成済み模型へまとめる主張はしない。Q1はW型ポテンシャルの最低2モードに限定したM47共同統計模型、Q2とQ3は有限配置グラフ上の物理的複素振幅場と実現配置を持つM42を暫定的に使う。共通なのは有限正準操作、局所記録、履歴保存、交換resetなどの装置部品であり、全系列を同じ物理ハードウェアと母測度へ統合したM0は未完成である。
...
Q2 & M42、M39、M41 & CNOT共同入力--出力統計、singlet余弦統計、非信号性、CHSH値 & 直接モード符号化、準備先行、非空間分離 \\
...
第8章と付録Hでは、周期反応座標、対数型捕捉エントロピー座標、熱雑音、散逸、能動自由エネルギー供給を持つM45開放捕捉器を扱う。旧M46のcurrent transducerは不採用であり、R130--R132の再利用可能な有限次元計算だけを補助結果として保持する。自然なM47 matching準備、切断後保存、連続空間の一様極限、多粒子、総エネルギー収支、多量子ビットの指数資源回避は未完成である。

本文は5部9章から成る。第1部は問題設定と模型地図、第2部はQ1、第3部はQ2、第4部はQ3、第5部は誤差、資源、反証条件、未完成目標、結論を扱う。長い証明と誤差評価は付録A--Iへ置く。
...
\begin{statusbox}
位置づけ：Q1のM47経路とQ2・Q3の暫定M42経路を分け、導出状態、達成判定、非主張を定める。
\end{statusbox}
...
\item
  Q2では、4頂点の物理的複素振幅場と共同実現配置からCNOT入力--出力統計とBell型測定周期を構成する。
\item
...
Q2-2の「条件付き達成」は、操作的接続という意味条件である。Q3-2は科学的には未達のまま凍結する。固定目標とモデル非依存な達成基準は \texttt{PROJECT\_STATUS.md} で管理する。
...
\begin{statusbox}
位置づけ：M42/R113をQ2・Q3だけの暫定模型へ縮小し、共通辺生成子、等変性、有限Hamiltonian近似、M35補助モジュールを整理する。
\end{statusbox}
...
本章のM42は、Q2とQ3にだけ残る\textbf{有限配置グラフ上の複素振幅場-実現配置模型}である。Q1は第3章のM47へ移行し、本章の場、最小率、R113を使わない。有限グラフを
...
Q2とQ3に共通なのは暫定有効模型である。M37は位置ばね網からQ3の複素振幅場を弱結合・有限時間で近似し、M35、M39、M41は設計済みの正準交換、分析器、記録、resetを使う。M47を含む全系列を同じ物理部品、時計周期、反復母測度へまとめたM0は未完成である。
...
R113はM42の旧来の操作模型に属する。M47のW型共同統計模型は、この最小率または旧M46のcurrent transducerを使わず、実現配置--浴共同測度から統計核を定義する。従ってM47がR113を導出、物理化または検証したとは扱わない。Q1はR139--R144へ移行した。M42/R113はR120、R121とQ3の固定時刻位置読出しに必要なためQ2・Q3だけに暫定保持する。全面退役には、Q2の共同配置とQ3の位置読出しを共同統計模型から再導出する必要がある。
...
\item
  M45または具体的回路からM47 matching準備を導出したこと。
\item
...
\begin{statusbox}
位置づけ：Q1のM47移行、Q2・Q3の暫定M42経路、証明済み部分、条件付き部分、凍結目標を総括する。
\end{statusbox}
...
\begin{statusbox}
位置づけ：M47の基礎として、実現配置--浴共同統計、開放Hopf準備、閉鎖作用角伝播、左右占有振動、条件付き閉包、枝別matching、逆設計存在構成を詳述する。
\end{statusbox}
