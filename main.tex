...
共通有効モデルをM42、Q1装置をM38、一般有限基底補助をM35、Q2結合ゲートをM39、Q2 Bell周期をM41、Q3ミクロ振動子網をM37、Q3有限環境純位相緩和をM43と呼ぶ。M44は、熱的入射場、非線形準安定捕捉器、同じ伝送路の出射部から、定常基底状態の初期共通原因測度を準備する現行補助モデルである。これらを1つのハードウェア、時計周期、反復母測度へまとめたM0は未完成である。
...
定常準備候補層M44 & 粒子、matched伝送路、反応座標、内部作用モード & 捕捉entropy型ready核、形成・保持分解、定常Nelson則 & R126。仮説依存の近似結果 \\
\end{longtable}
...
共通モデルは、量子力学の全構造、同一ハードウェアのM0、独立同分布型有限標本統計、総エネルギー収支、連続空間、多粒子、多量子ビットの多項式資源構成を与えない。M41は標準的な空間分離Bell実験ではなく、R123--R125は有限モード・有限時間・固定時刻読出しの範囲を越えない。M44は定常基底状態の準備候補であり、一般の時間依存Nelson流、励起状態、複素位相を導かない。結果別の非主張、資源上の未評価量、反証条件は第7章と第8章へ集約する。
...
付録Aは共通作用とM35、付録BはQ1完全周期と有限Zeno、付録CはQ2結合ゲート、付録DはM41、付録EはM37包絡誤差、付録Fは有限配置グラフ上の実現配置過程、付録GはQ3-3からQ3-5の証明を扱う。
...
M44捕捉準備 & 捕捉流束比に目的指数則と同次数の余分な位置依存prefactorが残る、基準核に非消失driftまたは位置依存拡散が残る、ready射影残差が \(o(\delta)\) でない、形成・保持分解が \(h^2\) へ収束しない、縮約時間対称加速度がミクロ条件付き加速度へ持ち上がらない \\
...
\item
  M44について、一つの滑らかな有限Hamiltonianから基準拡散核、捕捉entropy則、反復ready因子化、同じ作用scale、時間対称加速度への持ち上げを有限誤差付きで同時に導けるかを検査する。
\end{enumerate}

一般有限 \(L\) への拡張はM35の一般化課題として並行して管理する。M37、M38、M35、M39、M41、M42を1つのM0と統一母測度へまとめることは、その後の統合目標である。

\hypertarget{m44ux6355ux6349entropyux578bux521dux671fux5171ux901aux539fux56e0ux6e96ux5099}{%
\section{M44捕捉entropy型初期共通原因準備}\label{m44ux6355ux6349entropyux578bux521dux671fux5171ux901aux539fux56e0ux6e96ux5099}}

M42のBorn型等変性は、開始時に実現配置分布が複素振幅場の作用比と一致することを必要とする。R118は鋭い基準場・配置から同じ準備回路を場と実現配置へ作用させる。M35は任意の既知場に対する補助準備を与える。一方、物理potential \(V\) の定常基底状態について、目的固有関数を初期分布へ直接入力せずに同じ型の分布を作るミクロ準備原理は未構成であった。

M44は、この不足を定常基底状態に限って補う現行補助モデルである。物理構成は
...
\text{matched contact}
\longrightarrow
\text{metastable catcher}
\longrightarrow
\text{outgoing field}
\]

であり、一つの熱的入射場、一つの非線形準安定捕捉器、同じ伝送路の出射部を用いる。数式中の英語labelは装置の略号であり、二つの独立な平衡浴、外部clock、離散packet列、未来の観測境界を基本構成へ入れない。全Hamiltonianの候補は
...
である。\(V(X)\) は粒子部分に一度だけ入る。\(H_{\rm line+match}\) は座標--運動量matching、counterterm、裸質量調整を含み、\(H_{\rm cap}\) は入口鞍点、\(R\) 個の内部作用モード、弱非線形混合を持つ。readyとdumpの境界は粒子位置上の壁でなく、捕捉反応座標の鞍点である。出射波は捕捉失敗と脱出のenergy・相関を遠方へ運び、有限鎖では再帰前の観測窓だけを使う。
...
とする。入口鞍点を内向きに越えるミクロカノニカル流束を積分すると、理想化した \(R\) 調和内部モードについてready位相体積比は
...
これはBoltzmann分布または目的固有関数を仮定した重みではなく、捕捉可能な位相体積の比である。ただし \(E_*\) と入射場が決める \(\nu\) の係数一致は、現段階ではscale-matching条件である。
...
を仮定する。二つの \(A_{\delta,R}\) は二つの捕捉器ではなく、一つの準安定捕捉器の形成側と保持側である。このときR126は
...
を与える。主固有関数を \(h\) とすると、任意時刻のready占有は、過去からの形成重みと現在からの保持位相体積の積
...
なので、定常密度は \(\rho_V=\phi_0^2\) となる。保持因子は未来波形の準備ではなく、現在の内部energy分配が持つ残存位相体積と滞在時間の重みである。

主固有関数によるDoob変換\cite{doob1957}の連続極限は
...
を持つ。従って \(v=(b_++b_-)/2=0\)、\(u=(b_+-b_-)/2=\nu\partial_x\log\rho_V\) である。Nelsonの時間対称加速度\cite{nelson1966,guerra_morato1983,yasue1981,zambrini1986}
...
を与える。これは定常Fisher変分であり、一般の時間依存Nelson作用ではない。

R126は仮説依存の近似結果として管理する。ミクロカノニカル流束と有限 \(R\) 誤差、\(G=A K A\) 以後の主固有関数・Doob変換・定常Newton則は明示できる。一方、同じ一つの滑らかな有限Hamiltonianから、一般の非線形 \(V\) に対して基準拡散核、反復ready因子化、同じ \(\nu\) によるscale matching、ミクロ加速度への持ち上げを同時に導く部分は未証明である。正で既約な捕捉核は基底状態を選び、一般の励起状態または非零現在速度を持つ時間依存流を安定準備しない。

完全なHamiltonian、捕捉流束の積分、有限capacity・有限時間誤差、形成・保持分解、経路大偏差、Doob変換、旧二側条件付き経路および不採用Fisher閉包との違いは付録Hに示す。M44はM37、M42、R118、M35を置換せず、Q3の現行達成判定も変更しない。
...
\hypertarget{ux6355ux6349entropyux578bux521dux671fux5171ux901aux539fux56e0ux6e96ux5099ux3068ux5b9aux5e38nelsonux6d41}{%
\chapter{捕捉entropy型初期共通原因準備と定常Nelson流}\label{ux6355ux6349entropyux578bux521dux671fux5171ux901aux539fux56e0ux6e96ux5099ux3068ux5b9aux5e38nelsonux6d41}}

\begin{statusbox}
位置づけ：第8.13節のM44とR126について、ミクロHamiltonian候補、ready位相体積比、転送作用素、形成・保持分解、Doob変換、定常Nelson則、誤差と未証明点を詳述する。
\end{statusbox}

\hypertarget{ux76eeux7684ux7bc4ux56f2ux65e2ux5b58ux30e2ux30c7ux30ebux3068ux306eux95a2ux4fc2}{%
\section{目的、範囲、既存モデルとの関係}\label{ux76eeux7684ux7bc4ux56f2ux65e2ux5b58ux30e2ux30c7ux30ebux3068ux306eux95a2ux4fc2}}
...
\hypertarget{ux30dfux30afux30edux81eaux7531ux5ea6ux3068hamiltonianux5019ux88dc}{%
\section{ミクロ自由度とHamiltonian候補}\label{ux30dfux30afux30edux81eaux7531ux5ea6ux3068hamiltonianux5019ux88dc}}
...
と書ける。座標側の平方完成は浴が作る静的2次potentialを相殺するcountertermを含み、運動量側の平方完成は裸質量調整を必要とする。\(c_k\) と \(\eta_k\) のmatchingは、基準縮約核の拡散係数を \(\nu\) に固定し、時間反転対称な低速sectorへ不要な反応項が残らないように選ぶ。具体的な有限帯域列からこの性質を一様に導くことは、M44の未証明命題の一つである。有限調和浴から再帰前の有効記憶核を得る一般的な背景は\cite{ford1965,mori1965,zwanzig1973}にある。
...
\hypertarget{ux5165ux5c04ux5834ux51faux5c04ux5834ux521dux671fux5171ux901aux539fux56e0ux6e2cux5ea6}{%
\section{入射場、出射場、初期共通原因測度}\label{ux5165ux5c04ux5834ux51faux5c04ux5834ux521dux671fux5171ux901aux539fux56e0ux6e2cux5ea6}}
...
\hypertarget{readyux9818ux57dfux3068ux4f4dux76f8ux7a7aux9593ux5883ux754c}{%
\section{ready領域と位相空間境界}\label{readyux9818ux57dfux3068ux4f4dux76f8ux7a7aux9593ux5883ux754c}}
...
ready領域は全位相空間の真のattractorではない。有限Hamiltonian流はLiouville体積を保存する。局所変数だけを見ると捕捉に見えるのは、内部混合により逆脱出が遅くなり、失敗・脱出情報が遠方の出射自由度へ運ばれ、それらを縮約するからである。有限の混沌環境が局所系へ有効散逸を与え得ることとの比較は\cite{marchiori_deaguiar2011}にあるが、M44で必要な特定の捕捉核は別に証明しなければならない。

\hypertarget{readyux4f4dux76f8ux4f53ux7a4dux6bd4}{%
\section{ready位相体積比}\label{readyux4f4dux76f8ux4f53ux7a4dux6bd4}}
...
\hypertarget{ux71b1ux7684ux5165ux5c04ux5834ux304bux3089ux81eaux7531ux62e1ux6563ux6838ux3078ux306eux7e2eux7d04ux6761ux4ef6}{%
\section{熱的入射場から自由拡散核への縮約条件}\label{ux71b1ux7684ux5165ux5c04ux5834ux304bux3089ux81eaux7531ux62e1ux6563ux6838ux3078ux306eux7e2eux7d04ux6761ux4ef6}}
...
\hypertarget{ux6642ux9593ux53cdux8ee2ux5bfeux79f0ux306areadyux8ee2ux9001ux4f5cux7528ux7d20}{%
\section{時間反転対称なready転送作用素}\label{ux6642ux9593ux53cdux8ee2ux5bfeux79f0ux306areadyux8ee2ux9001ux4f5cux7528ux7d20}}
...
\begin{theorem}[R126：捕捉entropy型準備作用素と定常Nelson則]
...
\hypertarget{ux5f62ux6210ux679dux4fddux6301ux679dux5e73ux65b9ux5bc6ux5ea6}{%
\section{形成枝、保持枝、平方密度}\label{ux5f62ux6210ux679dux4fddux6301ux679dux5e73ux65b9ux5bc6ux5ea6}}
...
\hypertarget{ux7d4cux8defux6355ux6349ux91cdux307fux3068ux7d4cux9a13ux5bc6ux5ea6ux5927ux504fux5dee}{%
\section{経路捕捉重みと経験密度大偏差}\label{ux7d4cux8defux6355ux6349ux91cdux307fux3068ux7d4cux9a13ux5bc6ux5ea6ux5927ux504fux5dee}}
...
となる。これは前進経路と時間反転経路で同じ値を取る。時間反転で符号が変わるentropy生成ではなく、ready経路の利用可能位相体積を表す時間対称重みである。正の前後因子とreciprocal過程の関係は\cite{leonard2014,leonard_roelly_zambrini2014}と比較できるが、M44は未来観測境界でなく、常時進行する捕捉過程の形成・保持分解を使う。
...
であり、正の最小解は \(\phi_0\) である。この汎関数はGuerra--Morato作用のFisher項との関係を持つ\cite{knorst_lopes2024}が、ここで導いたのは定常経験密度の変分であり、一般の時間依存Nelson作用ではない。

\hypertarget{doobux5909ux63dbux3068ux524dux5f8cdrift}{%
\section{Doob変換と前後drift}\label{doobux5909ux63dbux3068ux524dux5f8cdrift}}
...
は \(x,y\) の交換で不変なので、\(h_\delta^2\) は不変密度であり詳細釣り合いを満たす。これはDoob変換\cite{doob1957}である。
...
\hypertarget{ux5b9aux5e38nelsonux52a0ux901fux5ea6ux3068newtonux5247}{%
\section{定常Nelson加速度とNewton則}\label{ux5b9aux5e38nelsonux52a0ux901fux5ea6ux3068newtonux5247}}
...
と定める\cite{nelson1966,guerra_morato1983,yasue1981,zambrini1986}。一般に
...
\hypertarget{ux6709ux9650ux8aa4ux5deeux6642ux9593ux5c3aux5ea6ux975eux6709ux754cpotential}{%
\section{有限誤差、時間尺度、非有界potential}\label{ux6709ux9650ux8aa4ux5deeux6642ux9593ux5c3aux5ea6ux975eux6709ux754cpotential}}
...
\tightlist
\item
  \(\varepsilon_K\)：有限浴、有限帯域、Markov近似の一段階核誤差。
\item
  \(\varepsilon_{\rm cap}\)：有限 \(R\) の捕捉entropy誤差。
\item
  \(\varepsilon_{\rm split}\)：対称作用素分割の誤差。
\item
  \(\varepsilon_{\rm mix}\)：一つの捕捉器を反復利用する際の残留内部記憶。
\item
  \(\varepsilon_{\rm rec}\)：有限伝送路の反射と再帰。
\item
  \(\varepsilon_{\rm tail}\)：有限capacityで扱えない高potential領域。
\item
  \(\varepsilon_{\rm lift}\)：Doob位置過程とミクロ条件付きHamilton軌道の時間対称加速度の差。
\end{enumerate}
...
\hypertarget{ux5c0eux51faux72b6ux614bux3068ux672aux8a3cux660eux547dux984c}{%
\section{導出状態と未証明命題}\label{ux5c0eux51faux72b6ux614bux3068ux672aux8a3cux660eux547dux984c}}
...
\tightlist
\item
  具体的な少数種類の滑らかな局所結合が、一般の非線形 \(V\) に対して基準拡散核と捕捉因子を同時に作ること。
\item
  一つの準安定捕捉器がfresh cell列なしに十分速く混合し、一段階残差 \(o(\delta)\) で反復因子化すること。
\item
  入射場が決める \(\nu\) と内部capacityが決める捕捉entropy係数を、同じ作用reservoirの保存量から固定すること。
\item
  縮約Doob過程の時間対称加速度が、元のHamilton軌道の条件付き前後加速度へ有限誤差で持ち上がること。
\end{enumerate}

これらを別の仮説へ移しただけで解決と呼ばない。M44は、この四点を一つの物理構成に対する検証課題へ集約した現行補助モデルである。

\hypertarget{ux65e7ux7d4cux8defux3068ux306eux9055ux3044ux3068ux975eux4e3bux5f35}{%
\section{旧経路との違いと非主張}\label{ux65e7ux7d4cux8defux3068ux306eux9055ux3044ux3068ux975eux4e3bux5f35}}
