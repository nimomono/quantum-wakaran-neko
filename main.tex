...
Q3ではM37の位置結合振動子網から局所回転包絡を作る。弱結合量 \(\eta<1\) の下で、有限時間の局所包絡を目標Schrödinger型複素振幅場へ \(O(\eta)\) で近づける。M42の共通実現配置則をこの空間グラフへ特殊化し、節を含む理想最小率を滑らかに正則化する。有限時間と精度を固定すれば、正則化、離散化、局所選択、1辺輸送、時計、検出の誤差を全変動距離で抑える有限Hamiltonian構成を選べる。固定時刻位置検出は得るが、連続空間極限、多粒子、質量・電荷・全エネルギー、初回到達を含む2重スリット完全装置は未達である。
...
\tightlist
\item
  量子力学の全構造、一般Born則、Wallstrom問題、連続スペクトルの導出。
\item
...
M42は、固有ベクトルを複素振幅場の初期条件として入れ、実現配置を対応する作用比で準備した場合の固定時刻位置分布を与える。しかし、固有ベクトルを初期条件として入れるだけでは状態選択を示したことにならない。

\hypertarget{ux30c8ux30f3ux30cdux30ebux52b9ux679cq3-4}{%
\section{トンネル効果（Q3-4）}\label{ux30c8ux30f3ux30cdux30ebux52b9ux679cq3-4}}

障壁高、幅、入射エネルギーを掃引し、ミクロ振動子網と目標Schrödinger型解の透過率差を有限領域で評価する。M42を使う場合は、障壁反対側の固定時刻占有だけでなく、初回通過、反射、有限検出窓を含む実現配置軌道の完全結果集合を定める。熱浴温度、古典的共鳴、障壁越えエネルギーを分離し、熱活性化だけで同じ透過率を説明できないことを検査する。
...
第9章は、複素振幅場の2経路干渉、空間実現配置の有限グラフ上の等変運動、予定時刻の局所位置検出までを別々に構成した。従って「複素振幅場の作用比を局所検出器へ接続する」という基本的な不足はM42で解消した。ただしQ3-5の達成には、次を1つの装置周期へ統合する必要がある。
...
\item
  Q3-2からQ3-5として、M37複素振幅場とM42実現配置を位相量子化、束縛状態選択、トンネルの初回通過、2重スリット完全検出周期へ拡張する。
\item
