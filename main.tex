...
Q2-2は、R151--R156によりM39からの固定singlet型破壊的受渡し、2翼強matching、切断後局所分析、局所記録、弱開放resetを同じ周期へ閉じる。固定singlet型、固定有限設定族、操作的接続、準備先行、非空間分離という解釈で条件付き達成とする。M41の旧結果は数学的に撤回せず、置換済み模型内の補助結果として保存する。Q3-2は科学的には未達のまま凍結する。固定目標とモデル非依存な達成基準は \texttt{PROJECT\_STATUS.md} で管理する。
...
置換済みM41の出力契約、R107--R111、R121は研究メモに保存する。Q2-1の根拠は引き続きR104--R106、R118、R120、R122であり、現行Q2-2のM39--M48周期と混同しない。

\hypertarget{m48ux3078ux306eux56faux5b9asingletux578bux7269ux7406ux53d7ux6e21ux3057}{%
\section{M48への固定singlet型物理受渡し}\label{m48ux3078ux306eux56faux5b9asingletux578bux7269ux7406ux53d7ux6e21ux3057}}
...
draft-45BのR151はこの代数契約を物理的な破壊的受渡しへ進める。4モード場を単一中央registerへ正準SWAPし、係数行列の反対称成分からpairing controllerを作る。共同実現配置 \(01,10\) は等重み枝seedへ履歴付きでroutingする。A設定生成後に各枝seedを安全なpaired-Hopf盆へ送るため、設定依存切断面測度を設定前開始面へ直接置かない。

受渡しは固定singlet型に限定する。M39の非因子化4モード状態全体をA、Bの2担体へ複製せず、元の4モードslotはSWAP後に空になる。任意M39状態の状態保存的分配または一般最大もつれ状態のM48化は主張しない。有限M39準備、反対称filter、SWAP、枝routingの誤差は \(\varepsilon_{39}+\varepsilon_{\rm link}\) として第5章の前向き誤差へ渡す。
...
\tightlist
\item
  M39の4モード場と共同実現配置をM48中央portへ破壊的に渡す。
\item
...
\hypertarget{ux8a2dux5b9aux524dux958bux59cbux9762ux3068m39ux51faux529b}{%
\section{設定前開始面とM39出力}\label{ux8a2dux5b9aux524dux958bux59cbux9762ux3068m39ux51faux529b}}
...
\hypertarget{ux56faux5b9asingletux578bux306eux7834ux58caux7684ux53d7ux6e21ux3057port}{%
\section{固定singlet型の破壊的受渡しport}\label{ux56faux5b9asingletux578bux306eux7834ux58caux7684ux53d7ux6e21ux3057port}}
...
\begin{theorem}[R151：M39からM48への固定singlet型破壊的受渡し]
...
\begin{theorem}[R155：M48完全周期の有限誤差Bell統計と前提監査]
...
置換済みM41の旧誤差分解と有限誤差Bell監査は研究メモに保存し、現行誤差台帳には重複掲載しない。M48の前向き誤差、非信号周辺差、CHSH値のずれは第8.16節の \(\varepsilon_{45B}\) で管理する。
...
\item
  M48の準備後設定変更、空間分離、一般M39状態の状態保存的分配、独立同分布型有限標本統計。
\item
...
\hypertarget{ux7f6eux63dbux6e08ux307fm41ux3068m48ux306eux672aux9054ux62e1ux5f35}{%
\section{置換済みM41とM48の未達拡張}\label{ux7f6eux63dbux6e08ux307fm41ux3068m48ux306eux672aux9054ux62e1ux5f35}}
...
文書上の範囲 & M37の時間非依存定理だけで時間依存傾斜をミクロ導出済みと扱う、M42/R113を現行Q1またはM48へ使う、R152を具体的ミクロ模型から導出済みと扱う、M39相関またはM48を空間分離Bell統計と扱う、生成物と正本が食い違う \\
\end{longtable}
...
\item
  M48の条件付き達成範囲を越える拡張として、R152のミクロ導出、準備後設定変更、空間分離、一般状態への受渡し、独立同分布型標本統計を検査する。
\item
...
と分ける。\(L_{\rm fib}<\infty\) は固定有限設定族のguard安全域上でfiber距離を結果分布の全変動距離へ移す一様Lipschitz定数である。\(\varepsilon_{\rm fib}\) をR153の右辺で展開するとき、同じpaired-Hopf誤差を \(\varepsilon_{\rm PH}\) と二重に加えない。各設定対の結果全変動距離が \(\varepsilon_{45B}\) 以下なら、R150、R155により一側周辺の設定差は \(2\varepsilon_{45B}\) 以下、CHSH値のずれは \(8\varepsilon_{45B}\) 以下である。
...
反証条件 & 受渡し、詳細釣合い、混合率、2翼fiber、局所条件付き確率、有限誤差上界、帰還上界のいずれかが破れること \\
\end{longtable}
...
Q2-2は条件付き達成である。条件は、固定singlet型、固定有限設定族、破壊的操作受渡し、準備先行、非空間分離、プロトコル面matchingである。交差共分散を単一試行頻度と同一視せず、R152の局所配置bathを新しい採用開放法則として明示した。具体的ミクロ回路からの導出を条件付き達成へ含めない。
...
Q2-2の次の検査は、R152の局所配置rateを具体的な衝突浴、回路または振動子装置から導出し、作用依存詳細釣合い、rate切断、局所記録までの熱・仕事・情報流を同じミクロ模型で閉じることである。別の強化課題として、準備後の設定変更と有限伝播を持つ空間分離版を検査する。Q2-3はこの検査範囲に含めない。
...
\hypertarget{r151ux306eux8a3cux660em39ux304bux3089ux306eux7834ux58caux7684ux53d7ux6e21ux3057}{%
\section{R151の証明：M39からの破壊的受渡し}\label{r151ux306eux8a3cux660em39ux304bux3089ux306eux7834ux58caux7684ux53d7ux6e21ux3057}}
...
\hypertarget{m39ux3068ux306eux5c04ux5f71ux5951ux7d04ux3068r151ux306eux7269ux7406ux7684ux53d7ux6e21ux3057}{%
\section{M39との射影契約とR151の物理的受渡し}\label{m39ux3068ux306eux5c04ux5f71ux5951ux7d04ux3068r151ux306eux7269ux7406ux7684ux53d7ux6e21ux3057}}
...
\hypertarget{draft-45bux3067ux9589ux3058ux305fux9805ux76eeux3068ux6b8bux308bux975eux4e3bux5f35}{%
\section{draft-45Bで閉じた項目と残る非主張}\label{draft-45bux3067ux9589ux3058ux305fux9805ux76eeux3068ux6b8bux308bux975eux4e3bux5f35}}
