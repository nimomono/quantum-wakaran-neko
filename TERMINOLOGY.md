...
| adopted open microscopic equation / 採用開放ミクロ方程式 / 採用open SDE | 直接定めた開放ミクロ方程式 | Hamiltonian浴などの上位模型からの導出を前提とせず、方程式自体をミクロ模型の基本発展則として明示する場合 |
| derived open equation | Hamiltonian浴から導出した開放方程式 | 浴、結合、縮約条件、適用範囲、必要な誤差評価を示して上位Hamiltonian模型から得た場合 |
...
「採用開放方程式」「採用open model」のように「採用」を名詞修飾語として使わない。文章では「開放ミクロ方程式として直接定める」を優先し、名詞として必要な場合だけ「直接定めた開放ミクロ方程式」と書く。「模型を採用する」「採用した近似」のように、「採用する」を通常の動詞として使うことは妨げない。
...
| 標準表記 | 意味 |
|---|---|
| ブラウン巨視的スピン | 熱揺らぎと散逸を受ける固定長単磁区磁化。退役R191で2結果指針変数として用いた旧実装の表記 |
| 確率的LLG方程式 | Landau--Lifshitz--Gilbert型磁化方程式へ熱揺らぎを加え、開放ミクロ方程式として直接定めたもの |
