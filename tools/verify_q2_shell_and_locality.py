...
def main() -> None:
    ...
    checks.append(record_max("r180c_product_normalization_error", abs(float(np.sum(conditional_product)) - 1.0), 2.0e-15))
    checks.append(record_max("r180c_a_marginal_error", float(np.max(np.abs(np.sum(conditional_product, axis=1) - local_a))), 2.0e-15))
    checks.append(record_max("r180c_b_marginal_error", float(np.max(np.abs(np.sum(conditional_product, axis=0) - local_b))), 2.0e-15))

    theta = 1.2
    additive_energy = -theta * np.log(conditional_product)
    expected_additive = -theta * np.log(local_a)[:, None] - theta * np.log(local_b)[None, :]
    checks.append(record_max("r180c_conditional_energy_additivity_error", float(np.max(np.abs(additive_energy - expected_additive))), 3.0e-15))
    ...
    checks.append(record_max("r180c_a_detailed_balance_error", abs(local_a[0] * rate_a_forward - local_a[1] * rate_a_backward), 2.0e-15))
    checks.append(record_max("r180c_b_detailed_balance_error", abs(local_b[0] * rate_b_forward - local_b[1] * rate_b_backward), 2.0e-15))
    ...
    checks.append(record_max("r180c_path_entropy_additivity_error", abs(joint_entropy - local_entropy), 2.0e-15))
