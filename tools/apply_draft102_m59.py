...
def prepend(path: str, block: str) -> None:
    rw(path, lambda t: block.rstrip() + "\n\n" + t.lstrip() if block.splitlines()[0] not in t else t)
...
def main() -> None:
    (ROOT / "sections" / "A23_q3_common_micro_model.md").write_text(A23, encoding="utf-8")
    (ROOT / "tools" / "verify_q3_common_micro_model.py").write_text(VERIFY, encoding="utf-8")

    draft = '''## draft-102：M59 Duffing shell＋二保存action reservoirへの置換
...
'''
    prepend("PROJECT_STATUS.md", draft)

    # Current Q3 evidence rows: replace M58 if present and append the new shell chain.
    def status_update(t: str) -> str:
        lines = []
        for line in t.splitlines():
            if line.startswith("| Q3-1 | 達成 |") or line.startswith("| Q3-2 | 達成 |"):
                line = line.replace("M58", "M59")
                if "R198A--R198D" not in line:
                    cells = line.split("|")
                    if len(cells) >= 7:
                        cells[-3] = cells[-3].rstrip() + "、R198A--R198D "
                        line = "|".join(cells)
            lines.append(line)
        return "\n".join(lines) + ("\n" if t.endswith("\n") else "")
    rw("PROJECT_STATUS.md", status_update)

    def enhance(t: str) -> str:
        t = t.replace("| Q3-1 | 達成 | 未監査 | — | — | — | — | M37、R86 |",
                      "| Q3-1 | 部分達成 | 未監査 | — | — | — | — | M37、R86、M59、R198A--R198D |")
        t = t.replace("| Q3-2 | 達成 | 未監査 | — | — | — | — | M57、R195A、R196A--R196C、R161、R185 |",
                      "| Q3-2 | 部分達成 | 未監査 | — | — | — | — | M57、M59、R195A、R196A--R196C、R198A--R198D、R161、R185 |")
        t = t.replace(
            ...
            "Q3-1-A1とQ3-2-A1は、M59/R198A--R198Cにより実Duffing内部自由度、二保存action reservoir、有限交換補正まで明示した。R198Dの具体的DNLS sectorに対するfinite-time mixing/homogenization witnessが未閉鎖なので部分達成とする。Q3-1-A2/Q3-2-A2はM59のDuffing＋DNLS＋exchange力学そのものの直接数値再現が未実施なので未監査のままとする。")
        t = t.replace("M58 common process--時計--終位置記録--reset", "M59 common process--時計--終位置記録--reset")
        return t
    rw("ENHANCEMENT_TARGETS.md", enhance)

    def readme(t: str) -> str:
        t = t.replace(
            "実在tracer $X$ は、この $Y_e$ と共に並進する通常の平衡oscillator bathへ結合します。Brownian noiseとFDTはこの平衡bathだけが担い、2作用状態数の自由エネルギーがosmotic driftを与えます。",
            "実在tracer $X$ は、この $Y_e$ と共に並進する通常の平衡oscillator bathへ結合します。tracerのBrownian noiseとFDTはこの平衡bathが担います。osmotic driftを与える2作用Gibbs shellは、M59では実2-mode Duffing内部自由度を二保存action DNLS reservoirへ弱結合し、そのmicrocanonical周辺化とfinite-time mixingから導きます。")
        old = "Q3では、M37局所古典振動子signal、thermostatted 2-action shell、M57 dual-ballistic-TL moving-bath tracerを同じM58古典開放模型へまとめています。signal marginalだけを見ればR86のSchrödinger型有効力学、同じ試行のtracerまで見ればR196A--R196C、R161、R185を通してNelson型の時間対称Newton則へ縮約します。井戸型・調和型・W型の束縛状態、トンネル効果、2経路干渉まで進んでおり、位相量子化は未達です。"
