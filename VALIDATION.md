...
- required検算では `verify_m65_open_selector.py`、`verify_q1_live_zeno.py`、`verify_r181d_projector_tree.py`、`verify_r180_m54_receiver.py` を現行M65主線の回帰検算として維持する。
- `verify_r191_macrospin.py` と `verify_q1_r193_macrospin_bridge.py` はrequired対象から外し、retired verifierとしてnotesへ保存する。
- migration checkでA20/A21とactive R191/R193 verifierが消え、M65 fixed-goal表記、Q1/Q2根拠、退役索引が同期したことを確認する。
- candidate込みphysics checks、原稿構造、生成物同期、LaTeX lintを最終headで確認する。
...
- active sourceへ旧M0必須文言が残っていないことを `tools/migrations/check_draft115_m0_open_device.py` でPR固有に検査する。恒久 `check_source.py` へ理論スナップショットを入れず、過去CHANGELOG/VALIDATIONとsuperseded notesは当時の履歴として書き換えない。
...
- Q3-2はR162/R188による固定有限時間・1次元有限格子での時間対称Newton則閉包へ統一し、READMEと第8章の旧残件を削除した。
- Q3-3A/B/Cは一般の環境弱結合を固定目標とし、現行の有限環境純位相緩和構成はその条件を満たす強い証人として維持した。
- 単一装置統合残件は有限能動部分系＋明示的Hamiltonian無限浴の単一ミクロ装置へ統一し、有限浴・有限閉鎖Hamiltonian化は独立の強化課題として分離した。
- 生成器とCIに、Q3-2旧状態、Q3-3有限環境再要求、旧M0有限局所統合表現の再混入防止検査を追加した。
- 一時生成run `34237252360`（run #345）では、全数値検算・意味論境界検査・論文再生成・生成物のbranchへのコミットまで成功し、生成コミット `9f5d86f6ee380288f3e727853cdf9ce0c0a28618` に `paper.md`、`main.tex`、`paper.pdf` を収録した。旧PDFとの同期比較だけは生成前の退避物を比較する一時runのため想定どおり失敗した。
- 一時生成処理を除去したread-only検算run `34237637822`（run #347）で、全 `verify_*.py`、新しい方針回帰、M54理論境界、M54整合理論境界、`tools/build_paper.py`、Markdown/TeX/PDF生成物同期、LaTeX禁止警告、標準構成がすべて成功した。

---
## Hamiltonian無限浴方針の正本化

- 固定目標Q2-1の新文言、Q2-3の有限能動部分系表現、Q3-3A/B/Cからの有限環境条件削除、M0「単一ミクロ装置統一目標」を静的検査対象へ追加した。
- `PROJECT_STANCE.md` の「有限閉鎖Hamiltonian実装は固定目標ではない」「Hamiltonian無限浴」「有限浴への持上げは強化結果」の正本語と、`PROJECT_GUIDE.md` の有限性監査規約を生成器・CIで検査する。
- 過去版のCHANGELOG/VALIDATIONにある有限閉鎖・有限浴方針は当時の履歴として書き換えない。現行正本、現行章、現在参照する研究メモだけを新方針へ同期する。
