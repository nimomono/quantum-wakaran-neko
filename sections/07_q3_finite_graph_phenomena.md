...
**M54 spatial profileへの同じ粒子の持上げ。** R164/R161/R162で開始面の初期matchingを一度だけ準備し、R161 moving specializationが同じ実在粒子を $0$ から $T_{\rm per}$ まで輸送する。終時刻に別のM50位置を再標本化しない。理想位置分布を $p_t$、有限衝突・正則化・clock・記録を含む観測分布を $q_t$ とし、
