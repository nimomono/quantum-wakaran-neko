...
@status: M35と同じ2モード正準信号担体を使うM38を構成し、Bloch 球、共通位相不変性、任意の $SU(2)$ 操作、自律Rabi 振動、任意軸の2段測定、永久外部記録、内部逆計算、外部セル交換 reset を接続する。Q1は厳密、Q2とQ3は無反応と装置誤差を含む制御された任意精度で達成する。M37との同一ハードウェア化は主張しない。
