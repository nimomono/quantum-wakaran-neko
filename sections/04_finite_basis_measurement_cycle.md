...
@chapter: 本文
@title: 明示的な有限基底測定周期
@status: 固定純粋準備・固定有限基底では、作用読出し、唯一結果、条件付き測定後状態、内部記録、逆計算、無理数回転 Poincaré 写像を1本の自律 Hamiltonian 周期として構成する。Q1の局所包絡方向誤差から Born 型分布の全変動距離への上界を与えるが、振動子網と測定装置を同じ完全反復系へ埋め込むことは未完成である。
