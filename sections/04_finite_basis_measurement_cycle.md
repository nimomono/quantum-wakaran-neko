...
@status: 固定純粋準備・固定有限基底・有限次元について、局所位相回転、隣接2モード交換、滑らかな累積比較、無反応結果、条件付き測定後状態、内部記録、逆計算、無理数回転 Poincaré 写像を1本の有限自律 Hamiltonian 周期として構成する。Born 型長期頻度は任意精度で得られる。M37の位置ばね網、局所時計配線、永久記録、弱開放 reset を同じ完全装置へ統合することは未完成であり、固定目標としてのQ2、Q3、Q5は部分達成である。
