...
零でないM54末端信号を正準SWAPで $\widetilde V=v$ と保持し、解析上だけ $V=\widetilde V/\|\widetilde V\|$ とする。A設定 $x$ に応じた $U_x^\dagger\otimes I_2$、R181Dの直交射影子作用保持機構、R164の2結果作用殻、R161/R162の有限再平衡化を順に作用させる。R181Dが固定する物理結果成分容量を
...
| 項目 | 明示する内容と限界 |
|---|---|
| M54 供給源 | R181B/R181Cの有限ハミルトニアン 持ち上げとゲート列、実際の末端信号 $\widetilde V$、同次元保持を使う。正準SWAPに規格化を含めず、末端逆演算用補助記憶部を共役信号に使わない |
