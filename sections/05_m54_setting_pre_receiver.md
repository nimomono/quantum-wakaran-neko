...
と書く。$A_\pm$ は結果成分容量指針変数、$S$ は中央の吸収pointer、$z_A,z_B$ は2翼受信機構信号、$X_A,X_B$ は局所選択変数、$R$ は外部記録である。R190の作用殻浴、R179のincoming/outgoing bath、reset環境は能動状態へ列挙せず環境interfaceとして扱う。$S$ は中央で形成される共通原因であり、この段階では外部結果として記録しない。
...
零でないM54末端信号を正準SWAPで $\widetilde V=v$ と保持し、解析上だけ $V=\widetilde V/\|\widetilde V\|$ とする。A設定 $x$ に応じた $U_x^\dagger\otimes I_2$、第2.13節の直交射影子作用保持機構で2結果の物理容量を保持し、その容量をR164--R190--R179の静的選択とR170 pointer固定へ渡す。共通補題が保持する物理結果成分容量を
...
を置く。R164が作用殻状態数、R161が静的平方根kernelを与え、R190がDrude作用殻混合と対称作用開口、R179が履歴条件付きrenewal、R170が吸収pointer固定を与える。ここで入力するのは各試行の $z_A,z_B$ であり、$V$ の集団平均ではない。
...
能動受信機構状態を $Y_n$、次周期の未使用基準状態を $Y_*$ とする。記録後に結果相関情報と散逸履歴をR179のoutgoing bathへ流し、能動受信機構を一様open resetで基準状態へ戻す。距離 $d_{\rm ret}$ について
...
を満たすとする。このresetは環境履歴を消去せず、stationary incoming bathとoutgoing bathを分離する。有限closed bankを事前配置する必要はない。帰還誤差は次周期入口へ渡し、既に記録した同じ周期の分布へ遡って加えない。
...
精度を上げると、M54 ゲート時間、保持品質、射影結果の固定機構精度、作用殻容量、混合時間、bath interface数、2端Hopf時間、W型粒子位置混合時間、未使用 貯蔵部容量が増える。一般状態で $\tau\downarrow0$ とすると節点質量は減るが $C_\tau$ が発散する。固定一重項では $p_s=1/2$ なので、この交換をQ2-2のCHSH ベンチマークへ持ち込む必要はない。
