@number: 6
@chapter: 本文
@title: 3モード境界作用殻と浴による等方準備
@status: 和・差作用、U(2) の不足、U(3) 不変測度、共通殻の周辺密度、残余ファイバー体積は厳密結果である。全殻等方拡散のミクロ導出と弱開放補正は未解決または近似結果である。
...
境界3モードの準備窓で、構造化浴の多数の未読自由度を消去した縮約方程式を考える。理想的な殻接方向生成子を
