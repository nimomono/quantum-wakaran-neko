@number: A8
@chapter: 付録
@title: 空間位相担体上の局在粒子トークン
@status: R113--R117の局所流、最小跳躍率、Born型等変性、有界率no-go、節一様正則化、初期トークン準備、有限Hamiltonian実装、M37誤差伝播を証明する。理想確率過程、有限装置、ミクロ担体を区別する。
...
M37の局所包絡 $b_{\rm mic}$ は反回転項を含むため、この式を厳密なM37局所作用流として使わない。M42が厳密な流として使うのは理想担体 $b_L$ の $J_{i\to j}$ である。$b_{\rm mic}$ から同じ式で計算する量は、目標流の局所推定値としてH.10節で誤差評価する。
...
この定理は初期分布を生成しない。初期分布を $p(0)$ として準備する物理過程はH.6節に分ける。また、連続方程式だけでは率は一意でない。任意の対称な非負辺流を両方向へ加えても同じ一時刻分布を保てる。上式は、余分な対称往復流を加えず、標準流を実現する率の中で期待跳躍頻度を最小にする選択である [44]。M42はこの最小性をモデル定義として採用する。
...
を満たし、有限時間爆発の確率は零である。節で特異な率を持つBell型過程の大域存在は既知であり [45]、有限状態・有界 $h$ はその十分条件の特別な場合である。本稿の新規結果はこの確率過程自体でなく、M37担体と有限Hamiltonian装置への接続にある。
...
を得る。従って有限時間の軌道分布も正則化極限で理想軌道分布へ近づく。ただし有限正則化では、理想重みが零となる時刻にトークンがその頂点へ残る確率を厳密零にはできない。

## H.6 初期トークンの作用比準備

等変性を使うには、初期トークンを $p_i(0)$ に従って準備する必要がある。初期作用を
...
へ分割し、選ばれたセル $i$ へトークンを運ぶ。作用読出しの共役運動量を入口で零に置けば、理想剪断は担体を変えない。比較境界を無反応結果とし、選択履歴を外部セルへ残せば、M35と同じ有限Hamiltonian機構で初期分布を任意精度に準備できる。

この準備は一般には全セル作用を集める大域過程であり、その後の局所トークン運動と区別する。初期準備誤差を $\varepsilon_{\rm init}$ とすると、後段の等変性はこの誤差を増幅せず、最終全変動距離へ加算する。
...
と書ける。入口で $P_p=P_{J,j}=0$ なら、読出し座標は変化するが担体への理想反作用は零である。比較幅、角切断、有限読出し窓に由来する誤差は無反応を含む1step核の全変動距離として数える。

トークンは新たな波動モードでなく、1個の古典局在自由度 $Z$ とする。有限グラフを互いに交わらない頂点領域と、端点以外では交わらない細い辺チャネルとして3次元空間へ埋め込み、その近傍をトークンの配置空間とする。粗視化位置を
...
と書く。トークンの正準位相空間は、この配置空間上の余接束であり、1個の位置 $Z$ とその共役運動量 $P_Z$ だけを持つ。各有向辺チャネル上で入口から出口へ向く滑らかなベクトル場を $v_{ij}(Z)$ とし、選択ラッチ $c_{i\to j}$ による輸送生成子を
...
とする。これはベクトル場 $v_{ij}$ の余接持上げであり、正準流を生成する。時計窓と $v_{ij}$ の積分を選び、入口Poincaré断面から出口断面までを有限時間で運ぶ。安全枝では1個のラッチだけが作動するため、1個のトークン位相点は1頂点領域または選択された1辺チャネル上にあり、非隣接セルへ移らず、複数辺へ分裂しない。これは有限幅チャネル内の連続軌道を与えるが、格子細分化で得る連続空間極限や、トークンの慣性質量を担体の有効質量 $m$ と同定することまでは主張しない。
...
M37担体は読出し、比較、輸送窓の間も発展する。理想的に停止した担体を仮定せず、窓内変化、読出し時刻と輸送完了時刻のずれ、時計窓との非可換性を $\varepsilon_{\rm win}$ として評価する。
...
を使う。理想空入口 $P_{D_i}=0$ ではトークンへの反作用なしに、トークンを含む1個の頂点検出器だけが作動する。検出誤差と外部記録誤差を $\varepsilon_{\rm det}$ とすれば、予定されたPoincaré断面で
...
これは固定時刻の局所位置読出しである。初回到達、吸収、有限検出窓、時間積分流束、スクリーン永久記録、検出後トークンの処理を含む2重スリット完全装置はQ3-5に残る。

## H.10 M37包絡誤差の伝播

固定した $\rho,\sigma>0$ について、正則化生成子を担体振幅の関数
...
## H.11 有限時間Hamiltonianトークン近似

<!-- theorem-start:theorem -->
**定理（節を含む有限時間Hamiltonianトークン近似）**
有限局所グラフ $G$、実対称な時間非依存生成子 $h_L$、有限時間 $T$、規格化初期担体 $b_0$、任意の $\epsilon>0$ を固定する。担体成分の正の下限は仮定しない。
...
本定理は、初期Born型準備を任意の初期トークン分布からの収束として導かない。トークンが担体の全エネルギー、質量、電荷を運ぶとも、連続空間の点粒子軌道、多粒子過程、固定帯域の同じ装置による $\epsilon\to0$、2重スリット完全検出周期を構成したとも主張しない。
