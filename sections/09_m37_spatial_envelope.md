...
これは予定時刻の局所読出しであり、Q3-4とQ3-5で採用する現在の検出方針である。初回到達、吸収条件、時間積分流束を与えるものではなく、これらを主張するには別の検出装置が必要である。ただし、これらはQ3-4またはQ3-5の固定達成条件ではない。Q3-4には単一障壁散乱と完全結果集合への統合、Q3-5には源、有限スリット、相対位相制御、単・両スリット対照、無検出を含む完全結果集合への統合が残る。
...
10. 井戸型・調和型ポテンシャルの束縛スペクトルと弱開放デコヒーレンス。
11. 単一障壁および有限2重スリット配置と、固定時刻局所検出を統合した完全結果分布。
12. 初回到達、吸収、時間積分流束、連続運転スクリーンを扱う、固定目標より強い検出模型。
