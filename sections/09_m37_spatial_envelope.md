@number: 9
@chapter: 本文
@title: 空間位相担体と局在1粒子トークン
@status: M37の有限局所実振動子網からSchrödinger型位相担体を有限時間誤差付きで導き、M42として独立の局在1粒子トークン、初期作用比準備、Born型等変性、節一様正則化、局所Hamiltonian更新、固定時刻位置検出を追加する。Q3-1の固定基準は維持し、粒子像を強化する。
...
3. 粒子層は、M42が独立に追加する1個の局在正準自由度 $(Z,P_Z)$ と、その粗視化セル位置 $X=X(Z)$ である。
4. 装置層は、初期準備、局所辺選択、トークン輸送、履歴保存、位置検出を行う有限正準制御系である。

ミクロ層から有効担体層への移行はQ3-1の固定達成基準である。M42はM37を上書きせず、位相担体を供給するサブモデルとして含む。完全状態は模式的に $(b,Z,P_Z,\mathcal R,\mathcal C)$ と書き、$\mathcal R$ は選択・履歴セル、$\mathcal C$ は時計・局所制御自由度を表す。理想Markov記述では粗視化した $X_t\in V$ を使う。M37だけから粒子変数や最小跳躍則が必然的に出るとは主張しない。
...
本節の受渡しは、位相担体の作用比を任意有限基底で標本化する補助診断として残す。M42の基本位置測定は、M35が測定時にセルを選ぶ方式ではなく、既に局在している粒子トークンを第9.16節の局所検出器が読む方式である。2モードの完全周期は第3章のM38で独立に扱う。
...
となる。M35だけを使う場合、これは位相担体の空間セル基底を標本化する式に留まる。M42は独立変数 $X$ とその等変力学を追加し、固定時刻の局所位置検出を別に構成する。従ってR87は削除せず、M42の初期準備と検算に使える補助的な作用標本化結果へ位置づける。

## 9.10 M42と独立粒子変数

R62が示すとおり、相関行列、階数1因子、位相担体の作用比だけでは粒子軌道、等変流、局所位置検出は決まらない。M42はこの不足を解釈で埋めず、独立の正準変数 $(Z,P_Z)$ を追加し、その粗視化セル位置を $X=X(Z)$ とする。

位相担体 $b$ はM37の全空間振動子網へ広がり、相対振幅、相対位相、局所辺流を担う。粒子トークン $(Z,P_Z)$ は1個の古典局在自由度であり、頂点断面では $X(Z)$ が1個のセルを、輸送窓中は $Z$ が選択された1本の隣接辺チャネル上の位置を表す。$|b_i|^2$ は担体の局所作用比であると同時に、適切に準備されたトークン集団の粗視化位置分布になる。担体作用が複数の粒子断片を表すとは解釈しない。

粒子トークンが担体の全エネルギー、慣性質量、電荷を運ぶことは示していない。有限幅辺チャネル内の軌道は構成するが、格子細分化で得る連続空間極限は本稿の範囲外である。
...
等変性は初期Born型分布を生成しない。初期粒子トークンを任意の分布から $p(0)$ へ収束させる定理も得ていない。M42では、M35の作用区間選択器を使う初期準備を別の物理過程として置く。
...
を全セルから読み、1個の初期選択器角を累積作用区間へ入れ、選択されたセルへ粒子トークンを運ぶ。選択履歴を外部セルへ残し、理想空読出し入口では担体を変えない。これは一般には大域準備である。準備後の辺流読出し、隣接移動、位置検出は局所的に行う。
...
粒子トークンは、グラフを頂点領域と細い辺チャネルで実現した配置空間上の1個の正準位置 $Z$ である。安全枝では選択された1本の辺の余接持上げだけを起動し、入口頂点断面から出口頂点断面へ有限時間で運ぶ。従ってPoincaré断面では $X(Z)$ が厳密に1セルを指し、輸送中も $Z$ は1本の辺チャネルにだけ存在する。単一占有の波動モードをSWAPする模型としては定義しない。
...
M37は読出し、比較、輸送中にも発展する。位相担体を都合よく停止せず、窓内変化、読出し時刻と輸送完了時刻のずれ、時計窓との非可換性を $\varepsilon_{\rm win}$ として数える。R116は固定 $K$ の局所更新、履歴保存、有限閉鎖埋込、無期限時の弱開放セル流、固定時刻検出をまとめる。

## 9.15 M37から粒子トークン分布への誤差伝播

固定した $\rho,\sigma>0$ では、担体振幅から正則化生成子への写像は有界球上で滑らかである。Lipschitz定数を $L_{\rho,\sigma,h,R}$ とし、R86の包絡誤差を使うと、M37入力と理想担体入力によるトークン分布の差は
...
を使えば、理想空入口 $P_{D_i}=0$ で粒子トークンへの反作用なしに、該当する1個の検出器だけを作動できる。従って
...
となる。Born型位置重みを検出器が測定時に新しく生成するのではない。初期作用比準備で与えた分布を粒子トークン力学が等変に保ち、検出器はそれを局所的に読む。

これは予定時刻の局所読出しである。初回到達、吸収条件、有限検出時間窓、時間積分流束、スクリーン永久記録、検出後トークン、スリット開閉時の総検出率を含む2重スリット完全装置はQ3-5に残る。
...
M42については、乱数種 `20260812` の6頂点鎖と、厳密節を持つ2頂点模型を使って35件を検査した。中心値は次のとおりである。
...
| 担体摂動全変動距離 | $1.359\times10^{-5}$ | $\leq2.401\times10^{-4}$ |

非隣接率が零であること、確率保存、非負性、待機作用、履歴セルの単射性も合格した。この表は `tools/verify_spatial_token.py` から再現できる。数値検算は解析証明の代わりではなく、添字、符号、係数、離散化を監査する回帰検査である。
...
Q3-1の固定基準自体はR83--R88で満たされ、今回の改訂で後から基準を広げたわけではない。R112--R117は、共通有限モード代数、独立粒子トークン、初期作用比準備、Born型等変性、節を含む有限率近似、局所Hamiltonian更新、固定時刻検出を追加する強化結果である。
...
4. 非線形ミクロ結合に対する閉包。
5. 一般連続極限と境界条件の一様誤差。
6. 格子細分化で得る連続空間の粒子軌道、位相量子化、多粒子トークン。
7. 任意の初期トークン分布からBorn型分布への収束、初期作用比準備を不要にする機構。
8. 粒子トークンの慣性質量、電荷、担体エネルギーとの同定。
9. 有限帯域装置による厳密節追跡、固定性能の同じ装置による誤差零極限。
10. 初回到達、吸収、時間積分流束、永久記録を含む2重スリット完全検出周期。
