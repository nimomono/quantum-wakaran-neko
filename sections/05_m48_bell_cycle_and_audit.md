...
である。これはM48で先に採用した開放粒子位置応答則であり、M42/R113の複素振幅場からcurrent rateを作る規則ではない。第3章R161は同じ数学的核を任意のM47 rayへ拡張し、付録MのR164は各翼の単一試行信号作用から $\pi^\delta$ の局所状態数起源を与え、付録LのR162は固定した単一試行bath座標に対する有限衝突熱浴実現を与える。
