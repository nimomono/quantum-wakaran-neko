...
全時刻でmatching多様体が厳密不変であるとは主張しない。切断面、局所分析器終了面、記録面というプロトコル面ごとにmatching誤差を評価する。M48内部のBell結果頻度に必要な橋はR151--R156、Q2-1から同じ試行registerを渡す橋はM49/R160が閉じる。R152はM47のR161と同じ平方根型核を持ち、R162の有限衝突実現を利用できるが、M47とM48を同一ハードウェアへ統合したことにはならない。
...
である。これはM48で先に採用した開放配置応答則であり、M42/R113の複素振幅場からcurrent rateを作る規則ではない。第3章R161は同じ数学的核を任意のM47 rayへ拡張し、付録LのR162は固定した単一試行bath座標に対する有限衝突熱浴実現を与える。
...
R152の正値性、詳細釣合い、有限時間収束は付録Dで直接証明する。さらにR161から全ray一様の明示ギャップ下界、R162から有限衝突セル、エネルギー保存散乱、overflow込み近似を利用できる。ただしM48のpaired-Hopf準備、seed routing、信号bath保持controllerまで同じ最小Hamiltonianから導いたことにはならない。
...
7. R161、R162を用いただけで一般状態M48 receiver、測定後状態、逐次測定を解いたこと。
