...
M49/M50とR112、R159、R164により、Q2-1は積入力を非分離な共同内部状態へ移すCNOT、固定有限benchmark、無反応込み、制御された任意精度の範囲で引き続き達成である。M50枝からの直接decodeを明示しても判定語は変えない。R160により、固定singlet programは付録Jの契約を満たしてM48へ物理的に接続されるが、これはQ2-1またはQ2-2の達成条件ではない。Q2-2はM48/M50とR147、R153、R155、R164、R168、R170を固有の根拠とする。
...
現行の直接モード一般化は $n$ 論理量子ビットに $L=2^n$ モードを要する。指数的なモードが受動的に存在すること自体はQ2-4の失敗条件ではない。未解決なのは、各モードの個別準備・較正・読出しを避け、一様な有限規則と多項式個の外部制御で運転することである。これはM49/M50型の直接符号化候補に対する資源監査であり、別の有限古典装置族まで排除するno-go定理ではない。
