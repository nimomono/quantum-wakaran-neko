...
と分ける。雑音零の採用開放方程式では $\varepsilon_{\rm Hopf}$ の信号bath方向成分を有限時間指数率で抑えられる。実現配置周辺と条件付き配置分布はR161、R162の後段再平衡化で作るため、Hopf準備誤差へ混ぜない。

R161、R162の1回の再平衡化誤差は
...
とする。第1項は有限混合、第2項は正則化、第3項は有限セル数、有限エネルギー、閾値平滑化、時計の衝突近似、第4項は再平衡化中の信号bath保持である。R161の一様下界 $\lambda_\delta\geq\kappa_Xa_{\min}m_\delta\lambda_G$ と $m_\delta=\delta q_{\min}/(1+\delta)$ を使い、$\delta$ を零と置いて一様率を主張しない。
...
| 構成 | 上界 | 非算入資源 |
|---|---:|---|
| M47 Q1能動測定器 | 固定有限段・固定精度で有限 | 条件付き障壁controller、辺ゲート、有限衝突セルの最小正準対数 |
...
M47の1段測定には、信号2モード、条件付き障壁controller、各辺の有限衝突セル、辺ゲート、左右テンプレート、左右記録ポインター、傾斜制御、時計、実現配置検出部、使用済み情報セルが必要である。固定段数、固定観測時間、固定精度では有限だが、最小正準対数は未評価である。$\delta\downarrow0$ では配置エネルギー幅が $O(\!\log\delta^{-1})$、必要衝突流束が少なくとも $O(\!\delta^{-1/2})$ まで増え得て、一様混合率下界は $O(\!\delta)$ まで低下し得る。Zeno用の反復セル数は現行資源表から外す。
...
M47では、傾斜切替をトンネル振動より速く、高モード間隔より遅くする必要がある。$J/G\to0$ の深いW型族は左右読出しと固定を高精度化するが、零傾斜の $x$ 回転時間は $O(\mathcal J_0/J)$ へ増える。さらに正則化誤差を下げるとR161の混合時間とR162の衝突資源が増える。精度、操作時間、傾斜帯域、エネルギー幅を同時に固定した任意精度定理ではない。
...
9. M47のHopf pump、条件付き障壁controller、有限衝突セル、傾斜切替、局所記録、枝別テンプレート交換、resetを含む総エネルギー・エントロピー収支。
...
| M47 Q1測定 | 左右効果がR142と一致しない、R161の定常分布・一様ギャップ上界・正則化誤差が破れる、R162の衝突率・エネルギー保存・有限セル誤差が破れる、辺閉鎖後の離脱率が $\varepsilon_{\rm res}$ を超える、局所記録が統計振幅や全密度を入力にする、枝別テンプレート交換後の条件付き共分散がR143上界を超える、無反応を事後除外する |
| M47 Q1周期 | 操作面ごとの再平衡化を実行せず無条件完全周期と記す、R163の配置部分台帳を全周期収支と扱う、外部記録または使用済み状態を消して非可逆化する、帰還誤差を同じ周期の記録分布へ二重加算する |
...
| M47 W型共同統計 | 古典作用角からR135の共分散回転が出ない、W型交差項がR136の左右占有振動と一致しない、R145の信号bath方向吸引が破れる、Born型配置エネルギーを位相体積から導出せず自然発生と扱う、R161の零node no-goを無視して $\delta=0$ の一様局所混合を主張する、準備散逸・位相固定・浴記憶・2モード外漏れが観測時間で残る、大域階数1を枝別固有状態と同一視する |
| 文書上の範囲 | M37の時間非依存定理だけで時間依存傾斜をミクロ導出済みと扱う、M42/R113を現行Q1・Q2-1・M48へ使う、R162だけでHopf準備またはpaired-Hopf周期全体をミクロ導出済みと扱う、M49相関またはM48を空間分離Bell統計と扱う、固定singlet接続から一般状態receiverを推論する、生成物と正本が食い違う |
...
1. M47のQ1について、R161のBorn型条件付き配置エネルギーをW型装置の局所位相体積または状態数から導き、R145のHopf pump、R162の衝突熱浴、信号bath保持、傾斜切替、局所記録、template交換、resetを同じ有限時間Hamiltonian台帳へ統合する。Q1-4は判定基準を保ったまま凍結する。
...
4. R160の固定singlet接続を基点に、R162をpaired-Hopf準備・seed routing・2翼controllerへ統合し、準備後設定変更、空間分離、一般状態receiver、独立同分布型標本統計を検査する。
...
R145は採用開放Hopf方程式により信号bath方向を階数1位相円へ準備する。R161は、その後の単一試行bath座標 $z$ に対し
...
を条件付きGibbs分布として定め、平方根型局所率の詳細釣合い、一意定常分布、全ray一様の有限時間混合率を与える。R162は、到着粒子の運動エネルギーが指数分布を持つ有限衝突bathとエネルギー保存散乱から同じ率を作り、有限セル、有限エネルギー、閾値平滑化、時計、保持の誤差を分ける。再平衡化後に入射を止めて辺ゲートを閉じることで、記録中の経路滞在失敗も有限誤差化する。
...
R144は固定有限段の記録、逆計算、外部空セル交換を合成する。各操作面でR161、R162を有限時間作用させるため、分析器中または周期間の連続matching保存を仮定しない。R163は配置部分の正逆経路確率比、積分ゆらぎ関係、quench散逸仕事と相対エントロピーの恒等式を与える。ただしBorn型条件付きエネルギーの位相体積起源と周期総収支は閉じていない。
...
| 配置quenchと再平衡化のゆらぎ関係 | R163、配置部分で厳密 |
| Born型条件付きエネルギーの位相体積起源 | 未導出 |
| Hopf pumpからresetまでの周期総収支 | 未導出 |

この改訂でQ1-1は達成、Q1-2とQ1-3は部分達成、Q1-4は未達・凍結を維持する。Q1-2の残件はBorn型地形の物理起源、Q1-3の残件は周期総収支であり、旧い連続matching保存問題ではない。M42/R113はQ2-3とQ3だけに残す。R139--R145、R161--R163の定義、証明、誤差目標は第3章、付録B、付録I、付録Lに示す。
...
| 単一試行bath条件付き配置matching | R152、R161のBell限定形。採用開放rate後に厳密、R162で有限衝突近似可能 |
...
| 未評価資源 | R162とpaired-Hopf・seed routing・2翼controllerの統合、一般状態Bell receiver、総エネルギー・総エントロピー収支、無期限cell供給、設定数に対する規模依存性 |
...
と加える。M48単独周期へは二重に加えない。M41のR107--R111、R121は置換済み模型内の結果として研究メモへ移す。R162だけでpaired-Hopf準備、seed routing、信号bath保持controllerを含むBell周期全体をミクロ導出したこと、連続時間の全区間でmatching fiberを保存すること、一般状態receiver、標準的な空間分離Bell実験は主張しない。
