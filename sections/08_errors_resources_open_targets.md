...
Q1、Q2、Q3の単一試行信号と前向き周期は別である。M47傾斜測定の中心誤差を
...
このM37からM35への受渡しは、任意有限基底の作用標本化を検査する補助診断である。Q3の主読出しは、固定入力時刻のM37標本をM50信号へ渡し、有限熱化後に局所枝位置を記録するR170である。中心誤差は
...
と分ける。$\varepsilon_{37\to50}$ はR168の階数1ray誤差またはR169の固定作用共分散誤差・radial補正のどちらか一方を含む。混合項は
...
である。同じM37標本偏差を、R167の共分散誤差、R168のray誤差、R169の作用変動として重複加算しない。入力標本時刻 $t_\star$ と出力記録時刻 $t_{\rm out}>t_\star$ を分け、熱化中の枝軌道をM37の物理空間軌道と解釈しない。
...
M47、M48、M49、R170の前向き分布は対象周期と入力信号が異なる。R161/R162の数学的核を共有しても、Q1、Q2、Q3を同一ハードウェアへ統合したことにはならない。M49は共同bath座標と粒子位置を同じ行枝から準備する。R170はM37標本を入力時刻で保持し、M50枝へ渡す。M37からM35への接続は状態方向誤差から有限基底分布誤差への補助診断であり、R170の粒子位置instrumentまたは同一ハードウェアの構成ではない。
...
| R170 Q3位置instrument | 完全結果集合上の全変動距離 $\varepsilon_{170}$ | M37受渡し、正則化、作用容量、有限幅、枝流束、混合、衝突、保持、ラッチ、記録 | 第6章、付録F |
...
| R170 固定入力instrument | 固定 $L$・固定精度・固定時刻で有限 | 作用容量結合から記録までの最小正準対数、単一Hamiltonian統合、連続空間極限 |

M35の密な基底回路は高々 $L(L-1)/2$ 個の隣接2モード変換を使う。M49が内部に保持する直接モード符号化を $n$ 論理量子ビットへ拡張すると、信号だけで $2^n$ モードを要する。従って現行値をQ2-3の多量子ビット資源評価へ外挿しない。

R170では、信号register、各枝2作用殻、容量controller、有限衝突セル、枝ゲート、局所記録、履歴セルが必要である。固定 $L$、固定精度、固定観測時刻では各部品を有限に選べるが、これらを一つの最小有限局所Hamiltonianへ統合した正準対数は未評価である。無期限運転では空セル流入と使用済みセル流出を持つ弱開放系として扱う。
...
M37のミクロエネルギーと各理想正準流は保存的である。R170の信号SWAP、M50作用殻、R162衝突bath、保持controller、記録媒体、外部セル輸送、低エントロピーセル供給まで含む総エネルギー収支は未評価である。粒子位置枝に独立の慣性質量やM37担体の保存エネルギーを割り当てたことにもならない。Landauer原理だけを引用して具体的resetの熱・仕事収支の代わりにはしない [17,18]。

R170の一般混合率下界は小さい $\delta$ で $O(\delta)$ まで低下し得る。目標混合誤差 $\varepsilon_X$ に必要な時間は
...
であり、有限幅作用殻を一様精度で保つ剛性には $\Omega(\delta^{-2})$ が必要である。任意精度は固定性能の同じ装置を意味しない。入力標本後は信号registerを保持し、M37包絡を処理窓中に連続追跡するとは仮定しない。
...
7. R170の格子細分化に対する連続空間極限、多粒子拡張、粒子位置枝の質量・電荷・M37担体エネルギーとの関係。
8. R170の信号SWAP、作用容量結合、殻内平衡化、信号保持、衝突bath、局所記録を含む反復周期、最大率・帯域・総エネルギーの精度依存性。
...
| R167--R170 Q3受渡し | R167の共分散上界が破れる、R168で階数1支持から外れた試行を無視する、R169を可変全作用へ補正なしに使う、入力時刻と出力時刻を同一視する、作用殻状態数を二重計数する、無反応を再規格化する、履歴を消して非可逆化する、固定入力instrumentを初回到達分布と同一視する |
...
| 文書上の範囲 | M37の時間非依存定理だけで時間依存傾斜をミクロ導出済みと扱う、R162またはR164だけでHopf準備・paired-Hopf周期・R170全体をミクロ導出済みと扱う、R163の有効仕事・熱を全微視的収支と扱う、M49相関またはM48を空間分離Bell統計と扱う、固定singlet接続から一般状態receiverを推論する、Q3-4・Q3-5を無条件達成と記す、生成物と正本が食い違う |
...
3. Q3-2は判定基準を保持したまま凍結する。Q3-3は達成を維持し、Q3-4・Q3-5はR170統合を条件に達成とする。R123--R125の有限モード・有限時間範囲を維持し、源、シャッター、全検出器、散乱極限、初回到達、吸収、時間積分流束、連続運転スクリーンを独立した強い拡張として管理する。
...
6. R170のM37信号SWAP、作用容量結合、殻内平衡化、信号保持、R162衝突bath、枝固定、局所記録を同じ有限局所Hamiltonianと総収支へ統合する。

一般有限 $L$ への拡張はM35のQ2・Q3一般化課題として並行して管理する。M47、M48、M49、M37、M50、R170、M35を1つのM0と統一母測度へまとめることは、その後の統合目標である。置換済み模型は統合対象から外す。

## 8.13 M45開放自己組織化準臨界準備

M45は、物理ポテンシャル $V(X)$ の正の定常基底状態に限り、目的固有関数を装置へ直接入力せずに定常位置分布を作れるかを調べる現行補助モデルである。R170の固定入力時刻M50読出しを置換せず、一般複素信号、高階数共分散、測定後状態を準備しない。
...
M45とR127--R129はM37、M47、M50、R170、M35を置換せず、M45単独でQ1--Q3の現行達成判定を変更しない。正の主固有関数を選ぶため一般の励起状態を準備せず、一般の時間依存Nelson流、複素位相、Schrödinger時間発展を導かない。完全な方程式、位相体積積分、数値事象定義、条件付き作用素、反証条件は付録Hに示す。
...
R143は、Hopf方向準備、初期再平衡化、解析器、分析後再平衡化、傾斜固定、辺閉鎖、左右井戸の局所記録、結果別2モードテンプレート交換、交換後再平衡化を順に合成する。記録相互作用は $X$ の局所関数だけを入力にし、統計振幅、共分散、全密度、確率流またはcurrent transducerを使わない。大域階数1共分散は枝別測定後状態を意味しないため、R143は枝別共分散を独立に評価する。
...
この改訂でQ1-1は達成、Q1-2とQ1-3は部分達成、Q1-4は未達・凍結を維持する。Q1-2の残件は作用殻起源を有限局所Hamiltonian周期へ統合すること、Q1-3の残件はfiberを含む周期総収支であり、旧い連続matching保存問題ではない。R139--R145、R161--R164の定義、証明、誤差目標は第3章、付録B、付録I、付録L、付録Mに示す。
...
が詳細釣合い、一意定常分布、有限時間全変動収束を持つことを示す。これは物理場のcurrentから全時刻位置率を作る因果律でなく、M48に採用する局所開放粒子位置応答則である。R152はR161のBell限定形であり、R161の一様ギャップ下界とR162の有限衝突熱浴実現を利用できる。
...
とする。Q1は全項を使う。Q2-1の中央4枝直接標本化は $\varepsilon_{\rm cap}$、$\varepsilon_{\rm width}$、$\varepsilon_{\rm sym}$、作用区間座標誤差、decode誤差を使い、配置混合と衝突熱浴を確率の起源にしない。Q2-2は各翼・各局所段階のM50誤差に $\varepsilon_{\rm prod}$ を加える。Q3のR170はR168またはR169の受渡し誤差、無反応、枝固定、局所記録を別に加え、同じM37標本偏差を複数のM50項へ重ねない。
...
$J,A,J_{\rm ref}$ は作用、$\Omega$ は無次元、$\Theta,E,F,B$ はエネルギー、$\beta$ はエネルギーの逆数、殻剛性はエネルギー毎作用2乗、率は時間の逆数である。有限幅を正則化極限で一様に保つ剛性には $\Omega(\delta^{-2})$ が必要で、$\Theta(\delta^{-2})$ は代表的な選択である。Q1-1達成、Q1-2・Q1-3部分達成、Q2-1達成、Q2-2条件付き達成、Q2-3未着手、Q3-1・Q3-3達成、Q3-4・Q3-5条件付き達成の判定を採用する。
