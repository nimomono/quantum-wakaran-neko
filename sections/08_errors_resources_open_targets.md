...
| Q2-1--Q2-2受渡し | cross matching、単一試行配置matching、同一レジスタ搬送、setting-free性 | 共同bath provider、$T_{\rm link}$、破壊的dilation、state・branch・provenance感度 | 付録K |
...
置換済みM41の旧誤差分解と有限誤差Bell監査は研究メモに保存し、現行誤差台帳には重複掲載しない。M48の前向き誤差、非信号周辺差、CHSH値のずれは第8.16節の $\varepsilon_{\rm Bell}^{48,{\rm cyc}}$ で管理する。将来の物理受渡し誤差 $\varepsilon_{\rm Q2-link}$ はprovider未構成のため、M48単独周期の台帳へ算入しない。
...
6. 付録Kを満たすQ2-1--Q2-2受渡し、M48の準備後設定変更、空間分離、一般状態、独立同分布型有限標本統計。
...
## 8.10 Q2-1--Q2-2受渡しとM48の未達拡張

M41は置換済み模型であり、その旧条件付き達成評価を現行Q2-2根拠へ戻さない。M48単独周期を越えるには、まず付録Kの共同状態、cross matching、単一試行配置matching、setting-free面、同一レジスタの $T_{\rm link}$、拡大系で一対一な破壊的dilationを構成する必要がある。その後に、準備後の自由な設定変更、空間的に隔たった設定選択、有限伝播速度を持つ長距離時計配線、一般状態、独立同分布型有限標本統計を別々に扱う。これらを現行singlet型の誤差係数へ吸収してはならない。

state-carrying接続を要求する場合は、Q2-1出力の2つ以上の射影類に応じてM48入口または出力法則が区別可能に変わる必要がある。branch-carryingなら入力枝bias $p$ を保存し、provenance-onlyなら履歴条件付きでも結果法則を変えないことを検査する。旧R151の反対称filterは入力状態を固定 $\mathsf E$ へ潰し、等重み枝は内部fair seedで代替できるため、state-carrying条件を満たさない。
...
| 文書上の範囲 | M37の時間非依存定理だけで時間依存傾斜をミクロ導出済みと扱う、M42/R113を現行Q1またはM48へ使う、R152を具体的ミクロ模型から導出済みと扱う、M39相関またはM48を空間分離Bell統計と扱う、M48単独周期をQ2-1接続済みと扱う、生成物と正本が食い違う |
...
4. Q2-1からM48への共同bath--実現配置providerを付録Kの契約に従って構成し、その後にR152のミクロ導出、準備後設定変更、空間分離、一般状態、独立同分布型標本統計を検査する。
...
R151--R154は、この抽象仮定を固定singlet型Bell装置のreceiver側について閉じる。R151は、setting-pre等重みseed $S_0$ と固定pairing tensor $\mathsf E$ から始め、A設定生成後に各枝を安全なpaired-Hopf盆へ送る。provenance履歴を付けても結果形成へ入力しない。M39の共同配置 $01,10$ をseedへ変換するadapterは任意だが、M48の必須入力でも、M39状態を運ぶ受渡しでもない。
...
| 未評価資源 | Q2共同bath providerと $T_{\rm link}$、R152の具体的ミクロ装置、総エネルギー・総エントロピー収支、無期限cell供給、設定数に対する規模依存性 |
...
R151--R156は、固定singlet型、固定有限設定族、準備先行、非空間分離、プロトコル面matchingという範囲でM48単独周期を局所記録、次周期入口まで閉じる。従ってM48単独周期を条件付き達成とする。一方、付録Kの共同bath providerと $T_{\rm link}$ は未構成なので、固定目標Q2-2全体は部分達成である。将来の受渡し誤差は
...
と予約するが、現行の $\varepsilon_{\rm Bell}^{48,{\rm cyc}}$ へは加えない。M41のR107--R111、R121は置換済み模型内の結果として研究メモへ移す。R152のrateを具体的ミクロ装置から導出したこと、連続時間の全区間でmatching fiberを保存すること、標準的な空間分離Bell実験は主張しない。
