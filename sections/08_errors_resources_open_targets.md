...
@status: M54から派生するQ1/Q2の静的open-bath選択とQ3の開放jump状態構成、R180受信機構、M37物理実装層を横断比較し、R162/R170/R179/R190の新責務、R181A--R181D、資源、反証条件、未完成目標を整理する。
...
## 8.3 共通R170選択・吸収pointer誤差

R170は容量生成や有限衝突装置を抱えず、上流の静的選択分布を吸収pointerへ固定する。選択終了時刻 $t_s$ で $D_{\rm TV}(\mathcal L(X_{t_s}),\widehat\pi)\leq\varepsilon_{\rm sel}$ とし、共通capture rate $\gamma$ のpointerを時間 $T_L$ 開く。pointer実装・有限漏れ誤差を $\varepsilon_{\rm ptr}$ とすれば
...
容量latch誤差は上流で1回だけ数える。外部記録まで含む場合だけ $\varepsilon_{170}^{\rm obs}\leq\varepsilon_{170}+\varepsilon_{\rm rec}$ とする。Q1/Q2の静的選択にR190を使う場合、$\varepsilon_{\rm sel}$ はR179 incoming誤差、R190 memory・mixing・scatterer誤差の和で抑える。同じ偏差をR161の抽象mixing誤差として再加算しない。有限衝突Hamiltonian、有限gate閉鎖、平坦域保持は中心誤差台帳から外す。
...
$\varepsilon_{\rm node}^{\rm dist}$ は出力側共通射影選別機構の容量latch、R190/R179静的選択、R170 pointer、局所記録、保護帯、制御付き選別機構、方向を変えない振幅再調整、転送の完全結果誤差を各1回だけ数える。$\eta_W$ は左右有限コントラストによる結果頻度の偏差である。
...
固定有限列ではpointerと選別機構用workspaceをR179のopen resetで再使用し、結果相関履歴をoutgoing bathへ流す。結果別の固有状態テンプレートと測定後再整合は不要である。R189CのQ1-2達成証人では $N=2$ を固定し、中間局所記録と方向を変えない振幅再調整を使わず、作用容量指針変数、R190/R179静的選択、R170 pointer、1個の選別機構用workspaceを追加する。永久記録、補助逆計算、周期末リセット、作用容量保持から選別機構までの有限能動部分系＋Hamiltonian無限浴の単一ミクロ装置統合、仕事・熱・エントロピー収支はQ1-2固定目標とは別の実装・熱力学的強化課題として残る。有限浴化はさらに別の強化課題である。
...
したがって $\nu c_{\rm ap}=\kappa_Xa_{ij}$ の校正では最悪attempt frequencyが $O(\delta^{-1/2})$ まで増大し得る。これは旧R162熱的有限衝突特殊化で得ていた正則化資源次数と同じであり、R190を使っても $\delta\downarrow0$ の資源発散は消えない。
...
Q2-4ではM54の $L=2^n$ 受動signal modeを使い、R181Cが局所gateを一括作用させ、R181Dが出力bitごとの未処理容量を保持する。各節点の静的選択は

```text
capacity latch
→ R164 action shell
→ R190 Drude mixing / aperture
→ R179 renewal
→ R170 absorbing pointer
→ reversible filter
→ radial repump
```

とする。R178Dの有限閉鎖reset境界と旧R179のcold/spent blank-bankは必須因果鎖から外す。結果相関情報や散逸履歴はoutgoing bathへ流し、能動workspaceとpointerだけをR179のopen resetで再使用する。

外部program、制御channel、準備・実行・reset時間、結合強度・bandwidth・精度、外部から個別指定するbath port数を $\operatorname{poly}(n,d,1/\epsilon)$ に抑える。受動signal mode、bath自由度、装置体積、総bath容量、総熱は報告対象の内部資源とし、それだけでは失敗としない。bathがmode別初期化・較正、回路出力確率、振幅表、指数長係数表を外部入力として要求する場合は失敗である。R186の加法noise障害も維持する。

Q2-4は条件付き達成を維持する。残る条件は、静的部分系配線、射影容量latch、R190/R179静的selector、R170吸収pointer、制御付きfilter、方向を変えない振幅再調整、open reset/supply interfaceを一つの一様装置族へ接続し、R186の許容noise条件を満たすことである。
...
| M54/R181A--R181D・R179・R186 | pointer固定前にfilterを開く、希少結果を事後除外する、状態依存除算を使う、open bathへ回路出力確率やmode別係数を外部注入する、一様装置族へ統合できない、またはR186の加法noise障害を回避できず指数精度を要求する |
...
固定目標上の未完成事項は、Q3-6の位相量子化、Q2-1/Q2-3の末端open-bath接続、Q2-4の一様装置族統合である。Q2-4では静的部分系配線、射影容量latch、R190/R179静的selector、R170吸収pointer、filter、radial repump、open reset/supply interfaceを一つの装置族へ接続し、R186のnoise条件を満たす必要がある。

固定目標と分ける強化課題は、採用開放方程式を明示的Hamiltonian無限浴から縮約すること、Q1/Q2の完全周期収支、R180Cの共通bath統合、Q3の新R162を特定Hamiltonian浴から導くこと、連続空間、多粒子である。旧R162有限衝突経路、旧R188、旧R179 partial-SWAP bank、旧R178D有限閉鎖reset境界は撤回せず、有限閉鎖実装を調べる強化結果として論文外メモへ保存する。

Q1-1、Q1-2、Q3-1、Q3-2、Q3-3A、Q3-3B、Q3-3Cは達成、Q2-1、Q2-2、Q2-3、Q2-4、Q3-4A、Q3-4B、Q3-5は条件付き達成、Q3-6は未達である。

## 8.14 ## 8.14 M37からQ1制御へ進む強化課題
