...
従って有限次元ユニタリ $U$ は、エルミート対数 $h$ と有限時間パルスを選ぶことで実正準流として実装できる。大域 位相も実正準回転であり、末端Born比には影響しない。
...
R181Bは未知の係数を外部で読み出すのでなく、入力モードと未使用 対象の局所ハミルトニアン 結合で積を生成する。従って制御器のプログラムは入力値に依存しない。
...
これは $|10\rangle$ と $|11\rangle$ を交換し、$|00\rangle,|01\rangle$ を固定する。よってCNOTに等しい。式(C.1)の正準座標へ展開すると、定数尺度を除いて本文式(4.17)の差モード 振動子を得る。
...
末端の実信号 $v$ をR112の正準 SWAPで未使用 保持-記憶部 $V$ へ移す。SWAPは同次元の正準置換であり、係数の推定、結果選択、再準備を含まない。
...
- 正準 SWAP出口と容量指針変数入口の共通安全 集合
- 指針変数容量からR164作用殻への有限ハミルトニアン境界
- R161/R162の有限ファイバー混合が保つ枝対称性
- 収集、固定、記録までを含む単一時計自由度 時刻割当
- すべての失敗素子と無反応を含む完全結果空間

これらは一般入力持ち上げや中間コヒーレント 復号器の欠落ではない。R181BとR181Cにより、その二つはそれぞれ明示的持ち上げと同じ永続記憶部上のゲート列へ置き換わった。
