@number: B
@chapter: 付録
@title: M38の2モード完全周期と有限Zeno周期
@status: Q1の操作、測定、記録、reset、固定有限Zeno反復の証明と資源計数をまとめる。
...
第 $j$ 選択器の作用区間幅は、過去履歴から決まる $K_j(s_j\mid s_{j-1})$ である。積Haar 測度により各履歴集合の体積が付録B.15の積になる。これは長期頻度を与えるが、結果列の独立同分布性または二項分布型有限標本揺らぎを与えない。
