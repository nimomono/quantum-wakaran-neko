@number: A3
@chapter: 付録
@title: 局所滑らか有限基底測定周期の完全正準構成
@status: 固定純粋準備・固定有限基底について、局所2モード回路、累積比較ポインター、無反応を含む結果集合、信号、テンプレート、レジスター、選択器、時計の全写像を追跡する。安全セクターでは測定後状態が厳密であり、全入力で内部逆計算が厳密に成立する。
