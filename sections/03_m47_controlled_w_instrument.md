...
@status: Q1を、2モード信号bathの仕事行程と有限衝突熱浴による配置再平衡化行程へ再編する。Q1-1は達成、Q1-2とQ1-3はBorn型自由エネルギー地形の起源と周期総収支を残して部分達成、Q1-4は未達のまま凍結する。
...
2. 信号bath方向を保持し、R161、R162で実現配置を条件付きGibbs分布へ近づける。
3. 衝突熱浴を切り、W型2モードの傾斜制御で測定軸の固有方向を左右局在方向へ写す。
4. 分析器終了後に再び衝突熱浴を有限時間だけ接続し、新しい信号bath方向へ配置を再平衡化する。
5. 入射セルと辺遷移を止め、トンネル振動より速く、高モード間隔より遅く傾斜を立ち上げる。
6. 左右井戸のエネルギー差と閉じた辺ゲートで、既存の実現配置を記録終了まで片側へ保持する。
7. 各井戸に置いた局所記録ポインターが、その場所にある $X$ だけを記録する。
8. 安全枝では、記録結果に対応する準備済み2モードテンプレートと信号bathを正準交換し、交換後方向へ配置を再平衡化する。
9. 測定前情報と使用済み装置状態を外部セルへ残し、内部補助を逆計算と交換resetで戻す。
...
付録IのR145は、雑音零の採用開放Hopf方程式について信号bath方向を目標位相円へ吸引する厳密解と有限時間率を与える。付録LのR161は任意のM47階数1方向に対する配置の一様指数再平衡化、R162はその局所詳細釣合い率の有限衝突実現、R163は解析器quenchと配置経路のゆらぎ関係を与える。R145とR161を順に作用させれば、信号bath方向と条件付き配置分布を同じ操作面へ有限時間で準備できる。

第5章のR152は、固定singlet型Bell装置に限って同じ平方根率を先に採用した結果である。R161はその数学的核を任意のM47 rayへ拡張し、R162は固定した単一試行信号bath座標に対する衝突熱浴実現を与える。R152を一般Q1定理として遡及的に読み替えず、R161--R163をQ1の新しい根拠とする。

一方、条件付き配置エネルギー $E_i^\delta=-\Theta\log\pi_i^\delta$ はBorn型対角から設計される。これをW型装置の自然な位相体積から導いた結果ではなく、Hopf pump、controller保持、記録、resetを含む周期全体の仕事・熱・エントロピー収支も未閉鎖である。このためQ1-2とQ1-3の判定は部分達成のまま維持する。
...
を作る。$q_i>0$ は固定背景分布、$\delta>0$ は有限装置の背景占有率である。$w_i(z)$ は集団共分散または統計振幅をcontrollerへ戻して作る量でなく、同じ試行に存在する2作用角 $z$ と固定W型mode係数から得る局所信号である。

熱作用尺度を $\Theta$ とし、条件付き配置エネルギーを
...
と置く。$z$ を固定したとき、$\pi^\delta(z)$ はこの有限配置系のGibbs分布である。任意の配置分布 $p$ の非平衡自由エネルギー差は
...
となる。従ってrematchingを、抽象多様体への射影でなく条件付き自由エネルギーの緩和として扱える。

このエネルギー地形はBorn型対角から設計されている。$E_i^\delta$ をM47内部状態数から導いたとは扱わず、その位相体積起源を未解決問題として残す。

## 3.7 任意の信号bath方向に対する配置再平衡化
...
信号bath方向 $c_t$ を外部protocolと見る。配置系の仕事と熱を
...
である。一般のエネルギー基準では $W$ を $W-\Delta F$ へ置き換える。
<!-- theorem-end:theorem -->

R163は、局所詳細釣合い率が単に目的分布を不変にするだけでなく、正逆経路確率比と散逸仕事を固定することを示す。ただしHopf pump、信号bath保持、傾斜回路、記録、template交換、resetを含む全周期の総収支は別に閉じる必要がある。
...
分析器終了後にR161、R162の配置再平衡化を時間 $T_X$ だけ作用させ、その誤差を $\varepsilon_{\rm eq}$ とする。左、右の実現配置読出しは、任意軸射影重み $p_+,p_-$ から各成分で高々 $\eta_W+\varepsilon_{\rm eq}$ ずれた2値分布を持つ。有限の分析器、傾斜切替、固定、局所記録、境界無反応を加えた結果分布 $p^{\rm obs}$ は、無反応質量を零とした理想分布 $p^{\rm id}$ に対して
...
とする。最初の項はR161の有限混合、2項目は理想Born型対角との差、後二項はR162の有限セル実現と信号bath保持の誤差である。
...
1. R145で信号bath方向を有限誤差 $\varepsilon_{\rm Hopf}$ 以内に準備し、R161、R162の初期再平衡化を $\varepsilon_{\rm eq}$ 以内で実行できる。
...
R143はR137の全時刻matching保存を仮定しない。分析器操作で配置--信号bath整合が崩れることを許し、操作面ごとにR161で回復する。記録中の経路滞在もR162の入射停止と辺ゲート閉鎖へ置き換えた。一方、条件付きエネルギー地形の位相体積起源、信号bath保持controllerの完全な反作用、Hopf準備からresetまでの総収支はR143から従わない。
...
R144は全時刻のmatching保存または周期間matching帰還を仮定しない。各操作面でR161、R162を有限時間だけ作用させる。一方、Hopf pump、信号bath保持controller、衝突セル準備、記録、template交換、resetを含む総仕事、総熱、総エントロピー生成を一つの恒等式へ閉じていない。このためQ1-3の完全達成定理ではない。
...
とする。$\varepsilon_{\rm eq}$ は有限時間混合、正則化、有限衝突セル、信号bath保持を含む。$\varepsilon_{\rm lock}$ は信号bath左右占有の変化、$\varepsilon_{\rm res}$ は辺ゲート閉鎖後の単一試行経路離脱であり、同じ量ではない。状態方向誤差、結果分布の全変動距離、記録ポインター誤差、周期末正準座標偏差は単位が異なるため、付録Bで対応するLipschitz定数を通してから合成する。
...
と分ける。R145は $\varepsilon_{\rm Hopf}$ の信号bath方向部分を有限準備時間で抑える。実現配置周辺と条件付き配置分布はR161、R162の後段再平衡化へ移し、R145単独の誤差へ入れない。零seedと位相基準の失敗は独立に完全結果集合へ残す。

熱力学台帳では、信号bath方向を変える解析器仕事 $W_{\rm ctrl}$、条件付き配置地形を切り替える仕事 $W_{\rm q}$、衝突セルとの熱交換 $Q_X$、Hopf pump仕事 $W_{\rm Hopf}$、記録・template交換・reset仕事を分ける。R163は配置部分について
...
1段の能動装置は、信号2モード、条件付き障壁controller、有限衝突セル、辺ゲート、左右テンプレート、左右記録セル、傾斜制御、時計、実現配置の局所検出部からなる。固定段数と固定観測時間では有限自由度である。正準対の最小数は評価しない。$K$ 周期の永久記録、衝突履歴、使用済みテンプレート保存は $O(K)$ 以上、固定有限段の制御時間は傾斜パルス数と再平衡化時間に比例する。

$\delta\downarrow0$ では配置エネルギー幅が $O(\!\log\delta^{-1})$、必要衝突流束が少なくとも $O(\!\delta^{-1/2})$ まで増え得て、R161の一般混合率下界は $O(\!\delta)$ まで低下し得る。有限資源のまま全bath方向の厳密nodeを追跡するとは主張しない。
...
| Q1-2 | 部分達成 | R141--R145、R161--R163 | 配置再平衡化、経路滞在、枝別整合は有限誤差化。Born型自由エネルギー地形の位相体積起源が残る |
| Q1-3 | 部分達成 | R143--R145、R161--R163 | 連続matching保存は不要化。Hopf pump、controller、記録、resetを含む周期総収支が残る |
...
3. 条件付き配置エネルギー $E_i^\delta$ がW型装置の自然な局所状態数から発生すること。
4. 信号bath保持controllerへの反作用が厳密に零であること。
5. 全時刻の配置--信号bath matching保存。改訂後の周期はこれを必要としない。
6. 有限障壁の左右位置読出しが厳密射影になること。
7. 傾斜切替で高モード漏れが厳密に零になること。
8. 局所記録が統計振幅、共分散、確率流を測っていること。
9. 無反応なしの滑らかな厳密2値写像。
10. 固定容量の閉鎖系による無期限の衝突熱浴、永久記録、reset。
11. Hopf pumpからresetまでの総仕事、総熱、総エントロピー収支。
12. Zenoまたは反Zeno効果。
13. M42/R113をQ2・Q3からも撤去したこと。
