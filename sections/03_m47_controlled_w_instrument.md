...
現行M65主線では測定終了時に粒子位置 $X$ を新しい信号方向へ再平衡化する必要はなく、選択後信号を同じ単一試行のまま次の制御区間へ渡す。3.11の代替位置診断を併用する場合だけ、その診断が完了するまで $X$ の安全井戸滞在を別条件として監査する。次の測定面ではその時点のW2射影作用を新しいR191へ渡すため、全時刻の配置--信号整合も、結果依存の外部状態再準備も仮定しない。
