...
@status: 設定前共通基準測度から2翼bath対を準備する開放古典模型を定義し、R146--R150の否定的結果、吸引率、singlet交差共分散、条件付き余弦則、有限誤差Bell監査を示す。完全matching周期は未完成である。
...
## J.15 非主張とdraft-45Bへの門

draft-45Aで主張しない事項は次である。

1. M48開放方程式の具体的回路、流体、振動子浴からの導出。
2. M48の有限閉鎖Hamiltonian系への持ち上げ。
3. $X_A,X_B$ とbath対を同時に吸引する完全matching定理。
4. 中央切断後のmatching保存、2局所分析器、傾斜固定、記録、resetを含む完全周期。
5. M39のQ2-1出力からM48準備portへの物理的受渡し。
