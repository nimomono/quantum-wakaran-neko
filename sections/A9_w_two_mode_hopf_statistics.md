@number: I
@chapter: 付録
@title: W型二mode Hopf共同統計模型
@status: 第8.15節のM47について、実現配置--浴共同統計、外部制御された開放Hopf準備、閉鎖作用角伝播、左右占有振動、条件付き閉包、逆設計存在構成を詳述する。R130--R132から再利用する補助結果と、未導出のmatching多様体準備を区別する。

## I.1 目的と主張範囲

本付録は、対称なW型ポテンシャルの最低二mode sectorだけを扱う。各試行で実在する基本状態は、実現配置 $X$ と浴変数 $\xi$ の組
...
この模型をM47と呼ぶ。M47はM42の場からrateを作るR113、M46の局所current transducerであるR133、R133を前提とするR134を使わない。M42と付録Fは既存Q1--Q3の操作模型として残るが、M47の因果的根拠ではない。
...
自然な局所Hopf--浴結合が所望のmatching多様体を吸引的に準備し、切断後の完全な $X$--bath流がその多様体を保存することは未導出である。この未導出部分をR135--R138の厳密部分へ含めない。
...
と読む。この作用素は量子力学から入力するのでなく、M37の古典正常mode問題の有効生成子である。
...
とする。位相規約を選び、$\phi_0$ を実偶関数、$\phi_1$ を実奇関数、両者を規格化直交とする。二mode埋込みを
...
とする。$g>0$ はHopf型動径供給と飽和、$\kappa>0$ は目標二mode方向から外れた成分の散逸、$\eta_t$ は開放portの雑音である。
...
となる。$\Pi_*=c_*c_*^\dagger$ である。$\kappa>g$ とし、初期値が $c_*$ 方向へ非零成分を持てば、直交成分は減衰し、動径は1へ近づく。共通位相方向は中立なので、吸引集合は一点でなく
...
自律Hopf limit cycleだけを各試行で独立に準備すると、共通位相は不要でも、二mode間の相対位相または切断時の位相が試行ごとに散る可能性がある。W型左右占有振動を同じ時刻原点で観測するには、準備中に次のいずれかが必要である。

1. 二modeが共有する外部pump位相。
2. 外部clockに同期した切断時刻。
3. 二作用角間の明示的な位相固定結合。
...
が二mode sectorで代数的に成り立つ。この式は古典作用角流から導いた統計因子の式であり、$X$ 周辺がその対角へ追随することはR135だけからは従わない。
...
仮定2からR135により $C(t)=U(t)C(t_0)U(t)^\dagger$ である。仮定1と3から全時刻でrank-one性と $\rho_i=K_{W,ii}$ が保たれる。rank-one因子を $c(t)=U(t)c(t_0)$ と選べば、$\psi_W^{\rm stat}=\Phi c$ は二mode生成子式を満たす。対角matchingから実現配置周辺がその絶対値二乗に一致し、R136を適用できる。証明終。
<!-- theorem-end:proof -->
...
とする。さらに二mode漏れ、bath記憶、切断残差をそれぞれ
...
今回の改訂ではM42を既存Q1--Q3の根拠から直ちに撤去しない。R113--R121、付録F、既存回帰がM42に依存するためである。ただしM42をM47の基礎、W型統計閉包の証拠、transducerなし模型の実装とは扱わない。M42を全面的に退役させるには、測定、共同配置、固定時刻位置読出しをM47型共同統計から再導出する別改訂が必要である。
...
3. W型偶奇二modeの交差項がR136の密度または左右占有率と一致しない。
4. $\mu\in\mathcal M_W$ としたにもかかわらず $P(X=i)=K_{W,ii}$ が破れる。
5. 切断後に準備散逸、雑音、位相固定、bath記憶が同じ次数で残る。
6. 二mode外への漏れが観測時間内で無視できない。
7. 開放Hopf方程式が目標回転軌道へ吸引せず、相対位相が試行間で拡散する。
8. matching条件を目的密度の直接入力だけで満たし、それを自然な導出と呼ぶ。

現在の未導出事項は次の通りである。

1. M45または具体的負性抵抗回路からI.5の二mode開放方程式を導くこと。
2. $X$ 周辺、bath共分散、条件付きbath分布を同時に $\mathcal M_W$ へ吸引すること。
3. 切断時の外部仕事、残留相関、bath記憶を評価すること。
4. 閉鎖した局所古典流がmatching fiberを有限時間ほぼ保存すること。
5. 逆累積分布によらない局所的な実現配置軌道を作ること。
6. W型最低二modeを越える一般ポテンシャル、多mode、node、連続空間一様極限へ拡張すること。

従って、M47が解析的に確立する中心は、古典作用角回転がrank-one統計核の相対位相を回し、matching条件下でW型左右占有分布を周期振動させることである。開放Hopf準備からそのmatching条件を自然に作る部分は、次の中心定理として残る。
