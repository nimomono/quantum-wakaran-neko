...
であり、独立した複素実体ではない。$X\in\mathcal I$ は状態構成に応じて測定結果成分選択機構または空間素子上の実在粒子位置を表す。$S_{\rm port}$ は供給源／テンプレート接続端、$G,W$ は逆演算用補助記憶部と可逆作業領域、$J,A^\delta$ は未処理・正則化作用容量、$X$ は配置または静的選択変数、$Y$ はR170の吸収pointer、$D$ は外部記録、$\tau$ は能動制御に必要な時計自由度である。確率的jump、作用殻混合、reset、排熱を担う浴は能動状態へ列挙せず、明示した環境接続端として別に扱う。$S_{\rm ref}$ はM37局所ばね物理実装層を使う空間状態構成だけが開始面で使う作用保持機構であり、他状態構成では空でよい。
...
を満たすとする。各有向辺へ独立なunit-rate Poisson reservoirを接続し、時刻 $t$ に配置が $i$ のとき、reservoir mark $u$ が $0<u<k_{i\to j}(t)$ を満たすincoming eventで $i\to j$ を起こす。この古典開放jump模型は固定有限時間で非爆発であり、生成子は
...
R161は静的・移動の率構成を共通に保つが、物理実現を一種類の有限衝突装置へ統一しない。Q3では新R162の開放jump過程を直接使う。Q1/Q2の静的状態構成では、作用殻状態数をR164、平方根kernelの具体的混合・開口をR190、反復時のfreshnessをR179が担う。旧R162の有限衝突・熱的特殊化は有限閉鎖実装の強化結果として退役メモに保存する。
...
## 2.9 R170：固定作用容量入力の静的選択・吸収pointer固定

R170を、Q1、Q2の段階的射影選別、R180Aの中央潜在選択、R180Cの局所読出し、Q3固定時刻診断で共有する静的選択・固定の正本とする。上流は正の固定済み作用容量 $\widehat A_i$ を保持し、R164/R190/R179が与える静的選択過程を有限時間走らせる。選択分布が目標容量比へ近づいた後は選択bathを切り、結果を開放吸収pointerへ写して固定する。有限閉鎖Hamiltonianで入射停止、辺閉鎖、平坦域保持、全微視的履歴の1対1保存まで構成することはR170の必要条件にしない。

<!-- theorem-start:theorem -->
**定理（R170：固定作用容量入力の静的選択・吸収pointer固定）**
...
を満たすとする。$t_s$ で選択bathを切り、pointer $Y\in\{\varnothing\}\cup\mathcal I$ を $Y=\varnothing$ から開始する。lock区間では現在の $X=i$ に対して $\varnothing\to i$ だけを共通capture rate $\gamma>0$ で許し、一度 $Y=i$ になった後は吸収状態とする。
...
であり、pointerの有限漏れまたはcapture実装誤差を $\varepsilon_{\rm ptr}$ とすれば
...
R170の吸収pointerをR112または系列固有記録機構へ誤差 $\varepsilon_{\rm rec}$ 以内で写すと、外部観測分布の全変動誤差は
...
従って理想分布間の分離が誤差和より大きければ、開放pointer固定後にも区別可能性が残る。この系をR124/R125の識別とR180CのBell監査へ共通に用いる。
...
## 2.15 R178Dの本線退役：有限閉鎖resetの情報容量境界

旧R178Dは、結果を保持したまま有限閉鎖系の全補助自由度を同じ初期点へ戻すことの制約と、使用済み側に必要な情報容量下界を与えていた。この内容は誤りとして撤回せず、有限閉鎖resetを検査する強化結果として論文外メモへ移す。現行Q2-4は開放resetとoutgoing bathを許すため、R178Dを必須依存に含めない。結果相関情報はR179のoutgoing channelへ移り、能動系だけを未使用状態へ戻す。

## 2.16 R179：一様開放供給・reset・renewal

M54の反復運転では、補助workspace、pointer、振幅再調整用接続端を有限closed bankから供給せず、固定した一様規則を持つincoming/outgoing reservoir interfaceへ接続する。未使用状態へのresetは開放収縮として扱い、結果相関情報と使用済み環境自由度はoutgoing channelへ流す。R190の反復作用殻混合では、各attemptが過去に装置と相互作用していないstationary incoming bath sectorを使う。

<!-- theorem-start:theorem -->
**定理（R179：一様開放供給・reset・renewal）**

補助能動自由度の未使用状態を分布 $\mu_{\rm blank}$ とし、一様reset半群 $P_t^{\rm reset}$ が
...
さらに反復attempt $m$ ごとに、過去履歴 $\mathcal H_m$ と独立なstationary incoming bath sectorを接続し、使用後のsectorをoutgoing channelへ送る。active shellの初期状態に一様な混合kernel $K_m$ が
...
理想stationary incoming sectorでは $\varepsilon_{{\rm in},m}=0$ である。R179は結果確率や振幅表を外部から供給せず、浴interfaceとreset規則は回路規模に対して一様な有限記述から生成される。有限bath容量、cold/spent cell数、partial-SWAP列は固定目標の必要条件にしない。
<!-- theorem-end:theorem -->

R179はR161/R162へ依存しない環境interface結果である。Q1/Q2のR190反復renewal、Q2-4のworkspace再使用、pointer初期化、散逸履歴のoutgoing排出に共通に使う。有限closed bankによる近似は強化課題として退役メモに保存する。
