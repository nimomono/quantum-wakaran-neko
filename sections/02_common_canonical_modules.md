...
各段階ではfreshな一様しきい値自由度を使い、現在配置に条件付けた各確率行を有限区間分割で実現する。比較境界を滑らかな有限幅帯へ押し込み、帯外では枝ラッチと配置輸送を厳密に理想枝へ一致させる。供給源、対象、段階番号、しきい値、待機または無反応を履歴へ残して全写像を1対1に保つ。時間凍結、比較境界、時計自由度、有限貯蔵部の誤差は段階別に分離する。
