...
@status: M41について、設定前の積Haar基準測度、M39の4モード状態、階数2枝選択、2つの物理的2モード担体への正準転送、互いに可換な局所測定、永久外部記録、内部逆計算、弱開放resetを1本の前向き周期へまとめる。singlet型固定プログラムでは余弦共同確率、非信号性、CHSH値 $2\sqrt2$、平面内2出力族のTsirelson上界を制御された任意精度で得る。Bellの前提違反は測定開始面の測定設定独立性にあり、準備後の設定変更、空間的分離、一般Tsirelson原理は含まない。
...
4. A設定 $x$ に対応する階数2ブロック作用を読み、源枝 $A$ を選ぶ。
5. 選択済み枝から2つの物理的2モード担体 $d^A,d^B$ を準備する。
6. 中央源と両担体の結合を停止する。
7. A側とB側で互いに可換な局所測定を行い、結果を別々の外部記録へ写す。
...
4. 階数2作用選択器で源枝を選ぶ。
5. A、B物理担体へ枝条件付き正準転送する。
6. 中央準備結合を停止する。
7. A、B局所測定を実行する。
8. A、B結果と無反応を外部記録へコピーする。
9. B測定、A測定、2担体転送、源選択、A基底回路、M39準備、設定生成を逆順で戻す。
...
1. 2担体準備後に自由に変更されるA設定またはB設定。
2. 空間的に隔たった設定選択、長距離空間分離、有限伝播速度を持つ時計配線。
3. 一般非singlet状態での作用側チャネルの不存在。
4. 有限資源で無反応なしの厳密2値測定。
5. 一般測定族を拘束するTsirelson原理。
6. 選択器列の独立同分布性と二項分布型有限標本揺らぎ。
7. 永久記録を含む有限閉鎖系全体の同一点帰還。
8. 多量子ビットへ多項式資源で拡張すること。
9. 標準的な空間分離Bell実験またはBell局所性の実験検証。
