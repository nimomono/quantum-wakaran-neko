@number: D
@chapter: 付録
@title: M35一般有限基底補助モジュールの完全正準構成
@status: M35の一般有限L構成について、局所2モード回路、累積比較ポインター、無反応を含む作用区間、場、テンプレート、レジスター、選択器、時計の全写像を追跡する。作用区間は実現配置準備、局所更新、条件付き場準備、分布検算に使い、主測定結果は別に生成しない。
