...
熱化後に新規入射素子を止めて結果成分間ゲートを閉じる。結果成分 $i$ の安全領域内で1、他の結果成分と通信路上で0となる滑らかな局所関数 $d_i(x)$ を選び、空の記録器へ
...
を作用させる。安全領域では1個の粒子位置結果成分だけが占有されるため、対応する1個の記録だけが動く。辺閉鎖、有限井戸幅、記録窓の偏差を $\varepsilon_{\rm lock}+\varepsilon_{\rm rec}$ とする。
...
履歴には入力記憶部、SWAP前後の時計、作用殻結果成分、衝突素子列、反射・透過ラベル、結果成分閉鎖状態、局所記録、無反応原因を残す。従って異なる入力または衝突履歴を同じ最終拡大状態へ潰さず、有限試行写像は単射である。全操作は有限時間なので $t_{\rm out}>t_\star$ を選べる。これでR170を得る。
