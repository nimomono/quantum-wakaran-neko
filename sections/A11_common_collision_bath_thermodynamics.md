...
各段階 $m$ へ、それ以前の窓で使わない一様Liouvilleしきい値座標 $u_m\in(0,1)$、その共役座標、時計自由度、空履歴、枝ラッチと作業記憶部を置く。現在配置が $i$ なら $(0,1)$ を長さ $P_m(i,j)$ の有限区間へ分ける。各内部境界の周囲に有限幅の比較境界帯を置き、その和集合を $B_{mi}$、最大Liouville測度を
...
比較境界帯を除く各枝区間上で厳密に0または1となる滑らかな台地関数 $\chi_{mir}(u_m)$ を選ぶ。source配置 $i$ の安全領域上で1となる滑らかな関数を $d_i(x)$、枝ラッチの正準対を $(C_{mir},P_{mir})$ とし、入口で $C_{mir}=P_{mir}=0$ とする。有限時計窓で
...
を作用させる。$P_{mir}=0$ は保存されるため、安全領域ではしきい値自由度とsource配置への理想反作用は零であり、ちょうど一つの枝ラッチだけが1となる。

source領域 $i$ からtarget領域 $j$ へ向かう滑らかなベクトル場を $v_{ij}(x)$ とする。その余接持上げ
...
を作用させる。安全な選択器領域では一つのラッチだけが作動するため、配置は理想枝と同じtargetへ有限時間で運ばれる。待機枝では $G_{ii}^X=0$ とする。source、target、段階番号、しきい値、待機または無反応を履歴へ残し、異なる履歴を消去しないため全有限時計写像は1対1に保たれる。
...
freshな $u_m$ は過去のHamiltonian窓に現れないため、任意の過去履歴で条件付けても同じ一様分布を持つ。従って初期配置分布誤差を $\varepsilon_{\rm init}$ とし、最初の不一致段階について逐次結合を行うと
...
同じ評価は、配置列にsource、target、段階番号、待機、無反応を含む保存履歴を加えた共同経路分布にも成立する。これをR162の有限骨格経路誤差 $\varepsilon_{162}^{\rm hist}$ と呼ぶ。各窓1段階の有限素子を事前配置すれば $\eta_{{\rm over},m}=0$ とできる。
...
一般部分では、各段階の厳密Markov核 $U_m$、Euler核 $P_m$、Hamiltonian条件付き核 $\widehat P_m$ を分ける。一様Liouvilleしきい値と滑らかな台地比較を同じ選択器上で結合すると、比較境界帯外では $\widehat P_m$ と $P_m$ の枝が厳密に一致する。freshな選択器は過去履歴で条件付けても一様なので、最初の不一致段階に対する逐次結合とunion boundから有限骨格全履歴の全変動距離が段階誤差の和で抑えられる。$C^1$ 率ではDuhamel表示から $U_m-P_m$ のrowノルムを二次で評価し、有限骨格経路誤差の表示式を得る。履歴を保存した枝ラッチと余接持上げ輸送は異なる履歴を消去せず、有限時計写像を1対1に保つ。熱的特殊化では指数入射流束の尾部へ活性化エネルギーを代入して平方根型率を得る。対称障壁と通過後エネルギー式が正逆散乱と詳細釣合いを与える。証明終。
<!-- theorem-end:proof -->
