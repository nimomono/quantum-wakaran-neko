...
本付録で厳密なのは、採用した有限次元paired-Hopf方程式、有限Markov生成子、有限設定controller、局所分析器、記録・帰還式の後段である。R152のrate形は第3章R161の一般形に含まれ、付録LのR162が固定bath座標に対する有限衝突熱浴実現を与える。一方、seed整列drift、paired-Hopf pump、信号bath保持、fresh cell流までを同じ具体的電子回路、流体装置、振動子浴、有限閉鎖Hamiltonianへ統合した結果ではない。Bell統計の条件付き達成範囲は変更しない。
