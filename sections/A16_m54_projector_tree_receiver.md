...
節点の完全能動状態には、信号 $Z_u$、未処理容量指針変数 $J_{u,b}$、作用殻容量 $A_{u,b}^\delta$、選択変数 $X$、R170の吸収pointer $Y$、選別機構用作業領域、外部記録を含める。R190の混合浴、R179のincoming/outgoing bath、方向を変えない振幅再調整用接続端の環境は環境interfaceとして別に扱い、解析上の条件付き確率を制御器へ書き込まない。
...
結果成分の排他的選択と固定は第2章R170が担う。R164の容量をR190の作用殻混合・作用開口へ渡し、R179のstationary incoming bathで反復renewalを保った静的選択を行う。選択bathを切った後、R170の吸収pointer $Y=b$ を固定してから選別機構を開く。保護帯またはpointer未captureは正式な無反応に残し、成功試行だけを再規格化しない。
...
$T_R$ は $\tau$ と安全集合から試行前に固定できる。未知の $p_{u,b}^{-1/2}$ を実装する状態依存スクイーズではない。これは採用開放法則であり、厳密なシンプレクティック リセットまたは無履歴逆掃除とは呼ばない。環境へ移った動径情報はR179のoutgoing bathへ流し、能動workspaceだけをopen resetする。
...
で抑えられる。作用殻剛性は $O(n^2/\epsilon^2)$、R190の選択attempt rateは $O(\sqrt{n/\epsilon})$、安全開口の制御範囲は $O(\log(n/\epsilon))$ で足りる。指数的な信号と受動bath容量、総熱はQ2-4の報告対象内部資源へ計上し、それだけでは失敗条件にしない。
