...
@status: M35について、固定純粋準備・固定有限基底・一般有限Lの局所基底回路、滑らかな比較、無反応結果、測定後状態、内部逆計算、無理数回転Poincaré 写像を構成する。M38で閉じたL=2のQ1--Q3と分離し、一般有限Lの外部記録、弱開放 reset、逐次基底一般化はQ5へ残す。
