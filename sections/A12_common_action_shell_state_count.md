...
現行Q2-3の3部分系候補では、回路末尾に $m=L=8$、$\Psi=I_8$ とする有限特殊化を使える。規格化出力信号を $c_{\rm out}$ とすればR164の枝比は $(|(c_{\rm out})_y|^2+\delta q_y)/(1+\delta)$ となり、正則化による理想Born分布からの全変動距離は高々 $\delta/(1+\delta)$ である。ただし、永続状態浴と二段ゲート列からこの読出しへ非破壊に接続する単一装置は未完成である。
