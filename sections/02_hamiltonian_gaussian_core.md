...
## 2.6 OU 補助モデルと Stratonovich 表現

$F=-\lambda I+\Omega J$ と選べば、2次元の回転を伴う OU 位相モデルが得られる。これは減衰する位相振幅を扱う便利な具体例である。しかし OU の摩擦は縮約後の有効係数であり、有限 Hamiltonian 部分の基本法則そのものが時間反転を破ることを意味しない。本論文では OU モデルを基礎仮定とせず、付録Dの例として用いる。

現行モデルの立場では、$\lambda$ は外部へのエネルギー流出と外部揺らぎの釣り合いを要約する有効係数である。$\lambda$ と拡散係数を特定の有限外部 Hamiltonian から導くこと、有限観測窓における Markov 近似誤差を評価することは別課題である。従って OU 表示は補助モデルであり、現行モデルそのものではない。

雑音係数が状態に依存しないため、Itô 表現と Stratonovich 表現の変換補正は零である。従って、どちらの記法を選んでも本論文の線形 Gauss 型定理は変わらない。Stratonovich 微分は中心論証に必要ないため、以後は Itô 表現に統一する。
