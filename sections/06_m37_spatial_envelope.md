...
@status: M37の有限時間Schrödinger型近似をR167で統計共分散へ持ち上げ、R168、R169のM50受渡しとR170の固定入力時刻粒子位置instrumentを構成する。
...
Q1はM47のW型2モード、Q2-2はM48のpaired-Hopf周期へ移行し、M35はQ2・Q3について一般有限 $L$ の作用選択器を与える。Q3の現行読出しはM50の一般有限枝状態数とR161/R162を使う。任意の装置用正準混合がM37の局所ばね網だけで実装できるとは仮定しない。M37がM47へ供給するのは、時間非依存の対称W型生成子、最低2固有モード、スペクトル間隔である。置換済み模型の旧利用関係は現行因果鎖に含めない。
...
本節の受渡しは、複素振幅場の作用比を任意有限基底で標本化するQ2・Q3用の補助診断として残す。Q3の主読出しは、固定入力時刻の単一試行標本をM50へ渡して後刻に局所記録するR170である。Q1のW型2モード測定は第3章で独立に扱い、本節の標本化式を使わない。
...
となる。M35だけを使う場合、これは複素振幅場の空間セル基底を標本化する式に留まる。R170は独立の有限枝位置 $X$ とM50熱化を追加し、入力標本時刻より後の局所位置記録を別に構成する。従ってR87は削除せず、R167--R170の分布誤差を独立に検算する補助的な作用標本化結果へ位置づける。
...
## 6.11 R167：M37標本力学から統計共分散への有限時間持上げ
...
## 6.12 R168：階数1支持からM50枝状態数への受渡し

階数1の場合、$C_Z(t_\star)=c_\star c_\star^\dagger$ なら、付録Mの支持補題から
...
<!-- theorem-start:theorem -->
**定理（R168：階数1支持からM50枝状態数へのQ3受渡し）**

$C_Z(t_\star)=c_\star c_\star^\dagger$、$Z_{t_\star}\neq0$ が安全試行上で成り立つとする。M50の等長埋込み $\Psi$ と正則化 $\delta>0$ に対して、全安全試行の枝状態数分布は

```math
\pi_i^\delta
\left(Z_{t_\star}(\omega)\right)
=
\frac{|(\Psi c_\star)_i|^2+\delta q_i}{1+\delta}
```

に一致する。近似ray $\widehat Z$ が $c_\star$ から純粋状態距離 $s$ 以内なら
...
零信号、閾値未満、保持失敗は $\varnothing$ とし、成功試行だけで再規格化しない。
<!-- theorem-end:theorem -->

R167の規格化方向誤差を使う場合、$q_T<1$ なら $\rho_T=q_T/(1-q_T)$ を安全なray誤差上界として選べる。

## 6.13 R169：固定全作用集団の高階数M50読出し

高階数集団では、各試行のray分布を先にM50へ渡し、その後で集団平均を取る。一般にはこの順序は共分散の規格化と可換でない。可換性を保証する最小条件として固定全作用を置く。

<!-- theorem-start:theorem -->
**定理（R169：固定全作用集団の高階数M50読出し）**

入力時刻に

```math
Z_{t_\star}^\dagger Z_{t_\star}=s_*>0
\qquad
\mu\text{-a.s.}
```

とする。$M_i=\Psi^\dagger|i\rangle\langle i|\Psi$ と置けば、M50枝状態数の集団平均は

```math
\mathbb E_\mu
\left[
\pi_i^\delta(Z_{t_\star})
\right]
=
\frac{\operatorname{tr}(M_iC_Z(t_\star))+\delta q_i}{1+\delta}
```

である。従って高階数共分散は、固定全作用という単一試行制約の下で、同じ試行のM50枝位置頻度として読める。
<!-- theorem-end:theorem -->

全作用が変動する場合、M50が平均するのは
...
であり、一般に $R_Z\neq C_Z$ である。$\overline S=\mathbb E[Z^\dagger Z]$ とすれば
...
M50正則化後の差はさらに $(1+\delta)^{-1}$ 倍で抑えられる。この補正を使う場合も、同じ全作用変動をR167の包絡誤差へ重ねて加算しない。

## 6.14 R170：固定入力時刻粒子位置instrument
...
<!-- theorem-start:theorem -->
**定理（R170：M37--M50固定入力時刻粒子位置instrument）**

固定有限M37入力集団、固定入力時刻 $t_\star$、固定有限枝グラフ、$\delta>0$ を考える。R168の階数1条件またはR169の固定全作用条件を満たし、M50作用容量準備、R161/R162の有限熱化、信号保持、枝固定、局所記録を各々指定誤差内で実行できるとする。このとき有限の $t_{\rm out}>t_\star$ と完全結果集合 $\mathcal I\cup\{\varnothing\}$ を持つinstrumentを選び、無反応質量を零とした理想分布 $p^{\rm id}$ に対して
...
とできる。安全結果 $i$ は1個の粒子位置枝 $X_{\rm out}=i$ と局所記録 $D_i=1$ を持ち、異なる初期状態、衝突履歴、結果記録を含む拡大写像は1対1である。
<!-- theorem-end:theorem -->

R170は、粒子位置が入力時刻以前からM37包絡に等変であることを仮定しない。$t_\star$ と $t_{\rm out}$ を同一時刻と書かず、装置内部の熱化軌道をSchrödinger型粒子軌道、初回到達、吸収、時間積分流束と解釈しない。

## 6.15 誤差、時間、資源、Q3-4・Q3-5への接続
...
と分ける。$\varepsilon_{37\to50}$ はR168のray誤差またはR169の共分散・全作用補正のどちらか一方を含む。R167の同じM37包絡差を別名で再加算しない。混合誤差は
...
R167--R170については `tools/verify_m37_m50_position_instrument.py` で、共分散持上げ、階数1支持、固定作用高階数公式、可変作用反例、正則化後のradial誤差、R161混合上界、完全結果集合、履歴単射性、R124・R125の正の余裕を検算する。数値検算は解析証明の代わりではなく、規格化順序、trace、全変動距離、時刻ラベル、誤差の二重計数を監査する回帰検査である。
...
Q3-1の固定基準自体はR83--R88で満たされ、今回の改訂で後から基準を広げたわけではない。R112は共通有限正準信号代数、R167はM37標本集団の共分散持上げ、R168とR169はM50枝状態数への受渡し、R170は固定入力時刻の有限粒子位置instrumentを追加する強化結果である。
...
M47ではM37を、対称W型生成子 $h_W$、最低2モード $\phi_0,\phi_1$、モード分裂 $E_1-E_0$ を供給する層としてだけ使う。M37の物理包絡 $b$ をM47の独立実在場とはせず、$b$ からrateを作って粒子位置を動かさない。M47の統計振幅は、粒子位置--浴共同測度のrank-one核として別に定義する。M37のHamiltonianと反回転項の評価は変更しない。外部 $\lambda_{\rm prep}(t)$ による開放準備と閉鎖作用角伝播、matching受渡しの条件は第8.15節と付録Iに示す。

R170はBorn型位置分布を「入力信号の標本化」「M50作用殻状態数」「有限熱化」「局所記録」に分ける。状態数だけで物理熱化を説明したとはせず、有限熱化だけで作用容量のミクロ起源を説明したとも扱わない。M35による直接セル標本化は補助診断として残し、R170の粒子位置instrumentとは混同しない。
