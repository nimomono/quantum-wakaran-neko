...
Q3の単一試行系は、M37担体層とM55粒子層を区別する。

| 層 | 単一試行で物理的に存在するもの | 派生表示・集団記述 | 役割 |
|---|---|---|---|
| M37担体 | 有限個の実振動子座標 $(q_i,p_i)$ と局所ばね結合 | 局所複素包絡 $b_i$、統計ray、規格化第2モーメント | R86の有限時間Schrödinger型担体 |
| M55粒子 | 1個の局在位置 $X_t$、局所辺bath、clock、履歴、記録 | 位置分布 $P(X_t=i)$ | R183--R184の局所輸送と終位置記録 |

複素包絡は実振動子状態の派生座標であり、複素rayと位置分布は多数試行の統計である。一方、$X_t$ は各試行に1つ存在する粒子位置である。M37だけから粒子位置や最小率が必然的に出るとは主張せず、M55の局所率、bath cell、更新則を追加の採用ミクロ法則として明示する。

共通M54/R181Aは、M37へ入れる前の実正準seed集団を開放driftでrank-one rayへ準備する。準備後の同じ単一試行信号にM50/R164の作用殻選択を一度だけ適用してM55の初期位置 $X_0$ を作る。その後はM37とM55を同時に進め、終時刻には新しい位置を再標本化せず、既存の $X_T$ をR112の局所記録回路で読む。このためM54、初期R164選択、終位置記録を独立なBorn型確率源として数えない。

ミクロ振動子層から有効担体への移行はQ3-1の固定達成基準であり、R86が満たす。M55は粒子実体と下流現象を追加する強化であって、Q3-1の判定へ遡及的に要求しない。M54のport、M37局所ばね網、初期作用殻、M55衝突bath、記録を同じ有限局所装置へ統合したとも扱わない。

Q1はM47のW型2モード、Q2-2はM54駆動R180 paired-Hopf receiver、固定時刻の一般枝instrumentはM50/R170を使う。M55はQ3の空間セルだけに採用する。任意の装置用正準混合がM37の局所ばね網だけで実装できるとは仮定しない。静的R86がM47へ供給するのは対称W型生成子、最低2固有モード、スペクトル間隔である。第6.17節と付録E.12では有限制御列への誤差接続を追加し、第3.5.1節の条件付き系へ渡す。
...
となる。これは空間セル基底の目標位置分布である。R183は同じ分布をM55トークンの等変分布として全有限時刻へ運び、R184は節正則化と有限衝突Hamiltonian近似を与える。R86の作用比だけを粒子実体と同一視せず、粒子層の追加方程式と初期選択を必要とする。
