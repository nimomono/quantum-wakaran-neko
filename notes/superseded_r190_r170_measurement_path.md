...
draft-89でR190A--R190C/R170はQ1/Q2 fixed-goal主線から外れ、draft-120でM65がcanonical open selectorへ正式に置換した。さらにdraft-124ではR164作用殻状態数とR170吸収pointerをactive paperから完全に退役し、旧作用殻測定経路全体をnotes/Git履歴へ移す。

ここに保存する定理群は反証されたものではない。2作用LC殻、Drude混合、対称作用開口、静的吸収pointerという別の物理実現候補としての研究記録である。現行Q1/Q2の結果形成はM65/R181D、Q3の粒子位置はM64/R203A--R203D/R161/R185が担い、この経路の誤差・資源をactive台帳へ加えない。

以下は旧版の数式・定理を追跡するための歴史記録であり、「現行」「正本」等の語が残る箇所は収録当時の意味で読む。R170の結果番号は再利用しない。R170の静的選択・固定中核への再定義はcommit `d6657b3cebc5f417a998ae1a2628e3873e4a90b0`、draft-82 mergeは `d7561f11ce7f8377bbfd72701f0dd493b3dc3661` を参照する。
