...
$C^1$ 収束から、収束する有限モデルの停留点列の極限がNelson作用の停留点であることは従う。逆に、任意のNelson停留点が有限モデルから得られること、無限次元の全変分、一般の非線形Hamiltonian系、収束率の最適性は未導出事項である。
...
を得る。これは旧モデルでreturn comparatorへ明示的に書き込んでいたterminal compatibilityと同じ支持条件である。比較演算は、浴modeの正準変換、全作用の保存、reservoir energyの非負性でまかなわれる。
...
ソフトモードと残余作用モードの作用を $J_s,J_0$ とし、

```math
J_s+J_0=\frac{E_\ell}{\omega_\ell}
```

という固定総作用殻上でLiouville測度を取ると、ソフトモードエネルギー $h=\omega_\ell J_s$ は $[0,E_\ell]$ 上で一様になる。結果領域の基準質量 $w_{AB}=1/4$ は、Hamiltonianの形ではなく対称な準備測度から従う。
...
と等価になる。全結果領域でしきい値が固定総作用殻の範囲内にあるとき、適合体積は
...
後発の候補では、終端フィルター $[R]$ を独立に置く代わりに、ソフトモード作用 $J_s$、帰還作用 $J_r$、比較器の差動作用 $\widetilde I_-$ から
...
のように設定 $a,b$ に依存する。したがって、このモデルで外れるBell前提は測定設定独立性である。
