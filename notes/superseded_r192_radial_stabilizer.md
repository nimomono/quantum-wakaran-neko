...
> **現行注記（draft-127）**：R192は反証されたのではない。旧Q2-4逐次binary readoutでは、R181D後の非終端branchを次のselectorへ渡すため絶対作用下限を回復する責務を持っていた。M66/R206のone-shot terminal multi-outcome samplerへQ2-4を切り替えると非終端branch自体が消えるため、fixed-goal主線から責務が消滅した。Q1/Q2-2のR181Dは固定有限深さで非規格化結果成分を直接渡し、R192を必要としない。結果ID R192は再利用しない。

以下は退役直前のactive付録内容を保存したもの。
