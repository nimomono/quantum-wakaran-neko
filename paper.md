...
Q3-2からQ3-5について本稿が変更するのは作業優先度と固定目標の判定範囲であり、新しい導出結果ではない。Q3-2は科学的には未達のまま凍結する。Q3-3はエネルギー保存型の純位相緩和を認め、Q3-4とQ3-5は有限グラフ上の最小例と位置読出しへの接続へ判定を絞る。従って、これらを第1.5節の主結果表へ追加せず、Q3-3からQ3-5の現在地も未達のまま保つ。モデル非依存な達成判定は `PROJECT_STATUS.md`、現行モデルによる次段階の導出方針と反証条件は第10章に置く。
...
9. 井戸型・調和型ポテンシャルのエネルギー保存型デコヒーレンス、障壁値未満状態の反対側への位置確率移動、2経路コヒーレント分布と非干渉混合の差を正式に導いたこと。
...
**次段階の導出方針。** まず時間非依存 $h_L$ について1次元有限井戸と調和型ポテンシャルを構成し、有限個の非縮退低位固有値、密度、節位置を求める。格子幅、領域幅、搬送周波数を変え、対象とする有限モード集合で収束を評価する。固有ベクトルを初期条件として入れた固定時刻位置分布はM42で読めるが、これはデコヒーレンスではない。
...
**次段階の導出方針。** 最小候補として有限鎖を障壁手前 $L$、有限障壁 $B$、反対側 $R$ に分け、時間非依存生成子 $h$ と障壁値 $V_B$ を定める。初期状態 $b_0$ について
...
**非主張。** 障壁高・幅・入射エネルギーに対する連続的な透過率曲線、半無限散乱極限、透過・反射・未確定を含む完全散乱装置、熱活性化との装置比較、初回通過、吸収器、到達時間分布は固定目標より強い拡張である。Q3-4は未達であり、上の不等式は次段階で証明する条件であって本版の結果ではない。

## 2重スリット干渉（Q3-5）

**固定目標と達成判定。** Q3-5は、有限空間グラフ上の2経路入力について、コヒーレント分布が非干渉混合と異なり、相対位相に応じて位置分布が変化することを示し、位置読出しへ接続する。固定目標名の「2重スリット」は、この最小の2経路干渉を指す。

**次段階の導出方針。** 有限グラフの直交する2経路入力を $|L\rangle,|R\rangle$ とし、同じ経路重みを持つコヒーレント入力と非干渉混合を
...
**非主張。** 幾何学的な開口、源、シャッター、多画素スクリーン、全検出器のHamiltonian、無検出を含む完全装置、初回到達、吸収、永久記録、反復resetは固定目標より強い拡張である。Q3-5は未達であり、上の全変動距離の正の下界は次段階で証明する条件であって本版の結果ではない。
...
2. Q3-2は判定基準を保持したまま凍結し、Q3-3からQ3-5について、1次元低位束縛スペクトルとエネルギー保存型デコヒーレンス、障壁値未満状態の位置確率移動、有限グラフ2経路入力のコヒーレント分布と非干渉混合の差を正式に導く。源、シャッター、全検出器、散乱極限、初回到達、吸収、時間積分流束、連続運転スクリーンは固定目標より強い拡張として分ける。
