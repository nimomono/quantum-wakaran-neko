...
**命題（R112：有限配置グラフ模型のQ1--Q2--Q3共通構造）**
...
**現在の実現方針。** M37から得る単一値の複素振幅場を使い、向き付けた閉路 $C$ の全頂点で $b_i\neq0$ のとき、候補巻数を
...
**非主張。** 密度と流れだけを基本変数とする一般的な確率力学への完全回答、節を横切る閉路の巻数、外部ゲージ場を含む一般化は主張しない。単価性を外から仮定する、閉路位相を整数へ丸める、整数巻数を許容条件として直接置く構成はQ3-2の達成としない。

## 束縛状態（Q3-3）

**固定目標と達成判定。** Q3-3は、井戸型・調和型ポテンシャルの固有値、密度、節構造、基底状態・励起状態を再現し、弱開放系でエネルギー固有状態間のデコヒーレンスと対角占有率の安定性を導く。1試行ごとの状態選択、基底状態への緩和、射影的な収縮、独立したエネルギー測定器は要求しない。

**現在の実現方針。** まず時間非依存 $h_L$ について有限井戸と調和型ポテンシャルを構成し、有限振動子パラメータから固有値、固有ベクトル、密度、節位置を求める。格子幅、領域幅、搬送周波数を変え、基底状態と複数励起状態の収束を評価する。固有ベクトルを初期条件として入れた固定時刻位置分布はM42で読めるが、これはデコヒーレンスではない。
...
**非主張。** 外部乱数位相を直接加えるだけの模型、Lindblad型減衰項をミクロ結合なしに置く模型、非対角成分と同程度に対角占有率を変える緩和模型はQ3-3の達成としない。状態選択、固有状態を吸引状態とする機構、基底状態への冷却は本目標から外す。

## トンネル効果（Q3-4）

**固定目標と達成判定。** Q3-4は、障壁高、幅、入射エネルギーに依存する透過率を、透過、反射、未確定を含む完全な散乱結果分布から導き、熱活性化や古典的障壁越えと区別する。初回到達時刻分布、吸収条件、時間積分流束は必須ではない。

**現在の実現方針。** 有限1次元格子を入射側 $L$、障壁領域 $B$、透過側 $R$ に分け、有限幅の入射波束を準備する。波束のエネルギー分布を求め、障壁高より上の成分を別に評価する。障壁高、幅、中心入射エネルギーを掃引し、M37のミクロ振動子網と目標Schrödinger型解の透過率差を評価する。
...
**非主張。** 波束分離前または端反射後の時刻だけを使う、障壁上成分を無視する、未確定結果を除外する、熱浴による越境を同じ透過率として数える構成は達成としない。初回通過、吸収器、到達時間分布は固定目標より強い拡張として別に扱う。

## 2重スリット干渉（Q3-5）

**固定目標と達成判定。** Q3-5は、準備、有限2重スリット配置、空間伝播、相対位相、干渉、局所検出を1つの装置へ統合し、無検出を含む位置検出分布を導く。左のみ、右のみ、両スリットの対照、相対位相による縞移動、排他的な局所結果、位置分布と総検出率の分離が必要である。

**現在の実現方針。** 有限グラフ上に源領域、左右スリット、伝播領域、画素集合 $S_k$ からなるスクリーン領域を置く。左のみ、右のみ、両方開放を同じ装置上で切り替え、一方の経路に制御可能な位相差を加える。M37で源からスクリーンまで伝播させ、M42で波束がスクリーン領域へ達した後かつ端反射が戻る前の固定時刻 $T$ に位置を読む。
...
**非主張。** 場の強度分布を描くだけで実現配置の局所検出を接続しない構成、単スリット対照がない構成、無検出を捨てる構成は達成としない。固定時刻読出しから初回到達、吸収、時間積分流束は従わない。永久外部記録、検出後処理、反復reset、連続運転スクリーンは追加できるが、固定達成条件より強い拡張である。Q3-5は未達のままである。
...
2. Q3-2からQ3-5として、限定された位相量子化回答、束縛スペクトルと弱開放デコヒーレンス、単一障壁散乱と固定時刻位置読出し、有限2重スリット伝播と固定時刻局所読出しを構成する。初回到達、吸収、時間積分流束、連続運転スクリーンは固定目標より強い拡張として分ける。
