...
本改訂の中心的な前進は、位相接続による Schrödinger 型力学、位置の Born 型入口標本化、同じ2成分場の低ランク対モードから得る Bell 余弦作用、共通未来比較器による2モード作用転送を1本の物理像へ整理したことである。最大の未解決問題は、殻容量に比例する共通境界測度を、結果別の事後選別を用いない実験周期として準備し、記録、逆計算、再初期化まで統合することである。

# 問題設定とモデル地図

> **位置づけ：** 位相接続を現行模型の力学的主線とし、Born 型入口殻と低ランク2モード Bell 境界構成を同じ2成分場の固定部分空間へ置く。配置拡散経路と旧3モード Bell 模型は現行模型に採用しない。
...
> **位置づけ：** 有限セルの正準変換、内部回転対称性、保存位相作用、固定作用下の局所作用分配、粒子運動量消去は厳密結果である。同じ場の位相・対・比較モード分解は固定射影として置く。coherent多様体の準備と交差応答の制御は未完成である。
...
> **位置づけ：** coherent縮約多様体に制限した Madelung 作用、変分、Schrödinger 型方程式、同期差保存は厳密結果である。確率過程を前提にしない。多様体の生成、安定化、微視的停留点選択は未完成である。
...
> **位置づけ：** 位相接続を主線とし、Born 型入口殻、低ランク Bell 対モード、2モード境界比較器の厳密部分と仮説依存部分を分離する。共通境界測度、一般測定器、完成 Hamiltonian 周期は未完成である。
...
| 構成 | 本論文で得た結果 | 未完成な接続 |
|---|---|---|
| 位相接続 | 有限セル正準構造、保存位相作用、固定作用下の局所分配、Madelung 作用、局所 Schrödinger 型PDE、同期差保存、条件付き循環量子化 | coherent多様体の準備・維持、節、係数整合、停留点選択 |
...
8. 直接作用分配方向が複数なら、入口重みは一般に作用の高いべきになる。
9. 低ランク相関 $C$ は独立した配置空間場ではない。
10. 低ランク相関と局所回転だけでは Bell 頻度は生じない。
11. 共通未来比較器だけでは、前向き因果として過去の結果頻度を変更できない。
12. 比較器の Hamiltonian 読み出しだけでは殻容量に比例するsector質量を生成しない。
13. $U(2)$ 混合は正規化されたsector総質量を変えない。
14. Bell 重みには共通境界測度の準備が必要である。
15. 現行局所装置は既存の2値sectorを符号化するが、一般的な唯一結果形成を与えない。
16. 異なる結果履歴を同じ全位相点へ押しつぶす完全resetは Hamiltonian ではない。
17. Bell 余弦則は、一般 Born 則、Wallstrom 問題、一般 Tsirelson 原理を単独では解かない。
18. 現Bell模型は平面内の2出力分析設定に限定される。
19. 配置拡散経路の Brown 極限、配置 Markov 閉鎖、時間対称 Newton 則は未完成であり、現行主線には使わない。
...
5. coherent多様体上の縮約作用が、有限 Hamiltonian 粗視化運動を実際に支配することを示す。
6. 節を含む位相接続と循環量子化を制御する。
7. 反対称対モードを生成する有限 Hamiltonian 源と長時間の位相保持を構成する。
8. 平面内2出力を超える一般測定器と複合状態構成へ拡張する。
9. 非対称準備と有限誤差を含む一般的な非信号条件を示す。
...
本論文は、Schrödinger 型力学、位置の Born 型入口重み、Bell 余弦作用、2モード比較作用を1つの2成分場アーキテクチャへ整理した。しかし、共通境界測度、一般的な結果形成、coherent場の再埋め込み、反復可能な完全周期を1本の有限 Hamiltonian から導いていない。この境界を超えて主張しない。
