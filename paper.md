...
4. M48の条件付き達成範囲を越える拡張として、R152のミクロ導出、準備後設定変更、空間分離、一般状態への受渡し、独立同分布型標本統計を検査する。
