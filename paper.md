...
6. M38の2段能動正準対数17と、Q4の $10+8N_{\rm Z}$ 資源上界。
...
9. Q4の全履歴分布が $E_{N_{\rm Z}}^{\rm fwd}$ の全変動距離上界を超える。
10. Q4の前向き測定窓でRabi 項の係数が零になる。
11. 観測中の摩擦または観測後reset だけでQ4の抑制が生じる。
12. 無反応または反転履歴を除外してQ4の分布を再規格化する。
...
1. Q5側では、M38の状態空間、操作、複数段測定、外部セル周期を一般有限 $L$ へ拡張する。
