...
この計算は、新しい種類の Gauss 型条件づけではない。有限次元の状態を拡大して Fourier 係数まで含めれば、固定区間の線形 Gauss 型平滑化と同じ Schur 補完になる [28]。経路測度の立場では相反過程および Schrödinger 橋の線形 Gauss 型部分に属し [15,26,27,37]、経路単位の Gauss 型条件づけとしても標準的に表せる [36]。本論文で必要なのは、この既知の条件づけを有限 Fourier 切断数 $N$ とパラメータ $\theta$ について一様に微分し、第4章の定量的 $C^1$ 評価へ接続することである。
...
ただし記録消去と再初期化では、$\Delta E_{\rm win}$ を無視しない。次試行の入口巨視状態を再現するため、仕事貯蔵系からの供給と外部への排熱が必要になる [17,18,33,34]。有限環境への散逸は長時間には戻り得るため、有限観測窓と再帰時間の両方を管理する [40]。
...
| 主張 | 確立度 | 理由 |
|---|---|---|
| `[S]`、`[M]`、`[R]` の下での $W_{AB}$ と規格化 | 厳密結果 | 有限次元の位相体積積分と代数 |
...
| 実験の無条件試行頻度が $P_R$ に従うこと | 仮説依存結果 | `[R]` の物理的生成が未導出 |
...
- `[R]` を生む試行周期：未構成であり、物理的 Bell 予測はこの仮説に依存する。
...
結果依存の未完了試行や時間切れ試行を除けば、事後選別が生じる。本論文の $W_{AB}$ は通過流束や完了率ではなく、`[R]` が物理的集団とする終端整合位相体積である。共有浴と待ち時間を含む一般的な否定結果は第8.5節と付録C.11にまとめる。
...
ただし実験の無条件試行頻度がこの共同法則に従うという主張は、`[R]` に依存する仮説依存結果である。弱開放な試行周期から `[R]` を導くこと、一般測定相互作用を構成すること、結果依存の欠測を排除することが残る。

# モデルの運用状態、結果の確立度、適用限界
...
| `[R]` の下での終端整合体積と Bell 型共同確率 | 厳密結果 | 肯定的結果・成立条件 |
| 実験の無条件試行頻度が `[R]` に従うこと | 仮説依存結果 | 肯定的主張 |
| 弱開放な試行周期から `[R]` が生じること | 予想・未解決 | 中心課題 |
| 弱開放結合、有限混合、有限パルスによる Bell 補正 | 近似結果 | 成立条件・適用限界 |
| `[S]` と `[R]` の下での非信号性 | 厳密結果 | 肯定的結果・成立条件 |
...
同じ Bell 共同確率についても、`[R]` を入力した後の積分と、`[R]` 自体の物理的起源は別の結果である。後段の代数が厳密であっても、未導出の入力が導出済みに変わるわけではない。
...
通常の順時間的リセットは入口巨視状態を再現できるが、それだけで初期・終端の積測度 `[R]` を与えない。結果依存の失敗を除外して Bell 重みを作るなら事後選別である。
...
従って、順時間的な共有浴雑音の漏れ、結果依存滞在時間、反応座標の通過速度は、それだけでは `[R]` の代わりにならない。
...
となり、A 側周辺は B 側の測定設定に依存する。従って `[S]` の非信号性は対称準備についての成立条件であり、任意準備についての定理ではない。
...
を有限幅でも厳密に読み出す。固定作用殻の一様密度、対称準備、`[R]` を仮定した後の Bell 型余弦共同確率も厳密に計算できる。Bell の前提違反は測定設定独立性に位置する。

一方、`[R]` は現行モデルの力学と試行周期から導かれておらず、局所装置も一般測定相互作用には達していない。従って物理的な Bell 予測は仮説依存結果である。現在の到達点は、Bell 型確率構造の完全な力学的導出ではなく、厳密に実現した部品、制御された近似、未導出の統計入力、否定的結果の境界を明示した構成である。
...
# Gauss 型 Nelson 方程式、Schrödinger 表示、OU 補助モデル
...
4. 装置が次試行の入口巨視状態へ戻った再初期化数 $N_{\rm reset}$。
...
通常の順時間的リセットが入口巨視状態を再現することは、`[R]` の2境界積測度を導くことと同じではない。逆に、$N_{\rm reset}<N_R$ となる結果依存の失敗を除外すれば、リセット段階の事後選別になる。
