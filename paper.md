...
7. Bell 統計は二側履歴支持に依存する条件付き模型として監査する。
...
1. 量子力学の全構造を導出したこと。
2. M37、M38、M35、M39、M36を1つのM0へ統合したこと。
...
5. 非対称誤差下の非信号上界。
6. 外部設定介入の意味論。
7. 共通源から生成され、空間的に分離する2物理担体。
8. 測定中のA--B直接結合を使わない2つの局所Hamiltonian 測定翼。
9. 各側の局所設定、不可逆な局所結果記録、記録後の照合。
10. M38の局所測定装置との接続。
11. 平面内2出力を超える一般Tsirelson 原理。
...
29. 生成物と章別正本で式、結果ID、目標の現在地が一致しない。

## 最重要の未解決問題

Q1-1からQ1-4をM38、Q2-1をM39で閉じた後の中心課題は、次の3本に分かれる。

1. Q2-2では、2物理担体と分離局所測定翼を構成し、集団交差相関を単一試行で読まず、余弦区間多重度と二側支持をミクロHamiltonian の解空間から作る。
2. Q2-3では、M39の $2^n$ 直接モードコストを監査し、多量子ビット回路の物理自由度、時間、精度、記録、reset の規模依存性を評価する。
