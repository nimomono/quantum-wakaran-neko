...
4. M44について、一つの滑らかな有限Hamiltonianから基準拡散核、捕捉entropy則、反復ready因子化、同じ作用scale、時間対称加速度への持ち上げを有限誤差付きで同時に導けるかを検査する。
...
を持つ。従って $v=(b_++b_-)/2=0$、$u=(b_+-b_-)/2=\nu\partial_x\log\rho_V$ である。Nelsonの時間対称加速度[3--6]
