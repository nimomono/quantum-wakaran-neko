...
> **位置づけ：** 位相接続を現行模型の力学的主線とし、局所作用整合・動径低速化の有限特異 Hamiltonian と時計制御準備浴を前段へ加える。Born 型入口殻と低ランク2モード Bell 境界構成は同じ2成分場の固定部分空間へ置く。配置拡散経路と旧3モード Bell 模型は現行模型に採用しない。
...
|---|---|---|---|
| 局所作用整合準備 | 有限特異 Hamiltonian、振幅浴、位相差に結合した作用交換浴、準備時計 | $P_i$ と $J_i-\mathcal J_\phi q_i$ の低速化、$\mathcal J_\phi$ の厳密保存 | coherent集中、密度同期、有限温度、再帰、一様収束 |
...
11. 場、装置、時計、準備浴、garbage自由度の再初期化。

本論文は、1から3のうち局所作用整合と動径低速化、5の局所モード回転、7の理想比較読み出し、8の条件付き測度、10の理想的な比較器逆計算を与える。coherent集中、密度同期、一般的な結果形成、境界測度の物理的準備、長期反復は未解決である。
...
- ミクロ時間発展が coherent集中、密度同期、共通境界測度を準備することは別の主張である。局所作用整合と動径低速化だけは、第2章と付録Eで部分的に具体化する。
...
> **位置づけ：** 有限セルの正準変換、内部回転対称性、保存位相作用、固定作用下の局所作用分配、特異 Hamiltonian の有限パラメータ消去、粒子運動量消去は厳密結果である。局所作用整合と動径低速化は低速枝および時計制御された準備浴として部分的に具体化する。coherent集中、密度同期、単流束化、交差応答の制御は未完成である。
...
> **位置づけ：** coherent縮約多様体に制限した Madelung 作用、変分、Schrödinger 型方程式、同期差保存は厳密結果である。確率過程を前提にしない。局所作用整合と動径低速化は有限特異 Hamiltonian の低速枝と準備浴により部分的に具体化する。coherent集中、密度同期、一様な特異極限、微視的停留点選択は未完成である。
...
付録Eの準備浴は、再埋め込み後に残る局所作用欠陥 $J_i-\mathcal J_\phi q_i$ と動径運動量 $P_i$ を再び小さくする候補である。しかし、標本化前の振幅 $q_i$ 自体を復元せず、結果情報の可逆転送、共有明反応座標の復元、garbage処理、全チャンネルで偏りのない再準備を行わない。従って、再埋め込み問題を部分的な法線欠陥除去へ分解するが、$\varepsilon_{\rm reset}$ 全体を解決しない。
...
> **位置づけ：** 位相接続を主線とし、局所作用整合・動径低速化の有限特異 Hamiltonian と時計制御準備浴を追加する。Born 型入口殻、低ランク Bell 対モード、2モード境界比較器の厳密部分と仮説依存部分は維持する。coherent集中、密度同期、共通境界測度、一般測定器、完成 Hamiltonian 周期は未完成である。
...
比較窓では $\varepsilon_{\rm ext}$ の効果を小さくし、有限閉鎖系の正準写像として計算する。試行間では、外部記録、garbage移送、再準備に弱開放流路を用い得る。
...
> **位置づけ：** 有限調和浴の Hamiltonian、共通位相回転対称性、全位相作用の保存、対称な余弦・正弦結合による位相固定項の消去は厳密結果である。記憶摩擦、欠陥減衰、有限温度床、有限時間準備は明記した短記憶・固定振幅近似の下での結果である。一般初期値に対する一様収束と長期反復は未完成である。
